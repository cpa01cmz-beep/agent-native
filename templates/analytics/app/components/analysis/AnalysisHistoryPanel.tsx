...
import {
  ...
  useAnalysisRevisions,
  useRestoreAnalysisRevision,
} from "@/hooks/use-analysis-revisions";
...
export function AnalysisHistoryPanel({
  ...
  const doRestore = async (revision: AnalysisRevision) => {
    try {
      await restoreRevision.mutateAsync({
        analysisId,
        revisionId: revision.id,
      });
      toast.success(t("analyses.historyRestored"));
      setPendingRestore(null);
      onOpenChange(false);
    } catch {
      toast.error(t("analyses.historyRestoreFailed"));
    }
  };
    ...
    <>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-xs font-medium">
                          {revision.name || t("common.untitledAnalysis")}
                        </p>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("analyses.historyRestoreQuestion")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("analyses.historyRestoreWarning")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
