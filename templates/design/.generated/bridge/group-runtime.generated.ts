...
(() => {
  ...
  (function() {
    ...
    function hasNonIdentityScale(value) {
      const scale = value.trim().toLowerCase();
      if (!scale || scale === "none") return false;
      return scale.split(/\\s+/).some((part) => Number(part) !== 1);
    }
    function hasNonZeroTranslate(value) {
      const translate = value.trim().toLowerCase();
      if (!translate || translate === "none") return false;
      return translate.split(/\\s+/).some((part) => !/^[-+]?0(?:\\.0+)?(?:[a-z%]+)?$/.test(part));
    }
    function transformed(element) {
      const computed = getComputedStyle(element);
      return computed.transform !== "none" || hasNonZeroTranslate(computed.translate) || computed.rotate !== "none" && computed.rotate !== "0deg" && computed.rotate !== "0" || hasNonIdentityScale(computed.scale) || !/^(?:1|normal)?$/.test(computed.getPropertyValue("zoom"));
    }
    function translationOnlyTransform(value) {
      if (typeof DOMMatrixReadOnly === "undefined") return false;
      try {
        const matrix = new DOMMatrixReadOnly(value);
        return matrix.is2D && Math.abs(matrix.a - 1) <= 1e-3 && Math.abs(matrix.b) <= 1e-3 && Math.abs(matrix.c) <= 1e-3 && Math.abs(matrix.d - 1) <= 1e-3;
      } catch (error) {
        console.debug(
    ...
    function distortsRelativeGeometry(element) {
      const computed = getComputedStyle(element);
      return computed.transform !== "none" && !translationOnlyTransform(computed.transform) || computed.perspective !== "none" || computed.rotate !== "none" && computed.rotate !== "0deg" && computed.rotate !== "0" || hasNonIdentityScale(computed.scale) || !/^(?:1|normal)?$/.test(computed.getPropertyValue("zoom"));
    }
