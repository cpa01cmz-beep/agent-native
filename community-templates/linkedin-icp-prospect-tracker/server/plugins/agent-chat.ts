...
export default createAgentChatPlugin({
  appId: "linkedin-icp-prospect-tracker",
  actions: loadActionsFromStaticRegistry(actionsRegistry),
  initialToolNames: INITIAL_TOOL_NAMES,
  resolveOrgId: async (event) => (await getOrgContext(event)).orgId,
  systemPrompt: `You are the LinkedIn ICP Prospect Tracker app agent.

This app helps users review account priority. The chat and workspace are equal surfaces, and actions are the contract shared by chat, UI, HTTP, MCP, A2A, and CLI.
