const messages = {
  home: {
    settingsTitle: "設定",
    settingsDescription: "語言和代理偏好設定",
    title: "你的應用正在執行",
    intro: "從編輯這裡開始建置",
    actionCardTitle: "由操作支援的資料",
    loadingAction: "正在載入操作結果...",
    docsTitle: "檔案",
    docsDescription: "了解框架",
    themeTitle: "主題",
    themeDescription: "切換深色 / 淺色",
  },
  common: {
    cancel: "取消",
    confirmDisconnect: "確認中斷連線",
    disconnect: "中斷連線",
    disconnecting: "正在中斷連線…",
  },
  settings: {
    title: "設定",
    description: "此應用的語言和工作區偏好設定。",
    languageTitle: "語言",
    languageDescription: "選取介面語言。此偏好會儲存到你的帳戶。",
    languageLabel: "介面語言",
    agentTitle: "代理設定",
    agentDescription:
      "開啟代理側邊欄設定，管理模型、API 金鑰、自動化、語音和其他代理控制項。",
    openAgentSettings: "開啟代理設定",
    workspaceTitle: "工作區",
    workspaceDescription: "管理團隊存取和共用工作區資源。",
    openTeamSettings: "開啟團隊設定",
    openResourceSettings: "開啟資源設定",
    backHome: "返回首頁",
    emailChange: "變更電子郵件",
    emailChangeSent: "請查看電子郵件以確認此變更。",
    emailChangeError: "無法傳送確認信。",
    builderConnection: {
      manage: "管理 Builder.io 連線",
    },
  },
  dispatch: {
    nav: {
      chat: "聊天",
      overview: "概覽",
      apps: "Apps",
      metrics: "指標",
      vault: "Vault",
      integrations: "整合",
      agents: "Agents",
      resources: "Resources",
      messaging: "Messaging",
      destinations: "Destinations",
      identities: "Identities",
      approvals: "核准",
      automations: "自動化",
      delivery: "投遞",
      audit: "Audit",
      dreams: "Dreams",
      threadDebug: "對話串偵錯",
      settings: "設定",
      team: "團隊",
      operations: "Operations",
      navigation: "導覽",
      navigationDescription: "工作區導覽連結",
    },
    sidebar: {
      workspaceControlPlane: "工作區控制平面",
      workspaceSubtitle_other: "工作區 · {{count}} 個應用程式",
      chats: "聊天",
      newChat: "新聊天",
      newDispatchChat: "新 Dispatch 聊天",
      renameChat: "重新命名聊天",
      renameThread: "重新命名 {{title}}",
      chatOptions: "{{title}} 的聊天選項",
      aboutPage: "關於 {{title}}",
      emptyAgentText: "建立應用程式、管理保管庫金鑰以及在工作區中路由工作。",
      suggestionBuildApp: "為 X 建置工作區應用程式",
      suggestionRouteSlack: "路由 Slack 提及我的分析應用程式",
      suggestionGrantKey: "將我的 OpenAI 金鑰授予此應用程式",
    },
    pages: {
      browserChatUnavailableTitle: "瀏覽器聊天工作階段無法使用",
      browserChatUnavailableDescription:
        "請從 Agent-Native 瀏覽器擴充功能重新連線。",
      browserChatPlaceholder: "詢問此頁面…",
      browserChatAttachedPlaceholder: "詢問 {{page}}…",
      browserConnectTitle: "連接瀏覽器聊天",
      browserConnectDescription:
        "允許 Agent-Native Chrome 擴充功能開啟此 Dispatch 聊天工作階段。連線使用一次性的短期票證。",
      browserConnectInvalid: "此連線要求無效。請從擴充功能重新開始。",
      browserConnectConnected: "瀏覽器聊天已連線。你可以關閉此分頁。",
      browserConnectConnecting: "連線中…",
      browserConnectButton: "連線",
      browserConnectOpenFromExtension:
        "請從 Agent-Native Chrome 擴充功能開啟此頁面。",
      browserConnectFailed: "瀏覽器擴充功能未能連線。",
      appsDescription: "開啟工作區應用程式並從 Dispatch 開始建立新應用程式。",
      appsDescriptionWithWorkspace:
        "「{{workspace}}」工作區中的應用程式。每個應用程式都會在此工作區下取得自己的路由，並共用資料庫、驗證與代理聊天。",
      appsInWorkspace: "{{workspace}} 中的應用程式",
      workspaceApps: "工作區應用程式",
      otherApps: "其他應用程式",
      otherAppsDescription: "開啟你可以使用的其他應用程式。",
      availableCount: "可用 {{count}} 個",
      activeCount: "{{count}} 個作用中",
      hiddenCount: "{{count}} 個已隱藏",
      createApp: "建立應用程式",
      templates: "範本",
      checkingTemplates: "檢查可用範本",
      templatesAvailable: "{{count}} 個可用於產生架構",
      curatedTemplates: "精選範本",
      curatedTemplatesDescription:
        "從經過審核的應用程式結構開始，打造屬於你的應用程式。",
      remix: "從範本建立",
      remixTemplate: "建立 {{name}} 應用程式",
      remixAppIdLabel: "新應用程式 ID",
      remixStarted: "已開始建立 {{name}}",
      viewLiveApp: "查看線上應用程式",
      alreadyInWorkspace: "已在工作區中",
      remixAppIdDescription: "為新的工作區應用程式選擇 URL 安全的 ID。",
      integrationSetup: "整合設定",
      source: "來源",
      remixing: "正在建立應用程式…",
      remixSuccess: "已開始從範本建立應用程式。",
      remixError: "無法從此範本建立應用程式",
      appIdRequired: "應用程式 ID 為必填項目。",
      hide: "隱藏",
      show: "Show",
      hiddenApps: "隱藏的應用程式",
      hiddenAppCount: "{{count}} 個隱藏的應用程式",
      workspaceAppFallback: "工作空間應用程式",
      workspaceAppDescription:
        "開啟已部署的應用程式或檢查正在建立的應用程式的狀態。",
      pageNotFound: "頁面未找到",
      pageNotFoundDescription: "該路由尚未出現在工作區應用程式清單中。",
      browseApps: "瀏覽應用程式",
      newApp: "新應用程式",
      newAppDescription: "根據提示建立工作區應用程式並應用工作區保管庫策略。",
      destinationsDescription:
        "已儲存出站 Slack 頻道、Telegram 聊天和對話串目標。",
      savedDestinations: "已儲存的目的地",
      addDestination: "新增目的地",
      quickTestMessage: "快速測試訊息",
      send: "傳送",
      delete: "刪除",
      cancel: "取消",
      messageSent: "訊息已傳送",
      unableToSendMessage: "無法傳送訊息",
      destinationSaved: "目的地已儲存",
      destinationRemoved: "目的地已刪除",
      deleteDestinationTitle: "刪除目的地？",
      deleteDestinationDescription:
        "「{{name}}」將會被移除。任何以此目的地為目標的已儲存工作流程或工作，會在下次傳送時開始失敗。",
      noDestinations:
        "尚未儲存目的地。在右側新增您的第一個 Slack 頻道或 Telegram 聊天。",
      dailyDigestChannel: "每日摘要頻道",
      optionalThreadOrTopicId: "可選對話串或主題 ID",
      destinationNotes: "這個目的地應該用來做什麼？",
      saveDestination: "儲存目的地",
      noWorkspaceApps: "還沒有工作區應用程式",
      noWorkspaceAppsDescription:
        "當工作流程需要有自己的集中居住地時，建立一個應用程式。",
      appNotFound: "找不到應用程式",
      building: "Building",
      appBuildingPrefix: "該應用程式正在建立中。它將在",
      appBuildingSuffix: "在其分支合併且工作區部署完成後。",
      branch: "分支：{{branch}}",
      openBuilderBranch: "開啟建置器分支",
      openingApp: "正在開啟 {{name}}",
      redirectingTo: "重新導向至",
      openApp: "開啟應用程式",
      notDispatchOrWorkspaceApp: "此工作區中的 Dispatch 分頁或工作區應用程式。",
      agentsDescription:
        "預設情況下，Dispatch 可以透過 A2A 委托給內建應用程式套件。僅當您想要將工作路由到該內建集之外的應用程式時，才在此處新增額外的代理。",
      mcpAccessUpdated: "MCP 應用程式存取已更新",
      selectAppForMcp: "至少選取一個應用程式，或公開所有應用程式。",
      mcpUrlCopied: "MCP URL 已複製",
      mcpUrlCopyFailed: "無法複製 MCP URL",
      unifiedMcpGateway: "統一 MCP 閘道",
      unifiedMcpGatewayDescription:
        "將外部代理連線到 Dispatch 一次，然後透過以下方式路由到授權的工作區應用程式",
      allApps: "所有應用程式",
      selectedApps: "選定的應用程式",
      loading: "Loading",
      grantedCount: "已授權 {{count}} 個",
      exposeAllAppsMcp: "透過 Dispatch MCP 暴露所有應用",
      copyUrl: "複製 URL",
      metricsUnavailable: "指標不可用",
      unableToLoadUsage: "無法載入使用情況。",
      metricsDescriptionBuilder:
        "工作區範圍內的 Builder.io 信用支出、聊天量、使用者活動和應用程式存取。",
      metricsDescriptionLlm:
        "工作區範圍內的 LLM 支出、聊天量、使用者活動和應用存取。",
      llmCalls: "LLM 來電",
      activeUsers: "活躍使用者",
      workspaceAppsStat: "工作區應用程式",
      chatThreads: "聊天話題",
      also: "還有",
      workspaceShortcutsAria: "工作區快捷入口",
      deliveryQueue: "投遞佇列",
      failedLastHour: "過去一小時失敗 {{count}} 次",
      processingCount: "{{count}} 個處理中",
      queued: "排隊中",
      active: "進行中",
      done1h: "完成 1 小時",
      failed1h: "失敗 1 小時",
      oldestQueued: "最早排隊：{{age}}",
      queueAgeNone: "無",
      queueFailureHint: "請檢查憑證、目的地和最近的佇列錯誤。",
      unknownPlatform: "未知",
      attemptsCount: "{{count}} 次嘗試",
      noErrorMessage: "（無錯誤訊息）",
    },
  },
  agentPanel: {
    useBuilder: "使用生成器",
    openDesktopToEditCode: "開啟桌面編輯程式碼",
    codeUnavailableDescription:
      "Agent-Native 桌面應用程式中提供來源程式碼更改和 CLI 存取。",
    downloadDesktop: "下載桌面",
    chatMode: "聊天模式",
    chat: "聊天",
    cliTerminalMode: "CLI 端子方式",
    cli: "CLI",
    workspaceMode: "檔案、代理、技能和工作",
    workspace: "資源",
    newChat: "新聊天",
    newTerminal: "新航站樓",
    panelOptions: "代理面板選項",
    collapseSidebar: "收起側邊欄",
    widenChat: "將聊天視窗加寬",
    returnChatToLayout: "將聊天視窗放回版面配置",
    hideChats: "隱藏聊天紀錄",
    allChats: "所有聊天紀錄",
    settings: "設定",
    feedback: "意見回饋",
    exitFullscreen: "退出全螢幕",
    fullscreen: "Fullscreen",
    closeTab: "關閉分頁",
    closeOtherTabs: "關閉其他標籤頁面",
    closeAllTabs: "關閉所有標籤頁面",
    clearChat: "清除聊天",
    cliRequiresDevMode: "CLI 需要開發模式",
    cliRequiresDevModeDescription:
      "使用 pnpm dev 在本機執行此應用程式，或使用 Builder.io 存取 CLI 終端。",
    toggleAgent: "切換代理",
    keyScope: "金鑰範圍",
    personalKeyScope: "個人",
    organizationKeyScope: "組織",
    personalKeyInEffect: "目前使用的是您的個人金鑰。",
    organizationKeyInEffect: "目前使用的是組織金鑰。",
    sharedKeyInEffect: "目前使用的是共用金鑰。",
    useOrganizationKey: "改用組織金鑰",
    keyStatusUnavailable: "無法取得金鑰狀態。",
  },
  share: {
    titleWithResource: "分享「{{title}}」",
    titleWithType: "分享 {{type}}",
    owner: "擁有者：{{name}}",
    close: "關閉",
    shareOptions: "分享選項",
    link: "連結",
    invite: "Invite",
    embed: "Embed",
    done: "完成",
    generalAccess: "一般存取",
    shareLink: "分享連結",
    addPeopleByEmail: "透過電子郵件新增人員",
    notifyPeople: "通知人們",
    peopleWithAccess: "有存取權限的人",
    ownerRole: "Owner",
    remove: "移除",
    noAccess: "目前還沒有人可以存取。",
    embedUrl: "嵌入 URL",
    embedCode: "嵌入程式碼",
    copy: "複製",
    role: "Role",
    private: "私人",
    privateDescription: "只有有權存取的人才能檢視",
    organization: "組織",
    organizationDescription: "您組織中的任何人都可以檢視",
    public: "Public",
    publicDescription: "使用連結登入的任何人都可以檢視",
    viewer: "Viewer",
    viewerDescription: "可以檢視",
    commenter: "評論者",
    commenterDescription: "可以檢視並新增評論",
    editor: "Editor",
    editorDescription: "可以編輯",
    admin: "管理員",
    adminDescription: "可以編輯和管理存取權限",
  },
  runsTray: {
    runs: "執行",
    agentRuns: "代理執行",
    activeRun_other: "{{count}} 個作用中的執行",
    failedRun_other: "{{count}} 個失敗的執行",
    recentRuns: "最近的執行",
    noRecentRuns: "最近沒有執行",
    ariaAgentRuns: "代理執行，{{label}}",
    summaryRunning: "{{activeCount}} 個執行中",
    summaryRunningRecent:
      "{{activeCount}} 個執行中 · {{terminalCount}} 個最近項目",
    summaryRecent_other: "{{count}} 個最近執行",
    noTrackedWorkYet: "尚未跟蹤工作",
    emptyDescription: "背景代理工作在執行時和完成後將顯示在此處。",
    open: "開啟",
    stopRun: "停止 {{title}}",
    hideRun: "隱藏 {{title}}",
    statusRunning: "Running",
    statusDone: "完成",
    statusFailed: "失敗",
    statusStopped: "Stopped",
    updatedJustNow: "剛剛更新了",
    finishedJustNow: "剛剛完成",
    updatedMinutes: "{{count}} 分鐘前更新",
    finishedMinutes: "{{count}} 分鐘前完成",
    updatedHours: "{{count}} 小時前更新",
    finishedHours: "{{count}} 小時前完成",
    updatedDate: "已於 {{date}} 更新",
    finishedDate: "完成於 {{date}}",
  },
  codeRequired: {
    fallbackDetail:
      "在本機編輯或使用 Builder.io 在雲端中編輯此程式碼並繼續以您喜歡的方式自訂應用程式。",
    defaultFeature: "對此應用程式進行請求的程式碼更改",
    branchError: "建立分支失敗",
    title: "需要更改程式碼",
    subtitleWithFeature:
      "「{{feature}}」會建立或修改原始碼，因此需要從此介面使用 Desktop 或 Builder。",
    subtitle: "此操作建立或修改來源程式碼，需要此介面的 Desktop 或 Builder。",
    desktopTitle: "使用 Agent-Native 桌面",
    desktopDescription:
      "在桌面應用程式中開啟專案以啟用來源程式碼編輯和 CLI 存取。",
    builderAgentTitle: "使用 Builder.io 代理",
    builderAgentDescription:
      "讓我們的雲端代理為您進行更改。您將獲得預覽和部署的連結。",
    codeChangeTitle: "這需要更改程式碼",
    codeChangeBadge: "程式碼變更",
    connectBuilderTitle: "連線 Builder.io",
    connectBuilderDescription:
      "Connect Builder（提供免費方案）可從此應用程式啟用基於雲端的程式碼更改。",
    setupRequired: "需要設定",
    branchCreated: "已建立分支",
    close: "關閉",
  },
  extensions: {
    createPrompt: "建立擴充功能：{{prompt}}",
    createWidgetPrompt:
      "建立一個適合「{{slotId}}」插槽的新小工具。我接著會描述它應該做什麼。",
    whatShouldItDo: "它應該做什麼？",
    createPlaceholder: "待辦事項清單、API 儀表板、計算器...",
    title: "擴充功能",
    hiddenShown: "顯示隱藏",
    showHidden: "顯示隱藏項",
    newExtension: "新擴充功能",
    newExtensionTitle: "新擴充功能",
    buildPlaceholder: "描述您想要建置什麼...",
    namePlaceholder: "我的擴充功能",
    descriptionPlaceholder: "這個擴充功能有什麼作用？",
    contentPlaceholder: "<html>...</html>",
    emptyTitle: "建立您的第一個擴充功能",
    emptyDescription: "描述一個小應用程式，代理將建置它。",
    hiddenFromEveryone: "對所有人都隱藏",
    localFile: "本機檔案",
    optionsFor: "{{name}} 的選項",
    unhideEveryone: "為所有人取消隱藏",
    hideEveryone: "向所有人隱藏",
    localFileDescription:
      "此擴充功能由 {{entryPath}} 支援。請從工作區編輯或移除它。",
    deleteQuestion: "要封存 {{name}} 嗎？",
    removeQuestion: "要移除 {{name}} 嗎？",
    hideForYouDescription:
      "這會將其從您的擴充功能清單中隱藏，而不會為其他任何人刪除它。",
    removeEverywhereDescription: "這會在共用的所有地方封存它。",
    cancel: "取消",
    remove: "移除",
    removing: "正在移除...",
    delete: "封存",
    deleting: "正在封存...",
    openFullView: "開啟全檢視",
    removeFromWidgetArea: "從此小工具區域中刪除",
    customBlockSandboxed: "自訂區塊 · 沙盒",
    sandboxedCustomBlock: "沙盒 SQL 自訂區塊",
    sandboxedCustomBlockCreatedBy: "由 {{email}} 建立的沙盒 SQL 自訂區塊",
    promoteToAppCode: "升級為應用程式碼",
    historyShowsSourceVersions: "歷史記錄包含原始碼版本",
    createdByHistoryShowsSourceVersions:
      "由 {{email}} 建立。歷史記錄包含原始碼版本。",
    createdByHistoryShowsSourceVersionsCompact:
      "由 {{email}} 建立 · 歷史記錄包含原始碼版本",
    deleteExtensionEllipsis: "封存擴充功能...",
    removeFromMyListEllipsis: "從我的清單中刪除...",
    removeFromWidgetAreaForMe: "從這個小工具區域移除（只套用於我）",
    deleteEverywhereConfirmation: "這會為所有與之共用的人封存該擴充功能。",
    addWidget: "新增小工具",
    loading: "正在載入...",
    noWidgetsAvailable: "此插槽尚無可用的小工具。",
    buildNewWidget: "建置一個新的小工具",
    contactSidebarWidget: "聯絡側邊欄小工具",
    contactSidebarDescription: "顯示在目前對話旁邊，包含聯絡人和對話串脈絡。",
    eventDetailWidget: "活動詳情小工具",
    eventDetailDescription: "顯示在開啟的日曆活動下方，包含活動與出席者脈絡。",
    addWidgetHere: "在這裡新增小工具",
    back: "返回",
    editExtension: "編輯擴充功能名",
    saving: "正在儲存...",
    save: "儲存",
    create: "建立",
    moreOptions: "更多選取",
    appearsIn: "出現在",
    noWidgetAreas: "未安裝在任何小工具區域。請代理將其新增到某處。",
    widgetAreaCount_other: "此擴充功能可在 {{count}} 個小工具區域中呈現。",
    nameLabel: "名稱",
    descriptionLabel: "描述",
    contentLabel: "內容",
    previewTitle: "擴充功能預覽",
    previewEmpty: "預覽將出現在這裡",
    loadError: "無法載入擴充功能。",
    widgetsLoadError: "無法載入小工具。",
    widgetAreasLoadError: "無法載入小工具區域。",
    historyLoadError: "無法載入擴充功能歷史記錄。",
    versionLoadError: "無法載入此擴充功能版本。",
    retry: "重試",
  },
  org: {
    createTitle: "建立您的組織",
    createDescription:
      "該應用程式按團隊組織您的內容。建立一個組織以繼續 - 之後您可以邀請隊友。",
    askAdminTitle: "請聯絡管理員",
    askAdminDescription: "請聯絡管理員取得邀請，以加入此工作區。",
    loadErrorTitle: "無法載入組織",
    loadErrorFallback: "無法載入組織。",
    tryAgain: "重試",
    yourOrganization: "您的組織",
    joinYourTeam: "加入你的團隊",
    openToDomainEmails: "開放 @{{domain}} 電子郵件加入",
    joinOrg: "加入 {{name}}",
    pendingInvitations: "待處理的邀請",
    invitedBy: "來自 {{name}}",
    accept: "Accept",
    createSeparate: "或建立另一個組織",
    organizationName: "組織名稱",
    organizationPlaceholder: "Acme 有限公司",
    createOrganization: "建立組織",
    create: "建立",
    creating: "創造...",
    team: "團隊",
    createOrgCardTitle: "建立組織",
    createOrgCardDescription:
      "Set up a team to collaborate with your colleagues.",
    joinDomainOne: "與您的電子郵件域匹配的組織已存在。加入它與您的隊友協作。",
    joinDomainMany:
      "與您的電子郵件域匹配的組織已存在。加入其中，與您的隊友協作。",
    join: "Join",
    invitedByLabel: "由 {{name}} 邀請",
    you: "You",
    owner: "擁有者",
    admin: "管理員",
    member: "Member",
    members: "Members",
    inviteMembers: "邀請成員",
    appRolesOptional: "應用程式角色",
    noAppRolesFound: "找不到角色",
    notAssigned: "未分配",
    appPermissions: "應用程式權限",
    applications: "應用程式",
    applicationsEmpty: "尚未註冊工作區應用程式。",
    applicationsLoadFailed: "無法載入工作區應用程式。",
    applicationAccess: "{{name}} 的存取權限",
    applicationAccessAll: "所有成員",
    applicationAccessRestricted: "受限",
    applicationAccessDisabled: "已停用",
    resetToDefaults: "重設為預設值",
    invited: "已邀請",
    role: "角色",
    status: "狀態",
    actions: "操作",
    noMembers: "尚無成員。",
    memberCount_other: "{{count}} 位成員",
    memberPagination: "成員列表分頁",
    previousMemberPage: "前往上一頁成員",
    nextMemberPage: "前往下一頁成員",
    memberPageStatus: "第 {{page}} 頁，共 {{totalPages}} 頁",
    youAreRole: "你的角色是 {{role}}",
    changeRole: "改變角色",
    removeMember: "刪除成員",
    transferTo: "轉移所有權給",
    cancel: "取消",
    remove: "移除",
    save: "儲存",
    loading: "正在載入...",
    dangerZone: "危險區域",
    ssoConfirm: {
      title: "要求使用此登入提供者？",
      description:
        "成員下次登入時必須使用此提供者。目前的工作階段可能會遭到撤銷。",
      confirm: "要求使用提供者",
    },
    ssoSetup: {
      idpSetup: "身分提供者設定",
      redirectUri: "重新導向 URI",
      spMetadataUrl: "服務提供者中繼資料 URL",
      dnsRecordName: "DNS TXT 記錄名稱",
      dnsRecordValue: "DNS TXT 記錄值",
      dnsPropagation: "請等待 DNS 變更傳播後再驗證。",
    },
    sso: {
      signIn: "組織登入",
      signInHelp: "選擇成員必須使用的登入提供者。",
      requiredProvider: "必要的登入提供者",
      optional: "選用",
      google: "Google",
      title: "單一登入",
      description: "連結 OIDC 或 SAML 身分提供者。",
      verified: "已驗證",
      verifyRequired: "需要驗證",
      verify: "驗證",
      remove: "移除",
      type: "身分提供者類型",
      providerId: "提供者 ID",
      issuer: "簽發者 URL",
      domain: "組織網域",
      noDomain: "未設定網域",
      clientId: "用戶端 ID",
      clientSecret: "用戶端密碼",
      discoveryEndpoint: "探索端點（選用）",
      entryPoint: "SAML 進入點 URL",
      entityId: "IdP 實體 ID",
      metadata: "貼上 IdP 中繼資料 XML",
      certificate: "簽署憑證（選用）",
      domainHelp:
        "網域必須與組織已驗證的網域相符。提交後，密碼與中繼資料會從表單中清除。",
      saveProvider: "儲存提供者",
      cancel: "取消",
      addProvider: "新增提供者",
    },
    scim: {
      title: "目錄佈建",
      description: "管理此組織的 SCIM 連線。",
      revoke: "撤銷",
      copyTokenOnce: "請立即複製此權杖。它不會再次顯示。",
      dismissToken: "關閉權杖",
      createConnection: "建立 SCIM 連線",
    },
    deleteOrg: "刪除組織",
    deleteOrgDescription:
      "永久刪除此組織及其成員和待處理的邀請。組織擁有的資料將變得無法存取。此操作無法復原。",
    deleteOrgConfirmPrompt: "輸入 {{name}} 以確認。",
    deleteOrgConfirmPlaceholder: "組織名稱",
    deleteOrgConfirmCta: "刪除組織",
    deleteOrgPending: "正在刪除...",
  },
  integrations: {
    webhookUrl: "檢視：Webhook URL",
    copyWebhookUrl: "複製 Webhook URL",
    notConfigured:
      "Not configured. Set the required secrets to enable this integration.",
    enable: "Enable",
    disable: "Disable",
    toggling: "...",
    networkError: "到達伺服器時出現網路錯誤",
    back: "返回",
    agentEngineRequired: "需要代理引擎",
    agentEngineDescription:
      "請先連線 Builder.io 或 LLM 金鑰，{{platform}} 才能回覆。",
    openLlm: "開啟 LLM",
    setup: "Setup",
    shareDocumentsWith: "與以下人員共用檔案",
    copyServiceAccountEmail: "複製服務帳戶電子郵件",
    requiredSecrets: "所需的秘密",
    envHelp: "Set these in your .env file or environment to connect.",
    copy: "複製",
    documentation: "檔案",
    clientAvailable: "該代理的 A2A 端點自動可用。無需設定。",
    addChatIntegration: "新增聊天整合",
    chatIntegrations: "聊天整合",
    chatIntegrationsDescription: "從其他平台與該代理交談",
    addIntegration: "新增整合",
    dispatchEntrypoint:
      "對於可以跨多個應用程式路由工作的中央 Slack 或 Telegram 入口點，請使用",
    sharedMessaging:
      "需要一個工作區共用的訊息介面嗎？將 Slack 或 Telegram 連線到 Dispatch 應用程式，讓它透過 A2A 委派給其他代理。",
  },
  observability: {
    noData: "無可用資料",
    noConversations: "尚未紀錄任何對話",
    noEvals: "尚未紀錄評估結果",
    noExperiments: "尚未建立實驗",
    totalRuns: "總跑數",
    totalCost: "總成本",
    avgLatency: "平均延遲",
    toolSuccess: "工具成功",
    thumbsUp: "豎起大拇指",
    avgEvalScore: "平均評估分數",
    run: "Run",
    model: "模型",
    duration: "Duration",
    cost: "Cost",
    tools: "工具",
    time: "Time",
    failedCount: "（{{count}} 個失敗）",
    backToList: "返回清單",
    spans: "Spans",
    type: "型別",
    name: "名稱",
    tokens: "Tokens",
    status: "狀態",
    totalEvals: "總評估數",
    avgScore: "平均分",
    scoresByCriteria: "按標準評分",
    overview: "概覽",
    conversations: "Conversations",
    evals: "埃瓦爾斯",
    experiments: "實驗",
    feedback: "意見回饋",
    variants: "Variants",
    created: "Created",
    backToExperiments: "回到實驗",
    metrics: "指標",
    level: "Level",
    results: "結果",
    variant: "Variant",
    metric: "Metric",
    value: "Value",
    totalFeedback: "總意見回饋",
    thumbsDown: "大拇指朝下",
    frustration: "Frustration",
    thumbsUpRate: "點讚率",
    categories: "分類",
  },
  error: {
    genericTitle: "出了點問題",
    genericDetails: "發生了意外錯誤。",
    notFoundTitle: "頁面未找到",
    notFoundDetails: "此頁面不存在。它可能已被移動或刪除。",
    statusTitle: "{{status}} 錯誤",
    goHome: "返回首頁",
  },
};

export default messages;
