...
**Developer page.** This page is for platform engineers setting up repo governance — branching, code review, CI. It covers **governance**: who reviews, approves, and owns what across many apps in one repo. For the agent customization layer (Agent Resources: `AGENTS.md`, memory, skills, custom agents) see [Agent Resources](/docs/agent-resources); for the deployment shape (one monorepo, many apps) see [Multi-App Workspaces](/docs/multi-app-workspace).
...
```text filename=".github/CODEOWNERS"
...
| **Schema migrations**             | Platform team + senior engineer       | Data safety, PostgreSQL compatibility                                     |
...
For a new workspace, after running `npx --yes @agent-native/core@latest create`:
...
- [ ] Configure shared MCP servers — see [Multi-App Workspaces — Shared MCP servers](/docs/multi-app-workspace#shared-mcp)
...
- [**Audit Log**](/docs/audit-log) — the action-level audit trail that complements `git log`/`git blame`
