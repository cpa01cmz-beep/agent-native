---
title: "Content: Collections, Properties, and Forms"
description: "Build Notion-style collections inside Content, connect them to an external source with reviewable sync, and collect submissions through intake forms."
---

# Collections, Properties, and Forms

A Content collection is a Notion-style table where every row is itself a full document. This page covers building one, giving it structured columns, optionally connecting it to an outside source, and using it to collect submissions through a form. It's useful for anyone setting up a tracker, roadmap, or intake queue, with a few details near the bottom for developers connecting an external source.

## Collections are pages with rows

Turn any page into a collection (or ask the agent to "create a collection called Feature Requests") and it renders as a table where each row opens into its own document — a row page can hold its own rich content, sub-pages, comments, and version history, just like any other document. Row pages are reached from the collection view rather than the ordinary sidebar tree, so a collection with hundreds of rows doesn't clutter your page list.
...
Save as many named views as you want per collection — table, list, gallery, board, calendar, timeline, or form — each with its own sort, filters, grouping, hidden columns, and layout settings. A calendar or timeline view plots rows by a date property; a board view groups rows into columns by status or a select property, and dragging a card between columns updates that row's property.

## Connecting an external source

A collection can mirror rows from an outside source instead of (or in addition to) rows you create by hand: a Notion database, a Builder CMS model, or a local table in your own app. Attaching a source reads a snapshot of its fields and rows into Content. Nothing is deleted or overwritten sight unseen.

<Steps>

### Attach a source

Point the collection at a Notion database, a Builder CMS model, or a local table.

### Choose an attach mode

**Add more rows** turns the source's entries into new rows in your collection. **Add details** joins the source's fields onto your _existing_ rows by a shared key, for example matching a Notion database row to a local row by email or slug, adding read-only columns without duplicating rows.
...
Turn a collection into a form to collect submissions from people, Slack, or another app without anyone touching the collection directly. A form view has its own question order and required fields, so the same collection can back two different forms for two different audiences.
...
Deleting a collection moves it (and its rows) to Trash separately from ordinary page deletion, so you can browse and restore a removed collection without hunting through deleted pages one by one.

## Data model

<DataModel
  id="doc-block-content5"
  title="Collections and external sources"
  summary={
    "A collection is a document with property definitions and rows; an attached external source layers read snapshots, field mappings, and reviewable change sets on top without touching the row until it's approved."
  }
  entities={[
    {
      id: "content_databases",
      name: "content_databases",
      note: "A collection object attached to a document (the collection's own page)",
      fields: [
    ...
    },
    {
      ...
      note: "One row in a collection, linking it to its row document",
      fields: [
        { name: "id", type: "id", pk: true },
        {
          name: "database_id",
    ...
    },
    {
      ...
      note: "Column definitions for a collection",
      fields: [
        { name: "id", type: "id", pk: true },
    ...
    },
    {
      ...
      note: "One attached external source per collection (per attach call)",
      fields: [
        { name: "id", type: "id", pk: true },
        { name: "database_id", type: "id", fk: "content_databases.id" },
        {
...
- [Writing and organizing documents](/docs/template-content-editing) — the editor, page tree, and sharing model a collection's row pages build on
- [Local files, Notion, and Builder CMS sync](/docs/template-content-sync) — syncing whole documents, as opposed to collection rows
