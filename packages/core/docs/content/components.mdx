...
tool-call display. `<AgentConversation>` already keeps itself pinned to the
bottom while streaming — do not add your own scroll handling on top of it.
...
- [**Actions**](/docs/actions-overview) — `defineAction()`, schemas, and the action surface these components call
