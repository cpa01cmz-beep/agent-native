---
title: "Extensions"
description: "An optional toolkit capability for one-off, sandboxed custom blocks. App code remains the default for durable product behavior."
---

# Extensions

Extensions are an **optional toolkit capability for sandboxed custom blocks**.
They are disabled by default. A template must opt in before its agent can create
or manage them; Analytics uses this capability for explicitly requested,
one-off custom visualizations.

For durable behavior, edit the app code. A custom block is an agent-authored
patch: useful when speed and local specificity matter, but not the default way
to add a reusable product feature.
...
Extensions are disabled by default. To expose user-authored extensions, set
`extensionTools: true` on both `createAgentChatPlugin()` and
`createCoreRoutesPlugin()`. The first adds the agent-facing actions and prompt
guidance; the second permits authenticated creation through
`POST /_agent-native/extensions`. The sandbox runtime and existing read, edit,
and deep-link routes remain available for compatibility either way.
...
| `agentNative.chat.send(message, opts?)`          | Send a prompt or selected value back to the agent chat    | `agentNative.chat.send('Use Q2', { submit: true, context: { q: 2 } })` |
...
- **Use `agentNative.ui.output` for inline control values the agent should read later.** It writes `inline-ui:<extensionId>:output` to application state; `agentNative.chat.send` is draft-only unless `{ submit: true }` is passed for a user-triggered submit/apply message. Never call it from polling, refresh, or error handlers.
...
- **Use a custom block only when the app opts in** and the user explicitly wants
  ...
  surface. It is fast and requires no build or deploy, but remains a sandboxed
  agent-authored patch.
- **Promote durable custom blocks.** If a block becomes useful beyond its
  original one-off context, use **Promote to app code** to hand it to the normal
...
- [**Actions**](/docs/actions-overview) — the operations an extension calls via `appAction`
