...
`app-skill pack` also emits a standard Agent Plugin at the bundle root:
`plugin.json`, `mcp.json`, and the exported `skills/` directories. Import that
portable bundle into an Agent-Native workspace with:

```bash
npx @agent-native/core@latest plugin import ./dist/assets-skill --into . --yes
```

The importer namespaces Skills under `skills/<plugin>/<skill>`, maps remote
Streamable HTTP MCP servers into `mcp.config.json`, and ignores package headers
so they cannot become workspace credentials. Stdio and SSE servers are reported
and skipped. Authentication remains an app or host Connections concern.
