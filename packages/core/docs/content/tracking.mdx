...
`source` is either an action's `ctx`, passed straight through, or a
`{ userId, anonymousId, sessionId }` meta object for callers that have no `ctx`.

The browser session is not something the caller has to supply: it comes from the
ambient request context (`RequestContext.browserSessionId`, set from the
`X-Agent-Native-Session-Id` header the action client, chat, and client `track()`
all send), so the same visit correlates whether the UI or the agent triggered the
event. Each provider maps it to its own session field — `$session_id` for
PostHog, which joins the event to session replay; a `session_id` property for
Mixpanel and Amplitude; a top-level `sessionId` for webhooks and Agent-Native
Analytics. It is absent for callers with no browser (cron, CLI, MCP, A2A), and a
`sessionId` in the meta object overrides it.
...
export default defineAction({
  ...
  run: async ({ name, template }, ctx) => {
    ...
    // Pass ctx straight through: the event is attributed to ctx.userEmail and
    // joins the browser session the call came from.
...
- [**Actions**](/docs/actions-overview) — where most tracking calls originate
- [**Server Plugins**](/docs/server-plugins) — `registerBuiltinProviders()` runs in the core-routes plugin at startup
