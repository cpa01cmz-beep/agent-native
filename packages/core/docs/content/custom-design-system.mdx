...
and agent-panel surfaces are useful design-system proving surfaces. The main
Chat canvas remains tokens-only because the assistant-ui renderer is a
documented v1 non-goal. Dark mode can be exercised from the `Cmd/Ctrl+K`
Appearance command rather than a second settings section.
...
Keep the build-time theme in a dependency-light module so Vite can load its
configuration without evaluating the customer React package.
