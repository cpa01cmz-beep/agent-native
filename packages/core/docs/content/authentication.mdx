...
description: "Magic-link-first Better Auth integration with password fallback, social providers, organizations, and MCP bearer credentials."
---
...
For an administered, invite-only deployment, set the signup and organization
creation policies in [Administered Deployments](/docs/administered-deployments).
This is separate from domain auto-join. A verified allowed domain can admit a
person to an organization, while an invitation is an explicit membership offer.

## Overview {#overview}

Auth is configured automatically via `autoMountAuth(app)` in the auth server plugin. There are three modes:

- **Default:** Magic-link-first Better Auth when outbound email is ready, with email/password fallback and social providers. Onboarding page shown on first visit.
  ...
  <div class="diagram-col">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >magic link / password &middot; Google &middot; GitHub</small
      >
    </div>
    <div class="diagram-card">
...
| **Production**   | Magic-link signup/login when outbound email is ready; email/password fallback otherwise | Required for magic links (when an email provider is configured) |
...
## Local development sign-in {#local-development-sign-in}

On a loopback development server, the shared sign-in page puts **Continue as
local dev** first. It creates or reuses the real Better Auth development
account and session, so this is the same authenticated path used elsewhere.

Choose **Show full sign in options** to reveal normal signup, password, magic
link, and social-provider options. If the local session cannot be created, the
page reveals those options automatically so you can continue with standard
authentication.

This convenience is available only when the browser connects through
`localhost`, `127.0.0.0/8`, or `::1` in development or test. It is disabled for
deploy previews and hosted production. It is not an authentication bypass.
Set `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` to opt out and show the normal
sign-in page in local development.

## Better Auth (Default) {#better-auth}

By default, Better Auth powers authentication. When outbound email is ready,
magic-link signup/login is the primary browser flow. Email/password remains
the fallback when email is unavailable, or when `AUTH_MAGIC_LINK=0` is set. It
provides:

- Magic-link registration and login, with email/password fallback
...
In an invite-only deployment, admission is checked before a Better Auth user
is created. The policy covers email/password, magic-link, and social sign-up.
See [Administered Deployments](/docs/administered-deployments#close-signup-and-organization-creation)
for the bootstrap-admin and allowed-domain rules.

- `GET /_agent-native/auth/session` — get current session
- `POST /_agent-native/auth/magic-link` — request a magic-link email
- `POST /_agent-native/auth/login` — email/password fallback login
...
The signed Agent-Native Desktop SSO Canary composes hosted federation into one
workspace sign-in for canonical first-party apps. Dispatch remains the identity
authority, while every app mints and owns its normal local session. Electron
keeps the authority session in a dedicated partition and copies only the target
app's allowlisted session cookie into `persist:app-<id>`; provider credentials,
identity tokens, and another app's cookies never enter renderer IPC.

The Dispatch-owned `desktop.workspace-sso` feature flag is default-off and
controls availability only. Missing, malformed, unreadable, and explicit-Off
state preserve ordinary per-app sign-in and sign-out. Nonce binding, signed
exchange validation, exact origin and callback allowlists, authenticated-session
requirements, session isolation, and revocation remain unconditional in every
flag state. Custom and third-party apps are excluded.
Canonical hosted apps recognize packaged Desktop requests without per-app
identity-hub environment configuration. Ordinary browsers stay on normal
per-app sign-in. Self-hosted apps need `AGENT_NATIVE_IDENTITY_HUB_URL` only for
the remaining explicit Desktop or server-side federation path.
...
Local development and tests skip password-signup email verification by default,
so the email/password fallback can be tested without waiting on an inbox. To
force verification locally while testing that fallback, set
`AUTH_SKIP_EMAIL_VERIFICATION=0`.
...
When this flag is set, password fallback signup does not require email
verification and the signup verification email is not sent. It does not change
magic-link delivery. Use it only for QA or preview environments, and name test accounts with a `+autoz` address
(`name+autoz@example.com`) so they are easy to identify.
...
When an anonymous viewer hits this URL, the framework's login page is served. After a successful sign-in (magic link, email/password, token, or Google OAuth), the browser returns to `return`.
...
Normal app pages use one impersonal, public-cacheable SSR shell for every visitor. `AppProviders` checks the session on the client; on a private path such as `/dashboard`, it redirects an anonymous visitor to `/sign-in?return=%2Fdashboard` before mounting private app UI. After sign-in, the visitor returns to `/dashboard`. Pass `isPublicPath` for public/SEO routes, or `sessionBypass` for a surface that authenticates through another scoped mechanism such as an MCP embed token.

How long that shell is cached is a deployment-wide setting — see [SSR Caching](/docs/ssr-caching) for `AGENT_NATIVE_SSR_CACHE`. Shortening or disabling the cache changes duration only; SSR still renders the anonymous branch with cookies stripped.

### Behind the scenes: Google OAuth

Both flows (the explicit `/sign-in` entrypoint and the bookmarked-path case) thread the return URL through the OAuth state. The legacy `/_agent-native/sign-in` alias remains accepted for older links. The state is HMAC-signed, so it can't be forged in transit. On the callback, the return URL is re-validated as same-origin before the redirect — so a leaked signing key still can't be turned into an open-redirect oracle.
