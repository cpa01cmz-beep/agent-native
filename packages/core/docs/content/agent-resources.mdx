...
description: "Claude-Code-level customization per user — skills, memory, instructions, custom agents, scheduled jobs, MCP servers — backed by PostgreSQL/PGlite, not a filesystem."
---
...
The twist: **it's PostgreSQL/PGlite rows, not filesystem files.** Each user gets their own set of agent resources stored in the database. There's no dev-box to spin up, no container per user, no files to mount. A multi-tenant SaaS can give every user a fully-customizable agent for essentially free, because all of it is rows — personal memory, personal MCP servers, personal skills, personal sub-agents — and the shared codebase hosts all of them at once.

<Diagram id="doc-block-13zbuxe" title="A Claude-Code setup, but stored as agent resources in PostgreSQL/PGlite" summary={"The same customization layer — instructions, skills, memory, agents, jobs, MCP — except every file is a row in a shared multi-tenant database."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-Native resources</span
    ><small class="diagram-muted">rows in one PostgreSQL/PGlite database</small>
    <div class="ws-rows">
...
| Claude Code / Codex              | Agent-Native resources                             |
| -------------------------------- | -------------------------------------------------- |
| Files on your local disk         | Rows in a shared PostgreSQL/PGlite database        |
...
| Per-project `CLAUDE.md` / skills | Per-app `AGENTS.md` + memory resources             |
...
The in-app Resources tab shows all three scopes. Personal and shared/organization resources are editable there. Workspace-scope resources are read-only in app panels and edited centrally from Dispatch, so every app sees the same canonical files without a sync step.
...
1. Open the **Resources** tab → **Shared** → `AGENTS.md` (create it with `+` → **File** if missing).
2. Add one rule, e.g.:
...
Click the `?` icon in the Resources toolbar to jump back to these docs at any time.
...
### Agent scratch vs workspace visibility {#agent-scratch}

Every resource has a `visibility`: `workspace` (the normal, user-visible kind shown in the Resources tab) or `agent_scratch` (hidden working files the agent creates for itself — draft notes, intermediate results, a script it's iterating on). Agent writes default to `agent_scratch` unless the path is a durable instruction/skill/job/memory path, so the agent can work with temporary files without cluttering the Resources tab.

Scratch resources auto-expire — 24 hours after creation unless the agent sets its own `expiresAt` — so abandoned working files don't accumulate. Use `action: "promote"` (personal or shared scope only, not `workspace`/`all`) to turn a scratch file into a permanent workspace resource once the user actually wants to keep it, e.g. "save that as a real file." Promote keeps the same path and content and clears the expiry.

| Visibility      | Shown in Resources tab? | Expires?                            |
| --------------- | ----------------------- | ----------------------------------- |
| `workspace`     | Yes                     | No                                  |
| `agent_scratch` | No (hidden by default)  | Yes — 24h unless `expiresAt` is set |
...
Agent resources have two current memory surfaces:
...
| `mcp-servers/…`    | Workspace / shared | Humans via Dispatch or app resources | MCP config refresh                     |

Users can edit these memory files directly in the Resources tab — they're regular resources. Delete lines the agent got wrong, keep personal preferences in `memory/MEMORY.md`, or promote team-wide rules into `AGENTS.md`.
...
<DataModel
  ...
  title="The agent resource model"
  summary="One resource shape backs every agent resource file. The runtime keys it by path and scope and resolves the effective value on read."
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "A single file in a user's agent resources — instructions, skill, memory, agent, MCP config, or job.",
      fields: [
        {
          name: "path",
          type: "string",
...
Skills are Markdown resource files under the `skills/` path (preferably `skills/<name>/SKILL.md`) that give the agent on-demand domain knowledge, invoked in chat with `/skill-name`. Add them from the Resources tab or, in Code mode, from `.agents/skills/`.
...
Agent resources include two agent types:
...
| ------------------ | ------------------------------------------------------------------- | -------------------------------------------------- |
| @ tagging          | Codebase files + agent resources + custom agents + connected agents | Agent resources + custom agents + connected agents |
| / slash commands   | .agents/skills/ + resource skills                                   | Resource skills only                               |
| Agent file access  | Filesystem + resources                                              | Resources only                                     |
| Resources tab      | Full access                                                         | Full access                                        |
| AGENTS.md / memory | Available                                                           | Available                                          |

## Workspace Connections {#workspace-connections}

Workspace Connections let apps share the same provider account (Slack, GitHub, HubSpot, etc.) without duplicating credentials. A connection records provider identity, account labels, status, scopes, app grants, and credential references in PostgreSQL/PGlite. Secrets stay in the credential store; connections only point at credential key names such as `SLACK_BOT_TOKEN`.
