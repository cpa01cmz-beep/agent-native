...
`connectorCatalog` decides which actions external callers can see. Which of them are _key_ is not a second list: the MCP `instructions` and the page's WebMCP manifest advertise `Key tools for this app: …` generated from the plugin's `initialToolNames` (override with `mcp.keyToolNames`), and the in-app `AGENTS.md` action table names the same actions in the same words — see [WebMCP](/docs/webmcp).
...
- [WebMCP](/docs/webmcp) — page-local tools, the key-tools manifest line, and the browser tab id header
