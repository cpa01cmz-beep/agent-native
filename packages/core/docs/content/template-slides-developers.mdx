  ...
  entities={[
    {
      id: "decks",
      name: "decks",
      note: "Slides live as JSON in data, with ownableColumns",
      fields: [
        {
          name: "id",
          type: "text",
    ...
    },
    {
      id: "design_systems",
      name: "design_systems",
      note: "Reusable brand tokens, with ownableColumns",
      fields: [
        {
          name: "id",
          type: "text",
...
- `request-deck-access`, `approve-deck-access-request`, `get-deck-access-status` — the private-deck access-request flow: a visitor without access requests it, the owner approves or denies from the app or the `/access-request/approve` route, and either side can poll the current status
...
- `get-layout-overflows` — read browser-measured slide-fit results, so an edit that no longer fits its layout can be caught and revised
...
- `import-google-slides-reference` — import a Google Slides deck as a reusable reference deck, rather than pulling its content into the working deck
...
- `create-design-system`, `update-design-system`, `delete-design-system`, `get-design-system`, `list-design-systems`, `set-default-design-system`, `apply-design-system` (link one to a deck)
- `get-workspace-defaults`, `set-workspace-defaults` — the workspace-wide default deck and design system. Setting one is admin-only
- `analyze-brand-assets`, `import-document`, `import-from-url`, `import-code` — gather raw brand signals (website CSS/fonts, uploaded document or code file contents) for a **local**, agent-drafted design system
- `index-design-system-with-builder`, `sync-design-system-with-builder`, `import-design-project` — the Builder-indexed path for Figma files, connected code/GitHub repos, and `design.md`. `import-design-project` starts from an existing design system rather than a raw source
...
- `get-deck-reference-context` — extract an existing deck's visual language as a reusable pattern library
...
| `/home`                          | Deck list, the private app home                                        |
| `/deck/:id`                      | The editor                                                             |
...
| `/access-request/approve`        | Approve a request for access to a private deck                         |
| `/agent`, `/extensions`, `/team` | Redirect into the relevant `/settings` tab                             |
...
<AnnotatedCode
  ...
  annotations={[
    ...
    {
      lines: "29-37",
      label: "Fail loud, don't return quietly",
      note: '`fail(...)` throws a structured error instead of returning a plain value. This action used to return `{ ok: false, message: "Text not found" }` for a bad match, which a caller could too easily read as success. Throwing is the only channel that reliably says the deck was not modified.',
    },
    {
      lines: "39-51",
      label: "Surgical edit",
      note: "A `find`/`replace` pair changes only the matched text, and now fails loudly on no match too. `fullContent` swaps the whole slide. Prefer the narrower form for real edits.",
    },
    {
      lines: "53-57",
...
The real action also validates that exactly one edit mode was provided, snapshots a version before writing, and writes with an optimistic-concurrency check so a stale read can never silently overwrite a newer one, on top of recording Creative Context reuse provenance and waiting briefly for the editor to report a layout-overflow measurement. This example trims all of that down to the core access, lock, read-modify-write, notify shape.
...
Most editor UI customization happens under `app/components/editor/`: the slide canvas, toolbar, sidebar, bubble menu, slash menu, and the history panel.
...
`templates/slides/AGENTS.md` is the short router the agent reads on every conversation. It points at the skills under `.agents/skills/` and lays out the core rules, persistence model, and application-state contract. Update the relevant skill whenever you add or change a slide layout or design-system source.

## What's next

- [**Features**](/docs/template-slides-features) — the user-facing editing, presenting, sharing, design-system, and import/export flow this data model backs
- [**Talk to the Agent**](/docs/template-slides-agent) — prompts by task and what the agent sees
- [**Slides in a Multi-App Workspace**](/docs/template-slides-integrations) — how these actions get called from outside Slides
