---
title: "Content: Local Files, Notion, and Builder CMS Sync"
...
# Local Files, Notion, and Builder CMS Sync
...
This starts normal database-backed Content and registers an opaque local-folder connection in `agent-native.json` — it does not switch the app into a separate local-only mode. See [Local folder sources](/docs/template-content-local-files) for the manifest format, truth policies, and Desktop bridge details.
...
Builder CMS models can also be attached as read-only inline-collection sources — see [Collections, properties, and forms](/docs/template-content-databases#connecting-an-external-source) for that workflow, which is a separate path from pulling/pushing a single document.
...
## Spaces and the Files collection underneath every sync surface

Every document belongs to a **space** — your personal space, or one shared with your organization — and every space owns one canonical Files collection. A local folder, once connected, is a source attached to that Files collection rather than a separate storage system, which is why imported files show up as normal pages with the same properties, sharing, and search as anything you typed by hand.
...
- [Writing and organizing documents](/docs/template-content-editing) — the editor and sharing model these synced documents use
- [Collections, properties, and forms](/docs/template-content-databases) — syncing rows inside a collection, rather than whole documents
- [Local folder sources](/docs/template-content-local-files) — full configuration, truth policies, and conflict handling reference
