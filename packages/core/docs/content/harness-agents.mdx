...
`installPackage` hint (for example `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` throws a clear install
...
For sandboxed built-in harnesses, pass a sandbox provider through
`agentOptions.sandbox` when registering them. The provider must implement
`resumeSession` for rehydration after a process restart; a caller-owned
`sandbox` session only covers initial creation.
...
## Codex auth: Code UI vs harness adapters {#codex-auth}

There are two Codex surfaces:

- **Agent-Native Code / Desktop** runs `codex exec` on the user's machine and
  reuses the local Codex CLI login reported by `codex login status`.
- **`ai-sdk-harness:codex`** uses the upstream AI SDK Harness native
  subscription flow where supported. No extra setting is required: credentials
  stay on the host, explicit API-key credentials take precedence, and the
  resulting auth is bridged into the sandbox without copying the local auth
  file. Set `harnessOptions.auth` to `"ai-gateway"` to use AI Gateway only;
  that mode never reads a native subscription.

The built-in `ai-sdk-harness:codex` adapter only supports
`permissionMode: "allow-all"`, because that is the stable Codex runtime's only
accepted mode. Omit `permissionMode` to use that default;
`allow-reads` and `allow-edits` are rejected for this adapter.
...
`permissionMode`, `harnessOptions`, and `agentOptions`). Use
`listAgentHarnesses()` to enumerate what is registered for a picker.
...
[Human Approval](/docs/actions-access-control#needs-approval) for the approval surface.
...
- [Human Approval](/docs/actions-access-control#needs-approval) — the approval surface harness runs use.
