...
description: "Multi-user collaborative editing where the AI agent is a first-class peer: CRDT merging, live presence, SSE fast-path, and granular server-side merge — on PostgreSQL/PGlite and any host."
---
...
- **PostgreSQL-backed** — Shared collaboration state uses PostgreSQL, including
...
The table is framework-managed and uses PostgreSQL. Each row carries an
...
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Postgres on a publicly reachable host**                      | The gateway dials the database from its own network. PGlite, `localhost`, `*.internal` and bare IP addresses are refused before the connection string ever leaves your machine. | `DATABASE_URL` points at a managed Postgres such as Neon                                                                                                                               |
...
The app posts its PostgreSQL connection string and public origin, gets back a channel id
...
**Self-registration means sending Builder your PostgreSQL connection string.**
...
Builder key, an unavailable PostgreSQL connection, or an unreachable gateway all leave the
