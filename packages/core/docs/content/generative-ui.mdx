...
**Developer page.** This page is for developers building agent-generated inline UI. For the end-user experience of chat, see [Using Your Agent](/docs/using-your-agent).

</Callout>

Ask for something like "give me sliders to tune these thresholds" or "let me pick from these options," and the agent writes and renders that control directly inside the chat transcript — no page reload, no separate app.
...
For generated extensions, chat messages are draft-only unless `submit: true` is
passed explicitly. Use that opt-in only from a user-triggered Apply or Submit
action. Do not call either helper from polling, refresh, or error handlers; use
`agentNative.ui.output` or application state for passive results.
...
| `submit`      | `false` | `true` auto-submits; `false` pre-fills/reviews   |
...
- [Actions](/docs/actions-overview) — the app/API surface generated UI should call.
