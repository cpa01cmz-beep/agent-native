...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `DATABASE_URL`                                 | Primary PostgreSQL connection. Local development falls back to PGlite when it is unset. Production also accepts `<APP_NAME>_DATABASE_URL` or Netlify's `NETLIFY_DATABASE_URL`. | A PostgreSQL URL, or unset for local PGlite.                                                                            |
...
persistent PostgreSQL URL. Workspace deployments can use the already-required
`A2A_SECRET` to derive the Better Auth secret and trusted internal signing
keys.

Local development intentionally uses PGlite when no database URL is present
and generates a Better Auth secret when no local secret is configured,
persisting it at `<app>/.agent-native/dev-auth-secret` so local sign-in
survives dev-server restarts. Those
...
The prefix form is supported for database URLs and app-local secret
