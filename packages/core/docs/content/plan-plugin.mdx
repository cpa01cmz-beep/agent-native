---
title: "Plan plugin and marketplace"
description: "Install the Agent-Native Plan skills (/visual-plan, /visual-recap, /visualize-repo) plus the hosted Plan MCP connector as a Claude Code or Codex plugin, or with the universal CLI. How updates work and whether you need to submit anything."
---

# Plan plugin and marketplace

The Agent-Native **Plan** app ships as one installable bundle. A single
install adds the Plan slash-command skills **and** wires up the hosted Plan
MCP connector (a remote tool server your agent talks to), so the agent can
generate plans and publish them straight into the Plan app in one step. This
page is for anyone choosing an install route, troubleshooting an install, or
deciding whether to submit Plan to a curated plugin catalog — see
[Visual Plans](/docs/template-plan) for what the skills actually do.

## What you get {#what-you-get}

One install gives you:

- **Three skills** — `/visual-plan` (the canonical entry point), `/visual-recap`, and `/visualize-repo`.
- **The Plan MCP connector** — registered against the hosted app at `https://plan.agent-native.com` (MCP endpoint `https://plan.agent-native.com/mcp`, server name `plan`).

<Diagram id="doc-block-i1sn3n" title="Three routes, one bundle" summary="The universal CLI, Claude Code plugin, and Codex plugin all install the same Plan skills plus the hosted Plan connector.">
...
By default, `/visual-plan` and `/visual-recap` publish to the hosted Plan app —
they create a plan via the MCP connector and hand you a link or inline plan to
review. They never dump an inline Markdown/ASCII plan into chat as the deliverable. If a Plan tool
...
<Diagram id="doc-block-zctmzc" title="Hosted vs. local-files mode" summary={"By default skills publish through the connector; local-files mode writes MDX to disk and previews via a localhost bridge instead."}>

```html
<div class="diagram-modes">
  <div class="diagram-card">
    <span class="diagram-pill accent">Default · hosted</span
    ><strong>Publish to the Plan app</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">Local-files privacy</span
    ><strong>Write MDX to disk</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No DB writes until
...
Works for any host — Claude Code, Codex, Cursor, Cline, Goose, ChatGPT custom MCP apps, Claude Cowork, and anything else MCP-compatible. The Agent-Native CLI installs the Plan skills, registers the hosted Plan MCP connector, **and runs auth for the selected local client(s) in the same step**, so your first tool call does not hit an OAuth wall:

```bash
npx @agent-native/core@latest skills add visual-plan
```

This installs `visual-plan` plus the companion `visual-recap` and `visualize-repo` skills, then registers the `plan` connector, then runs auth (OAuth prompt for hosted/account-backed sharing). Useful flags:
...
`/plugin install` adds the Plan skills and a **URL-only** MCP config (no secrets in the package); `/mcp` → **Authenticate** completes the OAuth handshake. Use the universal CLI route instead when you want local-files or self-hosted mode.
...
After install, **start a new Codex thread** so the skills and MCP tools load into the session. The plugin ships a URL-only connector (`[mcp_servers.plan]` → `https://plan.agent-native.com/mcp`); `codex mcp login plan` runs the OAuth flow. The universal CLI route above also works for Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) if you prefer one command that installs and authenticates together, or when you want local-files or self-hosted mode.
