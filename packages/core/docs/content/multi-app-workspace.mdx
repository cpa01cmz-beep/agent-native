...
## Standalone or workspace? {#standalone-or-workspace}

Choose a **workspace** when one company operates several internal apps that
should share deployment configuration, organization context, and team
administration. A workspace can also deploy its apps independently when
separate hostnames or release schedules are needed.

Choose a **standalone app** when you are building one independent app, such as
a one-off customer-facing tool or a prototype. Standalone is a supported
deployment shape. It does not provide the shared workspace package, root
configuration, or multi-app conventions. It can still use the framework's
organization and access features.

For a single-org internal deployment, start with a workspace and configure
invite-only access in [Administered Deployments](/docs/administered-deployments).
...
You have two options: **unified deploy** (the default for workspaces) or per-app independent deploy. [Workspace Deployment — One Origin, Many Apps](/docs/workspace-deployment#workspace-deploy) covers the full command, per-preset details (Cloudflare, Netlify, Vercel), and a diagram of the shape; the tradeoff:
