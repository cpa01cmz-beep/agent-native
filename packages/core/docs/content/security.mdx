...
The signed-in session carries `email` and (when an org is active) `orgId`. The framework establishes request context from that session and enforces ownership with `accessFilter()` and `assertAccess()` on application reads and writes. Agent SQL receives temporary filtered views for the active identity and organization. RLS is not enabled yet, so raw application queries must still use these helpers. The same enforcement contract applies whether the operation comes from the UI, an action, or the agent — the agent cannot read data for an org the user isn't a member of.
...
The `db-query` / `db-exec` tools reject schema-qualified table references (`public.<table>`) — a qualified name resolves to the base table and would bypass the temporary view above. Agents use bare table names; scoping is applied automatically.
...
Google Drive `changes.watch` notifications don't go through that generic adapter gate — they're opaque requests with no body, so the framework registers a random per-channel token and verifies the `X-Goog-Channel-ID`, `X-Goog-Channel-Token`, and `X-Goog-Resource-ID` headers before pulling changes. These are direct Drive notifications, not Pub/Sub messages, and they do not carry Pub/Sub OIDC bearer tokens.
...
- [**Organizations, Teams & Permissions**](/docs/organizations-teams-permissions) — the org-role vs. resource-role RBAC distinction and the org REST route table
