---
title: "Content: Writing and Organizing Documents"
...
If a commented passage gets edited later, the highlight follows the surrounding text instead of snapping to a fixed position. If the exact text is ever deleted, the comment thread still stays reachable from the sidebar instead of disappearing.

</Callout>

## Version history

Content keeps a snapshot of a page every time it changes significantly. Open a page's version history to see prior snapshots and restore one.

<Callout tone="warning">

Restoring replaces the current content with that version's content wholesale. There's no line-by-line diff or partial restore, so review the snapshot before you restore it.
...
Documents are **private by default** — only you can see a page you create. To share one, open the share dialog, or ask the agent to "share this doc with the team as an editor":

<Steps>

### Set who can see it

Change visibility to **org** so anyone in your organization can open it, or **public** so anyone with the link can open it at a read-only public URL.

### Grant a role

Grant specific people, or your whole org, a **viewer**, **editor**, or **admin** role.

### Optionally hide it from search

Make an org-visible page discoverable-off: it stays open to anyone with the link, but drops out of org-wide search and the sidebar list. Use this for pages that exist for reference but shouldn't show up in everyday browsing.
...
- [Collections, properties, and forms](/docs/template-content-databases) — turning a page into a structured table
- [Local files, Notion, and Builder CMS sync](/docs/template-content-sync) — editing the same pages from outside the app
