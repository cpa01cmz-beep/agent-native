...
The Analytics template's dev script pins `DATABASE_URL` to local PGlite at
`pglite:./data/pglite`, so a production `DATABASE_URL` in `.env` will not be
used during local development. To intentionally point local dev at another
PostgreSQL database, run `ANALYTICS_DATABASE_URL=<your dev database url> pnpm dev`.
