...
Consumer permission. Version 1.0 is the GA JSON-RPC endpoint; use its
`.../agents/{agent}/endpoint/protocols/a2a/agentCard/v1.0` card URL when available. Version 0.3 is the preview
endpoint returned when no version is selected. Pass `protocolVersion` to
`A2AClient` only when the card omits it. Foundry v1 does not provide SSE
streaming, so the client uses `message/send`. The token provider must obtain
and refresh an Entra access token. `A2A_SECRET` and a static API key do not
satisfy Foundry authentication.
...
and its managed assistants expose a standard A2A JSON-RPC endpoint. The
endpoint is under the assistant resource, for example
`.../assistants/default_assistant/agents/{id}/a2a`, while the agent card can be
published at a custom URL. Use the generic bearer client and pass that card URL:
...
The caller needs a Google OAuth bearer with the `discoveryengine.assist`
permission. Custom A2A-agent registration is Pre-GA, so confirm availability in
the target project before exposing it in a workspace picker. See [invoke a
Gemini Enterprise agent](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)
for the resource URL and IAM requirements.

### Anthropic Managed Agents {#anthropic-managed-agents}

Anthropic Managed Agents does not expose an inbound A2A endpoint. Use the
framework adapter when Agent-Native should own the A2A surface, workflow state,
and approvals while the hosted agent owns its session, context, and tools. The
adapter creates a Managed Agents session, sends `user.message` events, and reads
the session event stream. A blocking `agent.tool_use` or `agent.mcp_tool_use`
event becomes an A2A `input-required` task and an `approval-request` runtime
event. Resume it with explicit structured confirmations in the returned
metadata; never infer approval from a message:
...
`credentialRef` is a vault reference to the Anthropic API key. The adapter
resolves it in the caller's user and organization scope and sends the required
`anthropic-beta: managed-agents-2026-04-01` and `anthropic-version: 2023-06-01`
headers. Register `anthropic-managed-agents` in the workspace connection
catalog to reuse the existing connection grants for each app. The adapter is
mounted on the normal `A2AConfig.handler` seam:
