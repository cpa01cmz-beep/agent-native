...
| `useAppRole()`           | The current user's assigned roles in one app, or an empty array when unassigned.                |
| `useSetAppMemberRoles()` | Replace one member's app-role set. Org owner/admin only, enforced server-side.                  |
...
export function CoachOnlyControl() {
  ...
  return roles.includes("coach-admin") ? (
...
export function RoleAdmin() {
  ...
  const setRoles = useSetAppMemberRoles();
  if (!data?.canManage) return null;
  return (
    <button
      type="button"
      onClick={() =>
        setRoles.mutate({ appId: "coach", email: "dana@acme.com", roles: [] })
      }
    >
      Clear role
    </button>
...
`roles`, `myRoles`, and `canManage` are progressive disclosure only. The server
...
for the role model and [Actions → Authorization](/docs/actions-access-control#authorize) for
