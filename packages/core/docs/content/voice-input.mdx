...
<div class="diagram-voice">
  ...
  <div class="diagram-card col">
    ...
    <small class="diagram-muted">default when Builder connected</small>
...
Set `GEMINI_API_KEY`, `GROQ_API_KEY`, or `OPENAI_API_KEY` as an environment variable, or have a workspace admin add it in Settings → API Keys (stored as a shared, encrypted workspace secret). Every user's transcription hits the shared key.
...
The rest of this page is the transcription route's request/response contract and how to customize or replace it — useful if you're building a custom input surface or an on-prem provider, not required to use voice input day to day.
