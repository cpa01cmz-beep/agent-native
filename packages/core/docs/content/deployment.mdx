---
title: "Deployment"
description: "Deploy an Agent-Native app or workspace to a Nitro-compatible platform with a persistent PostgreSQL database."
...
Every deployed app needs a persistent PostgreSQL database. Local development can use
the PGlite database at `data/pglite`, but container, preview, and serverless
...
Use a hosted PostgreSQL provider such as [Neon](/docs/neon),
[Supabase](/docs/supabase), Railway, Render, [Amazon RDS for PostgreSQL](/docs/aws-rds),
[Cloud SQL for PostgreSQL](/docs/cloud-sql), or [Azure Database for PostgreSQL](/docs/azure-postgresql).
Use the [Plain Postgres](/docs/postgres) guide for other managed services and
self-hosted PostgreSQL servers. The framework configures PostgreSQL from the
connection URL.
