...
- **`type`** — `"content"` (default) keeps the work in the embedded app agent, which operates through app tools unless its host is intentionally granted workspace and write tooling. `"code"` routes to a separate code-capable host (for repository source changes, see [Agent-Native Code UI](/docs/code-agents-ui)).
...
Keep follow-up and revision prompts in the existing sidebar thread. Do not add
a second freeform input beside the generated result. A small local form or
popover is appropriate for structured values the UI must validate, but the
conversation itself belongs in the AgentSidebar composer.
...
[Agent Chat](/docs/client-agent-chat#sendtoagentchat) for the MCP App bridge behavior.
...
## Choosing agent chat versus an action {#agent-or-action}

Use `useActionQuery` or `useActionMutation` directly for deterministic local
operations such as CRUD, validation, provider reads, and persistence. If the
user experiences the workflow as research, analysis, generation,
recommendation, synthesis, or other multi-step reasoning, start it with
`sendToAgentChat({ openSidebar: true })` and let the agent orchestrate focused
actions. Deterministic implementation alone is not a reason to hide an
AI-shaped workflow inside one opaque action.
...
Type-safe arguments come from the zod schema in your `defineAction()`. See [Actions](/docs/actions-overview) for the full action system.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` and `useActionMutation()`
