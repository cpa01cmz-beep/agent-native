...
Retrieval itself has no vector-database dependency to run or pay for. Full-text search runs in local PGlite and hosted Postgres. Hosted Postgres can also build a pgvector-backed semantic index automatically and blend its ranking with full-text results - no separate service or configuration flag is required.
...
- [**Actions**](/docs/actions-overview) — the action system Brain is built on
