...
Chat ships with no `server/db/schema.ts` at all (see [What ships](/docs/template-chat#what-ships)), so the first change is a new file, following the same `table`/`text`/`ownableColumns` shape every other template uses:
  ...
  annotations={[
    {
      ...
      note: "`table`/`text`/`ownableColumns` come from `@agent-native/core/db/schema`, the PostgreSQL schema helpers every template uses.",
    },
    {
      lines: "13",
...
- [**Chat: Developer Guide**](/docs/template-chat-developers) — quick start, actions, core tables, auth, and the admin pages
- [**Actions**](/docs/actions-overview) — the action system chat and UI both call
