...
This walks through running Content from nothing installed to a live page you
can edit and ask the agent about. The commands are identical whether you're
in Terminal on macOS or PowerShell/Command Prompt on Windows, they're plain
Node.js tooling.
...
The app opens at `http://localhost:8083`. If it doesn't open automatically,
visit that URL in your browser.

### Sign in and connect an LLM

On the Welcome screen, select **Continue as local dev**. You don't need an
email or password for local development. Then connect an LLM: select
**Connect Builder.io** for free credits, or **Custom keys** to add an
Anthropic/OpenAI key or a local Ollama model.

</Steps>

Content provisions your personal space and its Files collection automatically,
so once an LLM is connected you can click **+ New page** and start writing,
...
Content's schema (`server/db/schema.ts`) currently defines 22 tables, not the nine a much earlier version of this doc described. Nine cover the document lifecycle, collaboration, and sync surfaces below; the other 13 belong to the inline-collection and external-source subsystem, documented in full on [Collections, properties, and forms](/docs/template-content-databases#data-model).

| Table                                      | What it holds                                                                                            |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `documents`                                | The page tree — title, markdown content, description, icon, parent/position, favorite, visibility, space |
| `content_spaces`                           | A personal or org workspace; owns one canonical Files collection                                         |
...
| `document_property_definitions`            | Column definitions for inline collections _(see Collections page)_                                       |
| `content_databases`                        | Inline collection objects _(see Collections page)_                                                       |
| `content_database_items`                   | Collection rows _(see Collections page)_                                                                 |
| `content_database_body_hydration_queue`    | Queues a body-content backfill pass for source-imported rows that need one                               |
| `content_database_sources`                 | An attached external source _(see Collections page)_                                                     |
| `content_database_source_fields`           | Field mappings _(see Collections page)_                                                                  |
| `content_database_source_rows`             | Row identity/provenance _(see Collections page)_                                                         |
| `content_database_source_change_sets`      | Proposed sync changes _(see Collections page)_                                                           |
| `content_database_source_change_reviews`   | Review decisions _(see Collections page)_                                                                |
| `content_database_source_executions`       | Guarded write-back attempts _(see Collections page)_                                                     |
| `content_database_source_execution_claims` | Idempotency keys so a retried write-back can't double-write                                              |
| `document_property_values`                 | Per-row property values _(see Collections page)_                                                         |
| `document_block_field_contents`            | Content for secondary Blocks fields _(see Collections page)_                                             |

<DataModel
  id="doc-block-content9"
  title="Documents, collaboration, and sync"
  summary={
    "documents is the page tree; every other table here hangs off it for spaces, versions, comments, Notion sync, Builder sidecars, and sharing. The inline-collection and external-source subsystem is a separate, larger block on the Collections page."
  }
  entities={[
    {
      id: "documents",
    ...
    },
    {
      id: "content_spaces",
      name: "content_spaces",
      note: "A personal or org workspace; owns one canonical Files collection",
      fields: [
        { name: "id", type: "id", pk: true },
...
| `_app.favorites.tsx`                                                                                               | Redirects into the current space's Favorites Files-collection document                                                                                       |
...
### Data grid boundary {#data-grid}

Content's collection table view composes the reusable `DataGrid` component from
`@agent-native/toolkit/data-grid`. The adapter is
`app/components/editor/database/DatabaseView.tsx`.

Toolkit supplies the shared spreadsheet mechanics: keyboard navigation,
active-cell behavior, the scroll surface, and the optional selection and column
width callbacks. Content supplies `ContentDatabaseItem` rows and typed
columns, then keeps collection-specific property rendering, grouping, filters,
row drag and drop, previews, access checks, and action-backed mutations in the
app.

The table preserves its existing Content surface through the grid's custom
slots and scroll-container props:

```tsx filename="app/components/editor/database/DatabaseView.tsx"
import { DataGrid, type DataGridColumn } from "@agent-native/toolkit/data-grid";
import type { ContentDatabaseItem } from "@shared/api";

const dataGridColumns: readonly DataGridColumn<ContentDatabaseItem>[] = [
  // Content maps its name, property, and action columns here.
];
...
The live implementation also supplies Content's custom `renderHeader` and
`renderBody` callbacks around this shell.

Do not move Content collection models, access checks, grouping, drag and drop,
or domain actions into Toolkit. If a change is reusable spreadsheet behavior,
extend the Toolkit contract first. If it describes how Content rows behave,
keep it in the app adapter and its actions.
...
<AnnotatedCode
  ...
  annotations={[
    {
      lines: "18-20",
      label: "System collections are protected",
      note: "A collection created by the framework itself (Files, Favorites, workspace catalog) carries a systemRole and refuses deletion here, so a stray delete call can't orphan a space's canonical storage.",
    },
    {
      lines: "24-31",
      label: "Reuse the document trash, don't reimplement it",
      note: "trashDocumentSubtree is the same helper delete-document uses. A collection's row documents get the identical soft-delete/restore semantics as any other page subtree instead of a second, collection-specific trash implementation.",
    },
    {
      lines: "33",
...
- **`app/components/editor/database/DatabaseView.tsx`** — the Content collection
  table adapter. It composes `@agent-native/toolkit/data-grid` while keeping
  collection-specific rendering and actions local.
...
- [Writing and organizing documents](/docs/template-content-editing) — the non-developer view of the editor and sharing model
- [Collections, properties, and forms](/docs/template-content-databases) — the full inline-collection and external-source data model
- [Local files, Notion, and Builder CMS sync](/docs/template-content-sync) — the non-developer view of every sync surface
- [Database](/docs/server-database) — the framework's Drizzle conventions and migration rules
- [Actions](/docs/actions-overview) — the action system every operation above is built on
