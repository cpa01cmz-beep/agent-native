...
`drizzle-kit push` against production. See [Database](/docs/server-database#migrations)
...
- [**Database**](/docs/server-database) — migrations and `getDb`/`schema`
- [**Actions**](/docs/actions-access-control#access-control) — the action system powering every entry above
