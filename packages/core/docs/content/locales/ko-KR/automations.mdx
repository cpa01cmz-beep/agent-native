...
**자동화**는 일정에 따라 또는 이벤트가 발생할 때 에이전트가 실행하는 작업입니다: _X가 발생하면 Y를 수행_. **Scheduled**와 **Event**는 두 가지 트리거 유형이며 별도 제품이 아닙니다.

프레임워크는 원래 예약 자동화를 **Recurring Jobs**로 제공했습니다. 이 이름과 `jobs/<name>.md` 경로, `manage-jobs` 도구는 호환성을 위해 유지됩니다. 새 가이드에서는 **Automations**를 포괄적인 이름으로 사용합니다. 기존 예약 작업, 조직 범위 또는 고급 필드는 [Recurring Jobs](/docs/recurring-jobs)를 참조하세요.

<Diagram id="doc-block-5aye3j" title={"X가 발생하면 Y를 수행하세요."} summary={"버스에서 이벤트가 발생하고 선택적 자연어 조건이 이를 게이트하며 에이전트는 전체 도구 액세스 권한으로 자동화 본체를 실행합니다."}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">에이전트가 body 실행</span
    ><small class="diagram-muted"
...
전체 Agent 페이지에서 **Automations**를 열어 예약 및 이벤트 트리거 자동화를 확인하고 관리합니다. 호환성 URL은 계속 `/agent#jobs`이므로 기존 링크와 북마크가 작동합니다.

<Callout tone="warning">

`manage-automations`는 Scheduled 및 Event 트리거의 개인·조직 자동화를 지원합니다. 조직 Event 자동화는 구성원에게 표시되지만 항상 생성자 권한으로 실행되며, 관리자는 그 실행 ID를 변경하지 않고 관리할 수 있습니다.
...
<AnnotatedCode
  id="doc-block-1apahpj"
  title={"이벤트로 트리거되는 자동화"}
  ...
  annotations={[
    {
      lines: "2",
      label: "크론 없음",
      note: '이벤트 트리거는 `schedule`을 `""`로 설정하므로 cron 필드는 비어 있습니다.',
    },
    {
      lines: "4-5",
      label: "방아쇠",
      note: "`triggerType: event`에 `event` 이름을 더하면 이 자동화가 버스에 등록됩니다.",
    },
    {
      lines: "6",
      label: "게이트",
      note: "발송 전 페이로드에 대해 Haiku가 평가하는 선택적 자연어 `condition`입니다.",
    },
    {
      lines: "12",
      label: "서버측 비밀",
...
| `mode`        | `"agentic"`               | `"agentic"`  | 전체 에이전트 루프(유일하게 지원되는 모드). `"deterministic"`는 제거되었습니다 — 구현된 적이 없으며, 이를 설정한 자동화는 실행된 적이 없습니다. |
...
이벤트 메타데이터의 `owner`가 실행 가능한 자동화를 결정합니다. 개인 및 조직 Event 자동화 모두 `createdBy`와 정확히 일치해야 합니다.
...
개인 및 조직 자동화는 `action` 매개변수와 함께 `manage-automations`를 사용합니다.
...
`manage-automations`는 조직 범위, 모델 재정의, 엄격히 적용되는 MCP 허용 목록을 지원합니다. `manage-jobs`는 기존 schedule-only 통합과 전달 메타데이터를 위한 호환성 도구로 유지됩니다.
...
<Endpoint id="doc-block-1mmnbgv" title={"테스트 이벤트 실행"} method="POST" path="/_agent-native/automations/fire-test" summary={"이벤트로 트리거되는 자동화를 검증하려면 test.event.fired 이벤트를 내보냅니다."} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-12c8gw1" title={"파견 경로"} summary={"실행된 이벤트부터 완료된 에이전트 실행까지 소유권 범위와 자연어 조건에 따라 제어됩니다."}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> body를 프롬프트로, payload를 컨텍스트로, 5분
      타임아웃</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
