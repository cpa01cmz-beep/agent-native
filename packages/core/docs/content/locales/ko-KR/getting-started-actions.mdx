...
에이전트가 채팅에서 이 액션을 실행하면 내장된 `data-chart` 렌더러가 구조화된 결과를 트랜스크립트 안의 차트로 변환합니다. 렌더러 파일을 만들거나 `app/root.tsx`에 임포트할 필요가 없습니다. 사용자 지정 네이티브 컴포넌트는 [Native Chat UI](/docs/native-chat-ui)에서 확인하세요.
