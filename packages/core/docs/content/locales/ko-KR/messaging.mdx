...
> **디스패치 템플릿을 사용하십니까?** 이 모든 것이 **관리 → 메시지**에 연결되어 있습니다. 각 플랫폼을 연결하려면 클릭하세요. 자신만의 템플릿을 사용자 정의하거나 구축하지 않는 한 이 페이지의 나머지 부분을 읽을 필요가 없습니다. [Dispatch](/docs/dispatch) 또는 [Dispatch template reference](/docs/template-dispatch)를 참조하세요.
...
- **메시지에서 다른 도구를 실행.** @mention 한 번으로 Notion의 행, Jira의 이슈, HubSpot의 레코드를 만들 수 있습니다. 전체 예제는 [Slack의 @mention → Notion의 행](#slack-to-notion)을 참고하세요.
- 단방향 인앱 알림(종 모양 아이콘, webhooks)은 [Notifications](/docs/notifications)를 참조하세요.

<Diagram id="doc-block-1hdzoku" title={"많은 채널, 하나의 에이전트"} summary={"모든 플랫폼은 동일한 에이전트 루프와 동일한 SQL 스레드 기록을 사용하므로 Slack DM 메시지과 이메일은 동일한 대화를 계속합니다."}>
...
<div class="msg-fanin">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">하나의 에이전트 루프</span
    ><small class="diagram-muted">같은 메모리 · 같은 도구</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    하나의 SQL 스레드 기록<br /><small class="diagram-muted"
...
- **채널 멘션** — 작업을 시작할 때 @mention을 사용합니다. 에이전트가 처리할 각 채널 메시지에는 명시적인 멘션이 필요하며, 일반 스레드 답글과 채널 대화는 에이전트를 실행하지 않습니다.
...
관리형 OAuth가 권장 설정입니다. **관리 → 메시징**에서 **Agent manifest**를 선택해
...
`app_home_opened`, `app_context_changed`, `app_mention`, `message.im`을 구독하세요.
`SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, `SLACK_SIGNING_SECRET`을
설정한 뒤 **관리 → 메시징**에서 **Add to Slack**을 선택합니다. 설치별 토큰은
...
각 채널 메시지에는 에이전트에 대한 명시적인 멘션이 필요하며, 일반 스레드 답글과
채널 대화는 에이전트를 실행하지 않습니다. 제한된 컨텍스트에는 리액션과 파일 참조가 포함되고 네이티브
...
### 전체 예제: Slack의 @mention에서 Notion의 행까지 {#slack-to-notion}

Slack 멘션이 동작하기 시작하면 거의 항상 나오는 다음 요청이 _"스레드에서 에이전트를 @mention하면 내가 쓴 내용을 Notion 데이터베이스에 행으로 넣어 달라"_ 입니다. 이미 가능한 일입니다. Slack 수신, Notion 공급자 API, 앱 액션이 모두 기본 제공됩니다. 다만 세 조각을 올바른 순서로 연결해야 하고, 그중 둘은 건너뛰면 아무 말 없이 실패합니다.

아래의 `<ANGLE_BRACKETS>`는 모두 자리 표시자입니다. 실제 Notion 시크릿을 소스, 커밋되는 `.env`, 채팅 메시지에 붙여 넣지 마세요.

#### 1. 멘션이 앱까지 도달하게 하기

Notion을 건드리기 전에 [Slack 설정](#slack)을 마치고 채널에서 `@mention`에 답변이 오는지 확인하세요. 이후 단계에서는 두 가지 동작이 중요합니다.

- `@mention`이 포함된 메시지가 호출이며, 그 텍스트가 에이전트에게 전달됩니다.
- 같은 스레드에 나중에 남기는 각 메시지에도 에이전트에 대한 명시적인 멘션이 필요합니다. 일반 답글은 도달하지 않으므로 새 요청마다 다시 @mention하세요.

#### 2. 공급자 API 액션 노출하기

Core에는 Notion 공급자 프리셋(기본 URL, `Notion-Version` 헤더, 인증, 문서 링크)이 포함되어 있지만, 에이전트가 이를 사용할 수 있게 해 주는 세 액션 — `provider-api-catalog`, `provider-api-docs`, `provider-api-request` — 은 **선택적 하위 경로 내보내기**입니다. 이를 추가한 적이 없는 앱에는 Notion으로 가는 경로가 전혀 없고, 에이전트는 Notion 도구가 없다고 답합니다. 대부분이 놓치는 단계입니다.
...
그다음 액션 ID마다 얇은 액션 파일을 추가합니다. 팩토리 옵션은 모두 선택 사항이라 각 파일은 호출 한 줄이면 됩니다.
...
`actions/`의 파일은 자동으로 탐지되므로 편집할 레지스트리가 없습니다. 공급자 목록을 좁히거나 HTTP로 노출하려면 [Core 액션 팩토리 재사용](/docs/integrations#provider-api-factories)을 참고하세요.

#### 3. Notion 키는 `.env`가 아니라 보관소에 저장하기

**[notion.so/my-integrations](https://www.notion.so/my-integrations)**에서 내부 통합을 만들고 시크릿을 복사하세요. 그런 다음 앱에서 **Settings → API Keys & Connections**를 열고 이름이 정확히 `NOTION_API_KEY`인 키를 추가한 뒤 값을 붙여 넣고 범위를 선택합니다.

- **Personal** — 본인 자격으로 실행되는 작업만 사용할 수 있습니다.
- **Workspace** — 조직과 공유됩니다. 본인 외의 사람이 에이전트를 @mention할 예정이라면 이 옵션을 선택하세요. Slack 실행은 메시지를 작성한 사람이 아니라 통합 소유자로 동작하기 때문입니다.

공급자 런타임은 암호화된 보관소에서 개인 → 조직 → 워크스페이스 순서로 자격 증명을 해석하며, **의도적으로 `process.env`를 전혀 읽지 않습니다**. 배포 환경 변수는 멀티테넌트 앱의 모든 사용자가 공유하게 되기 때문입니다. `.env`에 `NOTION_API_KEY` 줄을 넣어도 여기서는 사용되지 않습니다.

#### 4. 데이터베이스를 통합과 공유하기

Notion에서 대상 데이터베이스를 열고 **••• → Connections →** 에서 통합을 추가하세요. 이 단계를 건너뛰는 것이 Notion에서 가장 흔하고 가장 헷갈리는 실패입니다. API가 403이 아니라 **404**를 돌려주기 때문에 데이터베이스 ID가 틀린 것처럼 보입니다. 카탈로그 항목에 이 해결 안내가 들어 있고, `provider-api-request`는 401, 403, 404 응답의 `guidance` 필드에 그것을 덧붙입니다.

데이터베이스 ID는 데이터베이스 URL에 있습니다. `?` 앞의 32자 문자열입니다.

#### 5. 에이전트에게 만들어 달라고 하기

어떤 설정 화면도 알려 주지 않는 부분이 이것입니다. 두 절반을 직접 연결하는 대신 원하는 동작을 요청하면 됩니다. 앱의 에이전트 채팅에서:

> 공급자 API를 통해 Notion 데이터베이스 `<NOTION_DATABASE_ID>`에 행을 추가하는 `add-notion-row` 액션을 만들어 주세요. 메시지 본문, Slack 작성자, Slack 메시지로 돌아가는 링크를 받아야 합니다. 그런 다음 `AGENTS.md`를 업데이트해서, 누군가 Slack에서 @mention으로 기록하거나 저장해 달라고 하면 그 액션을 호출하고 새 Notion 페이지 URL로 답하도록 해 주세요.

에이전트가 작성해야 하는 것은 **액션 하나**입니다. Nitro 라우트도, 브라우저에서 보내는 `fetch`도 아닙니다.
...
이 샘플에서 핵심적인 세 가지는 다음과 같습니다.

- 최신 Notion에서는 행이 데이터베이스 안의 **데이터 소스**에 붙으므로, 먼저 데이터베이스를 읽고 그 `data_sources[0].id`를 부모로 사용하세요. 데이터베이스 ID를 곧바로 부모로 넘기는 것은 예전 API 형식입니다.
- `executeRequest`는 공급자 오류를 던지지 않고 반환하므로 `response.ok`를 직접 확인하고 반환된 `guidance`를 함께 전달하세요. "데이터베이스를 공유하라"는 힌트가 거기에 담깁니다.
- 속성 이름(`Name`, `Author`, `Source`)은 데이터베이스 열과 정확히 일치해야 하고 유형도 맞아야 합니다. 추측하게 두지 말고 에이전트가 먼저 데이터베이스의 `properties`를 읽게 하세요.

`AGENTS.md` 부분은 선택 사항이 아닙니다. 이것이 없으면 에이전트는 도구를 갖고 있어도 Slack 메시지가 "이거 기록해"라고 할 때 사용할 이유가 없습니다.

#### 6. 확인하기, 그리고 안 될 때 볼 곳

에이전트가 있는 Slack 채널에 `@YourAgent 이거 Notion에 기록해 줘: <SOME_TEXT>`라고 올리세요. 성공한 실행은 스레드에 새 페이지 URL로 답하고, Notion에 행이 생깁니다.

안 될 때는:

| 보이는 증상                                               | 확인할 곳                                                                                                                    |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Slack이 전혀 답하지 않음                                  | 멘션이 앱에 도달하지 못했습니다. [Slack 설정](#slack)의 Request URL과 서명 시크릿을 다시 확인하세요. Notion 문제가 아닙니다. |
| 에이전트가 답하지만 Notion 도구가 없다고 함               | 2단계. 공급자 API 액션이 `actions/`에 추가되지 않았습니다.                                                                   |
| `notion credential not configured. Tried: NOTION_API_KEY` | 3단계. 키가 `.env`에 있거나 실행 주체와 다른 계정의 **Personal**로 저장되어 있습니다. **Workspace**로 저장하세요.            |
| Notion이 `404`를 반환                                     | 4단계. 데이터베이스가 통합과 공유되지 않았습니다.                                                                            |
| Notion이 속성 관련 `400`을 반환                           | 액션의 속성 이름이나 유형이 데이터베이스 열과 일치하지 않습니다.                                                             |
| 한 번은 됐는데 이후 스레드 답글이 무시됨                  | 정상 동작입니다. 에이전트가 처리할 각 메시지에는 명시적인 멘션이 필요합니다. 다시 @mention하세요.                            |
...
- **구성이 아닌 클릭.** Dispatch의 **관리 → 메시징** 페이지에는 env-var 필드가 내장된 모든 플랫폼에 대한 연결 버튼이 있습니다.
...
<Diagram id="doc-block-179fw12" title={"인바운드 웹훅 수명 주기"} summary={"웹후크는 200을 확인하고 대기열에 추가하고 반환하기만 합니다. 새로운 함수 실행은 대기열을 비우고 에이전트 루프를 실행하며 60초 재시도 작업을 안전망으로 사용합니다."}>

```html
<div class="msg-flow">
  <div class="msg-row">
    <div class="diagram-node">
      Platform<br /><small class="diagram-muted">Slack · 이메일 · 등</small>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>
      <strong>/webhook</strong><br /><small class="diagram-muted"
        >verify signature + parse</small
      ><br /><span class="diagram-pill">INSERT 대기 작업</span>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<Endpoint id="doc-block-1czvts8" method="POST" path="/_agent-native/integrations/slack/webhook" summary={"Slack 이벤트 API 인바운드 웹훅"} auth={"HMAC-SHA256 of the raw body using `SLACK_SIGNING_SECRET`, checked against the `X-Slack-Signature` header. In production also gated by `SLACK_ALLOWED_TEAM_IDS` / `SLACK_ALLOWED_API_APP_IDS`."} params={[
  {
