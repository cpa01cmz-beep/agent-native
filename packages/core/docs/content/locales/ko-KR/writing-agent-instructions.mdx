...
<Diagram id="doc-block-z104n2" title={"작성된 표면 3개 + 런타임 표면 1개"} summary={"AGENTS.md 및 도구 설명은 매 턴마다 로드됩니다. 수요에 따른 기술 부하; application_state는 UI를 통해 실시간으로 작성됩니다."}>

```html
<div class="diagram-surfaces">
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">모든 턴</span><strong>AGENTS.md</strong
    ><small class="diagram-muted"
      >지도: 목적, 핵심 규칙, 상태 키, action + skills 색인</small
    >
  </div>
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">모든 턴</span><strong>도구 설명</strong
    ><small class="diagram-muted">도구 선택 유도 — 각각 정확한 한 문장</small>
  </div>
  <div class="diagram-card ondemand" data-rough>
    <span class="diagram-pill">요청 시</span><strong>기술</strong
    ><small class="diagram-muted">상세 가이드, 설명이 맞을 때 로드</small>
  </div>
  <div class="diagram-card runtime" data-rough>
    <span class="diagram-pill ok">Live</span><strong>application_state</strong
    ><small class="diagram-muted">UI가 작성: 탐색, 선택, 포커스</small>
  </div>
</div>
```
...
- **기술 인덱스** — 존재하는 skills 목록과 각각을 읽는 시기.
...
- Data lives in Drizzle 기반 SQL. Use actions for all writes.
...
`AGENTS.md`는 프레임워크가 기반으로 삼는 보편적인 표준이므로, 프로젝트 지침을 읽는 모든 AI 코딩 도구가 별도 설정 없이 바로 작동합니다:

- **Claude Code**: `CLAUDE.md`를 읽습니다(CLI 설정에 의해 `AGENTS.md`에 대한 심볼릭 링크로 생성됨)
- **Cursor**: `AGENTS.md`를 직접 읽거나, 있는 경우 `.cursorrules`(Cursor의 레거시 위치)를 읽습니다
- **Windsurf**: `AGENTS.md`를 직접 읽거나, 있는 경우 `.windsurfrules`(Windsurf의 레거시 위치)를 읽습니다
- **Codex, Gemini 및 기타**: 내장된 에이전트 패널을 통해 작동합니다
- **Builder.io**: 시각적 편집과 공동 작업을 지원하는 클라우드 호스팅 에이전트입니다
...
├── SKILL.md            # 규칙 + 행복한 경로 + do/don't
└── references/
    └── csv-format.md   # 전체 열 사양, 인코딩, 엣지 케이스
```

이것은 항상 로드되는 표면을 작게 유지하고 컨텍스트를 부풀리지 않고 깊이 확장을 가능하게 합니다. 전체 스킬 형식은 [기술 Guide](/docs/skills-guide)를 참조하세요.
...
## 기술 대 actions {#skills-vs-actions}

기술 및 actions는 상호보완적입니다. 스킬은 에이전트가 읽는 지침입니다. 와
...
| 기술 (`SKILL.md`)            | 귀하(개발자)       | 상담원이 해당 기술이 관련성이 있다고 판단하는 경우 | 특정 패턴에 대한 단계별 방법, 나열하거나 나열하지 않음 |
...
- **기술** — 요청 시 로드되는 특정 패턴에 대한 재사용 가능한 방법입니다. 앱에서 작업하는 모든 사람에게 적용됩니다.
- **메모리(`memory/MEMORY.md`)** — 작성된 지침이 아닌 사용자별 기본 설정 및 수정 사항입니다.

## 다음 단계 {#whats-next}

- [기술 Guide](/docs/skills-guide) — 스킬 파일 형식, 프레임워크 skills 및 앱 지원 skills.
