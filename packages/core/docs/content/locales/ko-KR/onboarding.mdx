...
<Diagram id="doc-block-uafnku" title={"설정 체크리스트"} summary={"AI 엔진 연결만 필요합니다. 패널은 완료를 추적하고 필요한 모든 작업이 완료되면 자동으로 숨겨집니다."}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span><strong>AI 엔진 연결</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>이메일 전달</strong
...
<Endpoint id="doc-block-ie2msr" title={"온보딩 단계 나열"} method="GET" path="/_agent-native/onboarding/steps" summary={"등록된 모든 단계를 완료 상태와 함께 나열합니다."} responses={[
  {
    "status": "200",
...
<AnnotatedCode
  id="doc-block-19uc21d"
  title={"사용자 정의 온보딩 단계 등록"}
  ...
  annotations={[
    {
      lines: "5",
      label: "자동 마운트",
      note: "Nitro 플러그인에서 등록하세요. 프레임워크가 렌더링, 완료 추적 및 해제를 처리합니다.",
    },
    {
      lines: "7",
      label: "안정적인 ID",
      note: "기본 로드 후 동일한 `id`을 다시 등록하면 기본 제공 단계가 재정의됩니다.",
    },
    {
      lines: "12-19",
      label: "기본 방법",
    ...
    },
    {
      lines: "20-26",
      label: "대리인 경로",
      note: '`kind: "agent-task"`는 프롬프트와 함께 설정 작업을 에이전트 채팅에 넘깁니다.',
    },
    {
      lines: "28-31",
      label: "완료 확인",
      note: "`isComplete`은 서버측에서 실행됩니다. OAuth 토큰은 `oauth_tokens` 저장소에 있습니다. `process.env.GMAIL_REFRESH_TOKEN`가 아니라 확인하세요.",
    },
  ]}
/>
...
| `database` | 아니요     | 호스팅 PostgreSQL URL 또는 로컬 PGlite용 빈 값 |
