---
title: "에이전트 리소스"
description: "Claude-사용자별 코드 수준 사용자 정의 — skills, 메모리, 지침, 사용자 정의 에이전트, 예약된 작업, MCP 서버 — 파일 시스템이 아닌 PostgreSQL/PGlite가 지원합니다."
---

# 에이전트 리소스

모든 에이전트 기반 앱에는 **에이전트 리소스**, 즉 에이전트를 나만의 것으로 만드는 사용자 정의 레이어가 함께 제공됩니다. 여기에는 팀 지침(`AGENTS.md`), 공유 학습(`LEARNINGS.md`), 개인 구조적 메모리(`memory/MEMORY.md`), 에이전트가 요청 시 가져오는 skills, 사용자 정의 하위 에이전트, 예약된 작업 및 연결된 MCP 서버 등 Claude 코드/Codex 설정에서 기대할 수 있는 모든 것이 포함되어 있습니다.

비틀림: **파일 시스템 파일이 아니라 PostgreSQL/PGlite 행입니다.** 각 사용자는 데이터베이스에 저장된 자신만의 에이전트 리소스를 얻습니다. 가동할 개발 상자도 없고, 사용자당 컨테이너도 없고, 마운트할 파일도 없습니다. 멀티 테넌트 SaaS는 개인 메모리, 개인 MCP 서버, 개인 skills, 개인 하위 에이전트 등 모든 행이 행으로 구성되어 있고 공유 코드베이스가 이 모든 것을 한 번에 호스팅하기 때문에 모든 사용자에게 본질적으로 무료로 완전히 사용자 정의 가능한 에이전트를 제공할 수 있습니다.

<Diagram id="doc-block-19r4wnf" title={"Claude-Code 설정이지만 PostgreSQL/PGlite에 에이전트 리소스로 저장됨"} summary={"동일한 사용자 정의 계층(지침, 기술, 메모리, 에이전트, 작업, MCP)은 모든 파일이 공유 다중 테넌트 데이터베이스의 행이라는 점을 제외합니다."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-Native 리소스</span
    ><small class="diagram-muted"
      >rows in one PostgreSQL/PGlite 데이터베이스</small
    >
    <div class="ws-rows">
...
| Claude 코드 / Codex                  | 에이전트 기반 리소스                           |
| ------------------------------------ | ---------------------------------------------- |
| 로컬 디스크의 파일                   | 공유 PostgreSQL/PGlite 데이터베이스의 행       |
| 개발자당 하나의 코드베이스           | 하나의 코드베이스, 많은 사용자                 |
| 개발 상자 또는 컨테이너가 필요합니다 | 모든 서버리스/에지 호스트에서 실행             |
| `~/.claude/`의 사용자 정의           | 사용자별 맞춤설정, 범위가 지정된 `u:<email>:…` |
| 프로젝트별 `CLAUDE.md` / skills      | 앱별 `AGENTS.md` + 메모리 리소스               |
| JSON 파일의 MCP 구성                 | JSON의 MCP 구성 _또는_ 범위당 UI 설정          |

동일한 기능. 다른 경제학. 이것이 SaaS에 중요한 이유는 [앱](/docs/cloneable-saas)를 참조하세요.
...
인앱 리소스 탭에는 세 가지 범위가 모두 표시됩니다. 개인 및 공유/조직 리소스를 편집할 수 있습니다. Workspace 범위 리소스는 앱 패널에서 읽기 전용이고 Dispatch에서 중앙에서 편집되므로 모든 앱은 동기화 단계 없이 동일한 표준 파일을 볼 수 있습니다.
...
<Diagram id="doc-block-1ckc1ao" title={"세 가지 범위, 하나의 유효 파일"} summary={"런타임은 읽기 시 작업 영역, 앱 및 개인 범위에서 동일한 경로를 확인합니다. 가장 구체적인 범위가 우선합니다."}>

```html
<div class="ws-stack">
  <div class="diagram-card">
    <span class="diagram-pill">Workspace</span
    ><small class="diagram-muted">Dispatch의 회사 전체 기본값</small
    ><code>context/brand.md</code>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">조직 / 앱</span
...
1. **리소스** 탭 → **공유** → `AGENTS.md`를 엽니다(누락된 경우 `+` → **파일**으로 생성).
2. 규칙 하나를 추가하세요. 예:
...
<Callout id="doc-block-1xaivt5" tone="info">

다시 시작하거나 재배포할 필요가 없습니다. `AGENTS.md`은 매 턴 시작 시 읽혀지므로 지금 저장하는 편집 내용은 바로 다음 메시지에서 에이전트의 행동을 변경합니다.
...
상담사 패널에는 Chat 및 CLI와 함께 **리소스** 탭이 포함되어 있습니다. 여기에는 모든 리소스의 폴더 구성 트리, 텍스트 파일용 인라인 편집기(Markdown, JSON, YAML, 일반 텍스트) 및 `+` 메뉴의 입력된 생성 흐름(파일, Skills, 에이전트, 예약된 작업)이 표시됩니다. 사용자는 상속된 작업 공간 기본값을 찾아보고 개인 또는 조직 리소스를 생성/편집/삭제할 수 있습니다.
...
언제든지 이 문서로 돌아가려면 리소스 도구 모음에서 `?` 아이콘을 클릭하세요.
...
에이전트 리소스에는 현재 두 개의 메모리 표면이 있습니다:
...
| `mcp-servers/…`    | 작업공간/공유 | Dispatch 또는 앱 리소스를 통한 인간 | MCP 구성 새로 고침                                   |

사용자는 리소스 탭에서 직접 이러한 메모리 파일을 편집할 수 있습니다. 이는 일반 리소스입니다. 상담원이 잘못 입력한 줄을 삭제하고, `memory/MEMORY.md`에서 개인 기본 설정을 유지하거나, 팀 전체 규칙을 `AGENTS.md`로 승격하세요.
...
<DataModel
  ...
  title={"에이전트 자원 모델"}
  summary={
    "하나의 리소스 형태는 모든 에이전트 리소스 파일을 백업합니다. 런타임은 이를 경로 및 범위별로 키를 지정하고 읽기 시 유효 값을 확인합니다."
  }
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "사용자의 에이전트 리소스에 있는 단일 파일(명령어, 스킬, 메모리, 에이전트, MCP 구성 또는 작업)입니다.",
      fields: [
        {
          name: "path",
          type: "string",
...
Skills는 `/skill-name`와의 채팅에서 호출되는 에이전트 주문형 도메인 지식을 제공하는 `skills/` 경로(바람직하게는 `skills/<name>/SKILL.md`) 아래의 Markdown 리소스 파일입니다. 리소스 탭에서 추가하거나 코드 모드의 `.agents/skills/`에서 추가하세요.
  ...
  annotations={[
    {
      ...
      note: "`name`은 `@`-드롭다운에 나타나는 내용이며 주 에이전트가 위임하는 내용입니다.",
    },
    {
      lines: "3-4",
      label: "위임할 시기",
      note: "`description`은 이 프로필이 작업에 적합한지 결정하기 위해 오케스트레이터가 읽는 것입니다.",
    },
    {
      lines: "5",
      label: "모델",
      note: "`inherit`는 주 에이전트의 모델을 재사용합니다. 프로필에 분명히 다른 프로필이 필요한 경우에만 재정의하세요.",
    },
    {
      lines: "6",
      note: "현재 `tools: inherit` — 이 필드는 향후 에이전트별 도구 정책을 위해 예약되어 있습니다.",
    },
  ]}
/>
...
## @ 태그와 / 슬래시 명령 {#at-tagging}

채팅 입력에 `@`를 입력해 에이전트, 파일 또는 리소스를 참조하거나 `/`를 입력해 skill을 호출하세요. 둘 다 커서 위치에 드롭다운을 열고 항목을 선택하면 인라인 칩을 추가합니다. 전체 작동 방식(라우팅, `MentionProvider`, 하위 에이전트 선택)은 [에이전트 멘션](/docs/agent-mentions)을, skills 작성 및 검색 방법은 [Skills 가이드](/docs/skills-guide)를 참조하세요.
...
| 기능                 | 코드 모드                                                                  | 앱 모드                                                  |
| -------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------- |
| @태그                | 코드베이스 파일 + 에이전트 리소스 + 사용자 정의 에이전트 + 연결된 에이전트 | 에이전트 리소스 + 사용자 정의 에이전트 + 연결된 에이전트 |
...
작업 공간 연결을 사용하면 앱이 자격 증명을 복제하지 않고도 동일한 공급자 계정(Slack, GitHub, HubSpot 등)을 공유할 수 있습니다. 연결은 공급자 ID, 계정 레이블, 상태, 범위, 앱 부여 및 자격 증명 참조를 PostgreSQL/PGlite에 기록합니다. 비밀은 자격 증명 저장소에 보관됩니다. 연결은 `SLACK_BOT_TOKEN`와 같은 자격 증명 키 이름만 가리킵니다.
...
- [**Skills 가이드**](/docs/skills-guide) — `skills/`에서 참조하는 skill 파일을 작성하고 범위 지정하기
- [**에이전트 멘션**](/docs/agent-mentions) — `@` 멘션과 `/` 명령의 전체 작동 방식
- [**반복 작업**](/docs/recurring-jobs) — Scheduled Tasks에서 만든 프롬프트 예약하기
- [**Workspace 거버넌스**](/docs/workspace-management) — workspace 변경을 검토하고 승인하며 소유하는 주체
- [**다중 앱 Workspace**](/docs/multi-app-workspace) — 하나의 workspace가 포괄하는 배포 형태
- [**Workspace 연결**](/docs/workspace-connections) — 앱 간에 제공업체 자격 증명 공유하기
