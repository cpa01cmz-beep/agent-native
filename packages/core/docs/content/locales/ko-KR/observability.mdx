...
<Diagram id="doc-block-1ua8f7i" title={"모든 실행은 루프에 공급됩니다."} summary={"한 번의 에이전트 실행으로 추적, 자동 점수 및 피드백 후크가 생성됩니다. 모두 앱 자체의 SQL에 저장되고 대시보드에 표시됩니다. 실험에서는 구성 변형 간에 트래픽을 분할합니다."}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    에이전트 실행<br /><small class="diagram-muted">production-agent.ts</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">자동으로 캡처됨</span
...
### PostHog의 피드백 {#posthog-feedback}

좋아요·싫어요, 카테고리, 자유 입력은 모두 `$ai_feedback`을 보냅니다. 반면 PostHog LLM 분석의 피드백 화면은 `survey sent` 이벤트를 읽습니다. 트레이스의 **Feedback** 탭에서 PostHog가 만든 설문을 `POSTHOG_AI_FEEDBACK_SURVEY_ID`에 설정하세요. 이 id 하나가 설정의 전부이며, 없으면 설문 이벤트는 전송되지 않습니다.

좋아요·싫어요 투표는 설문의 첫 질문에 PostHog의 선택지 번호(`1` 좋아요, `2` 싫어요)로 답하고, 싫어요 뒤에 오는 자유 입력은 후속 질문에 답하므로 텍스트가 평가 자리에 들어가는 일은 없습니다. 두 이벤트는 평가된 메시지마다 같은 제출 ID를 지니며, PostHog는 이를 기준으로 하나의 응답으로 묶습니다. 싫어요는 후속 답변이 도착할 때까지 미완료로 표시됩니다.
...
<Diagram id="doc-block-k2p8r6" title={"일관된 해시 변형 할당"} summary={"각 사용자는 안정적인 변형으로 해시하고 루프는 해당 변형의 구성 재정의를 적용하며 결과는 신뢰 구간과 함께 변형별로 롤업됩니다."}>

```html
<div class="exp">
  <div class="diagram-node">
    사용자 ID<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">구성 오버라이드 A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">구성 오버라이드 B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    결과 per variant<br /><small class="diagram-muted"
...
`defineAppConfig()`를 사용하여 서버 플러그인에서 설정합니다. 모든 필드에는 배포 환경 변수 별칭도 있습니다. 선언된 구성 참조를 참조하세요.
...
자체 앱에 OpenTelemetry 공급자를 등록하여 Langfuse, Datadog, Grafana 또는 모든 OTel 호환 백엔드로 추적을 보냅니다. 아래 [OpenTelemetry 범위](#otel)를 참조하세요.

코어는 의도적으로 내보내기 도구를 제공하지 않으며 내보내기 엔드포인트나 토큰도 보유하지 않습니다. 따라서 백엔드 자격 증명은 프레임워크 구성이 아니라 공급자 배선(및 볼트)에 있습니다.

## OpenTelemetry 범위 {#otel}

에이전트 루프는 모든 실행, 모델 호출 및 도구 호출에 대해 **실시간 OpenTelemetry 범위**를 내보냅니다. 따라서 이미 OTel 수집기를 실행하는 호스트는 나머지 분산 추적과 함께 에이전트 활동을 볼 수 있습니다.
...
- [**Actions**](/docs/actions-overview) — 추적에서 도구 호출로 나타나는 작업
