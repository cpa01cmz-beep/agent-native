...
<Diagram id="doc-block-1g03zkp" title={"하나의 실행 스토어에 3개의 레이어가 있음"} summary={"CLI, Desktop 및 공유 UI는 동일한 파일 지원 실행 저장소 및 실행기에서 서로 다른 표면입니다. 호스트는 CodeAgentsHost 계약을 통해 이를 조정합니다."}>
...
<div class="diagram-layers">
  <div class="diagram-row">
    <div class="diagram-card" data-rough>
      ...
      ><small class="diagram-muted">시작 · 재개 · 상태 · 중지</small>
    </div>
    <div class="diagram-card" data-rough>
      <span class="diagram-pill">Desktop</span
      ><small class="diagram-muted">네이티브 터미널 · webviews · 딥링크</small>
    </div>
    <div class="diagram-card" data-rough>
      <span class="diagram-pill accent">공유d UI</span
  ...
  <div class="diagram-box" data-rough>
    파일 기반 run store + 실행기<br /><small class="diagram-muted"
...
[Harness Agents](/docs/harness-agents); 이 어댑터는 이제 로컬 인증 파일을 샌드박스에
복사하지 않고 호스트에서 지원되는 네이티브 구독 자격 증명을 확인합니다.
...
<Diagram id="doc-block-1rn65mf" title={"원격 Dispatch은 아웃바운드 전용입니다."} summary={"모바일은 데스크톱과 직접 대화하지 않습니다. Desktop은 Dispatch을 길게 폴링하고, 명령을 요청하고, 로컬 실행 저장소를 구동하고, 결과를 다시 미러링합니다."}>

```html
<div class="diagram-remote">
  <div class="diagram-node" data-rough>
    모바일 / Telegram<br /><small class="diagram-muted">/code · 세션</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Dispatch 릴레이<br /><small class="diagram-muted"
      >호스트 · 실행 · 기록</small
    >
  </div>
...
<Endpoint id="doc-block-9q1n8v" title={"데스크톱 클레임 대기 중인 작업"} method="POST" path="/_agent-native/integrations/remote/poll" summary={"데스크톱은 대기열에 포함된 명령을 요청하기 위해 릴레이를 롱 폴링합니다."} auth="Desktop device token" responses={[
  {
    "status": "200",
