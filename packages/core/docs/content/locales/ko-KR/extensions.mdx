---
title: "확장"
description: "일회성 샌드박스 커스텀 블록을 위한 선택적 Toolkit 기능입니다. 지속 가능한 제품 동작에는 앱 코드가 기본입니다."
---

# 확장

확장 프로그램은 **샌드박스 커스텀 블록을 위한 선택적 Toolkit 기능**입니다. 기본적으로 꺼져 있으며 템플릿이 명시적으로 활성화해야 합니다. Analytics에서는 명시적으로 요청된 일회성 맞춤 시각화에 사용합니다. 지속 가능한 동작에는 앱 코드를 편집하세요.
...
> 더 깊은 사용자 지정은 자체 저장소와 개발 워크플로에서 템플릿을 변경하세요.
> 임베디드 에이전트는 호스트가 repository/workspace 쓰기 도구를 의도적으로
> 부여한 경우에만 지원할 수 있습니다. 확장 프로그램은 배포 없이 빠르게
> 추가하는 계층입니다.
...
확장 프로그램은 기본적으로 비활성화됩니다. 활성화하려면
`createAgentChatPlugin()`과 `createCoreRoutesPlugin()` 모두에
`extensionTools: true`를 설정하세요. 전자는 에이전트용 actions와 안내를
추가하고, 후자는 `POST /_agent-native/extensions`를 통한 인증된 생성을
허용합니다. 샌드박스 runtime과 기존 읽기, 편집, 딥 링크 routes는
호환성을 위해 유지됩니다.

<Diagram id="doc-block-gtw3mr" title={"샌드박스 브리지"} summary={"확장 HTML은 격리된 iframe에서 실행되며 고정된 브리지 도우미 세트를 통해서만 호스트에 도달합니다. 모든 호출은 범위가 지정되고 액세스가 확인됩니다."}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      호스트 템플릿<br /><small class="diagram-muted"
        >actions, 자동 범위 SQL</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>, 도메인 잠금</small
      >
    </div>
    <div class="diagram-box">
      외부 API<br /><small class="diagram-muted">extensionFetch로만</small>
    </div>
  </div>
</div>
```
...
1. **Settings → Extensions**를 열고(또는 채팅으로 물어보세요) **"새 확장 프로그램"**을 클릭하세요.
...
<Diagram id="doc-block-p2xcb4" title={"슬롯은 컨텍스트를 위젯에 푸시합니다."} summary={"호스트 템플릿은 명명된 슬롯을 소유합니다. 확장 기능을 설치하면 사용자가 현재 보고 있는 모든 항목에 대한 window.slotContext가 제공됩니다."}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">메일 스레드</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">연락처 메모</span
    ><small class="diagram-muted"
...
- `dbQuery`, `extensionFetch`는 현재 차단되었습니다.
...
## 확장 프로그램과 앱 소스 워크플로 중 무엇을 사용할까요? {#vs-app-code}

임베디드 에이전트는 기본적으로 소스 코드를 편집하지 않습니다. 호스트가 repository/workspace 쓰기 도구를 의도적으로 부여한 경우에만 편집할 수 있습니다. 지속적인 제품 동작에는 소스 워크플로를, 빠른 runtime 사용자 지정에는 확장 프로그램을 사용하세요.

- **앱 코드로 시작하세요.** 제품 동작, 네이티브 UI, 레이아웃, 경로, 비즈니스 로직 또는 폭넓게 재사용할 기능은 코드로 구현합니다.
- **앱이 활성화한 경우에만 커스텀 블록을 사용하세요.** 사용자가 기존 화면에서 일회성 시각화나 인터랙션을 명시적으로 요청한 경우에 적합합니다. 빠르고 배포가 필요 없지만 에이전트가 만든 샌드박스 패치입니다.
- **지속 가능한 블록은 앱 코드로 승격하세요.** 원래 맥락을 넘어 유용해지면 **앱 코드로 승격**을 사용하고 코드 변경이 검토·배포될 때까지 기존 블록을 유지하세요.

|                 | 확장                            | 앱 코드 편집                   |
| --------------- | ------------------------------- | ------------------------------ |
| **작성자**      | 런타임 시 에이전트(또는 사용자) | 사용자 또는 코드 지원 워크플로 |
| **저장 위치**   | 데이터베이스                    | git 저장소                     |
| **빌드 필요**   | 아니요                          | 예                             |
| **배포 필요**   | 아니요                          | 예                             |
| **범위**        | 사용자 1명(또는 조직과 공유)    | 전체 제품, 모든 사용자         |
| **최적의 용도** | 명시적인 일회성 커스텀 블록     | 지속 가능한 제품 동작          |

경험 법칙: **기본적으로 앱 코드를 사용하세요.** 확장 프로그램을 활성화한 앱에서는 일회성 시각화를 커스텀 블록으로 만들 수 있습니다. 모든 대시보드에서 쓸 새 차트 유형은 템플릿 코드에 속합니다.

커스텀 위젯 UI 대신 네이티브 차트와 테이블에 데이터를 공급해야 하는 커스텀 데이터 가져오기의 경우, [데이터 프로그램](/docs/data-programs)을 참조하세요 — 확장 기능 iframe 없이 dashboard 패널이 직접 바인딩하는, 저장되고 갱신 가능한 `run-code` 스크립트입니다.
...
- **인증 필요** — 확장 프로그램은 로그인한 사용자에 대해서만 실행되며 `dbQuery` 호출은 자동으로 범위가 지정됩니다.
...
- [**Actions**](/docs/actions-overview) — 확장 프로그램이 `appAction`를 통해 호출하는 작업
