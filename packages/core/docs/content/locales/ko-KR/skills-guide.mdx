...
`app-skill` 패키지는 표준 Agent Plugin 파일도 생성합니다. `agent-native plugin import <path> --into <workspace> --yes`로 Skills와 원격 Streamable HTTP MCP 서버를 가져올 수 있으며, 패키지 헤더는 무시되고 stdio 및 SSE 서버는 보고 후 건너뜁니다.
...
- 지침은 일회성입니다. 대신 `AGENTS.md` 또는 에이전트 메모리에 넣으세요
...
지식이 앱에서 작업하는 *모든 사람*에게 적용된다면("항상 하위 쿼리보다 CTE를 선호합니다"), 그것은 기술 또는 공유 `LEARNINGS.md`입니다. _이 특정 사용자_("Steve는 간결한 답변을 좋아합니다")에 관한 것이라면 `memory/MEMORY.md`에 속합니다. 전체 치료에 대해서는 [에이전트 리소스 — 메모리](/docs/agent-resources#memory)를 참조하세요.

## Skills와 SQL 리소스 {#skills-vs-resources}

이 페이지에서는 skill을 `.agents/skills/<name>/SKILL.md`의 파일로 설명합니다. [에이전트 리소스](/docs/agent-resources#skills)에서는 같은 skill을 `skills/<slug>/SKILL.md` 경로의 workspace/shared/personal 범위 SQL 리소스로 설명합니다. 두 설명은 같은 개념을 서로 다른 관점에서 본 것입니다. Code 모드는 파일을 직접 읽고 쓰고, App 모드는 파일 시스템 없이 작동하도록 리소스 행으로 저장합니다. 어느 방식이든 이 페이지의 형식으로 콘텐츠를 작성하세요.
...
- [**에이전트 리소스**](/docs/agent-resources#skills) — App 모드에서 skills가 SQL 리소스로 저장되는 위치
