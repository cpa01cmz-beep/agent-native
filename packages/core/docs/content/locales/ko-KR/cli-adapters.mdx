...
<Diagram id="doc-block-11h4al7" title={"CLI 어댑터 → 레지스트리 → 작업 표면"} summary={"ShellCliAdapter는 바이너리를 래핑합니다. CliRegistry는 검색을 위해 어댑터를 수집합니다. defineAction은 에이전트 + UI 작업 화면에서 하나의 호출을 노출합니다."}>
...
- [**Actions**](/docs/actions-overview) — 액션 표면 CLI 어댑터는 일반적으로 포장되어 있습니다.
