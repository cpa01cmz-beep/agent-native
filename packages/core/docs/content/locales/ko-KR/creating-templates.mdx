...
템플릿은 실제 워크플로를 해결하는 완전하고 포크 가능한 에이전트 기반 앱입니다. 자사 템플릿은 귀하가 사용하는 것과 동일한 프레임워크 표면으로 구축됩니다. UI용 React 경로, Drizzle 기반 PostgreSQL 데이터, 작업용 actions, 에이전트 동작용 작업 공간 리소스, 에이전트와 UI가 정렬 상태를 유지하도록 폴링 동기화합니다.
...
기능의 위치를 정할 때 세 계층을 염두에 두세요.

- **Core - 프레임워크:** 모든 앱이 사용할 수 있는 필수 런타임 및 데이터 계약입니다.
- **Toolkit - 선택적 재사용 부품:** 앱이 채택, 조합 또는 포크할 수 있는 공유 UI와 제품 시스템입니다.
- **Templates - 선택적 앱:** Core 위에 구축되고 흔히 Toolkit을 사용하는 완전한 도메인 제품이며 시작점으로 포크할 수 있습니다.

Toolkit과 Templates는 선택 사항입니다. Template 자체가 프레임워크는 아니며 Toolkit이 Core를 대체하지 않습니다.

좋은 템플릿:

- 유용한 시드 데이터 또는 빈 상태 흐름을 사용하여 하나의 워크플로를 엔드 투 엔드로 해결합니다.
- JSON 파일이 아닌 PostgreSQL/PGlite에 지속성 상태를 저장합니다.
...
Chat은 인증, 내구성 있는 채팅 스레드, PostgreSQL 지원 리소스, 도구, 애플리케이션 상태, actions 및 폴링 동기화를 제공합니다. 도메인 모델과 제품 UI를 추가합니다.

아직 재사용 가능한 UI 템플릿을 구축하지 않았다면 [Automation-First Apps](/docs/pure-agent-apps)를 사용하세요. 액션을 정의하고 브라우저 없이 실행한 다음 워크플로에 지속적인 화면이 필요할 때 UI를 추가하세요.
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "app/root.tsx",
      note: "HTML shell과 providers",
    },
    {
      path: "app/routes/",
      note: "React Router 파일 routes",
    },
    {
      path: "app/components/",
      note: "템플릿 UI",
    ...
    {
      path: "app/hooks/",
      note: "UI 상태 및 데이터 hooks",
    },
    {
      path: "actions/",
      note: "defineAction 작업: 단일 진실 공급원",
    },
    {
    ...
    {
      path: "server/plugins/",
      note: "시작 integrations",
    },
    {
      path: "server/routes/api/",
      note: "actions만으로 부족할 때만 custom routes 사용",
    },
    {
      path: "shared/types.ts",
      note: "공유 client/server 타입",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: 복잡한 workflow를 위한 에이전트 지침",
    },
    {
      path: "AGENTS.md",
      note: "템플릿별 에이전트 지침",
...
애플리케이션 상태에 대해 `data/` 디렉터리를 추가하지 마세요. 내구성 있는 앱 데이터는 PostgreSQL/PGlite에 속하며 UI는 actions 또는 형식화된 서버 핸들러를 통해 이를 읽습니다.

모든 템플릿의 네 가지 영역은 하나의 공유 작업 표면과 하나의 PostgreSQL/PGlite 데이터베이스를 통해 함께 연결됩니다. 에이전트와 UI는 동일한 작업에 대해 동등한 파트너입니다.

<Diagram id="doc-block-hjg2nf" title={"템플릿의 네 가지 영역이 연결되는 방식"} summary={"UI와 에이전트는 모두 동일한 작업을 통해 PostgreSQL/PGlite에 도달합니다. 애플리케이션 상태와 폴링 동기화가 정렬된 상태를 유지합니다."}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      React 사용자 인터페이스 화면<br /><small class="diagram-muted"
        >app/routes · 컴포넌트</small
      >
    </div>
    <div class="diagram-node">
...
## PostgreSQL의 모델 데이터 {#data-models}

프레임워크 Drizzle 도우미를 사용하여 도메인 테이블을 정의하고 스키마가 PostgreSQL 및 PGlite를 사용하도록 하세요.
...
스키마 변경은 추가 방식이어야 합니다. 새 앱 소유 테이블과 열에는
`drizzle-kit generate`로 `server/db/migrations/<id_name>.sql`에 검토
가능한 SQL을 만들고 Node.js 배포의 릴리스 단계에서
`runDrizzleMigrations()`로 로드하세요. Cloudflare Workers/Pages와 기타 edge 런타임에는 배포
가능한 파일 시스템이 없으므로, 생성된 SQL을 worker bundle에 임베드하고
`runMigrations()`에 전달하세요. 수동으로 작성한 이력과 특수한 경우에는
`runMigrations()`를 사용하세요. 파괴적인 SQL, `drizzle-kit push`, 테이블 이름
바꾸기 또는 열 삭제를 사용하지 마십시오.

앱 읽기 및 쓰기의 경우 Drizzle의 쿼리 빌더와 `drizzle-orm`의 PostgreSQL 호환 연산자를 사용하세요. Drizzle가 쿼리를 표현할 수 있는 경우 원시 SQL로 제품 코드를 작성하지 말고 템플릿에서는 `@agent-native/core/db/schema`의 스키마 도우미를 사용하세요.
...
사용자 또는 조직 데이터를 보관하는 스키마를 추가하기 전에 [Database](/docs/server-database) 및 [Security](/docs/security) 문서를 사용하세요.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "8",
      label: "도구 표면",
      note: "`description`는 이 작업을 도구로 호출할 시기를 결정하기 위해 에이전트가 읽는 것입니다.",
    },
    {
      lines: "9-11",
      label: "타입 계약",
      note: "하나의 zod `schema`가 에이전트, UI, HTTP, MCP, A2A의 입력을 검증합니다.",
    },
    {
      lines: "18-19",
      label: "범위가 지정된 쓰기",
      note: "공유 및 액세스 확인을 위해 행의 범위가 올바르게 지정되도록 `ctx`에서 `ownerEmail` / `orgId`을 스탬프 처리합니다.",
    },
  ]}
/>
...
- Data lives in Drizzle 기반 PostgreSQL. Use actions for all writes; schema is additive.
...
커뮤니티 템플릿은 공개 GitHub 저장소에서 직접 설치합니다. Agent-Native 앱을 저장소 루트에 두세요.

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

특정 리비전을 고정하려면 `#branch`, `#tag` 또는 `#commit`을 추가하세요.

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

CLI는 소스만 다운로드하며 종속성을 설치하거나 템플릿 스크립트를 실행하지 않습니다. 커뮤니티 템플릿은 타사 코드이므로 `pnpm install`을 실행하기 전에 저장소, 라이선스, 종속성 및 설치 스크립트를 검토하세요.

카탈로그에 등록하려면 루트의 완성된 앱, 라이선스와 설정 README가 있는 공개 저장소, `@agent-native/core` 종속성, 통과하는 타입 검사와 테스트가 필요합니다. 호스팅 데모와 스크린샷은 선택 사항입니다.

저장소가 준비되면 [커뮤니티 템플릿을 제출](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml)하세요. 커뮤니티 목록은 공식 카탈로그와 분리되며 작성자가 유지 관리합니다.
