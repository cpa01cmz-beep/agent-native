...
<Diagram id="doc-block-1kr0pmm" title={"한 번의 통화, 다양한 목적지"} summary={"통지()는 항상 소유자 범위의 받은 편지함 행을 작성하고 등록된 모든 채널에 병렬로(최선의 노력) 팬아웃한 다음 이벤트 버스에 통지를 내보냅니다."}>
...
이메일, Slack, webhook은 받은 편지함과 함께 항상 등록됩니다. 전송 시 알림별 메타데이터의 URL/수신자를 우선 사용하고, 그다음 일치하는 환경 변수로 대체합니다. 둘 다 설정되지 않으면 해당 채널은 알림 전송 시 아무 작업도 하지 않습니다.
...
<Diagram id="doc-block-1lw1rdr" title={"채널 및 심각도"} summary={"받은 편지함은 항상 켜져 있습니다. 웹훅에는 환경 변수가 필요합니다. 맞춤 채널은 시작 시 등록됩니다. 심각도는 배지 스타일을 결정하고 모든 채널에 전달됩니다."}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>심각도가 배지를 결정</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">확인, FYI</span>
    </div>
    <div class="sev-row">
...
<Endpoint id="doc-block-1232thx" title={"알림 나열"} method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary={"현재 사용자에 대한 최근 알림 나열"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "ISO 타임스탬프 커서 — 페이지네이션을 위해 이 시간 전에 생성된 알림을 반환합니다."
  }
]} responses={[
  {
    "status": "200",
...
- **폴 통합** — 모든 변이는 `recordChange()`를 호출하므로 [`useDbSync`](/docs/client-data#usedbsync)를 사용하는 템플릿은 추가 배선 없이 자동 무효화됩니다.
...
- [**Server plugins**](/docs/server-overview) — 시작 시 맞춤 채널이 등록되는 곳
