...
<Diagram id="doc-block-15vqyqp" title={"하네스는 루프를 소유합니다. Agent-Native이 세션을 구동합니다."} summary={"AgentHarness 기판은 기본 세션을 creates/resumes하고 해당 이벤트를 일반 기록으로 스트리밍하며 턴 사이에 SQL에서 이력서 상태를 유지합니다."}>
...
`installPackage` 힌트(예: `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness`는 명확한 설치를 발생시킵니다.
...
## Codex 인증: 코드 UI 대 하네스 어댑터 {#codex-auth}

Codex 표면은 두 가지입니다.

- **Agent-Native 코드 / 데스크톱**은 사용자 컴퓨터에서 `codex exec`를 실행하고
  `codex login status`가 보고하는 로컬 Codex CLI 로그인을 재사용합니다.
- **`ai-sdk-harness:codex`**는 기반 하네스가 지원하는 경우 upstream AI SDK Harness의
  네이티브 구독 흐름을 사용합니다. 추가 설정은 필요하지 않습니다. 자격 증명은 호스트에
  남고, 명시적인 API 키 자격 증명이 우선하며, 로컬 인증 파일을 복사하지 않고 결과 인증이
  샌드박스로 전달됩니다. AI Gateway만 사용하려면 `harnessOptions.auth`를
  `"ai-gateway"`로 설정하세요. 이 모드는 네이티브 구독을 읽지 않습니다.

내장 `ai-sdk-harness:codex` 어댑터는 안정적인 Codex 런타임에서 허용하는 유일한
모드인 `permissionMode: "allow-all"`만 지원합니다. 이 기본값을 사용하려면
`permissionMode`를 생략하세요. 이 어댑터에서는 `allow-reads`와 `allow-edits`가
거부됩니다.
...
`permissionMode`, `harnessOptions`, `agentOptions`)에 매핑됩니다.
`listAgentHarnesses()`를 사용하여 등록된 내용을 열거하세요.
선택기.

샌드박스를 사용하는 기본 하네스는 등록할 때 `agentOptions.sandbox`를 통해
샌드박스 공급자를 전달하세요. 프로세스 재시작 후 재수화하려면 공급자가
`resumeSession`을 구현해야 하며, 호출자가 제공한 `sandbox` 세션은 초기
생성에만 사용됩니다.
...
<Diagram id="doc-block-v2i6m2" title={"차례, 프로세스, 배포 전반에 걸쳐 재개"} summary={"매 턴마다 불투명한 이력서 상태를 SQL로 분리합니다. 다음 차례에서는 채팅 기록을 재생하는 대신 createSession에 다시 피드백합니다."}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    턴 N<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >불투명 · SQL harness 세션</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    턴 N+1<br /><small class="diagram-muted">createSession.resumeState</small>
  </div>
</div>
```
...
승인 표면용 [Human Approval](/docs/actions-access-control#needs-approval).
...
- [Human Approval](/docs/actions-access-control#needs-approval) — 승인 표면 하니스 실행에 사용됩니다.
