...
<Diagram id="doc-block-1rgn59l" title={"두 레이어는 이력서의 중복 부작용을 차단합니다."} summary={"저널은 내구성 있는 원장을 읽고 이전 호출을 분류합니다. 레이어 1은 모델에 알려주고, 레이어 2는 완료된 항목과 일치하는 재배송된 쓰기를 하드 차단합니다."}>
...
- [**Actions**](/docs/actions-overview) — `readOnly`는 읽기를 다시 실행해도 안전한 것으로 표시합니다. 다른 모든 것은 부작용으로 간주됩니다.
