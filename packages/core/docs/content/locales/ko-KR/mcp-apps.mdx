...
<Diagram id="doc-block-9m5f26" title={"MCP 앱 삽입 왕복"} summary={"작업의 링크 대상도 포함 대상입니다. 가능한 호스트는 동일한 서명된 앱 경로를 인라인으로 로드합니다. 다른 모든 사람들은 딥 링크로 돌아갑니다."}>
...
<div class="diagram-embed">
  ...
  <div class="diagram-card" data-rough>
    ...
    ><small class="diagram-muted">일회용 SQL ticket 교환</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card" data-rough>
    <strong>Signed app route</strong
    ><span class="diagram-pill ok">실제 React 라우트</span
    ><small class="diagram-muted">short-lived browser session</small>
  </div>
  <div class="diagram-fallback">
    <span class="diagram-pill warn">MCP Apps 미지원</span>
  ...
  annotations={[
    {
      lines: "6",
      label: "점진적인 향상",
      note: "`mcpApp.resource`은 MCP 앱 확장을 지원하는 호스트에 대한 인라인 UI를 광고합니다. 작업의 `link` 빌더도 유지하세요. CLI만 있고 오래된 호스트는 UI 메타데이터를 무시하고 여전히 딥 링크가 필요합니다.",
    },
    {
      lines: "7",
    ...
    },
    {
      lines: "11",
      label: "범용 대체 라벨",
      note: '`openLabel`은 호스트가 인라인 iframe을 렌더링하지 않을 때 딥 링크 탈출구로 쓰이는 표시 텍스트 `"Open in … →"`입니다.',
    },
  ]}
/>
...
<Diagram id="doc-block-1fvbryv" title={"호스트 브리지 경로 2개, 서명된 경로 1개"} summary={"Claude는 수화된 경로를 이식하고 직접 ui/_bridge을 사용합니다. ChatGPT은 window.openai를 통해 제어된 iframe을 가져오고 postMessage를 통해 호스트 작업을 중계합니다. 둘 다 동일한 서명된 앱 경로를 가리킵니다."}>
  ...
  <div class="diagram-col">
    <div class="diagram-card" data-rough>
      ...
      ><small class="diagram-muted"
        >Claude iframe에서 서명된 앱 HTML을 hydrate한 뒤 직접 `ui/_` 호스트
        브리지</small
      >
    </div>
    <div class="diagram-card" data-rough>
...
- [Actions](/docs/actions-overview) — `defineAction`, `link` 빌더, `publicAgent`.
