---
title: "공유 및 개인정보 보호"
description: "프레임워크에 내장된 익숙하고 일관된 공유 기능입니다. 문서, 대시보드, 디자인, 자료, 클립, 녹화, 양식 등 사용자가 만든 모든 리소스에 동일한 기본 비공개 모델과 일관된 공유 UI가 적용됩니다."
---

# 공유 및 개인정보 보호

문서, 대시보드, 디자인, 데크, 화면 녹화, 회의 기록, 양식, 예약 링크 등 사용자가 에이전트 기반 앱에서 생성하는 모든 리소스는 **기본적으로 작성자에게 공개됩니다**. 다른 사람들은 작성자가 명시적으로 공유하거나 공개 상태를 `org` 또는 `public`로 변경한 경우에만 볼 수 있습니다.

어디서나 하나의 익숙한 공유 모델을 사용합니다. 모든 템플릿에서 앱별로 새로 만들 필요 없이 동일한 공유 버튼, 동일한 대화 상자, 동일한 3단계 공개 범위 모델, 동일한 사용자별/조직별 권한 부여를 사용합니다.
...
<Diagram id="doc-block-1iuidh0" title={"가시성, 바깥쪽으로 넓어짐"} summary={"리소스에 대한 대략적인 가시성은 바닥을 설정합니다. 컴패니언 테이블의 명시적인 공유 부여는 이름이 지정된 사람을 맨 위에 추가합니다."}>
...
- **뷰어** — 읽기 전용.
- **댓글 작성자** — 읽고 댓글을 추가할 수 있지만 리소스를 편집하거나 공유를 관리할 수 없음.
- **편집기** — 읽기 + 쓰기.
...
- **개인 데이터 앱**(메일) - 사용자 범위로 설계되었습니다. '받은편지함 공유' 개념은 없습니다.
...
- [**보안 — 데이터 범위 지정**](/docs/security#data-scoping) — 공유의 기반이 되는 액세스 필터 및 소유권 모델
- [**인증**](/docs/authentication) — 세션, 조직, 요청 컨텍스트로 ID가 전달되는 방식
- [**Extensions**](/docs/extensions#sharing) — 샌드박스 미니 앱 표면에서의 공유
- [**템플릿 만들기**](/docs/creating-templates) — 새 템플릿 스키마에 `ownableColumns` 연결하기
