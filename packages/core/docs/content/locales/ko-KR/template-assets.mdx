...
Assets는 브랜드와 일관된 미디어를 생성하고 관리하기 위한 에이전트 네이티브 작업 공간이며, 특히 이미지 또는 비디오 모델이 뚜렷한 장점을 보이는 경우에 적합합니다. 업로드 및 생성된 결과를 라이브러리와 폴더에 정리하고, 팀이 제품 사진, 로고, 소셜 이미지, 포토리얼한 장면 및 기타 미디어의 승인된 참조 자료를 수집하여 Design 구성이나 다른 곳에서 사용할 수 있도록 합니다. 생성은 에이전트 채팅을 통해 라우팅되므로 모든 자산을 검토하고 개선할 수 있습니다.

<WireframeBlock id="doc-block-hibfdf">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-4kw99k" title={"생성, 검토, 재사용"} summary={"참조 및 프롬프트는 생성 및 선택 세션을 제공합니다. 선택한 자산은 라이브러리에 저장되고 선택기나 A2A을 통해 다른 앱으로 흘러나갑니다."}>
...
<div class="diagram-assets">
  <div class="diagram-col">
    ...
    <div class="diagram-node">
      프롬프트<br /><small class="diagram-muted"
        >채팅 또는 Generate 컨트롤</small
      >
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    <span class="diagram-pill accent">생성 세션</span
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">선택된 브랜드 일관 assets</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-node">
      Picker<br /><small class="diagram-muted">iframe / MCP 앱</small>
    </div>
    <div class="diagram-node">
      A2A<br /><small class="diagram-muted">슬라이드 · 디자인 · 콘텐츠</small>
    </div>
  </div>
</div>
```
...
대부분의 시각 작업에는 **Design**을 사용하세요. 랜딩 페이지, 광고, 다이어그램 및 기타 구성을 HTML/CSS, 정확한 레이아웃, 편집 가능한 구조 또는 디자인 도구와의 가져오기/내보내기가 필요할 때는 Design에서 시작하세요.

이미지 또는 비디오 모델이 작업에 더 적합하고 결과를 조각별로 편집하기보다 미디어로 사용할 수 있다면 **Assets**를 사용하세요. 여기에는 포토리얼한 이미지나 픽셀 기반 이미지, 제품 사진, 소셜 이미지 및 비디오가 포함됩니다. 생성된 이미지를 그대로 사용하려는 경우 손으로 그린 다이어그램이나 Excalidraw 스타일 다이어그램은 예외가 될 수 있습니다.

의도한 워크플로는 Design에서 시작하는 것입니다. PNG 또는 다른 미디어 에셋이 적합할 때 Design 에이전트가 보조 이미지 생성을 Assets에 위임할 수 있습니다. 결과는 Design 구성이나 다른 앱에서 사용하세요.
