...
- **기본값:** 발신 이메일을 사용할 수 있으면 Better Auth에서 magic link를 우선 사용하고, 이메일/비밀번호를 대체 흐름으로 사용하며 소셜 제공자를 지원합니다. 대체 흐름을 강제하려면 `AUTH_MAGIC_LINK=0`을 설정하세요.
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code, ChatGPT 커넥터</small>
    </div>
    <div class="diagram-card">
...
## 로컬 개발 로그인 {#local-development-sign-in}

루프백 개발 서버에서는 공유 로그인 페이지에 **Continue as local dev**가 먼저 표시됩니다. 실제 Better Auth 개발자 계정과 세션을 생성하거나 재사용하므로 앱의 다른 곳과 동일한 인증 흐름을 사용합니다.

**Show full sign in options**를 선택하면 일반 가입, 비밀번호, magic link, 소셜 공급자 옵션이 표시됩니다. 로컬 세션을 만들 수 없으면 페이지가 이 옵션을 자동으로 표시하므로 표준 인증을 계속할 수 있습니다.

이 기능은 개발 또는 테스트 환경에서 브라우저가 `localhost`, `127.0.0.0/8` 또는 `::1`을 통해 연결할 때만 사용할 수 있습니다. 배포 미리보기와 호스팅된 프로덕션에서는 비활성화됩니다. 인증 우회가 아닙니다. `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1`을 설정하면 이 기능을 선택 해제하고 로컬 개발에서 일반 로그인 페이지를 표시합니다.

## 더 나은 인증(기본값) {#better-auth}

기본적으로 Better Auth는 인증을 강화합니다. 다음을 제공합니다:

- magic link 등록 및 로그인, 이메일/비밀번호 대체 흐름 지원
...
- `POST /_agent-native/auth/magic-link` — magic link 이메일 요청
...
또는 환경을 미리 보고 `+autoz` 주소로 테스트 계정 이름을 지정
(`name+autoz@example.com`) 쉽게 식별할 수 있습니다.

## 이메일 확인 정책 {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION`은 로컬 및 QA용 편의 기능이며 호스팅된 배포에서는
무시됩니다. 프로덕션을 포함한 모든 환경에 대해 정책을 명시하려면 구성에서
선언하세요:
...
배포 별칭은 `AUTH_REQUIRE_EMAIL_VERIFICATION=0`입니다. 선언된 값은
`AUTH_SKIP_EMAIL_VERIFICATION`과 환경별 기본값보다 모두 우선합니다.

`false`는 확인되지 않은 주소를 로그인 자격 증명으로 허용합니다. 누구나 어떤
이메일이든 사용할 수 있으므로 가입이 이미 다른 방법으로 제한된 곳에서만
사용하세요. 이메일 공급자가 없는 배포에서 `true`로 설정하면 확인을 전달할 수
없으므로 대신 비밀번호 가입이 비활성화됩니다.
...
<Diagram id="doc-block-g7a3qa" title={"원격 MCP OAuth 핸드셰이크"} summary={"OAuth 가능 클라이언트는 MCP URL(챌린지, 검색, 동적 등록, PKCE 코드 교환)에서 부트스트랩합니다."}>
...
익명의 뷰어가 이 URL를 조회하면 프레임워크의 로그인 페이지가 제공됩니다. 로그인에 성공하면(토큰, 이메일/비밀번호 또는 구글 OAuth 등 모든 흐름) 브라우저는 `return`로 돌아갑니다.
...
일반 앱 페이지는 모든 방문자에게 동일한 비개인화 공개 캐시 가능 SSR 셸을 사용합니다. `AppProviders`가 클라이언트에서 세션을 확인하며, `/dashboard` 같은 비공개 경로에서는 비공개 UI를 마운트하기 전에 익명 방문자를 `/sign-in?return=%2Fdashboard`로 리디렉션합니다. 로그인 후에는 `/dashboard`로 돌아옵니다. 공개/SEO 경로에는 `isPublicPath`를, MCP 임베드 토큰처럼 별도의 제한된 인증 방식을 사용하는 화면에는 `sessionBypass`를 지정하세요.

셸이 캐시되는 기간은 배포 전체 설정입니다 — `AGENT_NATIVE_SSR_CACHE`에 대해서는 [SSR Caching](/docs/ssr-caching)을 참조하세요. 캐시를 단축하거나 비활성화해도 기간만 바뀔 뿐, SSR은 여전히 쿠키가 제거된 익명 분기를 렌더링합니다.

### 비하인드 스토리: 구글 OAuth

두 흐름(명시적 `/sign-in` 진입점 및 북마크된 경로 사례) 모두 OAuth 상태를 통해 반환 URL를 스레드합니다. 상태는 HMAC로 서명되어 있으므로 전송 중에 위조될 수 없습니다. 콜백에서 반환된 URL는 리디렉션 전에 동일 출처로 재검증되므로 유출된 서명 키는 여전히 공개 리디렉션 오라클로 전환될 수 없습니다.
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | `0` 또는 `1`로 설정하면 프로덕션을 포함한 모든 환경에서 비밀번호 가입 확인 정책을 선언합니다. `AUTH_SKIP_EMAIL_VERIFICATION`보다 우선합니다                                      |
...
기본적으로 꺼진 Dispatch의 `desktop.workspace-sso` 플래그는 사용 가능 여부만 제어하며 인증, 서명, nonce, origin, callback, 세션, 해지 검사는 항상 적용됩니다. 플래그가 꺼져 있거나 읽을 수 없으면 Desktop은 앱별 로그인, 로그아웃 또는 설정을 변경하지 않습니다.
User-Agent의 Canary 표시는 가용성 힌트일 뿐 원격 증명이 아니며, 인증과 권한 부여는 이를 신뢰하지 않습니다.
정식 호스팅 앱은 앱별 identity hub 환경 설정 없이 패키지된 Desktop SSO Canary 요청을 인식합니다. 일반 브라우저는 일반 앱별 로그인을 계속 사용합니다. 자체 호스팅 앱에는 남아 있는 명시적 Desktop 또는 서버 측 페더레이션 경로에만 `AGENT_NATIVE_IDENTITY_HUB_URL`이 필요합니다. 안정적인 Desktop 빌드는 이후 릴리스가 별도로 승인될 때까지 브로커를 초기화하지 않습니다.
Canary는 익명 앱 로그인 탐색을 가로채지 않습니다. 사용자는 활성 등록 앱에 대해 Desktop 설정에서 워크스페이스 로그인을 명시적으로 선택해야 하며, 일반 앱 로그인은 기본 경로로 유지됩니다.

## 관리 배포와 초대

관리 배포에서는 `AUTH_SIGNUP=invited`, `ORG_CREATION=closed`, `AUTO_CREATE_DEFAULT_ORG=0`을 설정합니다. [관리 배포 가이드](/docs/administered-deployments)에서 부트스트랩 관리자, 초대, 허용 도메인을 확인하세요. 멤버 초대와 도메인 가입은 별도의 규칙입니다.
