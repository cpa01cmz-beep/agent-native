...
Agent-Native의 기본 아이디어는 에이전트와 UI가 **동등한 파트너**라는 것입니다. 이유는 [Agent-Native란?](/docs/what-is-agent-native)를 참조하세요. 이 섹션은 그 약속의 나머지 절반, 즉 앱 옆에 도킹된 에이전트와 실제로 작업하는 느낌에 관한 것입니다.
...
<Diagram id="doc-block-1ohc2nc" title={"일상적인 루프"} summary={"도킹된 에이전트로 작업하는 5가지 방법 - 각각이 이 섹션의 페이지입니다."}>
...
<div class="diagram-loop">
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <strong>가벼운 UI</strong
    ><small class="diagram-muted">chat is the product</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card accent-card">
    <span class="diagram-pill accent">공동 편집</span
    ><small class="diagram-muted">실시간, 나란히</small>
  </div>
</div>
```
...
모든 앱에 전체 대시보드가 필요한 것은 아닙니다. 상담원이 제품인 경우 대부분의 맞춤 UI를 건너뛸 수 있습니다. 앱을 열고 원하는 것을 요청하면 상담원이 나머지 작업을 수행하게 됩니다. 에이전트에는 여전히 기록, 에이전트 리소스, 설정 등 관리 영역이 있지만 주요 상호 작용은 클릭이 아닌 대화입니다.
