...
<Diagram id="doc-block-30s6xp" title={"계층별 방어"} summary={"프레임워크는 대부분의 위협 표면을 소유합니다. 당신은 외부 입력의 범위 지정 및 유효성 검사를 위한 태그 지정 테이블이라는 두 가지를 소유하고 있습니다."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">프레임워크 소유</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">직접 소유</span
    ><small class="diagram-muted"
...
프레임워크는 세션에서 요청 컨텍스트를 만들고 앱 읽기 및 쓰기에 `accessFilter()`와 `assertAccess()`로 소유권을 적용합니다. 에이전트 SQL에는 활성 ID와 조직으로 필터링된 임시 뷰가 제공됩니다. RLS는 아직 활성화되지 않았으므로 앱의 직접 쿼리에서도 이 도우미를 계속 사용해야 합니다.
...
<Callout id="doc-block-91h30" tone="risk">

문자열 연결이나 템플릿 리터럴로 SQL를 빌드하지 마세요. 사용자 입력을 `args`에서 `exec` / `db-query`로 전달하거나 Drizzle을 사용하세요. 둘 다 항상 매개변수화됩니다. `pnpm guards` 검사는 CI 시간에 범위가 지정되지 않은 쿼리와 연결된 쿼리를 포착합니다.
...
<Diagram id="doc-block-adkvgz" title={"범위 지정 파이프라인"} summary={"SQL 에이전트는 기본 테이블을 직접 건드리지 않습니다. 현재 ID로 범위가 지정된 임시 뷰를 통해 읽으므로 기본 테이블 이름은 소유한 행만 반환할 수 있습니다."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    로그인된 세션<br /><small class="diagram-muted">email &middot; orgId</small>
  </div>
  ...
  <div class="diagram-node ok">
    에이전트 SQL<br /><small class="diagram-muted">bare table names only</small>
  </div>
</div>
```
...
`db-query` / `db-exec` 도구는 스키마 한정 테이블 참조(`public.<table>`)를 거부합니다. 정규화된 이름은 기본 테이블로 확인되고 위의 임시 뷰를 우회합니다. 에이전트는 기본 테이블 이름을 사용합니다. 범위 지정은 자동으로 적용됩니다.
...
<DataModel
  id="doc-block-197tsny"
  title="What ownableColumns() adds"
  summary={"테이블을 테넌트 인식하고 공유 가능하게 만드는 세 개의 열입니다."}
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "생성 시 소유자의 활성 조직입니다. 조직 가시성 검사를 진행합니다.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "비공개 | 조직 | 공개 — 대략적인 기본값, 기본값은 비공개입니다.",
        },
      ],
    },
  ]}
...
사용자별/조직별 자격 증명과 vault는 저장 시 AES-256-GCM으로 암호화됩니다. 공유 워크스페이스 vault는 `WORKSPACE_SECRETS_ENCRYPTION_KEY`를 우선하고 `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET`, `BETTER_AUTH_SECRET`에서 파생된 레거시 후보를 사용합니다. 회전 중에는 `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS`를 설정해 읽기가 행을 마이그레이션하도록 하세요. 기존 평문 행은 `pnpm action db-migrate-encrypt-credentials`(멱등성, 비파괴)로 암호화할 수 있습니다.
...
인바운드 웹훅 핸들러(Resend, SendGrid, Slack, Telegram, WhatsApp, Recall.ai, Deepgram, Zoom)는 프로덕션에서 기본적으로 위조된 요청을 거부합니다. 해당 서명 비밀 env var가 누락되면 핸들러는 수락 및 전달 대신 401을 반환합니다.
...
Google Drive `changes.watch` 알림은 이 일반 어댑터 게이트를 통과하지 않습니다. 본문이 없는 불투명한 요청이므로 프레임워크는 채널별로 임의 토큰을 등록하고 변경사항을 가져오기 전에 `X-Goog-Channel-ID`, `X-Goog-Channel-Token`, `X-Goog-Resource-ID` 헤더를 확인합니다. 이는 Pub/Sub 메시지가 아닌 Drive의 직접 알림이며 Pub/Sub OIDC bearer 토큰을 포함하지 않습니다.
...
- [ ] 공유 멀티앱 vault에는 `WORKSPACE_SECRETS_ENCRYPTION_KEY`를 동일하게 설정하고, 앱별 암호화에는 `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` fallback을 유지하세요 - [Secrets Management](#secrets) 참조
...
`agent-native doctor`(스캐폴딩된 앱에서는 `pnpm doctor`)를 실행하면 이 체크리스트의 스키마, 자격 증명, 환경 변수 항목을 자동으로 확인할 수 있습니다 — 각 결과는 guard, 파일, 줄을 알려줍니다. 자세한 내용은 [Doctor](/docs/doctor) 문서를 참조하세요.
...
- [**Authentication**](/docs/authentication) — 이 페이지가 기반으로 하는 인증 모드, cookie realm, 세션 세부 정보
- [**Multi-Tenancy**](/docs/multi-tenancy) — `org_id` 스코핑 뒤에 있는 조직 멤버십 모델
- [**Sharing**](/docs/sharing) — 공유 가능한 리소스에 적용되는 `ownableColumns()` 및 access-guard 패턴
- [**Messaging**](/docs/messaging#env-vars) — 위 체크리스트에서 참조하는 통합별 webhook 서명 비밀
- [**Deployment**](/docs/deployment) — 이 체크리스트가 설정되어 있다고 가정하는 프로덕션 환경 변수
