...
<Diagram id="doc-block-unss0s" title={"출처에서 인용 답변까지"} summary={"Brain은 승인된 소스를 원시 캡처로 수집하고, 내구성 있는 메모리를 추출하고, 검토를 통해 이를 게이트화한 다음 인용으로 답변합니다."}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    원시 캡처<br /><small class="diagram-muted">중복 제거됨, 수정됨</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted">사실 · 결정 · 프로세스</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">승인됨, 원자적</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-vvekcs">
  <Screen
    surface="desktop"
    html={
...
벡터 데이터베이스 요구 사항이 없으므로 템플릿은 PGlite 전체에서 이식성을 유지합니다.
Postgres, Neon, hosted PostgreSQL 및 유사한 호스트. 애플리케이션 상태는
현재 경로, 필터 및 선택된 ID를 통해 상담원은 항상 현재 정보를 알 수 있습니다.
탐색 및 선택.

Brain의 스키마는 `templates/brain/server/db/schema.ts`에 있습니다. 테이블 9개:
...
<Endpoint id="doc-block-1rqor2m" title={"서명된 수집 웹훅"} summary={"클립 및 일반 transcript/capture 가져오기는 소스별 전달자 토큰과 함께 RawCapturePayload을 게시합니다."} method="POST" path="/api/_agent-native/brain/ingest" summary={"Clips 또는 일반 소스에서 원시 캡처 가져오기"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
이러한 변경은 저장소 개발 워크플로에서 수행하세요. 임베디드 에이전트는
호스트가 repository/workspace 쓰기 도구를 의도적으로 부여한 경우에만 소스
편집을 도울 수 있습니다. 그 외에는 actions와 데이터를 통해 작업합니다.
[Key Concepts](/docs/key-concepts#agent-modifies-code)를 참조하세요.

## 다음 단계

- [**Dispatch**](/docs/dispatch) — 작업 공간 제어 평면
- [**Dispatch template**](/docs/template-dispatch) — 비계 조정 앱
- [**에이전트 리소스**](/docs/agent-resources) — 앱 전체에서 공유 리소스
- [**A2A Protocol**](/docs/a2a-protocol) — 교차 앱 위임
- [**템플릿**](/docs/cloneable-saas) — 복제 후 소유하는 모델과 시작할 수 있는 모든 도메인 템플릿
- [**시작하기**](/docs/getting-started) — Actions, 애플리케이션 상태, 실시간 동기화 등 프레임워크의 기본 사항
