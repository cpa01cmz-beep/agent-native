---
title: "드롭인 에이전트"
description: "<AgentPanel>, <AgentSidebar> 및 sendToAgentChat()을 사용하여 상담원 채팅 + 에이전트 리소스를 React 앱에 마운트하세요."
---
...
처음부터 에이전트 네이티브를 구축할 필요는 없습니다. 상담원 채팅, 에이전트 리소스 탭, CLI 터미널, 음성 입력 및 모든 관련 인프라는 앱에 드롭하는 소수의 React 구성 요소로 제공됩니다.

> **전제 조건:** 서버는 `agent-chat-plugin`를 실행해야 합니다(모든 템플릿에 자동으로 마운트됨). 처음부터 시작하는 경우 [Server](/docs/server-overview)를 참조하세요.
>
...
| `<AgentPanel>`        | 원시 패널 자체 — 채팅 + CLI + 에이전트 리소스 탭                                       | 레이아웃 또는 전담 에이전트 페이지에 대한 완전한 제어를 원하는 경우 |
...
<Diagram id="doc-block-1jfm3tw" title={"마운트 모델"} summary={"<AgentSidebar>은 기존 레이아웃을 래핑합니다. 경로는 주요 영역에서 렌더링됩니다. 에이전트 패널이 그 옆에 마운트됩니다. <AgentPanel>은 래퍼가 없는 동일한 패널입니다."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        귀하의 앱<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        에이전트 패널<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1ildzlq"
  title={"<AgentSidebar>으로 루트 레이아웃 래핑"}
  ...
  annotations={[
    ...
    },
    {
      lines: "8-12",
      label: "시작 메시지",
      note: "`suggestions` 빈 채팅에서 클릭 가능한 칩으로 렌더링됩니다.",
    },
    {
      lines: "13",
      label: "상황 인식 칩",
...
그렇습니다. 이제 사용자는 채팅 기록, 에이전트 리소스 탭, CLI 터미널, 음성 입력 및 더 넓은 드로어를 포함하여 모든 페이지에 전환 가능한 에이전트를 갖게 됩니다. `localStorage`를 통해 다시 로드해도 상태가 유지됩니다.

### 소품

- **`children`** — 앱의 일반 레이아웃 및 경로입니다. 주요 영역에서 렌더링됩니다. 상담사 패널은 데스크톱에서는 옆에, 모바일에서는 그 위에 설치됩니다.
...
`<AgentPanel>`는 사이드바 래퍼, 축소 버튼 또는 상태 지속성 없이 원시 탭(Chat / CLI / 에이전트 리소스)을 제공합니다. 원하는 곳에 두십시오. 레이아웃을 처리합니다.
...
- **`isFullscreen`** — 페이지 또는 사용자 지정 패널에서 Claude 스타일의 중앙 열을 원할 때 설정합니다. 사이드바는 대신 더 넓은 드로어를 사용합니다.
...
인라인 `llm()` 호출로 별도의 AI 경로를 만드는 대신 에이전트에 작업을 전달하는 버튼입니다. 이 차이가 중요한 이유는 [왜 이 방식으로 앱을 만들까요](/docs/what-is-agent-native#why-build-apps-this-way)를 참조하세요.
...
- **사용자가 시작한 표시 가능한 AI 작업**에는 `openSidebar: true`를 사용하세요. `false`는 명시적으로 조용한 백그라운드 또는 시스템 시작 작업에만 사용하고 후속 작업은 같은 AgentSidebar 스레드에서 계속하세요.
- **`type`** — `"content"`(기본값)는 앱 도구를 사용하는 임베디드 에이전트에서 작업을 유지합니다. `"code"`는 저장소 소스 변경을 위한 별도의 코드 지원 호스트로 라우팅합니다. 쓰기는 호스트가 repository/workspace 도구를 의도적으로 부여한 경우에만 사용할 수 있습니다. [Agent-Native 코드 UI](/docs/code-agents-ui)를 참조하세요.
...
후속 작업과 수정 프롬프트는 기존 사이드바 스레드에서 계속하세요. 결과 옆에 두 번째 자유 입력란을 추가하지 말고, UI에서 검증해야 하는 구조화된 값에만 로컬 양식을 사용하세요.

동일한 경로가 MCP 앱으로 삽입된 경우 제출
`sendToAgentChat()` 통화는 지원되는 호스트 채팅으로 전달됩니다. 참조
MCP 앱 브리지 동작을 위한 [Agent 채팅](/docs/client-agent-chat#sendtoagentchat).
...
## 에이전트 채팅과 action 중 선택하기 {#agent-or-action}

CRUD, 검증, provider 읽기, 영속화와 같은 결정적인 로컬 작업에는 `useActionQuery` 또는 `useActionMutation`을 직접 사용하세요. 사용자가 워크플로를 조사, 분석, 생성, 추천, 합성 또는 다단계 추론으로 경험한다면 `sendToAgentChat({ openSidebar: true })`로 시작하고 에이전트가 집중된 action을 조정하게 하세요. 구현이 결정적이라는 이유만으로 AI처럼 보이는 워크플로를 하나의 불투명한 action 안에 숨기지 마세요.

## UI: `useActionMutation()`의 형식 안전 actions {#use-action-mutation}

UI가 에이전트 도구와 같은 작업을 실행해야 하는 경우 [`useActionMutation`](/docs/client-data#use-action-mutation)을 사용하세요.
...
유형이 안전한 인수는 `defineAction()`의 zod 스키마에서 나옵니다. 전체 동작 시스템은 [Actions](/docs/actions-overview)를 참조하세요.

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation`과 `sendToAgentChat`.** 사용자가 결정적인 버튼("Send reply")을 클릭했을 때는 `useActionMutation`으로 작업을 직접 실행하세요. 작업에 에이전트의 추론, 도구 또는 여러 단계의 계획이 필요하면 `sendToAgentChat`에 맡기세요. UI에서 인라인 `llm()`을 호출하지 마세요. 공유 에이전트 루프 대신 별도의 AI 경로가 만들어집니다.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` 및 `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — 선택, 탐색, 보기 화면
- [**에이전트 리소스**](/docs/agent-resources) — 에이전트 리소스 탭에 포함된 내용(skills, 메모리, MCP 서버, 예약된 작업)
