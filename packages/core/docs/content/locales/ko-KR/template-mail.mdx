...
<WireframeBlock id="doc-block-93d63u">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-d9e9pa" title={"메일 요청 흐름 방식"} summary={"키보드 단축키와 상담원 프롬프트는 동일한 작업을 실행합니다. 이메일은 Gmail에 있습니다. 초안, 자동화 및 추적이 SQL 및 application_state에 실시간으로 제공됩니다."}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      사용자가 조작<br /><small class="diagram-muted">J/K/E/R 단축키</small>
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Gmail<br /><small class="diagram-muted">여러 계정, OAuth 경유</small>
    </div>
    <div class="diagram-box">
  ...
  <div class="diagram-box">받은편지함 실시간 새로고침</div>
</div>
```
...
<DataModel
  ...
  title={"메일 SQL 테이블"}
  summary={
    "이메일 자체는 Gmail에 있습니다. SQL 테이블에는 Gmail에 없는 내용(대기 중인 초안, 전송 추적 이벤트, OAuth 토큰)이 들어 있습니다. 설정 및 임시 상태는 settings 및 application_state 저장소에 있습니다."
  }
  entities={[
    {
...
**에이전트 기능 추가.** `defineAction`를 사용하여 `templates/mail/actions/` 아래에 새 파일을 추가합니다. 귀하의 작업은 에이전트 도구, CLI 명령(`pnpm action <name>`) 및 `useActionQuery`/`useActionMutation`를 통해 입력된 프런트엔드 후크 표면이 됩니다. 간단한 예를 보려면 `templates/mail/actions/star-email.ts`를 보거나 여러 개의 하위 actions가 있는 경우에는 `templates/mail/actions/manage-automations.ts`를 살펴보세요. 전체 패턴은 [actions](/docs/actions-overview) 문서를 참조하세요.
...
이러한 변경은 자체 저장소와 개발 워크플로에서 수행합니다. 임베디드 에이전트는 호스트가 repository/workspace 쓰기 도구를 의도적으로 부여한 경우에만 소스 편집을 도울 수 있습니다. 그 외에는 actions와 데이터를 통해 작업합니다. [Key Concepts](/docs/key-concepts#agent-modifies-code)를 참조하세요.

## 다음 단계

- [**템플릿**](/docs/cloneable-saas) — 복제해 소유하는 모델과 시작점으로 사용할 수 있는 다른 모든 도메인 템플릿
- [**시작하기**](/docs/getting-started) — actions, application state, 실시간 동기화 등 프레임워크의 기본 사항
- [**캘린더**](/docs/template-calendar) — 일정 관리와 예약을 다루는 관련 도메인 템플릿
- [**자동화**](/docs/automations) — Mail의 자동 분류 규칙이 기반으로 삼는 이벤트 트리거 엔진
- [**Actions**](/docs/actions-overview) — 채팅, 단축키, 자동화가 호출하는 action 시스템
