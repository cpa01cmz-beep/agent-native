---
title: "실시간 협업"
description: "AI 에이전트가 최고 수준의 피어인 다중 사용자 공동 편집: CRDT 병합, 실시간 존재, SSE 빠른 경로 및 세분화된 서버측 병합 — PostgreSQL/PGlite 및 모든 호스트에서 가능합니다."
---
...
상담원이나 다른 사용자가 SQL에 쓸 때 새로 고치기 위해 UI만 필요한 경우에는 이 중 어떤 것도 필요하지 않습니다. [`useDbSync`](/docs/client-data#usedbsync)를 사용하세요. 이 페이지는 단일 서식 있는 텍스트 문서의 문자 수준 공동 편집을 위한 것입니다(공유 커서, 충돌 없는 병합). 둘 다 동일한 `/_agent-native/poll` 채널을 타고 있습니다.
...
이 테이블은 프레임워크에서 관리되며 공급자에 구애받지 않습니다(PGlite 및 Postgres 사용
...
데이터 모델에 맞는 액세스 모드를 선택하세요. 소유자가 있거나 공유 가능한
레코드는 `registerShareableResource`로 등록한 유형과 함께
`mode: "resource"`를 사용합니다. 배포의 모든 인증된 사용자에게 의도적으로
공개하는 레코드는 `mode: "all-authenticated"`를 명시합니다.
경고를 없애기 위해 리소스 유형을 임의로 만들지 마세요.
...
`access` 생략은 더 이상 권장되지 않는 레거시 동작입니다. `all-authenticated`처럼
동작하고 경고를 기록하며 Doctor가 이를 표시합니다.
...
- 업데이트 및 삭제는 `update-comment` / `delete-comment` actions를 통해 실행됩니다. 생성 및 나열은 `add-comment` / `list-comments` actions를 통해 실행됩니다. 사용자 정의 템플릿은 핵심 `POST /_agent-native/collab/:docId/search-replace` 경로에 대해 자체적으로 동등한 엔드포인트를 구현합니다.
...
## 호스팅형 실시간 동기화 게이트웨이 {#hosted-gateway}

앱은 기본적으로 위에서 설명한 poll/SSE 전송을 사용합니다. 어떤 앱이든 대신
호스팅형 실시간 동기화 게이트웨이를 선택할 수 있습니다. 게이트웨이는 앱의
`sync_events` 테이블을 중앙에서 추적하고 연결된 모든 클라이언트에 변경을
전달하므로, 변경 전달이 더 이상 각 서버리스 인스턴스가 자체 장기 연결을
유지하는 데 의존하지 않습니다.

설정 방법은 누가 앱을 배포하는지에 따라 다릅니다.

### Builder 호스팅 앱: 할 일 없음 {#hosted-gateway-builder}

Builder 호스팅 파이프라인으로 배포된 앱은 자동으로 프로비저닝됩니다. 배포 시
전송 방식, 게이트웨이 URL, 채널 ID, 서명 시크릿
(`AGENT_NATIVE_REALTIME_HMAC_SECRET`)이 예약된 환경 변수로 주입됩니다.
게이트웨이가 읽는 데이터베이스는 파이프라인이 이미 프로비저닝했기 때문입니다.

프로젝트를 옵트아웃하려면 해당 프로젝트의 `builder.config.json`에 `realtime`
블록을 추가하세요.
...
### 그 외 환경에 배포한 경우: 자체 등록 {#hosted-gateway-self}

자체 Netlify, Vercel, 컨테이너, VM에 있는 앱에는 그런 프로비저닝이 없으므로 앱이
직접 게이트웨이에 자신을 소개합니다. 배포에 환경 변수 하나만 설정하세요.

```bash
AGENT_NATIVE_REALTIME_TRANSPORT=hosted
```

이 변수를 설정하는 것이 곧 명시적 동의이며, 이 기능이 추가하는 변수는 이것
하나뿐입니다. 설정하지 않은 배포에서는 여기 적힌 어떤 일도 일어나지 않습니다.

배포는 또한 네 가지 조건을 충족해야 하는데, 일반적인 프로덕션 앱이라면 모두 이미
만족합니다.

| 요구 사항                                           | 이유                                                                                                                                                                           | 확인 방법                                                                                                                                                                                    |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **공개적으로 접근 가능한 호스트의 Postgres**        | 게이트웨이는 자체 네트워크에서 데이터베이스에 접속합니다. PGlite, `localhost`, `*.internal`, 순수 IP 주소는 PostgreSQL 연결 문자열이 사용자의 머신을 떠나기도 전에 거부됩니다. | `DATABASE_URL`이 Neon 같은 관리형 Postgres를 가리킬 것                                                                                                                                       |
| **`BUILDER_PRIVATE_KEY`를 «배포» 환경 변수로 설정** | 등록을 인증하고, 어떤 Builder 조직이 채널을 소유하는지 결정합니다.                                                                                                             | 아래 주의 사항 참조 — 가장 자주 놓치는 단계입니다                                                                                                                                            |
| **프로덕션 배포일 것**                              | 배포 프리뷰는 로컬 동기화를 유지하므로, 일회성 브랜치 자격 증명이 머신을 벗어나지 않고 활발한 저장소가 풀 리퀘스트마다 채널을 만들지도 않습니다.                               | `CONTEXT=production` / `BRANCH=production` 또는 `VERCEL_ENV=production`                                                                                                                      |
| **플랫폼이 배포를 식별할 것**                       | 등록은 앱의 공개 오리진을 키로 삼으므로, 실제로 그 배포가 아닌 프로세스가 오리진을 주장해서는 안 됩니다.                                                                       | Netlify, Vercel, AWS Lambda, Cloud Run, Render, Cloudflare Pages는 모두 스스로를 표시합니다. 순수 컨테이너나 VM에서는 `AGENT_NATIVE_REALTIME_APP_URL`에 해당 배포 자신의 오리진을 설정하세요 |

<Callout tone="warning">

**앱 안에서 Builder를 연결하는 것만으로는 부족합니다.** 앱 내 Builder 연결은
자격 증명을 사용자별 또는 조직별로 앱의 시크릿 볼트에 저장합니다. 자체 등록은
이 자격 증명을 의도적으로 무시합니다 — 한 사용자의 연결이 우연히 첫 요청을 보낸
조직 아래에 배포 전체의 데이터베이스를 등록해서는 안 되기 때문입니다 — 그리고
배포 수준의 `BUILDER_PRIVATE_KEY` 환경 변수만 읽습니다. "Builder 연결됨"이라고
표시되더라도 그 변수가 없으면 앱은 로컬 동기화에 머무릅니다.

</Callout>

이후 등록은 채널이 필요한 첫 요청에서 온디맨드로 이루어집니다. 앱은 PostgreSQL 연결 문자열과
공개 오리진을 전송하고 채널 ID와 서명 시크릿을 돌려받은 뒤, Builder 호스팅 앱과
똑같이 구독 토큰을 발급합니다. 데이터베이스 URL, 앱 오리진, Builder 자격 증명 중
하나라도 바뀌면 스스로 다시 실행되므로, 자격 증명 교체가 다른 어떤 것도 재배포할
필요 없이 반영됩니다.

워크스페이스 배포(하나의 오리진에서 여러 앱을 제공)는 자체 등록하지 않습니다.
형제 앱들이 오리진을 공유하고 등록은 그 오리진을 키로 삼으므로 하나의 채널에서
충돌하게 되기 때문입니다. 그런 경우 해당 사실을 로그에 한 줄 남기고 로컬 동기화를
유지합니다.

### 정상 동작 확인 {#hosted-gateway-verify}

`GET /_agent-native/health`가 결과를 알려주며, 이를 해석하는 과정에서 등록이
수행되므로 curl 한 번으로 활성화와 확인이 동시에 됩니다.
...
| 필드          | 의미                                                                                                                                                                        |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `transport`   | 환경 변수가 설정되면 `"hosted"`, 아니면 `"local"`. 여기에 `local`이 표시되면 변수가 실행 중인 배포에 전달되지 않은 것입니다.                                                |
| `registered`  | 채널이 해석되었습니다. `false`는 앱이 여전히 자체 `/_agent-native/poll`을 사용하고 있다는 뜻입니다.                                                                         |
| `channelHash` | 채널 ID의 SHA-256 앞 8자 — 두 배포를 구분하거나 교체를 확인하기에는 충분하지만 접속에는 쓸 수 없습니다. 이 엔드포인트는 공개되어 있으므로 ID 자체는 절대 공개하지 않습니다. |
| `unavailable` | 해석이 «실패»했을 때만 존재하며, "채널 없음"으로 해석된 것과는 다릅니다. 도달할 수 없는 게이트웨이와 등록되지 않은 앱은 서로 다른 문제입니다.                               |

`registered`가 `false`이면 배포 로그에 이유를 알려주는 `[realtime]` 줄이
남습니다 — 거부된 자격 증명, 파싱할 수 없는 데이터베이스 URL, 배포처럼 보이지
않는 프로세스 등. 위 요구 사항 표를 순서대로 확인하세요.

### Builder가 저장하는 것과 신뢰 경계 {#hosted-gateway-trust}

**자체 등록은 Builder에 PostgreSQL 연결 문자열을 보낸다는 뜻입니다.** Builder는
이를 저장하고 `sync_events`를 추적하기 위해 사용자의 데이터베이스에 접속합니다.
이 연결은 읽기 전용이 아닙니다. 버전 할당과 대역 외 변경 감지 모두 쓰기를
수행합니다. 원하지 않는다면 전송 변수를 설정하지 마세요. 결정은 그것뿐이며, 이
자격 증명을 공유하는 다른 경로는 없습니다.

**조직이 신뢰 경계입니다.** 등록할 수 있는 자격 증명은 기존 오리진을 다시 등록할
수도 있으며, 그러면 해당 채널의 서명 시크릿이 반환되고 데이터베이스 대상이
변경됩니다. 그 시크릿을 가지면 앱의 임의 사용자에 대한 구독 토큰을 발급할 수 있어
그 사용자의 _변경 스트림_ — 어떤 레코드가 언제 바뀌었는지 — 이 노출됩니다. 다만
레코드 자체는 노출되지 않습니다. 클라이언트는 자신의 세션으로 앱에서 콘텐츠를
다시 읽기 때문입니다. 등록 자격 증명은 조직 전체에 해당하는 것으로 취급하세요.

### 동작 방식 {#hosted-gateway-mechanics}

- `GET /_agent-native/realtime-token`은 브라우저가 게이트웨이 연결을 여는 데
  사용하는, 수명이 짧고(10분) 세션 범위로 한정된 구독 토큰을 발급합니다.
  동일 오리진만 허용하며 활성 세션이 없으면 401을 반환합니다.
- `GET /_agent-native/can-see`는 게이트웨이의 이벤트별 가시성 검사입니다. 서명된
  게이트웨이 액세스 토큰을 검증한 뒤 앱 자체의 `resolveAccess`를 다시 실행하므로,
  이미 접근 권한을 잃은 공유 대상자에게 게이트웨이가 이벤트를 전달하는 일은
  없습니다. 알 수 없는 리소스 유형이나 조회 실패는 닫힌 쪽으로 실패합니다
  (`{ allowed: false }`).
- Postgres에서 호스팅형 앱은 동기화 버전을 인메모리 카운터가 아니라 데이터베이스
  (`dbAssignedVersions`)에서 할당합니다. 게이트웨이 인스턴스와 앱 자체의 서버리스
  인스턴스가 하나의 데이터베이스를 공유하므로 쓰기 순서에 대해 합의해야 하기
  때문입니다.
- `AGENT_NATIVE_REALTIME_GATEWAY_URL`은 스테이징용으로 게이트웨이 엔드포인트를
  재정의합니다. 설정하지 않으면 `BUILDER_GATEWAY_BASE_URL`에서 파생됩니다.

여기서의 모든 실패는 설계상 부드럽게 처리됩니다. 전송 변수 없음, 배포 수준의
Builder 키 없음, Postgres가 아닌 데이터베이스, 도달할 수 없는 게이트웨이 —
어느 경우든 오류를 내지 않고 앱은 로컬 폴링에 머무릅니다.
`GET /_agent-native/realtime-token`이 404를 반환하면 채널이 해석되지 않았다는
뜻입니다.
...
소유자가 있거나 공유 가능한 리소스에는 `registerShareableResource`로
등록한 유형을 사용합니다.
...
경고를 없애기 위해 `resourceType`를 만들지 마세요. 해당 소유권 및 공유 모델이
존재할 때만 `resource` 모드가 올바릅니다. 레거시 호환성을 위해 `access` 생략은
계속 지원되지만 `all-authenticated`처럼 동작하고 경고를 기록하며 Doctor가 이를
표시합니다. 명시적 액세스 권한이 있는 비소유자는 상태 벡터 따라잡기를 사용합니다
(안전하지만 대기 시간이 더 깁니다).

### 액세스 확인

모든 협업 경로에는 인증이 필요합니다. `resource` 모드에서는 읽기에
