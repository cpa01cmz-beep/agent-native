...
> **Qual documento do espaço de trabalho?** Esta página aborda o **formato de implantação** — um monorepo, muitos aplicativos, autenticação compartilhada e uma implantação unificada. Para a camada de personalização do agente (Recursos do Agente: `AGENTS.md`, memória, skills, agentes personalizados) consulte [Recursos do Agente](/docs/agent-resources); para governança (quem analisa, aprova e possui o quê), consulte [Workspace Governance](/docs/workspace-management).

## Aplicativo independente ou workspace? {#standalone-or-workspace}

Escolha um workspace quando uma empresa opera vários aplicativos internos que precisam compartilhar configurações de implantação e contexto da organização. Escolha um aplicativo independente para uma ferramenta isolada ou um protótipo. Esse formato de implantação é compatível, mas não inclui pacotes compartilhados nem configuração raiz do workspace. Consulte [Implantações administradas](/docs/administered-deployments).
...
<FileTree
  id="doc-block-vhde85"
  title="Um workspace gerado"
  entries={[
    {
      path: "package.json",
      note: "Declara agent-native.workspaceCore",
    },
    {
    ...
    {
      path: ".env.example",
      note: "ANTHROPIC_API_KEY, A2A_SECRET, DATABASE_URL, ... compartilhados",
    },
    {
    ...
    },
    {
      ...
      note: "Overrides de plugin somente quando necessário",
    },
    {
      path: "packages/shared/src/client/",
      note: "Código React compartilhado somente quando necessário",
    },
    {
      path: "packages/shared/AGENTS.md",
      note: "Instruções para todo o workspace",
    },
    {
      path: "apps/mail/",
    },
...
<Diagram id="doc-block-1y2xf74" title={"Três camadas, mescladas por nome de arquivo"} summary={"Cada aplicativo resolve plug-ins, habilidades, ações e AGENTS.md primeiro do app-local, depois do pacote compartilhado e, em seguida, do padrão da estrutura."}>
...
Os recursos de despacho têm como escopo **Todos os aplicativos** (cada aplicativo os herda em tempo de execução, sem etapa de cópia ou sincronização) ou **Aplicativos selecionados** (concedido por aplicativo para contexto específico do aplicativo). Consulte [Recursos do Agente](/docs/agent-resources#global-resources) para obter a tabela completa de modelos de recursos, convenções de caminho e o pacote inicial recomendado.
...
<Diagram id="doc-block-1l5ncko" title={"Implantação unificada: uma origem, um caminho por aplicativo"} summary={"Cada aplicativo é fornecido com uma única origem, portanto, as sessões de login e o A2A entre aplicativos são gratuitos."}>
...
- [Recursos do Agente](/docs/agent-resources) — a camada de personalização (`AGENTS.md`, `LEARNINGS.md`, memória pessoal, skills, agentes personalizados) compartilhada por todos os aplicativos no espaço de trabalho.
