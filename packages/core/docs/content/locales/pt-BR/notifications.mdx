...
<Diagram id="doc-block-16x7wpy" title="Uma chamada, muitos destinos" summary={"notify() sempre grava a linha da caixa de entrada com escopo do proprietário, espalha todos os canais registrados em paralelo (melhor esforço) e emite warning.sent no barramento de eventos."}>
...
E-mail, Slack e webhook são sempre registrados junto com a caixa de entrada. A entrega prioriza a URL ou os destinatários nos metadados da notificação e, em seguida, usa a variável de ambiente correspondente. Se nenhum deles estiver definido, esse canal não faz nada durante a emissão.
...
<Diagram id="doc-block-1b6ozy6" title="Canais e gravidade" summary={"a caixa de entrada está sempre ativada; o webhook precisa de um env var; canais personalizados são registrados na inicialização. A severidade impulsiona o estilo do emblema e é transmitida a todos os canais."}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>A gravidade define o badge</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">confirmações, FYI</span>
    </div>
    <div class="sev-row">
...
<Endpoint id="doc-block-11yfpn9" title={"Listar notificações"} method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary={"Listar notificações recentes para o usuário atual"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "Cursor de timestamp ISO — retorna notificações criadas antes desse horário para paginação."
  }
]} responses={[
  {
    "status": "200",
...
- **Integração de pesquisa** — cada mutação chama `recordChange()` para que os modelos que usam [`useDbSync`](/docs/client-data#usedbsync) sejam invalidados automaticamente sem qualquer conexão extra.
...
- [**Server plugins**](/docs/server-overview) — onde os canais personalizados são registrados na inicialização
