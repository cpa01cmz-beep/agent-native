...
<Diagram id="doc-block-1yyr150" title={"Da fonte à resposta citada"} summary={"O Brain ingere fontes aprovadas em capturas brutas, destila memória durável, passa por revisão e só então responde com citações."}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    Capturas brutas<br /><small class="diagram-muted"
      >deduplicado, redigido</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted"
      >fatos · decisões · processos</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">aprovado, atômico</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-10sg8a3">
  <Screen
    surface="desktop"
    html={
...
não há necessidade de banco de dados vetorial, portanto o modelo permanece portátil em PGlite,
Postgres, Neon, hosted PostgreSQL e hosts semelhantes. O estado do aplicativo reflete o
rota atual, filtros e IDs selecionados para que o agente sempre saiba a rota atual
navegação e seleção.

O esquema do cérebro reside em `templates/brain/server/db/schema.ts`. Nove tabelas:
...
<DataModel
  id="doc-block-blbynx"
  title="Modelo de dados cerebrais"
  summary={
  ...
  entities={[
    ...
    },
    {
      ...
      note: "Entradas atômicas destiladas",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Itens de revisão pendentes",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Sincronizar registro de auditoria",
      fields: [
        {
          name: "source_id",
          type: "id",
    ...
    },
    {
      ...
      note: "Fila de destilação em segundo plano",
      fields: [
        {
          name: "operation",
          type: "text",
...
<Endpoint id="doc-block-1kkpo0w" title={"Webhook de ingestão assinado"} summary={"Clipes e importações transcript/capture genéricas postam um RawCapturePayload com um token de portador por origem."} method="POST" path="/api/_agent-native/brain/ingest" summary={"Importe uma captura bruta de Clips ou de uma fonte genérica"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
Faça essas alterações no fluxo de desenvolvimento do repositório. O agente
incorporado só pode ajudar a editar o código-fonte quando o host concede
intencionalmente ferramentas de escrita de repository/workspace; caso
contrário, trabalha por meio de actions e dados. Veja
...
- [**Recursos do Agente**](/docs/agent-resources) — recursos compartilhados entre aplicativos
- [**A2A Protocol**](/docs/a2a-protocol) — delegação entre aplicativos
- [**Modelos**](/docs/cloneable-saas) — o modelo de clonar e possuir e todos os outros modelos de domínio para começar
