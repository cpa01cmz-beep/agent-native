...
<Diagram id="doc-block-13mdnqm" title="MCP Aplicativo incorporado de ida e volta" summary={"O destino do link da ação também é o destino incorporado. Hosts capazes carregam a mesma rota de aplicativo assinada em linha; todos os outros voltam para o link direto."}>
...
<div class="diagram-embed">
  ...
  <div class="diagram-card" data-rough>
    ...
    ><small class="diagram-muted">troca ticket SQL de uso único</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card" data-rough>
    <strong>Signed app route</strong
    ><span class="diagram-pill ok">rota React real</span
    ><small class="diagram-muted">short-lived browser session</small>
  </div>
  <div class="diagram-fallback">
    <span class="diagram-pill warn">sem suporte a MCP Apps</span>
...
<AnnotatedCode
  id="doc-block-1vcqj5m"
  title={"A configuração do recurso mcpApp"}
  ...
  annotations={[
    {
      lines: "6",
      label: "Aprimoramento progressivo",
      note: "`mcpApp.resource` anuncia um UI embutido para hosts que suportam a extensão MCP Apps. Mantenha o construtor `link` da ação também – hosts somente CLI e mais antigos ignoram os metadados UI e ainda precisam do link direto.",
    },
    {
      lines: "7",
    ...
    {
      lines: "11",
      label: "Rótulo substituto universal",
      note: '`openLabel` é o texto visível `"Open in … →"` usado como saída de link profundo quando um host não renderiza o iframe inline.',
    },
  ]}
/>
...
<Diagram id="doc-block-14hyaa7" title="Dois caminhos de ponte de host, uma rota assinada" summary={"Claude transplanta a via hidratada e utiliza a ui/_bridge direta; ChatGPT obtém um iframe controlado via window.openai e retransmite as ações do host por postMessage. Ambos apontam para a mesma rota de aplicativo assinada."}>
...
<div class="diagram-bridge">
  <div class="diagram-col">
    <div class="diagram-card" data-rough>
      ...
      ><small class="diagram-muted"
        >hidrata o HTML assinado do app no iframe do Claude, depois ponte direta
        do host `ui/_`</small
      >
    </div>
    <div class="diagram-card" data-rough>
  ...
  <div class="diagram-box" data-rough>
    Same signed app route<br /><small class="diagram-muted"
      >rota normal + componentes React</small
    >
  </div>
</div>
```
...
- [Actions](/docs/actions-overview) — `defineAction`, o construtor `link`, `publicAgent`.
