...
<Diagram id="doc-block-1gqtn6n" title={"Adaptador CLI → registro → superfície de ação"} summary={"ShellCliAdapter encapsula um binário; CliRegistry coleta adaptadores para descoberta; defineAction expõe uma chamada na superfície de ação do agente + UI."}>
...
- [**Actions**](/docs/actions-overview) — os adaptadores CLI da superfície de ação geralmente são incluídos.
