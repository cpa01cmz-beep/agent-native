...
Os pacotes `app-skill` também geram arquivos Agent Plugin padrão. Use `agent-native plugin import <path> --into <workspace> --yes` para importar Skills e servidores MCP remotos via Streamable HTTP; os cabeçalhos são ignorados e os servidores stdio e SSE são informados e ignorados.
...
- A orientação é única: coloque-a em `AGENTS.md` ou na memória do agente
...
- [**Recursos do Agente**](/docs/agent-resources#skills) — onde os skills ficam como recursos SQL no modo App
