...
<WireframeBlock id="doc-block-4yvkvj">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-1y413ry" title={"Como flui uma solicitação de email"} summary={"Os atalhos de teclado e os prompts do agente executam as mesmas ações. O e-mail reside em Gmail; rascunhos, automações e rastreamento ao vivo em SQL e application_state."}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      Você conduz<br /><small class="diagram-muted">atalhos J/K/E/R</small>
    </div>
    <div class="diagram-node">
      Você pede ao agente<br /><small class="diagram-muted"
        >"redija uma recusa amigável"</small
      >
    </div>
  </div>
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Gmail<br /><small class="diagram-muted">várias contas, via OAuth</small>
    </div>
    <div class="diagram-box">
      SQL + application_state<br /><small class="diagram-muted"
        >rascunhos · automações · rastreamento</small
      >
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">Caixa de entrada atualiza ao vivo</div>
</div>
```
...
<DataModel
  id="doc-block-8jxgi9"
  title="Enviar tabelas SQL"
  summary={
    "O próprio e-mail reside em Gmail. As tabelas SQL contêm o que Gmail não contém: rascunhos em fila, eventos de rastreamento de envio e tokens OAuth. As configurações e o estado efêmero residem nas configurações e nos armazenamentos application_state."
  }
  entities={[
    {
...
**Adicionando um recurso de agente.** Adicione um novo arquivo em `templates/mail/actions/` usando `defineAction`. Sua ação se torna uma ferramenta de agente, um comando CLI (`pnpm action <name>`) e uma superfície de gancho de frontend digitada por meio de `useActionQuery`/`useActionMutation`. Veja `templates/mail/actions/star-email.ts` para um pequeno exemplo ou `templates/mail/actions/manage-automations.ts` para um com vários sub-actions. Consulte a documentação do [actions](/docs/actions-overview) para ver o padrão completo.
...
Faça essas alterações no seu repositório e fluxo de desenvolvimento. O agente incorporado só pode ajudar a editar o código-fonte quando o host concede intencionalmente ferramentas de escrita de repository/workspace; caso contrário, trabalha por meio de actions e dados. Consulte [Key Concepts](/docs/key-concepts#agent-modifies-code).

## O que vem a seguir

- [**Modelos**](/docs/cloneable-saas) — o modelo de clonar e possuir, além de todos os outros modelos de domínio para começar
- [**Primeiros passos**](/docs/getting-started) — os fundamentos da estrutura: ações, estado do aplicativo e sincronização ao vivo
- [**Calendário**](/docs/template-calendar) — um modelo de domínio relacionado para agendamento e reservas
- [**Automações**](/docs/automations) — o mecanismo acionado por eventos sobre o qual as regras de triagem automática do Mail são criadas
- [**Ações**](/docs/actions-overview) — o sistema de ações que chat, atalhos e automações chamam
