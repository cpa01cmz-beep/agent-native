...
<Diagram id="doc-block-1r2cc85" title={"Um arnês possui seu laço; Agent-Native conduz a sessão"} summary={"O AgentHarness substrato creates/resumes a sessão nativa, transmite seus eventos para a transcrição normal e persiste resumeState em SQL entre os turnos."}>
...
Dica `installPackage` (por exemplo `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` lança uma instalação clara
...
## Autenticação Codex: Código UI versus adaptadores de chicote {#codex-auth}

Existem duas superfícies do Codex:

- **Agent-Native Code / Desktop** executa `codex exec` na máquina do usuário e
  reutiliza o login local do Codex CLI informado por `codex login status`.
- **`ai-sdk-harness:codex`** usa o fluxo de assinatura nativa do AI SDK Harness
  quando compatível com o chicote subjacente. Não é necessária configuração extra:
  as credenciais permanecem no host, as credenciais explícitas de API têm prioridade
  e a autenticação resultante é encaminhada ao sandbox sem copiar o arquivo de
  autenticação local. Defina `harnessOptions.auth` como `"ai-gateway"` para usar
  somente o AI Gateway; esse modo nunca lê uma assinatura nativa.

O adaptador integrado `ai-sdk-harness:codex` aceita apenas
`permissionMode: "allow-all"`, pois esse é o único modo aceito pelo runtime Codex
estável. Omita `permissionMode` para usar esse padrão; `allow-reads` e
`allow-edits` são rejeitados por este adaptador.
...
`permissionMode`, `harnessOptions` e `agentOptions`). Use `listAgentHarnesses()` para enumerar o que está registrado
um seletor.

Para harnesses integrados com sandbox, passe um provedor de sandbox por
`agentOptions.sandbox` ao registrá-los. O provedor deve implementar
`resumeSession` para a reidratação após a reinicialização do processo; uma sessão
`sandbox` fornecida pelo chamador cobre apenas a criação inicial.
...
<Diagram id="doc-block-1gmpipb" title={"Retome entre turnos, processos e implantações"} summary={"Cada turno separa um resumeState opaco em SQL; no próximo turno, ele retorna para createSession em vez de reproduzir o histórico do bate-papo."}>
...
<div class="diagram-resume">
  ...
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >opaco · sessão de harness SQL</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
...
[Human Approval](/docs/actions-access-control#needs-approval) para a superfície de aprovação.
...
- [Native Interface de chat](/docs/native-chat-ui) — coloque seu próprio agente por trás do bate-papo UI com `AgentChatRuntime`.
...
- [Human Approval](/docs/actions-access-control#needs-approval) — o chicote de superfície aprovado é usado.
