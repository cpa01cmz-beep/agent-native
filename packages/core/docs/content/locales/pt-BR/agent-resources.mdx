...
description: "Claude – Personalização em nível de código por usuário — skills, memória, instruções, agentes personalizados, trabalhos agendados, servidores MCP — apoiados por PostgreSQL/PGlite, não por um sistema de arquivos."
---

# Recursos do Agente

Todo aplicativo nativo do agente vem com **recursos do agente**: a camada de personalização que torna o agente seu. Ele contém instruções de equipe (`AGENTS.md`), aprendizados compartilhados (`LEARNINGS.md`), memória estruturada pessoal (`memory/MEMORY.md`), skills que o agente recebe sob demanda, subagentes personalizados, trabalhos agendados e servidores MCP conectados — tudo o que você esperaria de uma configuração do Código Claude/Codex.

A diferença: **são linhas PostgreSQL/PGlite, não arquivos do sistema de arquivos.** Cada usuário obtém seu próprio conjunto de recursos do agente armazenado no banco de dados. Não há dev-box para ativar, nem contêiner por usuário, nem arquivos para montar. Um SaaS multilocatário pode oferecer a cada usuário um agente totalmente personalizável, essencialmente gratuito, porque tudo isso são linhas – memória pessoal, servidores MCP pessoais, skills pessoais, subagentes pessoais – e a base de código compartilhada hospeda todos eles de uma vez.

<Diagram id="doc-block-1l7jkfd" title={"Uma configuração Claude-Code, mas armazenada como recursos do agente em PostgreSQL/PGlite"} summary={"A mesma camada de personalização — instruções, habilidades, memória, agentes, trabalhos, MCP — exceto que cada arquivo é uma linha em um banco de dados multilocatário compartilhado."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Recursos agent-native</span
    ><small class="diagram-muted"
      >rows in one banco de dados PostgreSQL/PGlite</small
    >
    <div class="ws-rows">
    ...
    <small class="diagram-muted"
      >uma base de código, muitos usuários, com escopo
...
| Código Claude / Codex                | Recursos nativos do agente                                    |
| ------------------------------------ | ------------------------------------------------------------- |
| Arquivos em seu disco local          | Linhas em um banco de dados PostgreSQL/PGlite compartilhado   |
...
| `CLAUDE.md`/skills por projeto       | `AGENTS.md` por aplicativo + recursos de memória              |
| Configuração MCP em um arquivo JSON  | Configuração MCP em JSON _ou_ as configurações UI, por escopo |

Mesmas capacidades. Economia diferente. Consulte [Aplicativos](/docs/cloneable-saas) para saber por que isso é importante para SaaS.
...
A guia Recursos do aplicativo mostra todos os três escopos. Recursos pessoais e compartilhados/organizacionais são editáveis ​​lá. Os recursos no escopo do espaço de trabalho são somente leitura em painéis de aplicativos e editados centralmente no Dispatch, para que cada aplicativo veja os mesmos arquivos canônicos sem uma etapa de sincronização.
...
<Diagram id="doc-block-e47wl0" title={"Três escopos, um arquivo eficaz"} summary={"O tempo de execução resolve o mesmo caminho no espaço de trabalho, no aplicativo e nos escopos pessoais na leitura – o escopo mais específico vence."}>

```html
<div class="ws-stack">
  <div class="diagram-card">
    <span class="diagram-pill">Workspace</span
    ><small class="diagram-muted">padrões da empresa pelo Dispatch</small
    ><code>context/brand.md</code>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Organização / app</span
...
1. Abra a guia **Recursos** → **Compartilhado** → `AGENTS.md` (crie-o com `+` → **Arquivo** se estiver faltando).
...
<Callout id="doc-block-1tw74fq" tone="info">

Sem reinicialização, sem reimplantação. `AGENTS.md` é lido no início de cada turno, portanto, uma edição que você salva agora altera o comportamento do agente na próxima mensagem.
...
## Guia Recursos {#resources-tab}

O painel do agente inclui uma guia **Recursos** junto com Chat e CLI. Ele mostra uma árvore organizada por pastas de todos os recursos, um editor embutido para qualquer arquivo de texto (Markdown, JSON, YAML, texto simples) e os fluxos de criação digitados do menu `+` (Arquivos, Skills, Agentes, Tarefas agendadas). Os usuários podem navegar pelos padrões herdados do espaço de trabalho e criar/editar/excluir recursos pessoais ou da organização.
...
Clique no ícone `?` na barra de ferramentas de Recursos para voltar a esses documentos a qualquer momento.
...
Os recursos do agente têm duas superfícies de memória atuais:
...
| `mcp-servers/…`    | Espaço de trabalho/compartilhado | Humanos via Dispatch ou recursos do aplicativo | Atualização de configuração MCP              |

Os usuários podem editar esses arquivos de memória diretamente na guia Recursos — eles são recursos regulares. Exclua as linhas que o agente errou, mantenha as preferências pessoais em `memory/MEMORY.md` ou promova regras para toda a equipe em `AGENTS.md`.
...
<DataModel
  id="doc-block-qxh1b7"
  title={"O modelo de recursos do agente"}
  summary={
    "Um formato de recurso suporta cada arquivo de recurso do agente. O tempo de execução classifica-o por caminho e escopo e resolve o valor efetivo na leitura."
  }
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "Um único arquivo nos recursos do agente de um usuário — instruções, habilidade, memória, agente, configuração MCP ou trabalho.",
      fields: [
        {
          name: "path",
          type: "string",
        ...
        },
        {
          ...
          note: "Em qual nível esta linha reside",
        },
        {
          name: "owner",
          type: "string",
        ...
        },
        {
          name: "content",
          type: "text",
          note: "Remarcação / JSON / YAML corpo",
        },
      ],
    },
  ]}
...
Skills são arquivos de recursos Markdown no caminho `skills/` (de preferência `skills/<name>/SKILL.md`) que fornecem ao agente conhecimento de domínio sob demanda, invocado no bate-papo com `/skill-name`. Adicione-os na guia Recursos ou, no modo Código, em `.agents/skills/`.
...
<AnnotatedCode
  id="doc-block-1mwgb3r"
  title="Um perfil de agente personalizado"
  ...
  annotations={[
    {
      ...
      note: "`name` é o que aparece no menu suspenso `@` e o que o agente principal delega.",
    },
    {
      lines: "3-4",
      label: "Quando delegar",
      note: "O `description` é o que o orquestrador lê para decidir se esse perfil se ajusta a uma tarefa.",
    },
    {
      lines: "5",
      label: "Modelo",
      note: "`inherit` reutiliza o modelo do agente principal. Substitua somente quando o perfil claramente precisar de um perfil diferente.",
    },
    {
      lines: "6",
      note: "`tools: inherit` por enquanto — o campo está reservado para futuras políticas de ferramentas por agente.",
    },
  ]}
/>
...
Os recursos do agente incluem dois tipos de agentes:
...
## Marcação @ e comandos / {#at-tagging}

Digite `@` na entrada do chat para referenciar um agente, arquivo ou recurso, ou `/` para invocar um skill. Ambos abrem uma lista suspensa no cursor e adicionam um chip em linha quando você seleciona um item. Consulte [Menções de agentes](/docs/agent-mentions) para conhecer toda a mecânica (roteamento, `MentionProvider`, seleção de subagente) e o [Guia Skills](/docs/skills-guide) para saber como os skills são criados e descobertos.
...
| --------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| @ marcação                  | Arquivos de base de código + recursos do agente + agentes personalizados + agentes conectados | Recursos do agente + agentes personalizados + agentes conectados |
...
- [**Guia Skills**](/docs/skills-guide) — criar e definir o escopo dos arquivos de skill referenciados em `skills/`
- [**Menções de agentes**](/docs/agent-mentions) — mecânica completa das menções `@` e dos comandos `/`
- [**Trabalhos recorrentes**](/docs/recurring-jobs) — agendar os prompts criados a partir de tarefas agendadas
- [**Governança do Workspace**](/docs/workspace-management) — quem revisa, aprova e é responsável pelas alterações no workspace
- [**Workspaces de vários aplicativos**](/docs/multi-app-workspace) — a forma de implantação abrangida por um workspace
- [**Conexões do Workspace**](/docs/workspace-connections) — compartilhar credenciais de provedores entre aplicativos
