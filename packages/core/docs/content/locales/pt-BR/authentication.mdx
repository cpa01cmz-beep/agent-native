...
- **Padrão:** Magic link como fluxo principal do Better Auth quando o e-mail de saída está pronto, com e-mail/senha como fallback e provedores sociais. Defina `AUTH_MAGIC_LINK=0` para forçar o fallback.
...
<Diagram id="doc-block-1rujd8s" title={"Três entradas, uma sessão"} summary={"Visitantes do navegador, hosts MCP e provedores personalizados resolvem para a mesma AuthSession que o escopo downstream lê."}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">conectores Claude Code e ChatGPT</small>
    </div>
    <div class="diagram-card">
...
## Login no desenvolvimento local {#local-development-sign-in}

Em um servidor de desenvolvimento com endereço de loopback, a página compartilhada de login mostra primeiro **Continue as local dev**. Ela cria ou reutiliza a conta e a sessão de desenvolvimento reais do Better Auth, usando o mesmo fluxo autenticado que existe no restante do aplicativo.

Escolha **Show full sign in options** para mostrar as opções normais de cadastro, senha, magic link e provedores sociais. Se a sessão local não puder ser criada, a página mostrará essas opções automaticamente para que você possa continuar com a autenticação padrão.

Essa conveniência está disponível apenas quando o navegador se conecta por `localhost`, `127.0.0.0/8` ou `::1` em desenvolvimento ou teste. Ela fica desativada em visualizações de implantação e na produção hospedada. Isso não é um desvio de autenticação. Defina `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` para desativá-la e mostrar a página normal de login no desenvolvimento local.

## Melhor autenticação (padrão) {#better-auth}

Por padrão, o Better Auth possibilita a autenticação. Ele fornece:

- Registro e login por magic link, com e-mail/senha como fallback
...
- `POST /_agent-native/auth/magic-link` — solicitar um e-mail com magic link
...
ou visualize ambientes e nomeie contas de teste com um endereço `+autoz`
(`name+autoz@example.com`) para que sejam fáceis de identificar.

## Política de verificação de e-mail {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` é uma conveniência local e de controle de
qualidade — uma implantação hospedada a ignora. Para declarar a política em
qualquer ambiente, inclusive produção, declare-a na configuração:

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

O alias de implantação é `AUTH_REQUIRE_EMAIL_VERIFICATION=0`. Um valor
declarado prevalece sobre `AUTH_SKIP_EMAIL_VERIFICATION` e sobre o padrão de
cada ambiente.

`false` aceita um endereço não verificado como credencial de login — qualquer
pessoa pode reivindicar qualquer e-mail —, portanto use-o apenas onde a
inscrição já esteja restrita de outra forma. `true` em uma implantação sem
provedor de e-mail desativa a inscrição por senha, porque a verificação nunca
...
<Diagram id="doc-block-1485xrl" title={"Aperto de mão remoto MCP OAuth"} summary={"Um cliente compatível com OAuth inicializa apenas MCP URL - desafio, descoberta, registro dinâmico e, em seguida, uma troca de código PKCE."}>
...
Quando um visualizador anônimo acessa este URL, a página de login do framework é exibida. Após um login bem-sucedido (qualquer fluxo – token, e-mail/senha ou Google OAuth), o navegador retorna a `return`.
...
As páginas normais do aplicativo usam um único shell SSR impessoal e armazenável em cache público para todos os visitantes. O `AppProviders` verifica a sessão no cliente; em um caminho privado como `/dashboard`, ele redireciona o visitante anônimo para `/sign-in?return=%2Fdashboard` antes de montar a interface privada. Após o login, o visitante retorna a `/dashboard`. Use `isPublicPath` para rotas públicas/SEO ou `sessionBypass` para uma superfície autenticada por outro mecanismo limitado, como um token de incorporação MCP.

Por quanto tempo esse shell é armazenado em cache é uma configuração de toda a implantação — consulte [SSR Caching](/docs/ssr-caching) para `AGENT_NATIVE_SSR_CACHE`. Encurtar ou desativar o cache altera apenas a duração; o SSR continua renderizando o branch anônimo com os cookies removidos.

### Nos bastidores: Google OAuth

Ambos os fluxos (o ponto de entrada `/sign-in` explícito e o caso do caminho marcado) encadeiam o retorno URL através do estado OAuth. O estado é assinado por HMAC, portanto não pode ser falsificado em trânsito. No retorno de chamada, o retorno URL é revalidado como de mesma origem antes do redirecionamento. Portanto, uma chave de assinatura vazada ainda não pode ser transformada em um oráculo de redirecionamento aberto.
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | Defina como `0` ou `1` para declarar a política de verificação da inscrição por senha em qualquer ambiente, inclusive produção; prevalece sobre `AUTH_SKIP_EMAIL_VERIFICATION`                                   |
...
A flag do Dispatch `desktop.workspace-sso`, desativada por padrão, controla apenas a disponibilidade; as verificações de autenticação, assinatura, nonce, origem, callback, sessão e revogação continuam sempre obrigatórias. Com a flag desativada ou ilegível, o Desktop não altera o login, o logout nem os ajustes de cada app.
O marcador Canary do agente do usuário é apenas uma indicação de disponibilidade, não uma atestação remota; autenticação e autorização não confiam nele.
Os apps hospedados canônicos reconhecem solicitações do Desktop SSO Canary empacotado sem configuração de hub de identidade por app. Navegadores comuns continuam usando o login normal por app. Apps auto-hospedados exigem `AGENT_NATIVE_IDENTITY_HUB_URL` apenas para o caminho explícito restante do Desktop ou da federação no servidor. Builds estáveis do Desktop não inicializam o intermediário até que uma versão posterior seja aprovada separadamente.
A versão Canary não intercepta a navegação de login anônimo dos aplicativos. O usuário deve escolher explicitamente o login do workspace nas configurações do Desktop para o aplicativo registrado ativo; o login normal do aplicativo continua sendo o caminho padrão.

## Implantação administrada e convites

Para uma implantação administrada, defina `AUTH_SIGNUP=invited`, `ORG_CREATION=closed` e `AUTO_CREATE_DEFAULT_ORG=0`. O [guia de implantações administradas](/docs/administered-deployments) explica administradores de bootstrap, convites e domínios permitidos. Convites de associação e entrada por domínio são regras distintas.
