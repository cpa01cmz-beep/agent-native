...
<Diagram id="doc-block-1p9httw" title="Executar ciclo de vida" summary={"startRun abre uma linha em execução; updateRunProgress corrige; completeRun move-o para um status de terminal e marca completed_at."}>
...
<Endpoint id="doc-block-kvc3lz" title={"Listar execuções ativas"} method="GET" path="/_agent-native/runs" summary={"List the caller's runs. The RunsTray polls this with active=true."} auth="Session cookie (owner-scoped)" params={[
  {
    "name": "active",
    "in": "query",
    "type": "boolean",
...
- [**Dados e sincronização**](/docs/client-data) — `useDbSync` para invalidação de cache em tempo real
