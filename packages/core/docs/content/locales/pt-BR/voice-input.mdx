...
A estrutura lida com tudo isso automaticamente. Os usuários conectados ao Builder (nível gratuito disponível) obtêm o Gemini Flash-Lite hospedado no Builder por padrão; caso contrário, os usuários poderão trazer sua própria chave de provedor ou recorrer ao reconhecimento de fala do navegador.
...
1. **Builder Gemini Flash-Lite (padrão quando Builder está conectado).** O navegador envia áudio para `/_agent-native/transcribe-voice`, que faz proxy através de Builder.io (nível gratuito disponível) usando Gemini Flash-Lite. Não é necessária nenhuma chave Google API.
...
<div class="diagram-voice">
  ...
  <div class="diagram-card col">
    ...
    <small class="diagram-muted">padrão quando Builder está conectado</small>
...
Builder é o caminho mais fácil: conecte Builder.io (nível gratuito disponível) em Configurações e o provedor padrão se tornará Builder Gemini Flash-Lite. Para provedores BYOK, adicione a chave correspondente em Configurações → Chaves API.
