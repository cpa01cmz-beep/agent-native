...
<Diagram id="doc-block-ss2sdw" title="Cada corrida alimenta o loop" summary={"Uma execução de agente produz um rastreamento, pontuações automatizadas e um gancho de feedback – tudo armazenado no próprio SQL do aplicativo e exibido no painel. Os experimentos dividem o tráfego entre variantes de configuração."}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    Execução do agente<br /><small class="diagram-muted"
      >production-agent.ts</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Capturado automaticamente</span
...
### Feedback no PostHog {#posthog-feedback}

Polegares, categorias e texto livre emitem todos `$ai_feedback`. Já a visão de feedback do LLM analytics do PostHog lê um evento `survey sent`, então defina `POSTHOG_AI_FEEDBACK_SURVEY_ID` como a pesquisa que o PostHog cria pela aba **Feedback** de um rastreamento. Esse único id é toda a configuração; sem ele nenhum evento de pesquisa é enviado.

Um voto de polegar responde à primeira pergunta da pesquisa com o índice de escolha do PostHog (`1` para cima, `2` para baixo), e o texto livre após um polegar para baixo responde à pergunta de acompanhamento — assim o texto nunca cai onde vai a avaliação. Ambos levam um mesmo id de envio por mensagem avaliada, e é assim que o PostHog os une em uma única resposta; um polegar para baixo permanece marcado como incompleto até chegar o acompanhamento.
...
<Diagram id="doc-block-815qg2" title={"Atribuição de variante de hash consistente"} summary={"Cada usuário faz hash para uma variante estável, o loop aplica a substituição de configuração dessa variante e os resultados são acumulados por variante com intervalos de confiança."}>

```html
<div class="exp">
  <div class="diagram-node">
    ID do usuário<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">override de configuração A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">override de configuração B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    Resultados per variant<br /><small class="diagram-muted"
...
Definido a partir de um plugin de servidor com `defineAppConfig()`. Cada campo também tem um alias de variável de ambiente de implantação — consulte a referência de configuração declarada.
...
Envie rastreamentos para Langfuse, Datadog, Grafana ou qualquer backend compatível com OTel registrando um provedor OpenTelemetry no seu próprio aplicativo — consulte [Espaços OpenTelemetry](#otel) abaixo.

O núcleo deliberadamente não inclui nenhum exportador e não guarda endpoint ou token de exportação, portanto a credencial do backend fica na configuração do seu provedor (e no cofre), nunca na configuração do framework.

## Extensões do OpenTelemetry {#otel}

O loop do agente emite **espaços OpenTelemetry ao vivo** para cada execução, chamada de modelo e chamada de ferramenta. Assim, um host que já executa um coletor OTel vê a atividade do agente junto com o restante de seus rastreamentos distribuídos.
...
- [**Actions**](/docs/actions-overview) — as operações que aparecem como chamadas de ferramenta em rastreamentos
