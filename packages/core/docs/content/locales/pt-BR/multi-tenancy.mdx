...
Um novo andaime `npx --yes @agent-native/core@latest create` já vem com:
...
Se você estiver avaliando agente nativo para um CRM, rastreador de projeto, caixa de entrada de suporte ou qualquer ferramenta de equipe, a base multilocatário já está lá. Todos os modelos próprios são multilocatários. Consulte [Modelos](/docs/cloneable-saas) para ver a lista.

<Diagram id="doc-block-nfti3p" title={"Associação e isolamento da organização"} summary={"Os usuários ingressam em organizações como owner/admin/member. Cada linha proprietária carrega o org_id do locatário que a possui e nenhuma linha vaza através do limite."}>
...
<Diagram id="doc-block-uppcw" title={"Da sessão ao escopo SQL"} summary={"A organização ativa na sessão torna-se AGENT_ORG_ID, que a estrutura incorpora na cláusula WHERE de cada consulta."}>
...
- [**Multi-App Workspace**](/docs/multi-app-workspace) — hospede vários aplicativos Agent-Native em um monorepo com autenticação compartilhada e RBAC

## Modelo de isolamento

Use `accessFilter()` e `assertAccess()` para leituras e gravações no servidor. O escopo SQL do agente cria views temporárias filtradas para tabelas próprias. `pnpm guards` inclui a verificação de consultas sem escopo. RLS ainda não é uma camada ativa e está planejado como reforço após a versão 1.0. Consulte [Implantações administradas](/docs/administered-deployments).
