---
title: "Extensões"
description: "Um recurso opcional do toolkit para blocos personalizados pontuais e isolados. O código do aplicativo continua sendo o padrão para comportamento duradouro."
---

# Extensões

Extensões são um **recurso opcional do toolkit para blocos personalizados isolados**. Elas ficam desativadas por padrão e o template precisa habilitá-las explicitamente. O Analytics usa esse recurso para visualizações pontuais solicitadas de forma explícita. Para comportamento duradouro, edite o código do aplicativo.
...
> personalização mais profunda, altere o template no seu repositório e fluxo
> de desenvolvimento. O agente incorporado só pode ajudar quando o host concede
> intencionalmente ferramentas de escrita de repository/workspace; as extensões
> continuam sendo a camada rápida, sem implantação.
...
As extensões ficam desativadas por padrão. Para habilitá-las, defina
`extensionTools: true` em `createAgentChatPlugin()` e
`createCoreRoutesPlugin()`. O primeiro adiciona as actions e orientações do
agente; o segundo permite a criação autenticada por
`POST /_agent-native/extensions`. O runtime isolado e as rotas existentes de
leitura, edição e links profundos permanecem disponíveis por compatibilidade.

<Diagram id="doc-block-2dtgp8" title="A ponte da caixa de areia" summary={"A extensão HTML é executada em um iframe isolado e atinge o host apenas por meio de um conjunto fixo de auxiliares de ponte - cada chamada tem escopo e acesso verificado."}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Template host<br /><small class="diagram-muted"
        >ações, SQL com escopo automático</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>, travado ao domínio</small
      >
    </div>
    <div class="diagram-box">
      APIs externas<br /><small class="diagram-muted"
        >somente via extensionFetch</small
      >
    </div>
  </div>
</div>
...
1. Abra **Settings → Extensions** (ou simplesmente pergunte no chat) e clique em **"Nova extensão"**.
...
<Diagram id="doc-block-1mbdamx" title="Os slots empurram o contexto para o widget" summary={"O modelo de host possui slots nomeados; instalar uma extensão em um alimenta window.slotContext para tudo o que o usuário está visualizando no momento."}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">Thread de e-mail</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Notas de contato</span
    ><small class="diagram-muted"
...
- `dbQuery` e `extensionFetch` estão bloqueados por enquanto.
...
## Devo usar uma extensão ou o fluxo de código-fonte? {#vs-app-code}

O agente incorporado não edita o código-fonte por padrão; só pode fazer isso quando o host concede intencionalmente ferramentas de escrita de repository/workspace. Use o fluxo de código-fonte para comportamento duradouro e extensões para personalização rápida em runtime.

- **Comece com o código do aplicativo** para comportamento do produto, UI nativa, layout, rotas, lógica de negócio ou qualquer recurso que deva ser reutilizado.
- **Use um bloco personalizado somente se o aplicativo habilitar o recurso** e o usuário pedir explicitamente uma visualização ou interação pontual em uma superfície existente. É rápido e não exige deploy, mas continua sendo um patch isolado criado pelo agente.
- **Promova blocos duradouros.** Se um bloco se tornar útil além do contexto original, use **Promover para código do aplicativo** e mantenha o bloco até a revisão e implantação da mudança.

|                            | Extensão                                        | Edição do código do aplicativo            |
| -------------------------- | ----------------------------------------------- | ----------------------------------------- |
| **Criado por**             | Agente (ou usuário) em tempo de execução        | Usuário ou fluxo com capacidade de código |
...
| **Melhor para**            | Blocos personalizados pontuais                  | Comportamento duradouro do produto        |

Regra geral: **use o código do aplicativo por padrão.** Em um aplicativo que habilita extensões, uma visualização pontual pode ser um bloco personalizado; um novo tipo de gráfico disponível em todos os dashboards pertence ao código do template.

Para busca de dados personalizada que deva alimentar gráficos e tabelas nativos em vez de uma UI de widget sob medida, veja [Programas de dados](/docs/data-programs) — um script `run-code` armazenado e atualizável ao qual os painéis de dashboard se vinculam diretamente, sem nenhum iframe de extensão envolvido.
...
- **Autenticação necessária** — as extensões são executadas apenas para usuários logados, e as chamadas `dbQuery` têm escopo automático.
...
- [**Actions**](/docs/actions-overview) — as operações que um ramal chama via `appAction`
