...
<Diagram id="doc-block-1k5139p" title={"O caminho de renderização nativo"} summary={"Uma ação retorna JSON; o tempo de execução corresponde a um discriminante de widget explícito ou chatUI.renderer; AssistantChat monta um componente React real. Sem iframe, sem execução de HTML."}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    Ação executa<br /><small class="diagram-muted"
      >retorna JSON estruturado</small
    >
  </div>
...
Dimensione os renderers em unidades absolutas, não em porcentagens. A transcrição
cresce para acomodar suas mensagens, então o contêiner de um renderer não tem altura
definida para resolver uma porcentagem — `height: 40%` na barra de um gráfico colapsa
para zero em vez de gerar erro, o que parece um renderer que falhou ao desenhar. Dê aos
gráficos e outros visuais dependentes de altura uma base em pixels (ou `rem`); larguras
em porcentagem funcionam, pois a coluna da transcrição tem largura definida.
...
<Diagram id="doc-block-2rq7jl" title={"O tempo de execução BYO mantém o shell de bate-papo Agent-Native"} summary={"Seu agente externo transmite eventos normalizados por meio de um conector; Agent-Native mantém o compositor, a transcrição, os cartões de ferramentas, as aprovações e os widgets nativos."}>

```html
<div class="diagram-byo">
  <div class="diagram-box" data-rough>
    Seu agente<br /><small class="diagram-muted"
...
- [Actions](/docs/actions-overview) — defina as operações que retornam dados nativos do widget.
