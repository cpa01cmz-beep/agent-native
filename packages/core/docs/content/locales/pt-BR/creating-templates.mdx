...
Os modelos são aplicativos nativos do agente completos e bifurcáveis que resolvem um fluxo de trabalho real. Os modelos primários são criados com a mesma superfície de estrutura que você usa: rotas React para UI, dados PostgreSQL com Drizzle, actions para operações, recursos de espaço de trabalho para comportamento do agente e sincronização de sondagem para que o agente e UI permaneçam alinhados.
...
Considere as três camadas ao decidir onde uma capacidade pertence:

- **Core - o framework:** o contrato obrigatório de runtime e dados que qualquer app pode usar.
- **Toolkit - peças reutilizáveis opcionais:** UI e sistemas de produto compartilhados que os apps podem adotar, compor ou bifurcar.
- **Templates - apps opcionais:** produtos de domínio completos construídos sobre Core, normalmente usando Toolkit, que podem ser bifurcados como pontos de partida.

Toolkit e Templates são opcionais. Um template não é o próprio framework, e Toolkit não substitui Core.

Um bom modelo:

- Resolve um fluxo de trabalho de ponta a ponta, com dados iniciais úteis ou um fluxo de estado vazio.
- Armazena estado durável em PostgreSQL/PGlite, não em arquivos JSON.
...
O Chat oferece autenticação, threads de bate-papo duráveis, recursos apoiados por PostgreSQL, ferramentas, estado do aplicativo, actions e sincronização de pesquisa. Você adiciona o modelo de domínio e o produto UI.

Se você ainda não estiver criando um template de UI reutilizável, use [Apps orientados à automação](/docs/pure-agent-apps): defina ações, execute-as sem um navegador e adicione UI depois, quando o fluxo de trabalho precisar de uma superfície durável.
...
<FileTree
  id="doc-block-1mjrrpg"
  title="Layout do projeto de template"
  entries={[
    {
      path: "app/",
      note: "Frontend React",
    ...
    {
      path: "app/root.tsx",
      note: "Shell HTML e providers",
    },
    {
      path: "app/routes/",
      note: "Rotas de arquivo do React Router",
    },
    {
      path: "app/components/",
      note: "UI do template",
    ...
    {
      path: "app/hooks/",
      note: "Hooks de estado e dados da UI",
    },
    {
      path: "actions/",
      note: "Operações defineAction: a única fonte de verdade",
    },
    {
    ...
    },
    {
      ...
      note: "Migrações aditivas",
    },
    {
      path: "server/plugins/",
      note: "Integrações de inicialização",
    },
    {
      path: "server/routes/api/",
      note: "Rotas personalizadas somente quando actions não bastam",
    },
    {
      path: "shared/types.ts",
      note: "Tipos compartilhados cliente/servidor",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: orientação do agente para workflows complexos",
    },
    {
      path: "AGENTS.md",
      note: "Instruções de agente específicas do template",
    },
    {
      path: "package.json",
    },
...
Não adicione um diretório `data/` para o estado do aplicativo. Os dados duráveis do aplicativo pertencem ao PostgreSQL/PGlite, e o UI os lê por meio do actions ou de manipuladores de servidor digitados.

As quatro áreas de cada modelo se conectam por meio de uma superfície de ação compartilhada e um banco de dados PostgreSQL/PGlite — o agente e o UI são parceiros iguais nas mesmas operações:

<Diagram id="doc-block-pp7sdt" title={"Como as quatro áreas de um modelo se conectam"} summary={"A UI e o agente alcançam PostgreSQL/PGlite através das mesmas ações; o estado do aplicativo e a sincronização de pesquisa os mantêm alinhados."}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      UI React<br /><small class="diagram-muted"
        >app/routes · componentes</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Ações</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    PostgreSQL com Drizzle<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">Sincronização por polling</div>
</div>
```
...
## Dados do modelo em PostgreSQL {#data-models}

Defina tabelas de domínio com os auxiliares da estrutura Drizzle para que os esquemas usem PostgreSQL e PGlite:
...
As alterações de esquema devem ser aditivas. Para novas tabelas e colunas próprias
do aplicativo, use `drizzle-kit generate` para criar SQL revisável em
`server/db/migrations/<id_name>.sql` e carregue-o com
`runDrizzleMigrations()` na etapa de release em implantações Node.js. Cloudflare
Workers/Pages e outros runtimes edge não têm um sistema de arquivos implantável. Nesses ambientes,
incorpore o SQL gerado ao bundle do worker e passe-o para `runMigrations()`.
Mantenha `runMigrations()` para o histórico escrito à mão e casos especiais.
Nunca use SQL destrutivo, `drizzle-kit push`, renomeações de tabelas ou eliminações
de colunas.

Para leituras e gravações de aplicativos, use o construtor de consultas do Drizzle e os operadores compatíveis com PostgreSQL de `drizzle-orm`. Não escreva o código do produto com SQL bruto quando Drizzle puder expressar a consulta e use os auxiliares de esquema de `@agent-native/core/db/schema` nos modelos.
...
Use os documentos [Database](/docs/server-database) e [Security](/docs/security) antes de adicionar esquemas que contenham dados do usuário ou da organização.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      ...
      note: "O `description` é o que o agente lê para decidir quando chamar esta ação como ferramenta.",
    },
    {
      lines: "9-11",
      label: "Contrato tipado",
      note: "Um zod `schema` valida a entrada do agente, da UI, de HTTP, MCP e A2A.",
    },
    {
      lines: "18-19",
      label: "Escrita com escopo",
      note: "Carimbe `ownerEmail` / `orgId` de `ctx` para que a linha tenha o escopo correto para verificações de compartilhamento e acesso.",
    },
  ]}
/>
...
- Data lives in PostgreSQL com Drizzle. Use actions for all writes; schema is additive.
...
Os modelos da comunidade são instalados diretamente de um repositório público do GitHub. Coloque o aplicativo Agent-Native na raiz:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

Adicione `#branch`, `#tag` ou `#commit` para fixar uma revisão específica:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

A CLI baixa apenas o código-fonte. Ela não instala dependências nem executa scripts do modelo. Modelos da comunidade são código de terceiros; revise o repositório, a licença, as dependências e os scripts antes de executar `pnpm install`.

Para entrar no catálogo, inclua um aplicativo completo na raiz, um repositório público com licença e README de configuração, uma dependência de `@agent-native/core`, e verificações de tipo e testes aprovados. Uma demo hospedada e uma captura de tela são opcionais.

[Envie um modelo da comunidade](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml) quando o repositório estiver pronto. Esses modelos permanecem separados do catálogo próprio e são mantidos por seus autores.
