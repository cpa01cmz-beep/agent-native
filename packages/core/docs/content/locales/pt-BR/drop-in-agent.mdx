...
description: "Monte o chat do agente + recursos do agente em qualquer aplicativo React com <AgentPanel>, <AgentSidebar> e sendToAgentChat()."
---
...
Você não precisa criar o Agent-Native do zero. O bate-papo do agente, a guia Recursos do Agente, o terminal CLI, a entrada de voz e toda a infraestrutura relacionada são fornecidos como um punhado de componentes React que você coloca em qualquer aplicativo.

> **Pré-requisito:** o servidor deve estar executando o `agent-chat-plugin` (ele é montado automaticamente em cada modelo). Se você está começando do zero, consulte [Server](/docs/server-overview).
>
...
| `<AgentPanel>`        | O próprio painel bruto — chat + CLI + guias Recursos do Agente                                             | Você deseja controle total sobre o layout ou uma página de agente dedicada            |
...
<Diagram id="doc-block-kyw6fz" title="O modelo de montagem" summary={"<AgentSidebar> envolve seu layout existente. Suas rotas são renderizadas na área principal; o painel do agente é montado ao lado deles. <AgentPanel> é o mesmo painel sem o wrapper."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        Seu aplicativo<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        Painel do agente<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-406m1h"
  title={"Envolvendo o layout raiz com <AgentSidebar>"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "Inicia prompts",
      note: "`suggestions` renderiza como chips clicáveis ​​no chat vazio.",
    },
    {
      lines: "13",
      label: "Chips sensíveis ao contexto",
      note: '`dynamicSuggestions` merges screen-aware prompts (e.g. "Summarize this selection") with your static ones. On by default.',
    },
    {
      lines: "18-20",
      label: "Botão de alternância",
      note: "Put `<AgentToggleButton />` anywhere in your header to open and close the panel.",
    },
    {
      lines: "22-24",
      label: "Seu aplicativo",
      note: "`<Outlet/>` (your routes) renders in the main area, untouched.",
    },
  ]}
/>

É isso. O usuário agora tem um agente alternável em cada página – com histórico de bate-papo, guia Recursos do Agente, terminal CLI, entrada de voz e um painel mais largo. O estado persiste durante as recargas via `localStorage`.

### Acessórios

- **`children`** — layout e rotas normais do seu aplicativo. Renderizado na área principal; o painel do agente é montado ao lado dele no desktop e acima dele no celular.
...
`<AgentPanel>` fornece as guias brutas (Chat / CLI / Recursos do Agente) sem o wrapper da barra lateral, o botão de recolhimento ou qualquer persistência de estado. Coloque onde quiser; você cuida do layout.
...
- **`isFullscreen`** — defina esta opção se quiser uma coluna centralizada no estilo Claude em uma superfície de página ou painel personalizado. A barra lateral usa o painel mais largo.
...
Um botão que transfere o trabalho para o agente em vez de criar um caminho de IA separado com uma chamada `llm()` em linha. Consulte [Por que criar apps dessa forma](/docs/what-is-agent-native#why-build-apps-this-way) para entender por que essa distinção importa. Para trabalho de IA visível iniciado pelo usuário, abra explicitamente a barra lateral para que ele possa ver, conduzir e continuar na mesma conversa:
...
- **Para trabalho de IA visível iniciado pelo usuário**, use `openSidebar: true`. Use `false` apenas para trabalho em segundo plano ou iniciado pelo sistema que seja explicitamente silencioso e mantenha os acompanhamentos no mesmo thread do AgentSidebar.
- **`type`** — `"content"` (padrão) mantém o trabalho no agente incorporado, que usa ferramentas do aplicativo. `"code"` roteia para um host separado com capacidade de código para alterações no repositório; a escrita só fica disponível quando o host concede intencionalmente ferramentas de repository/workspace. Consulte [Agent-Native Código UI](/docs/code-agents-ui).
...
Mantenha os prompts de acompanhamento e revisão no thread existente da barra lateral. Não adicione um segundo campo de texto livre ao lado do resultado; use formulários locais apenas para valores estruturados que a interface precise validar.
...
[Chat do Agent](/docs/client-agent-chat#sendtoagentchat) para o comportamento da ponte do aplicativo MCP.
...
## Escolhendo entre o chat do agente e uma ação {#agent-or-action}

Use `useActionQuery` ou `useActionMutation` diretamente para operações locais determinísticas, como CRUD, validação, leituras de provedores e persistência. Se o usuário vivenciar o fluxo como pesquisa, análise, geração, recomendação, síntese ou raciocínio em várias etapas, inicie-o com `sendToAgentChat({ openSidebar: true })` e deixe o agente orquestrar ações focadas. Uma implementação determinística, por si só, não é motivo para esconder um fluxo semelhante a IA dentro de uma ação opaca.

## Typesafe actions do UI: `useActionMutation()` {#use-action-mutation}

Quando a UI precisar executar a mesma operação que uma ferramenta de agente executaria, use [`useActionMutation`](/docs/client-data#use-action-mutation):
...
Argumentos de tipo seguro vêm do esquema zod em seu `defineAction()`. Consulte [Actions](/docs/actions-overview) para o sistema de ação completo.

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation` vs `sendToAgentChat`.** Execute a operação diretamente com `useActionMutation` quando o usuário clicar em um botão determinístico ("Send reply"). Entregue-a a `sendToAgentChat` quando o trabalho precisar do raciocínio, das ferramentas ou do planejamento em várias etapas do agente. Nunca chame um `llm()` em linha pela UI; isso cria um caminho de IA separado em vez de usar o loop compartilhado do agente.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` e `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — seleção, navegação, tela de visualização
- [**Recursos do Agente**](/docs/agent-resources) — o que a guia Recursos do Agente contém (skills, memória, servidores MCP, trabalhos agendados)
