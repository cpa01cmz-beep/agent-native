...
- **Subagentes** — executam com seu próprio thread, seu próprio prompt de sistema, seu próprio conjunto de ferramentas. Cada um mapeia para um perfil de "agente personalizado" no [Recursos do Agente](/docs/agent-resources).
...
<Diagram id="doc-block-1feksqh" title="Orquestrador e especialistas" summary={"O chat principal delega para subagentes que executam seus próprios threads e reportam como chips embutidos."}>

```html
<div class="at-orc">
  <div class="diagram-card main">
    <span class="diagram-pill accent">Chat principal</span
  ...
  <div class="at-fan">
    ...
    <div class="at-subs">
      <div class="diagram-box">
        Revisão de código<br /><small class="diagram-muted"
          >own thread &amp; prompt</small
        >
      </div>
      <div class="diagram-box">
        Análise BigQuery<br /><small class="diagram-muted">own tools</small>
      </div>
      <div class="diagram-box">
        E-mail por voz<br /><small class="diagram-muted">own context</small>
      </div>
    </div>
  </div>
...
Os agentes personalizados residem no espaço de trabalho em `agents/<slug>.md` — um arquivo Markdown com frontmatter YAML. Consulte [Custom Agents](/docs/agent-resources#custom-agents) para obter o formato.
...
<Diagram id="doc-block-14x2n99" title="O que spawnTask() faz" summary={"Cada spawn cria um thread, persiste o estado para SQL e transmite eventos de chip até a conclusão."}>
...
Mapeamento de subagentes para perfis de agentes personalizados — arquivos Markdown em `agents/<slug>.md` no espaço de trabalho que aparecem no menu suspenso `@mention` e servem como alvos de delegação. [Recursos do Agente — Custom Agents](/docs/agent-resources#custom-agents) possui o formato completo (frontmatter, `tools`, `delegate-default`, substituições de modelo).
...
<Diagram id="doc-block-ipohq9" title={"Proteção de profundidade de delegação (cap 2 padrão)"} summary={"Cada nível pode gerar um nível mais profundo até o limite; um spawn passado é recusado no lado do servidor."}>

```html
<div class="at-depth">
  <div class="diagram-card ok">
    <span class="diagram-pill">depth 0</span
    ><strong>Chat de nível superior</strong
    ><small class="diagram-muted ok">may spawn &darr;</small>
  </div>
  <div class="diagram-card ok">
    <span class="diagram-pill">depth 1</span><strong>Subagente</strong
    ><small class="diagram-muted ok">may spawn &darr;</small>
  </div>
  <div class="diagram-card warn">
    <span class="diagram-pill warn">depth 2</span
    ><strong>Subagente's sub-agent</strong
...
- [**Recursos do Agente — Custom Agents**](/docs/agent-resources#custom-agents) — o formato do perfil
- [**A2A Protocol**](/docs/a2a-protocol) — quando o "subagente" mora em um aplicativo totalmente diferente
- [**Actions**](/docs/actions-overview) — as ferramentas que um subagente chama
