...
<Diagram id="doc-block-1pkuuh" title={"Direção do cliente: muitas fontes, um registro de ferramenta"} summary={"Arquivos de configuração, env e UI de tempo de execução são mesclados no gerenciador MCP; suas ferramentas aparecem prefixadas e pesquisáveis ​​junto com as ações do seu aplicativo. Este é o espelho da direção do servidor."}>
  ...
  <div class="diagram-col sources">
    <div class="diagram-box" data-rough>
      Workspace <code>mcp.config.json</code><br /><small class="diagram-muted"
        >compartilhado entre apps</small
      >
    </div>
    <div class="diagram-box" data-rough>
      Raiz do app <code>mcp.config.json</code><br /><small class="diagram-muted"
        >substituição por app</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      Remoto pela UI de configurações<br /><small class="diagram-muted"
  ...
  annotations={[
    {
      ...
      note: "A chave se torna o prefixo da ferramenta: as ferramentas deste servidor aparecem como `mcp__claude-in-chrome__*` no registro do agente, para que possam't collide with your template's ações.",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` gera um binário local. Os servidores Stdio são destinados ao **desenvolvimento local** — eles não funcionam em tempos de execução de borda.",
    },
    {
      lines: "6",
      label: "Ambiente do processo",
...
Os IDs de preset são `context7`, `sentry`, `notion`, `semgrep`, `linear`,
`atlassian`, `supabase`, `neon`, `stripe`, `cloudflare`, `gitlab`, `figma`,
`vercel`, `github`, `slack`, `asana`, `hubspot`, `intercom`, `monday`, `webflow`, `paypal`, `box`, `netlify` e `zapier`.

O diretório diferencia endpoints que aceitam o fluxo OAuth genérico de provedores
que exigem um cliente aprovado pelo provedor ou um aplicativo OAuth pré-registrado.
Essas entradas continuam visíveis para descoberta e apontam para as instruções
oficiais, mas não exibem um botão enganoso de conexão com um clique. Extensões
isoladas usam `agentNative.mcp.listTools`, `agentNative.mcp.callTool` e
`agentNative.providerApi.catalog(...)`/`agentNative.providerApi.docs(...)` por
meio das ações do host para descobrir dados conectados; elas nunca recebem
URLs de conexão, tokens de acesso nem tokens de atualização. A chamada bruta
`provider-api-request` é intencionalmente restrita ao host e ao agente: expor
requisições arbitrárias de método/caminho a extensões compartilhadas
transformaria uma extensão em um proxy de escrita autenticado. Em vez disso,
as extensões devem usar `agentNative.mcp.callTool(...)` ou uma ação de
aplicativo criada especificamente para operações de provedor com escopo
limitado.

Extensões de arquivo local devem permitir explicitamente `list-mcp-tools`,
`call-mcp-tool` e qualquer ação de provedor compatível que usem em
`extension.json`. Os helpers de descoberta também estão disponíveis em
`agentNative.connectors.mcp` e `agentNative.connectors.providerApi`.

### Atlassian / Jira

O preset Atlassian usa o endpoint OAuth 2.1 do Atlassian Rovo MCP,
`https://mcp.atlassian.com/v1/mcp/authv2`, e disponibiliza ferramentas do Jira
e do Confluence. A criação de tickets no Jira exige as permissões da Atlassian
que autorizam gravações no Jira; o usuário continua vendo apenas os projetos e
conteúdos permitidos pela sua conta da Atlassian.

Administradores de organizações da Atlassian podem controlar se um cliente
OAuth tem permissão para se conectar. Antes da conexão, talvez um administrador
precise adicionar a origem HTTPS real do aplicativo à lista de domínios
confiáveis em **Atlassian Administration → Rovo → Rovo MCP server → Domains** e
ativar as permissões necessárias de **Read**, **Write** e **Search** em
**Permissions**. O domínio deve ser o host em que o aplicativo Agent-Native é
executado; não copie `https://builder.io/**` para outra implantação, a menos que
essa seja a origem real dela. Consulte o [guia de configuração do Rovo MCP](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/)
e os [controles administrativos](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/) da Atlassian.

O catálogo fornece o endpoint e as orientações de configuração. O formulário
genérico de configurações armazena o endpoint e cabeçalhos criptografados
opcionais, mas não substitui o fluxo interativo de consentimento OAuth da
Atlassian. Use um cliente ou runtime MCP compatível com OAuth quando a Atlassian
exigir OAuth interativo. Se o provedor ou runtime aceitar um token Bearer
estático, siga o processo aprovado de gerenciamento de segredos e nunca cole
tokens em prompts ou configurações versionadas.
...
HTTPS URLs são aceitos em todos os lugares; `http://` simples só é permitido para `localhost` durante o desenvolvimento. A autenticação opcional entra como um token de portador enviado via `Authorization: Bearer …` em cada solicitação. Provedores que aceitam somente OAuth, incluindo Atlassian Rovo MCP, exigem o fluxo OAuth do provedor em vez de um cabeçalho estático.
...
<Diagram id="doc-block-u8qid6" title="Modelo de hub: um aplicativo atende servidores MCP de escopo organizacional" summary={"Dispatch mantém os servidores MCP de escopo organizacional; aplicativos de consumo os extraem e mesclam como mcp__hub_<orgId>_<name>__*. Apenas as linhas do escopo organizacional são compartilhadas – as credenciais pessoais permanecem inalteradas."}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">servidores MCP no escopo da organização</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">pull + merge a cada ~60 s</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-4wxhm5" method="GET" path="/_agent-native/mcp/status" summary={"MCP status do cliente para ferramentas e integração"} responses={[
  {
    "status": "200",
...
- Um `command` configurado incorretamente → o servidor é ignorado, seu erro aparece em `/_agent-native/mcp/status` em `errors.<server-id>` e todos os outros servidores continuam funcionando.
