...
<Diagram id="doc-block-162mde6" title={"A lista de verificação de configuração"} summary={"Apenas é necessário conectar um mecanismo de IA. O painel rastreia a conclusão e se oculta automaticamente quando tudo o que é necessário é feito."}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span
    ><strong>Conectar um motor de IA</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>Entrega de e-mail</strong
...
<Endpoint id="doc-block-i3rakc" title={"Listar as etapas de integração"} method="GET" path="/_agent-native/onboarding/steps" summary={"Liste todas as etapas registradas com seu status de conclusão"} responses={[
  {
    "status": "200",
...
<AnnotatedCode
  id="doc-block-1jk54dm"
  title={"Registrando uma etapa de integração personalizada"}
  ...
  annotations={[
    {
      lines: "5",
      label: "Montado automaticamente",
      note: "Registre-se a partir de um plugin Nitro — a estrutura lida com renderização, rastreamento de conclusão e dispensa.",
    },
    {
      lines: "7",
      label: "ID da pilha",
      note: "O novo registro com o mesmo `id` após o carregamento dos padrões substitui uma etapa integrada.",
    },
    {
      lines: "12-19",
      label: "Método primário",
      note: '`primary: true` marks the big CTA. `kind: "link"` sends the user into the OAuth flow.',
    },
    {
      lines: "20-26",
      label: "Caminho do delegado",
      note: '`kind: "agent-task"` entrega a configuração ao chat do agente com um prompt.',
    },
    {
      lines: "28-31",
      label: "Verificação de conclusão",
      note: "`isComplete` é executado no lado do servidor. Os tokens OAuth residem na loja `oauth_tokens` – verifique, não `process.env.GMAIL_REFRESH_TOKEN`.",
    },
  ]}
/>
...
| `database` | não         | URL do PostgreSQL hospedado ou vazio para PGlite local |
