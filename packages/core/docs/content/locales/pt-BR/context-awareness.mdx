...
<Diagram id="doc-block-104616q" title={"Como o agente vê o que você vê"} summary={"A UI grava chaves de estado leves; view-screen os hidrata em registros reais; o agente pode escrever e navegar de volta para mover a IU."}>

```html
<div class="diagram-ctx">
  <div class="diagram-card col">
    <span class="diagram-pill">UI escreve</span>
    <div class="diagram-node">
      navigation<br /><small class="diagram-muted"
        >visualização, IDs abertos</small
      >
    </div>
    <div class="diagram-node">
    ...
    <div class="diagram-node">
      selection<br /><small class="diagram-muted">linhas, blocos, formas</small>
    </div>
  </div>
  ...
  <div class="diagram-box">
    Agente age<br /><small class="diagram-muted">on the real object</small>
  </div>
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&#8635;</div>
  <div class="diagram-box diagram-accent">
    navigate<br /><small class="diagram-muted">agente move a UI</small>
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-9e0r3k"
  title={"view-screen — os olhos do agente"}
  ...
  annotations={[
    {
      ...
      note: "O agente lê esta descrição para saber que pode chamar `view-screen` para ver o UI atual.",
    },
    {
      lines: "13",
      label: "http: false",
      note: "Ação interna — não exposta em HTTP. O agente e `pnpm action` chamam isso, não o navegador.",
    },
    {
      lines: "15-16",
      label: "Estado de leitura",
      note: "Extrai as chaves leves `navigation` e `selection` que UI escreveu.",
    },
    {
      lines: "23-37",
      label: "Hidratar",
      note: "Transforma esses IDs em registros **frescos** direto de SQL, para que o agente verifique o objeto ativo antes de agir.",
    },
  ]}
/>
...
<Callout id="doc-block-1d0fh4i" tone="info">

**Mantenha `navigation` e `selection` pequenos.** Armazene IDs mais rótulos curtos, não registros inteiros. `view-screen` busca a fonte da verdade sob demanda, para que o estado obsoleto ou volumoso nunca chegue ao agente.
...
O `TAB_ID` importado de `app/lib/tab-id.ts` é o valor de `getBrowserTabId()` do framework. Chamadas WebMCP locais à página enviam o mesmo id em `X-Agent-Native-Browser-Tab`, de modo que o servidor resolve o estado `navigation`/`selection` por aba exatamente para a aba que o agente ou usuário está controlando — não há um segundo esquema de abas para manter sincronizado.
...
<Diagram id="doc-block-kn5blq" title={"A marcação de origem interrompe o jitter de auto-rebusca"} summary={"Uma guia ignora eventos de sincronização marcados com seu próprio TAB_ID, mas ainda reage às gravações do agente e de outras guias."}>

```html
<div class="diagram-jitter">
  <div class="diagram-node">
    Esta aba escreve<br /><small class="diagram-muted"
      >X-Request-Source: TAB_ID</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Servidor armazena origem<br />on the event
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
    <div class="diagram-pill warn">source == TAB_ID &rarr; ignored</div>
    <small class="diagram-muted">sem refetch, sem piscar</small>
    <div class="diagram-pill ok">agent / other tab &rarr; applied</div>
    <small class="diagram-muted">A UI atualiza ao vivo</small>
  </div>
</div>
```
