...
<Diagram id="doc-block-dgl70m" title={"Três superfícies de autoria + uma superfície de tempo de execução"} summary={"AGENTS.md e descrições de ferramentas são carregadas a cada turno; carga de habilidades sob demanda; application_state é escrito ao vivo pela sua UI."}>
...
<div class="diagram-surfaces">
  <div class="diagram-card always" data-rough>
    ...
    ><small class="diagram-muted"
      >o mapa: propósito, regras centrais, chaves de estado, índice de ações +
      skills</small
    >
  </div>
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">Cada turno</span
    ><strong>Descrições de ferramentas</strong
    ><small class="diagram-muted"
      >orienta a seleção de ferramentas — uma frase precisa cada</small
    >
  </div>
  <div class="diagram-card ondemand" data-rough>
    <span class="diagram-pill">Sob demanda</span><strong>Habilidades</strong
    ><small class="diagram-muted"
      >guia aprofundado, carregado quando a descrição dispara</small
    >
  </div>
  <div class="diagram-card runtime" data-rough>
    <span class="diagram-pill ok">Live</span><strong>application_state</strong
    ><small class="diagram-muted"
      >escrito pela sua UI: navegação, seleção, foco</small
    >
  </div>
</div>
```
...
`AGENTS.md` é o padrão universal sobre o qual o framework se baseia, então qualquer ferramenta de codificação com IA que leia instruções de projeto funciona de imediato:

- **Claude Code**: lê `CLAUDE.md` (vinculado por link simbólico a `AGENTS.md` pela configuração da CLI)
- **Cursor**: lê `AGENTS.md` diretamente, ou `.cursorrules` (localização legada do Cursor), se presente
- **Windsurf**: lê `AGENTS.md` diretamente, ou `.windsurfrules` (localização legada do Windsurf), se presente
- **Codex, Gemini e outros**: funcionam por meio do painel de agente incorporado
