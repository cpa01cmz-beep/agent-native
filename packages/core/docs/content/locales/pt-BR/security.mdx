...
<Diagram id="doc-block-5ijovz" title="Defesa em camadas" summary={"A estrutura possui a maior parte da superfície de ameaças; você possui duas coisas: marcar tabelas para definir o escopo e validar entradas externas."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">Pertence ao framework</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">Você controla</span
    ><small class="diagram-muted"
...
A estrutura cria o contexto da solicitação a partir da sessão e aplica a propriedade com `accessFilter()` e `assertAccess()` nas leituras e gravações do aplicativo. O SQL do agente recebe visualizações temporárias filtradas para a identidade e organização ativas. O RLS ainda não está habilitado, portanto consultas diretas do aplicativo devem continuar usando esses auxiliares.
...
<Callout id="doc-block-7a5j5d" tone="risk">

Nunca construa SQL por concatenação de strings ou literais de modelo. Passe a entrada do usuário como `args` para `exec` / `db-query` ou use Drizzle — ambos sempre parametrizam. As verificações `pnpm guards` capturam consultas sem escopo e concatenadas no momento do CI.
...
<Diagram id="doc-block-16s8kyp" title="O pipeline de escopo" summary={"O agente SQL nunca toca as tabelas base diretamente - ele lê uma visão temporária com escopo para a identidade atual, portanto, um nome de tabela vazio só pode retornar linhas próprias."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    Sessão conectada<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    SQL do agente<br /><small class="diagram-muted"
      >bare table names only</small
    >
  </div>
</div>
```
...
As ferramentas `db-query`/`db-exec` rejeitam referências de tabela qualificadas pelo esquema (`public.<table>`) — um nome qualificado é resolvido para a tabela base e ignoraria a visualização temporária acima. Os agentes usam nomes de tabelas simples; o escopo é aplicado automaticamente.
...
<DataModel
  ...
  summary={
    "As três colunas que tornam uma tabela compatível com locatários e compartilhável."
  }
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Organização ativa do proprietário na criação. Impulsiona verificações de visibilidade organizacional.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "privado | organização | público — padrão grosseiro, o padrão é privado.",
        },
      ],
    },
  ]}
...
As credenciais por usuário/organização e o cofre são criptografados em repouso com AES-256-GCM. O cofre compartilhado do workspace prefere `WORKSPACE_SECRETS_ENCRYPTION_KEY`, depois candidatos legados derivados de `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET` e `BETTER_AUTH_SECRET`; durante uma rotação, `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` mantém as linhas antigas legíveis até a migração. Para criptografar linhas existentes em texto simples, execute `pnpm action db-migrate-encrypt-credentials` (idempotente, não destrutivo).
...
Os manipuladores de webhook de entrada (Resend, SendGrid, Slack, Telegram, WhatsApp, Recall.ai, Deepgram, Zoom) recusam solicitações forjadas por padrão na produção: quando a variável de env secreta de assinatura correspondente está faltando, o manipulador retorna 401 em vez de aceitar e despachar.
...
As notificações `changes.watch` do Google Drive não passam por esse gateway genérico de adaptadores: são solicitações opacas sem corpo. Por isso, o framework registra um token aleatório por canal e verifica os cabeçalhos `X-Goog-Channel-ID`, `X-Goog-Channel-Token` e `X-Goog-Resource-ID` antes de buscar as alterações. São notificações diretas do Drive, não mensagens Pub/Sub, e não carregam tokens OIDC bearer do Pub/Sub.
...
- [ ] `WORKSPACE_SECRETS_ENCRYPTION_KEY` definido de forma consistente para cofres compartilhados multi-app; a criptografia local mantém os fallbacks `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` — consulte [Secrets Management](#secrets)
...
Execute `agent-native doctor` (`pnpm doctor` em um app gerado pelo scaffold) para verificar automaticamente os itens de esquema, credenciais e variáveis de ambiente desta checklist — cada descoberta indica o guard, o arquivo e a linha. Consulte a documentação do [Doctor](/docs/doctor).
...
- [**Authentication**](/docs/authentication) — modos de autenticação, cookie realms e detalhes de sessão sobre os quais esta página se baseia
- [**Multi-Tenancy**](/docs/multi-tenancy) — o modelo de associação à organização por trás do escopo `org_id`
- [**Sharing**](/docs/sharing) — o padrão `ownableColumns()` e de access-guard aplicado a recursos compartilháveis
- [**Messaging**](/docs/messaging#env-vars) — os segredos de assinatura de webhook por integração referenciados na lista de verificação acima
- [**Deployment**](/docs/deployment) — as variáveis de ambiente de produção que esta lista de verificação pressupõe estarem definidas
