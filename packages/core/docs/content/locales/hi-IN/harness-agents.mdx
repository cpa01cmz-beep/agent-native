...
<Diagram id="doc-block-19f4tip" title={"एक हार्नेस का अपना लूप होता है; Agent-Native सत्र चलाता है"} summary={"एजेंटहार्नेस सब्सट्रेट creates/resumes मूल सत्र, इसकी घटनाओं को सामान्य प्रतिलेख में स्ट्रीम करता है, और मोड़ों के बीच SQL में फिर से शुरू करता है।"}>
...
`installPackage` संकेत (उदाहरण के लिए `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` एक स्पष्ट इंस्टॉल फेंकता है
...
## Codex प्रमाणन: कोड UI बनाम हार्नेस एडाप्टर {#codex-auth}

Codex की दो सतहें हैं:

- **Agent-Native कोड / डेस्कटॉप** उपयोगकर्ता की मशीन पर `codex exec` चलाता है
  और `codex login status` में रिपोर्ट किए गए स्थानीय Codex CLI लॉगिन का पुन: उपयोग करता है।
- **`ai-sdk-harness:codex`** जहां समर्थित हो, upstream AI SDK Harness के native
  subscription flow का उपयोग करता है। किसी अतिरिक्त सेटिंग की आवश्यकता नहीं है:
  credentials host पर रहते हैं, स्पष्ट API-key credentials को प्राथमिकता मिलती है,
  और स्थानीय auth फ़ाइल कॉपी किए बिना परिणामी auth sandbox में पहुंचाया जाता है। केवल
  AI Gateway उपयोग करने के लिए `harnessOptions.auth` को `"ai-gateway"` पर सेट करें;
  यह मोड native subscription को कभी नहीं पढ़ता।

अंतर्निहित `ai-sdk-harness:codex` एडाप्टर केवल
`permissionMode: "allow-all"` का समर्थन करता है, क्योंकि स्थिर Codex रनटाइम में
यही एकमात्र स्वीकार्य मोड है। इस डिफ़ॉल्ट का उपयोग करने के लिए `permissionMode`
छोड़ दें; यह एडाप्टर `allow-reads` और `allow-edits` को अस्वीकार करता है।
...
`permissionMode`, `harnessOptions`, और `agentOptions`) पर मैप होता है।
किसके लिए पंजीकृत है यह गिनने के लिए `listAgentHarnesses()` का उपयोग करें
एक पिकर।

सैंडबॉक्स वाले बिल्ट-इन हार्नेस के लिए, पंजीकरण करते समय `agentOptions.sandbox`
के माध्यम से सैंडबॉक्स प्रोवाइडर दें। प्रक्रिया पुनः आरंभ होने के बाद रीहाइड्रेशन
के लिए प्रोवाइडर को `resumeSession` लागू करना होगा; कॉलर द्वारा दी गई `sandbox`
session केवल शुरुआती निर्माण को कवर करती है।
...
<Diagram id="doc-block-2mf2og" title={"मोड़ों, प्रक्रियाओं और तैनाती को फिर से शुरू करें"} summary={"प्रत्येक मोड़ एक अपारदर्शी बायोडेटा को SQL में अलग कर देता है; अगला चरण चैट इतिहास को दोबारा चलाने के बजाय इसे वापस createSession में फीड कर देता है।"}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    टर्न N<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >अपारदर्शी · SQL harness सेशन</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    टर्न N+1<br /><small class="diagram-muted">createSession.resumeState</small>
  </div>
</div>
```
...
अनुमोदन सतह के लिए [Human Approval](/docs/actions-access-control#needs-approval).
...
- [Human Approval](/docs/actions-access-control#needs-approval) - अनुमोदन सतही हार्नेस का उपयोग।
