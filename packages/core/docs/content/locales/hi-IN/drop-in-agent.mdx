...
description: "एजेंट चैट + एजेंट संसाधन को <AgentPanel>, <AgentSidebar>, और sentToAgentChat() के साथ किसी भी React ऐप में माउंट करें।"
---
...
आपको शुरुआत से एजेंट-नेटिव बनाने की आवश्यकता नहीं है। एजेंट चैट, एजेंट संसाधन टैब, CLI टर्मिनल, वॉयस इनपुट, और सभी संबंधित बुनियादी ढांचे को मुट्ठी भर React घटकों के रूप में शिप किया जाता है जिन्हें आप किसी भी ऐप में डालते हैं।

> **पूर्वावश्यकता:** सर्वर को `agent-chat-plugin` चलाना होगा (यह प्रत्येक टेम्पलेट में स्वचालित रूप से माउंट होता है)। यदि आप शून्य से शुरू कर रहे हैं, तो [Server](/docs/server-overview) देखें।
>
...
| `<AgentPanel>`        | कच्चा पैनल स्वयं - चैट + CLI + एजेंट संसाधन टैब                                            | आप लेआउट, या एक समर्पित एजेंट पेज पर पूर्ण नियंत्रण चाहते हैं |
...
<Diagram id="doc-block-hwvbl4" title={"माउंट मॉडल"} summary={"<AgentSidebar> आपके मौजूदा लेआउट को लपेटता है। आपके मार्ग मुख्य क्षेत्र में प्रस्तुत होते हैं; एजेंट पैनल उनके बगल में लगा होता है। <AgentPanel> बिना रैपर वाला वही पैनल है।"}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        आपका ऐप<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        एजेंट पैनल<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-5cav6f"
  title={"रूट लेआउट को <AgentSidebar> के साथ लपेटना"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "संकेत प्रारंभ करता है",
      note: "`suggestions` खाली चैट पर क्लिक करने योग्य चिप्स के रूप में प्रस्तुत करें।",
    },
    {
      lines: "13",
      label: "संदर्भ-जागरूक चिप्स",
...
बस इतना ही। उपयोगकर्ता के पास अब प्रत्येक पृष्ठ पर एक टॉगल करने योग्य एजेंट है - चैट इतिहास, कार्यक्षेत्र टैब, CLI टर्मिनल, वॉयस इनपुट और एक व्यापक ड्रॉअर के साथ। स्थिति `localStorage` के माध्यम से पुनः लोड करने पर बनी रहती है।

### प्रॉप्स

- **`children`** - आपके ऐप का सामान्य लेआउट और रूट। मुख्य क्षेत्र में प्रस्तुत; एजेंट पैनल डेस्कटॉप पर इसके बगल में और मोबाइल पर इसके ऊपर लगा होता है।
...
`<AgentPanel>` आपको साइडबार रैपर, कोलैप्स बटन या किसी भी स्टेट पर्सिस्टेंस के बिना रॉ टैब (चैट / CLI / एजेंट संसाधन) देता है। जहां चाहो वहां रख दो; आप लेआउट संभालें.
...
- **`isFullscreen`** - यदि आप किसी पृष्ठ या कस्टम पैनल पर Claude-शैली का केंद्रित कॉलम चाहते हैं तो इसे सेट करें। साइडबार इसके बजाय व्यापक ड्रॉअर का उपयोग करता है।
...
एक बटन जो inline `llm()` call से अलग AI path बनाने के बजाय agent को काम सौंपता है। यह अंतर क्यों महत्वपूर्ण है, इसके लिए [ऐप्स इस तरह क्यों बनाएं](/docs/what-is-agent-native#why-build-apps-this-way) देखें:
...
- **उपयोगकर्ता द्वारा शुरू किए गए दिखाई देने वाले AI कार्य** के लिए `openSidebar: true` का उपयोग करें। `false` का उपयोग केवल स्पष्ट रूप से शांत बैकग्राउंड या सिस्टम-आरंभित कार्य के लिए करें, और follow-up को उसी AgentSidebar thread में रखें।
- **`type`** — `"content"` (डिफ़ॉल्ट) कार्य को embedded app agent में रखता है, जो app tools का उपयोग करता है। `"code"` repository source changes के लिए अलग code-capable host तक route करता है; write access केवल तभी उपलब्ध है जब host जानबूझकर repository/workspace tooling दे। [Agent-Native कोड UI](/docs/code-agents-ui) देखें।
...
Follow-up और revision prompts को मौजूदा AgentSidebar thread में रखें। परिणाम के पास दूसरा freeform input न जोड़ें; local form केवल उन structured values के लिए हो जिन्हें UI को validate करना है।
...
MCP ऐप ब्रिज व्यवहार के लिए [एजेंट चैट](/docs/client-agent-chat#sendtoagentchat)।
...
## एजेंट चैट और action के बीच चुनाव {#agent-or-action}

CRUD, validation, provider reads और persistence जैसे deterministic local operations के लिए सीधे `useActionQuery` या `useActionMutation` का उपयोग करें। यदि उपयोगकर्ता workflow को research, analysis, generation, recommendation, synthesis या multi-step reasoning के रूप में अनुभव करता है, तो इसे `sendToAgentChat({ openSidebar: true })` से शुरू करें और agent को focused actions orchestrate करने दें। केवल implementation का deterministic होना AI-जैसे workflow को एक opaque action में छिपाने का कारण नहीं है।

## UI से टाइपसेफ actions: `useActionMutation()` {#use-action-mutation}

जब UI को वही operation चलाने की आवश्यकता हो जो agent tool चलाएगा, तो [`useActionMutation`](/docs/client-data#use-action-mutation) का उपयोग करें:
...
टाइप-सुरक्षित तर्क आपके `defineAction()` में ज़ोड स्कीमा से आते हैं। संपूर्ण कार्य प्रणाली के लिए [Actions](/docs/actions-overview) देखें।

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation` बनाम `sendToAgentChat`.** User ने deterministic button ("Send reply") click किया हो, तो operation सीधे `useActionMutation` से चलाएं। काम को agent के reasoning, tools या multi-step planning की जरूरत हो, तो `sendToAgentChat` को दें। UI से inline `llm()` कभी call न करें; इससे shared agent loop का उपयोग करने के बजाय अलग AI path बनता है।
...
- [**Actions**](/docs/actions-overview) - `defineAction()` और `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) - चयन, नेविगेशन, व्यू-स्क्रीन
- [**एजेंट संसाधन**](/docs/agent-resources) - एजेंट संसाधन टैब में क्या शामिल है (skills, मेमोरी, MCP सर्वर, शेड्यूल किए गए कार्य)
