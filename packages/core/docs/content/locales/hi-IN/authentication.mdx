...
- **डिफ़ॉल्ट:** आउटबाउंड ईमेल तैयार होने पर Better Auth में magic link पहले, ईमेल/पासवर्ड fallback और सामाजिक प्रदाताओं के साथ। fallback को बाध्य करने के लिए `AUTH_MAGIC_LINK=0` सेट करें।
...
<Diagram id="doc-block-isxt5n" title={"तीन तरह से, एक सत्र"} summary={"ब्राउज़र विज़िटर, MCP होस्ट और कस्टम प्रदाता सभी उसी AuthSession को हल करते हैं जिसे डाउनस्ट्रीम स्कोपिंग पढ़ता है।"}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code, ChatGPT कनेक्टर</small>
    </div>
    <div class="diagram-card">
...
## स्थानीय विकास साइन-इन {#local-development-sign-in}

लूपबैक डेवलपमेंट सर्वर पर साझा साइन-इन पेज सबसे पहले **Continue as local dev** दिखाता है। यह वास्तविक Better Auth डेवलपमेंट अकाउंट और सेशन बनाता या फिर से उपयोग करता है, इसलिए ऐप के बाकी हिस्सों में इस्तेमाल होने वाला वही प्रमाणित प्रवाह बना रहता है।

**Show full sign in options** चुनने पर सामान्य साइनअप, पासवर्ड, magic link और सोशल प्रोवाइडर विकल्प दिखते हैं। यदि स्थानीय सेशन नहीं बनाया जा सकता, तो पेज इन विकल्पों को अपने आप दिखाता है ताकि आप मानक प्रमाणीकरण के साथ आगे बढ़ सकें।

यह सुविधा केवल तब उपलब्ध है जब ब्राउज़र `localhost`, `127.0.0.0/8` या `::1` के माध्यम से विकास या परीक्षण वातावरण में कनेक्ट हो। डिप्लॉयमेंट प्रीव्यू और होस्टेड प्रोडक्शन में यह बंद रहती है। यह प्रमाणीकरण बायपास नहीं है। `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` सेट करके इसे बंद करें और स्थानीय विकास में सामान्य साइन-इन पेज दिखाएं।

## बेहतर प्रमाणीकरण (डिफ़ॉल्ट) {#better-auth}

डिफ़ॉल्ट रूप से, Better Auth प्रमाणीकरण को शक्ति प्रदान करता है। यह प्रदान करता है:

- magic link पंजीकरण और लॉगिन, ईमेल/पासवर्ड fallback के साथ
...
- `POST /_agent-native/auth/magic-link` - magic link ईमेल का अनुरोध करें
...
या परिवेश का पूर्वावलोकन करें, और `+autoz` पते के साथ परीक्षण खातों को नाम दें
(`name+autoz@example.com`) ताकि उन्हें पहचानना आसान हो।

## ईमेल सत्यापन नीति {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` एक स्थानीय और QA सुविधा है — होस्ट की गई तैनाती
इसे अनदेखा करती है। उत्पादन सहित किसी भी वातावरण के लिए नीति बताने हेतु, इसे
कॉन्फ़िगरेशन में घोषित करें:

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

तैनाती उपनाम `AUTH_REQUIRE_EMAIL_VERIFICATION=0` है। घोषित मान
`AUTH_SKIP_EMAIL_VERIFICATION` और प्रति-वातावरण डिफ़ॉल्ट, दोनों से ऊपर रहता है।

`false` एक असत्यापित पते को लॉगिन क्रेडेंशियल के रूप में स्वीकार करता है — कोई भी
किसी भी ईमेल का दावा कर सकता है — इसलिए इसका उपयोग केवल वहीं करें जहाँ साइनअप
पहले से किसी अन्य तरीके से सीमित हो। बिना ईमेल प्रदाता वाली तैनाती पर `true`
इसके बजाय पासवर्ड साइनअप को अक्षम कर देता है, क्योंकि सत्यापन कभी वितरित नहीं
हो सकता।
...
<Diagram id="doc-block-16envrs" title={"रिमोट MCP OAuth हैंडशेक"} summary={"एक OAuth-सक्षम क्लाइंट केवल MCP URL से बूटस्ट्रैप करता है - चुनौती, खोज, गतिशील पंजीकरण, फिर एक PKCE कोड एक्सचेंज।"}>
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | उत्पादन सहित किसी भी वातावरण के लिए पासवर्ड साइनअप की सत्यापन नीति घोषित करने हेतु `0` या `1` सेट करें; यह `AUTH_SKIP_EMAIL_VERIFICATION` से ऊपर रहता है                                    |
...
Dispatch का default-off `desktop.workspace-sso` flag केवल उपलब्धता नियंत्रित करता है; authentication, signature, nonce, origin, callback, session और revocation जाँच हमेशा अनिवार्य रहती हैं। flag बंद या अपठनीय होने पर Desktop प्रति-app sign-in, sign-out या Settings नहीं बदलता।
Canary user-agent marker केवल availability hint है, remote attestation नहीं; authentication और authorization उस पर भरोसा नहीं करते।
हर Desktop build broker शुरू कर सकता है जब device preference `desktopSsoEnabled` true (default) हो; स्पष्ट रूप से persist किया गया `false` opt-out बना रहता है। Dispatch flag `desktop.workspace-sso` भी enabled होना चाहिए। Canary user-agent marker अब broker initialization को gate नहीं करता; यह केवल update channel बताता है।
Canary अनाम ऐप sign-in navigation को intercept नहीं करता। उपयोगकर्ता को सक्रिय registered app के लिए Desktop Settings में workspace sign-in स्पष्ट रूप से चुनना होगा; सामान्य app sign-in default path बना रहता है।
सामान्य browsers normal per-app sign-in पर रहते हैं। Self-hosted apps को `AGENT_NATIVE_IDENTITY_HUB_URL` केवल बचे हुए explicit Desktop या server-side federation path के लिए चाहिए।

## प्रशासित डिप्लॉयमेंट और आमंत्रण

प्रशासित डिप्लॉयमेंट के लिए `AUTH_SIGNUP=invited`, `ORG_CREATION=closed` और `AUTO_CREATE_DEFAULT_ORG=0` सेट करें। [प्रशासित डिप्लॉयमेंट गाइड](/docs/administered-deployments) बूटस्ट्रैप एडमिन, आमंत्रण और अनुमत ईमेल डोमेन समझाती है। सदस्यता आमंत्रण और डोमेन से जुड़ना अलग नियम हैं।
