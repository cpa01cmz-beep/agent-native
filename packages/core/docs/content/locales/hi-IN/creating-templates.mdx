...
टेम्प्लेट पूर्ण, फोर्केबल एजेंट-नेटिव ऐप्स हैं जो वास्तविक वर्कफ़्लो को हल करते हैं। प्रथम-पक्ष टेम्पलेट आपके द्वारा उपयोग की जाने वाली समान फ़्रेमवर्क सतह के साथ बनाए गए हैं: UI के लिए React रूट, Drizzle के साथ PostgreSQL डेटा, संचालन के लिए actions, एजेंट व्यवहार के लिए कार्यक्षेत्र संसाधन, और पोलिंग सिंक ताकि एजेंट और UI संरेखित रहें।
...
किसी capability का स्थान तय करते समय तीन layers ध्यान में रखें:

- **Core - framework:** आवश्यक runtime और data contract जिसे हर app उपयोग कर सकता है।
- **Toolkit - वैकल्पिक reusable pieces:** shared UI और product systems जिन्हें apps अपना, compose या fork कर सकते हैं।
- **Templates - वैकल्पिक apps:** Core पर बने complete domain products, जो अक्सर Toolkit उपयोग करते हैं और starting point के रूप में fork किए जा सकते हैं।

Toolkit और Templates वैकल्पिक हैं। Template स्वयं framework नहीं है और Toolkit, Core की जगह नहीं लेता।

एक अच्छा टेम्पलेट:

- उपयोगी बीज डेटा या खाली-अवस्था प्रवाह के साथ एक वर्कफ़्लो को शुरू से अंत तक हल करता है।
- टिकाऊ स्थिति को PostgreSQL/PGlite में संग्रहीत करता है, JSON फ़ाइलों में नहीं।
...
चैट आपको प्रामाणिक, टिकाऊ चैट थ्रेड, PostgreSQL-समर्थित संसाधन, उपकरण, एप्लिकेशन स्थिति, actions और पोलिंग सिंक प्रदान करता है। आप डोमेन मॉडल और उत्पाद UI जोड़ें।

यदि आप अभी तक पुन: उपयोग योग्य UI टेम्पलेट नहीं बना रहे हैं, तो [Automation-First Apps](/docs/pure-agent-apps) का उपयोग करें: actions परिभाषित करें, उन्हें ब्राउज़र के बिना चलाएं, और जब workflow को टिकाऊ surface की आवश्यकता हो तब UI जोड़ें।
...
<FileTree
  id="doc-block-339wig"
  title={"Template project की संरचना"}
  entries={[
    {
      path: "app/",
      note: "React फ्रंट एंड सतह",
    },
    {
      path: "app/root.tsx",
      note: "HTML shell और providers",
    },
    {
      path: "app/routes/",
      note: "React Router की file routes",
    },
    {
      path: "app/components/",
      note: "Template की UI",
    ...
    {
      path: "app/hooks/",
      note: "UI state और data hooks",
    },
    {
      path: "actions/",
      note: "defineAction operations: single source of truth",
    },
    {
      path: "server/db/schema.ts",
      note: "Drizzle स्कीमा परिभाषा",
    },
    {
      path: "server/plugins/db.ts",
      note: "additive migrations केवल जोड़ने वाली",
    },
    {
      path: "server/plugins/",
      note: "startup integrations setup",
    },
    {
      path: "server/routes/api/",
      note: "custom routes केवल तब जब actions पर्याप्त न हों",
    },
    {
      path: "shared/types.ts",
      note: "shared client/server type definitions",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: जटिल workflows के लिए agent guidance",
    },
    {
      path: "AGENTS.md",
      note: "template-specific agent निर्देश",
    },
    {
      path: "package.json",
    },
...
एप्लिकेशन स्थिति के लिए `data/` निर्देशिका न जोड़ें। टिकाऊ ऐप डेटा PostgreSQL/PGlite में है, और UI इसे actions या टाइप किए गए सर्वर हैंडलर के माध्यम से पढ़ता है।

प्रत्येक टेम्प्लेट के चार क्षेत्र एक साझा क्रिया सतह और एक PostgreSQL/PGlite डेटाबेस के माध्यम से एक साथ जुड़ते हैं - एजेंट और UI समान संचालन में समान भागीदार हैं:

<Diagram id="doc-block-1l3djej" title={"किसी टेम्प्लेट के चार क्षेत्र कैसे जुड़ते हैं"} summary={"यूआई और एजेंट दोनों समान क्रियाओं के माध्यम से PostgreSQL/PGlite तक पहुंचते हैं; एप्लिकेशन स्थिति और पोलिंग सिंक उन्हें संरेखित रखते हैं।"}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      React इंटरफेस<br /><small class="diagram-muted"
        >app/routes · कंपोनेंट्स</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">क्रियाएँ</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Drizzle के ज़रिए PostgreSQL<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">पोलing sync</div>
</div>
```
...
## PostgreSQL में मॉडल डेटा {#data-models}

फ़्रेमवर्क Drizzle सहायकों के साथ डोमेन तालिकाएँ परिभाषित करें ताकि स्कीमा PostgreSQL और PGlite का उपयोग करें:
...
स्कीमा परिवर्तन योगात्मक होने चाहिए। नए ऐप-स्वामित्व वाले टेबल और कॉलम के लिए
`drizzle-kit generate` से `server/db/migrations/<id_name>.sql` में
समीक्षा योग्य SQL बनाएं और Node.js deployment के release चरण से
`runDrizzleMigrations()` के माध्यम से लोड करें। Cloudflare Workers/Pages और अन्य edge रनटाइम में
deployable फ़ाइल-सिस्टम नहीं होता, इसलिए वहाँ generated SQL को worker bundle में
embed करके `runMigrations()` को दें। हाथ से लिखे इतिहास और विशेष मामलों के लिए
`runMigrations()` रखें। विनाशकारी SQL, `drizzle-kit push`, तालिका का नाम बदलने या
कॉलम ड्रॉप का उपयोग न करें।

ऐप पढ़ने और लिखने के लिए, Drizzle के क्वेरी बिल्डर और `drizzle-orm` के PostgreSQL-संगत ऑपरेटरों का उपयोग करें। जब Drizzle क्वेरी को व्यक्त कर सकता है, तो कच्चे SQL के साथ उत्पाद कोड न लिखें, और टेम्पलेट्स में `@agent-native/core/db/schema` के स्कीमा हेल्पर्स का उपयोग करें।
...
उपयोगकर्ता या संगठन डेटा रखने वाले स्कीमा जोड़ने से पहले [Database](/docs/server-database) और [Security](/docs/security) दस्तावेज़ों का उपयोग करें।
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "8",
      label: "उपकरण सतह",
      note: "`description` वह है जिसे एजेंट यह तय करने के लिए पढ़ता है कि इस क्रिया को टूल के रूप में कब कॉल करना है।",
    },
    {
      lines: "9-11",
      label: "टाइप किया हुआ अनुबंध",
      note: "एक zod `schema` agent, UI, HTTP, MCP और A2A से आने वाले input को validate करता है।",
    },
    {
      lines: "18-19",
      label: "दायरा लिखें",
      note: "`ctx` से `ownerEmail` / `orgId` स्टांप करें ताकि पंक्ति साझा करने और पहुंच जांच के लिए सही ढंग से दायरा हो।",
    },
  ]}
/>
...
- Data lives in Drizzle के ज़रिए PostgreSQL. Use actions for all writes; schema is additive.
...
कम्युनिटी टेम्पलेट सीधे सार्वजनिक GitHub रिपॉज़िटरी से इंस्टॉल होते हैं। Agent-Native ऐप को रिपॉज़िटरी के रूट पर रखें:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

किसी खास संशोधन को पिन करने के लिए `#branch`, `#tag` या `#commit` जोड़ें:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

CLI केवल स्रोत डाउनलोड करती है; यह डिपेंडेंसी इंस्टॉल या टेम्पलेट स्क्रिप्ट नहीं चलाती। कम्युनिटी टेम्पलेट तृतीय-पक्ष कोड हैं, इसलिए `pnpm install` चलाने से पहले रिपॉज़िटरी, लाइसेंस, डिपेंडेंसी और इंस्टॉल स्क्रिप्ट की समीक्षा करें।

कैटलॉग में सूचीबद्ध होने के लिए रूट पर पूरा ऐप, लाइसेंस और सेटअप README वाला सार्वजनिक रिपॉज़िटरी, `@agent-native/core` डिपेंडेंसी, और सफल टाइपचेक व टेस्ट शामिल करें। होस्टेड डेमो और स्क्रीनशॉट वैकल्पिक हैं।

रिपॉज़िटरी तैयार होने पर [कम्युनिटी टेम्पलेट सबमिट करें](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml)। कम्युनिटी सूची पहले-पक्ष कैटलॉग से अलग रहती है और उसके लेखक उसका रखरखाव करते हैं।
