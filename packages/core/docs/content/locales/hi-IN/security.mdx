...
<Diagram id="doc-block-50mrb0" title={"परतों में बचाव"} summary={"ढाँचा अधिकांश खतरे की सतह का मालिक है; आपके पास दो चीजें हैं - स्कोपिंग के लिए टेबल टैग करना और बाहरी इनपुट को मान्य करना।"}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">फ्रेमवर्क के पास</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">नियंत्रण आपका है</span
    ><small class="diagram-muted"
...
फ़्रेमवर्क सेशन से अनुरोध संदर्भ बनाता है और ऐप के पढ़ने-लिखने पर `accessFilter()` तथा `assertAccess()` से स्वामित्व लागू करता है। एजेंट SQL को सक्रिय पहचान और संगठन के लिए अस्थायी फ़िल्टर किए गए दृश्य मिलते हैं। RLS अभी सक्षम नहीं है, इसलिए सीधे ऐप क्वेरीज़ में इन सहायकों का उपयोग जारी रखें।
...
<Callout id="doc-block-1g4m9jl" tone="risk">

कभी भी SQL को स्ट्रिंग कॉन्सटेनेशन या टेम्प्लेट अक्षरशः द्वारा न बनाएं। उपयोगकर्ता इनपुट को `args` से `exec` / `db-query` के रूप में पास करें, या Drizzle का उपयोग करें - दोनों हमेशा पैरामीटराइज़ करते हैं। `pnpm guards` चेक सीआई समय पर अस्पष्ट और संक्षिप्त प्रश्नों को पकड़ते हैं।
...
<Diagram id="doc-block-kmvua" title={"स्कोपिंग पाइपलाइन"} summary={"एजेंट SQL कभी भी आधार तालिकाओं को सीधे नहीं छूता है - यह वर्तमान पहचान के दायरे में एक अस्थायी दृश्य के माध्यम से पढ़ता है, इसलिए एक खाली तालिका नाम केवल स्वामित्व वाली पंक्तियों को वापस कर सकता है।"}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    साइन-इन सेशन<br /><small class="diagram-muted">email &middot; orgId</small>
  </div>
  ...
  <div class="diagram-node ok">
    एजेंट SQL<br /><small class="diagram-muted">bare table names only</small>
  </div>
</div>
```
...
`db-query` / `db-exec` उपकरण स्कीमा-योग्य तालिका संदर्भों (`public.<table>`) को अस्वीकार करते हैं - एक योग्य नाम आधार तालिका में हल हो जाता है और उपरोक्त अस्थायी दृश्य को बायपास कर देगा। एजेंट नंगे टेबल नामों का उपयोग करते हैं; स्कोपिंग स्वचालित रूप से लागू होती है।
...
<DataModel
  id="doc-block-16nt3zl"
  title="What ownableColumns() adds"
  summary={
    "तीन कॉलम जो तालिका को किरायेदार-जागरूक और साझा करने योग्य बनाते हैं।"
  }
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "निर्माण के समय स्वामी का सक्रिय संगठन। ऑर्ग-विजिबिलिटी जांच चलाता है।",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "निजी | संगठन | सार्वजनिक - मोटे तौर पर डिफ़ॉल्ट, डिफ़ॉल्ट से निजी।",
        },
      ],
    },
  ]}
...
प्रति-उपयोगकर्ता/प्रति-संगठन क्रेडेंशियल्स और वॉल्ट AES-256-GCM से एन्क्रिप्ट होते हैं। Shared workspace vault पहले `WORKSPACE_SECRETS_ENCRYPTION_KEY`, फिर `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET` और `BETTER_AUTH_SECRET` से बने legacy candidates उपयोग करता है; rotation में `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` rows को migration तक पढ़ने योग्य रखता है। मौजूदा plaintext rows के लिए `pnpm action db-migrate-encrypt-credentials` (idempotent, non-destructive) चलाएँ।
...
इनबाउंड वेबहुक हैंडलर (रीसेंड, सेंडग्रिड, Slack, टेलीग्राम, व्हाट्सएप, रिकॉल.एआई, डीपग्राम, ज़ूम) उत्पादन में डिफ़ॉल्ट रूप से जाली अनुरोधों को अस्वीकार कर देते हैं: जब संबंधित हस्ताक्षरित गुप्त एनवी संस्करण गायब होता है, तो हैंडलर स्वीकार करने और भेजने के बजाय 401 लौटाता है।
...
Google Drive की `changes.watch` सूचनाएँ उस सामान्य अडैप्टर गेट से नहीं गुजरतीं: ये बिना बॉडी वाले अपारदर्शी अनुरोध हैं। इसलिए फ़्रेमवर्क हर चैनल के लिए एक यादृच्छिक टोकन दर्ज करता है और बदलाव लेने से पहले `X-Goog-Channel-ID`, `X-Goog-Channel-Token` और `X-Goog-Resource-ID` हेडर सत्यापित करता है। ये सीधे Drive नोटिफ़िकेशन हैं, Pub/Sub संदेश नहीं, और इनमें Pub/Sub OIDC bearer token नहीं होते।
...
- [ ] Shared multi-app vaults के लिए `WORKSPACE_SECRETS_ENCRYPTION_KEY` समान रूप से सेट करें; app-local encryption में `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` fallback रहते हैं - [Secrets Management](#secrets) देखें
