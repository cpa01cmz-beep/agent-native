...
**ऑटोमेशन** एजेंट द्वारा चलाया जाने वाला काम है जो किसी समय-सारणी पर या घटना होने पर शुरू होता है: _जब X हो, तो Y करें_। **Scheduled** और **Event** दो ट्रिगर प्रकार हैं, अलग उत्पाद नहीं।

फ़्रेमवर्क ने मूल रूप से शेड्यूल किए गए ऑटोमेशन को **Recurring Jobs** के रूप में दिखाया था। वह नाम, `jobs/<name>.md` पथ और `manage-jobs` टूल संगतता के लिए बने हुए हैं। नई गाइड **Automations** को व्यापक नाम मानती है; पुराने शेड्यूल किए गए जॉब, संगठन स्कोप या उन्नत फ़ील्ड के लिए [Recurring Jobs](/docs/recurring-jobs) देखें।

<Diagram id="doc-block-umhh0u" title={"जब X हो तो Y करें"} summary={"एक घटना बस में सक्रिय हो जाती है, एक वैकल्पिक प्राकृतिक-भाषा की स्थिति इसे गेट करती है, और एजेंट पूर्ण टूल एक्सेस के साथ ऑटोमेशन बॉडी चलाता है।"}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">एजेंट body चलाता है</span
    ><small class="diagram-muted"
...
शेड्यूल और इवेंट-ट्रिगर ऑटोमेशन देखने और प्रबंधित करने के लिए पूर्ण Agent पेज पर **Automations** खोलें। संगतता URL `/agent#jobs` ही रहता है, इसलिए मौजूदा लिंक और बुकमार्क काम करते रहेंगे।

<Callout tone="warning">

`manage-automations` Scheduled और Event ट्रिगर के लिए व्यक्तिगत और संगठन ऑटोमेशन का समर्थन करता है। संगठन Event ऑटोमेशन सदस्यों को दिखाई देते हैं, लेकिन हमेशा अपने निर्माता के रूप में चलते हैं; व्यवस्थापक उस पहचान को बदले बिना उनका प्रबंधन कर सकते हैं।
...
<AnnotatedCode
  id="doc-block-1ur2gfy"
  title={"एक इवेंट-ट्रिगर स्वचालन"}
  ...
  annotations={[
    {
      lines: "2",
      label: "कोई क्रॉन नहीं",
      note: 'इवेंट ट्रिगर `schedule` को `""` पर सेट करते हैं — cron फ़ील्ड खाली रहता है।',
    },
    {
      lines: "4-5",
      label: "ट्रिगर",
      note: "`triggerType: event` प्लस `event` नाम बस के लिए इस स्वचालन की सदस्यता लेता है।",
    },
    {
      lines: "6",
      label: "गेट",
      note: "एक वैकल्पिक प्राकृतिक-भाषा `condition`, प्रेषण से पहले पेलोड के विरुद्ध Haiku द्वारा मूल्यांकन किया गया।",
    },
    {
      lines: "12",
      label: "सर्वर-साइड रहस्य",
...
इवेंट मेटाडेटा में `owner` तय करता है कि कौन से ऑटोमेशन सक्रिय होंगे। व्यक्तिगत और संगठन Event ऑटोमेशन दोनों का `createdBy` से सटीक मेल होना आवश्यक है।
...
व्यक्तिगत और संगठन ऑटोमेशन `action` पैरामीटर के साथ `manage-automations` का उपयोग करते हैं:
...
`manage-automations` संगठन स्कोप, मॉडल ओवरराइड और सख्ती से लागू MCP अनुमत-सूचियों का समर्थन करता है। `manage-jobs` पुराने केवल-शेड्यूल इंटीग्रेशन और डिलीवरी मेटाडेटा के लिए संगतता टूल बना रहता है।
...
<Endpoint id="doc-block-1yvchl3" title={"एक परीक्षण ईवेंट सक्रिय करें"} method="POST" path="/_agent-native/automations/fire-test" summary={"इवेंट-ट्रिगर ऑटोमेशन को मान्य करने के लिए एक test.event.fired इवेंट का उत्सर्जन करें"} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1fvs24e" title={"प्रेषण पथ"} summary={"एक निकाल दिए गए इवेंट से लेकर एक पूर्ण एजेंट रन तक, जो स्वामित्व के दायरे और प्राकृतिक-भाषा की स्थिति से निर्धारित होता है।"}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> body को prompt, payload को context, 5-मिनट
      timeout के साथ</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-overview) - ऑटोमेशन एजेंट लूप के माध्यम से किसी भी पंजीकृत कार्रवाई को कॉल कर सकता है
