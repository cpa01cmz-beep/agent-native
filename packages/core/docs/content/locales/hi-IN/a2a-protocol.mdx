...
<Diagram id="doc-block-romgc7" title={"एक एजेंट दूसरे को काम सौंपता है"} summary={"एक मेल एजेंट एनालिटिक्स एजेंट के कार्ड का पता लगाता है, एक JSON-RPC संदेश भेजता है, और पूरा किया गया कार्य वापस प्राप्त करता है।"}>

```html
<div class="diagram-handoff">
  <div class="diagram-card">
    <strong>Mail एजेंट</strong
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-pill ok">कार्य · पूरा</div>
  </div>
  <div class="diagram-card" data-rough>
    <strong>Analytics एजेंट</strong
    ><small class="diagram-muted">run-query चलाता है, परिणाम लौटाता है</small>
  </div>
</div>
```
...
<Endpoint id="doc-block-wng0ch" title={"प्राथमिक A2A समापन बिंदु"} summary={"सभी JSON-RPC विधियां यहां पोस्ट की गई हैं। message/send दिखाया गया।"} method="POST" path="/_agent-native/a2a" summary={"एक संदेश भेजें और कार्य पूरा होने की प्रतीक्षा करें"} auth="JWT bearer signed with A2A_SECRET (or legacy apiKeyEnv static token)" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-18sqzpi" title={"सर्वर रहित पर Async कार्य जीवनचक्र"} summary={"async:true, मिलीसेकेंड में काम करके रिटर्न देता है, फिर एक ताज़ा निष्पादन एजेंट लूप चलाता है जबकि कॉलर पोल करता है।"} caption="A recurring sweeper re-claims any task left in flight if the function execution dies mid-run.">
...
<div class="diagram-async">
  ...
  <div class="diagram-panel">
    ...
    ><small class="diagram-muted">~मिलीसेकंड</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>
    स्व-ट्रिगर POST /_agent-native/a2a/_process-task<br /><small
      class="diagram-muted"
      >HMAC token · नया execution · पूरा timeout</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
...
<AnnotatedCode
  id="doc-block-w4jzyy"
  title={"टाइप किए गए भागों के साथ A2A संदेश"}
  language="json"
  code={
  ...
  annotations={[
    {
      lines: "4",
      label: "text part",
      note: "एजेंट द्वारा पढ़ा जाने वाला सादा प्राकृतिक-भाषा निर्देश।",
    },
    {
      lines: "5",
      label: "data part",
      note: "संरचित JSON तर्क - उदा. एक तिथि सीमा - संकेत के साथ पारित की गई।",
    },
    {
      lines: "6-9",
      label: "file part",
      note: "नाम, `mimeType`, और बेस64 `bytes` द्वारा एक फ़ाइल संलग्न करें।",
    },
  ]}
/>
...
## होस्ट किए गए एजेंट कनेक्शन {#hosted-agent-connections}

होस्ट किया गया एजेंट तब A2A में भाग ले सकता है जब वह संगत A2A एंडपॉइंट उपलब्ध
कराता है। केवल मॉडल प्रदाता या एजेंट SDK A2A पीयर नहीं होता। ऐप का UI,
actions और डेटाबेस सामान्य ऐप होस्ट पर रखें, फिर एजेंट रनटाइम को उसके A2A
एंडपॉइंट या किसी अडैप्टर के माध्यम से जोड़ें।

होस्ट किए गए कनेक्शन में एंडपॉइंट मेटाडेटा और auth descriptor होता है। यह
मेटाडेटा remote-agent resource या connection record के साथ रखें। manifest में
कभी token या client secret न रखें:
...
`url` invocation endpoint या base URL है। जब discovery `/.well-known/agent-card.json`
का उपयोग करता है तब `cardUrl` वैकल्पिक है। जब provider card को किसी दूसरे पथ
पर प्रकाशित करता है तब इसे इस्तेमाल करें। Client agent card से protocol version
पढ़ता है। यदि provider card में version नहीं है, तो `protocolVersion` सीधे
`A2AClient` को दें। `auth` बताता है कि server-side code bearer token कैसे प्राप्त
करेगा। समर्थित descriptor हैं vault-backed
`credentialRef` वाला `bearer`, या `tokenUrl`, `clientId`, `clientSecretRef` और
`scope` वाला `oauth-client-credentials`। `credentialRef` और `clientSecretRef`
secret के references हैं, secret values नहीं।

### सामान्य bearer A2A {#generic-bearer-a2a}

जब peer `Authorization: Bearer ...` स्वीकार करता है, संगत card देता है और
Agent-Native JSON-RPC स्वीकार करता है, तब standard client इस्तेमाल करें:
...
`token` को workspace connection या vault से server-side हल करें। इसे browser
code से न भेजें, manifest में न लिखें और prompt में न रखें। Static API key
केवल remote service के लिए transport credential है। यह Agent-Native caller
identity स्थापित नहीं करती और `approvedActions` नहीं ले जाती।

### Microsoft Foundry {#microsoft-foundry}

[Azure AI Foundry hosted agents](https://learn.microsoft.com/en-us/azure/foundry/agents/overview)
आपके agent code या container को managed endpoint, identity, scaling, session
state और observability के पीछे चलाते हैं। यह agent runtime के लिए hosting model
है। Agent-Native UI, actions और PostgreSQL deployment को app host पर रखें और
Foundry endpoint को server-side code से call करें।

Foundry [Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/enable-agent-to-agent-endpoint)
के माध्यम से A2A endpoint expose कर सकता है। इसके लिए Microsoft Entra auth
चाहिए और caller को Foundry Agent Consumer permission चाहिए। जब endpoint
Agent-Native JSON-RPC बोलता है तब provider के v0.3 card और endpoint के साथ
`cardUrl` और `protocolVersion` का उपयोग करें। Token provider को Entra access
token प्राप्त और refresh करना होगा। `A2A_SECRET` और static API key Foundry auth
के लिए पर्याप्त नहीं हैं।

### Gemini Enterprise {#gemini-enterprise}

[Gemini Enterprise external A2A agents को register कर सकता है](https://docs.cloud.google.com/gemini/enterprise/docs/register-and-manage-an-a2a-agent)
और उसका Agent Registry proxy endpoint प्रकाशित कर सकता है। इसका HTTP+JSON
binding provider-specific paths उपयोग करता है: discovery के लिए `GET {url}/v1/card`,
invocation के लिए `POST {url}/v1/message:send` और streaming के लिए
`POST {url}/v1/message:stream`। Requests Google OAuth या ADC bearer tokens और
Google Cloud IAM का उपयोग करती हैं। Endpoint और permissions के लिए [registry A2A endpoint से agent को call करें](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)।

ये paths Agent-Native JSON-RPC paths (`/.well-known/agent-card.json` और
`/_agent-native/a2a`) से अलग हैं। Endpoint को register करने से पहले ऐसा
server-side adapter जोड़ें जो wire format translate करे और Google credentials
प्राप्त तथा refresh करे। यदि target अलग standard A2A endpoint देता है, तो
generic bearer guidance का उपयोग करें। Gemini Enterprise registration traffic
Agent Gateway policies से होकर नहीं जाता, इसलिए target की अपनी auth और
authorization policy लागू करें।

### Anthropic Managed Agents {#anthropic-managed-agents}

Anthropic Managed Agents कोई inbound A2A endpoint उपलब्ध नहीं कराता। जब Agent-Native को A2A surface, workflow state और approvals का स्वामित्व रखना हो, जबकि hosted agent अपने session, context और tools का स्वामित्व रखता हो, तब framework adapter का उपयोग करें। Adapter Managed Agents session बनाता है, `user.message` events भेजता है और session event stream पढ़ता है। Blocking `agent.tool_use` या `agent.mcp_tool_use` event A2A `input-required` task और `approval-request` runtime event बन जाता है। लौटाए गए metadata में स्पष्ट structured confirmations के साथ इसे जारी रखें; किसी message से approval का अनुमान न लगाएँ:
...
`credentialRef` vault में रखी Anthropic API key का reference है। Adapter इसे caller user और organization scope में resolve करता है और आवश्यक `anthropic-beta: managed-agents-2026-04-01` तथा `anthropic-version: 2023-06-01` headers भेजता है। प्रत्येक app के लिए मौजूदा connection grants का पुनः उपयोग करने हेतु workspace connection catalog में `anthropic-managed-agents` register करें। Adapter सामान्य `A2AConfig.handler` seam पर mount होता है:
...
## प्रत्यायोजित एजेंट गतिविधि {#delegated-agent-activity}

Agent-Native पीयर कार्य स्थिति संदेशों में
`kind: "agent-native/agent-activity"` वाला वैकल्पिक `data` भाग जोड़ सकते हैं।
इसमें उपयोगकर्ता को दिखाई देने वाले रीजनिंग सारांश, टूल के नाम और पूर्णता
स्थिति, बीता हुआ समय और क्रमशः बनता उत्तर सीमित स्नैपशॉट के रूप में होता है।
कॉल करने वाला ऐप इन अपडेट को नेस्टेड एजेंट रन के रूप में दिखाता है और
प्रत्यायोजित कार्य पूरा होने पर कार्य इतिहास को समेट देता है।

गतिविधि भाग में टूल इनपुट, टूल परिणाम, क्रेडेंशियल या प्रदाता का छिपा रीजनिंग
कभी शामिल नहीं होता। इसे अविश्वसनीय प्रस्तुति डेटा मानें; पहचान, प्राधिकरण,
अनुमोदन, रूटिंग या आर्टिफैक्ट सत्यापन के लिए इसका उपयोग न करें। बाहरी A2A पीयर
इस एक्सटेंशन को छोड़ सकते हैं; उनकी सामान्य स्थिति और अंतिम प्रतिक्रिया उसी
प्रत्यायोजित-एजेंट दृश्य में दिखाई देती रहती है।
...
### संगठन सीक्रेट सिंक {#organization-secret-sync}

**संगठन → क्रॉस-ऐप प्रमाणीकरण** सेटिंग, एक ही संगठन में अलग-अलग डिप्लॉय किए गए ऐप्स के बीच डेलीगेशन को प्रमाणित करने वाले संगठन-स्कोप्ड सीक्रेट को प्रबंधित करती है। यह उपयोगकर्ता साइन-इन या Cross-App SSO सेटिंग नहीं है। यह सीक्रेट डिप्लॉयमेंट के `A2A_SECRET` का पूरक है: `org_domain` क्लेम वाला A2A JWT संबंधित संगठन के सीक्रेट से सत्यापित किया जा सकता है।

संगठन के मालिक को हर भाग लेने वाले ऐप को एक ही अनुमत ईमेल डोमेन और एक ही संगठन-स्कोप्ड सीक्रेट देना चाहिए:

1. पहले ऐप में **संगठन → क्रॉस-ऐप प्रमाणीकरण** से सीक्रेट जनरेट या प्रकट करें।
2. हर अतिरिक्त ऐप में **दूसरे ऐप से सीक्रेट पेस्ट करें** चुनें और वही मान पेस्ट करें। पहली कनेक्शन के लिए यह मैन्युअल चरण ज़रूरी है: जिस ऐप के पास अभी सीक्रेट नहीं है, वह दूरस्थ सिंक अनुरोध को सत्यापित नहीं कर सकता।
3. ऐप्स द्वारा सीक्रेट साझा करने के बाद, जुड़े और खोजे गए ऐप्स पर भविष्य के अपडेट भेजने के लिए **ऐप्स में सिंक करें** का उपयोग करें। परिणाम उन ऐप्स की जानकारी देता है जिन्हें अपडेट नहीं किया जा सका।

जब आप सीक्रेट **पुनः जनरेट** करते हैं, सेटिंग्स UI उसे तुरंत सिंक करता है और पिछले सीक्रेट से अपडेट पर हस्ताक्षर करता है, ताकि पहले से जुड़े ऐप बदलाव सत्यापित कर सकें। यदि कोई ऐप उस सिंक से छूट जाए, तो वहाँ नया सीक्रेट मैन्युअल रूप से पेस्ट करके फिर से सिंक करें। इस सीक्रेट को किसी भी अन्य क्रेडेंशियल की तरह रखें: इसे क्लाइंट कोड, लॉग, या तीसरे-पक्ष की क्लाइंट कॉन्फ़िगरेशन में न रखें।
