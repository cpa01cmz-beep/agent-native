---
title: "消息传递"
description: "通过 Slack、Microsoft Teams、Discord 交互、电子邮件、Telegram 或 WhatsApp 与您的代理交谈。"
---

# 消息传递

将您的代理连接到 Slack、Microsoft Teams、Discord 交互、电子邮件、Telegram 或 WhatsApp。它仍使用相同的记忆、工具和对话历史。

> **准确的支持范围：** Teams 会验证官方 Bot Framework JWT，并回复原始会话。Discord 支持通过 Ed25519 签名的 HTTP 交互执行斜杠命令和延迟确认；普通服务器消息和私信仍需要 Gateway 运行时。Telegram 会保留 `message_thread_id`，WhatsApp 会使用传入的 `wamid` 发送上下文回复。
>
> Teams 需要 `MICROSOFT_TEAMS_APP_ID`、`MICROSOFT_TEAMS_APP_PASSWORD` 和 `MICROSOFT_TEAMS_ALLOWED_TENANT_IDS`。Discord 需要 `DISCORD_APPLICATION_ID` 和 `DISCORD_PUBLIC_KEY`。Telegram 还需要 `TELEGRAM_WEBHOOK_SECRET`，WhatsApp 还需要 `WHATSAPP_APP_SECRET`。

> **使用调度模板？** 所有这些都已在 **管理 → 消息传送** 中为您做好准备。单击以连接每个平台 - 您无需阅读本页的其余部分，除非您正在自定义或构建自己的模板。请参阅 [Dispatch](/docs/dispatch) 或 [Dispatch template reference](/docs/template-dispatch)。
...
- **从一条消息操作其他工具。** 一次 @mention 可以变成 Notion 里的一行、Jira 里的一个工单，或 HubSpot 里的一条记录 —— 完整示例见 [Slack 中的 @mention → Notion 中的一行](#slack-to-notion)。
- 有关单向应用内提醒（响铃图标、webhooks），请参阅 [Notifications](/docs/notifications)。

<Diagram id="doc-block-iujha0" title={"多渠道，一位代理商"} summary={"每个平台都进入相同的代理循环和相同的 SQL 线程历史记录 - 因此 Slack 私信 和电子邮件继续相同的对话。"}>
...
<div class="msg-fanin">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">一个代理循环</span
    ><small class="diagram-muted">相同记忆 · 相同工具</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    一条 SQL 线程历史<br /><small class="diagram-muted"
...
- **频道提及** - 使用 @mention 开始工作。代理要处理的每条频道消息都必须明确提及代理；普通线程回复和一般频道聊天不会调用代理。
- **私信** — 已接受的个人私信属于私密会话，并要求已验证的组织成员或已关联的 Slack 身份。
- **身份** — 私信可把经过验证的 Slack 邮箱映射到同一应用用户；托管频道使用自己的服务身份和策略。
...
推荐使用托管 OAuth。在**管理 → 消息**中选择 **Agent manifest**，下载当前主机专用的
`agent-native-slack-manifest.json`，然后从 Slack 的 **App Manifest** 导入。
该清单会启用当前的 `agent_view` 体验，以及可写入的 **Messages** 私信标签页。
请确认包含以下 Bot Token Scopes：

应用图标不包含在清单中。请在 **Basic Information → Display Information** 中上传
Agent-Native 图标，使 Agents 选择器与产品品牌一致。可以使用应用公开的
`icon-512.svg` 作为源文件，并在需要时转换为 Slack 支持的上传格式。
...
Event Subscriptions Request URL 设置为
`https://your-app.example.com/_agent-native/integrations/slack/webhook`，
Interactivity Request URL 设置为
`https://your-app.example.com/_agent-native/integrations/slack/interactions`。
订阅 `app_home_opened`、`app_context_changed`、`app_mention` 和 `message.im`。
配置 `SLACK_CLIENT_ID`、`SLACK_CLIENT_SECRET` 和
`SLACK_SIGNING_SECRET` 后，在**管理 → 消息**中点击 **Add to Slack**。
每个安装使用独立的加密令牌，并按工作区、应用、频道和线程隔离会话键。

更改清单、权限范围、事件、Agent view 或 Messages 标签页后，请重新安装 Slack 应用。
`app_home_opened` 用于检测私信是否打开，`app_context_changed` 提供当前查看的 Slack
上下文，`message.im` 负责传递私信。Slack 管理员可以为整个组织隐藏已安装的代理，
每位成员也可以从自己的顶部栏隐藏代理。如果安装成功但代理未显示，请检查这些显示
设置，或从 **Tools → Apps** 打开。

个人 Slack 私信只接受已验证的 Agent-Native 组织成员，或已关联到该成员的 Slack
身份。频道服务主体的策略和预算与个人身份分开管理。

代理要处理的每条频道消息都必须明确提及代理；普通线程回复和一般频道聊天不会调用代理。有限上下文包含
反应和文件引用，原生任务卡显示计划、进度、取消和批准/拒绝。私信使用个人身份；
托管频道使用明确的服务身份以及访客、Slack Connect 和模型策略。频道记忆仅在明确
要求时保存或删除，绝不自动记忆。在频道中创建的例程会把结果发送回同一 Slack。
组织、请求者和频道的每日或每月预算会随完整审计链一起执行。`assistant:write`
启用 Agent View 上下文事件，`chat:write` 用于回复和实时状态更新。

### 完整示例：Slack 中的 @mention → Notion 中的一行 {#slack-to-notion}

Slack 提及跑通之后，最常见的下一个需求大致是 _“我在会话串里 @mention 代理时，把我说的内容作为一行写进我的 Notion 数据库”_。这件事已经可以做到 —— Slack 入站、Notion 供应商 API 和应用操作都是内置的 —— 但需要按正确顺序连接三个部分，其中两个如果跳过会静默失败。

下面所有 `<ANGLE_BRACKETS>` 都是占位符。切勿把真实的 Notion 密钥粘贴到源码、提交到仓库的 `.env` 或聊天消息中。

#### 1. 让提及先到达应用

在动 Notion 之前，先完成 [设置Slack](#slack)，并确认频道中的 `@mention` 能得到回复。后续步骤有两点行为很关键：

- 含 `@mention` 的那条消息就是调用，它的文本就是代理看到的内容。
- 同一会话串里之后的每条消息也必须明确提及代理；普通回复不会到达代理。每次有新请求时都请重新 @mention。

#### 2. 暴露供应商 API 操作

Core 自带 Notion 供应商预设（基础 URL、`Notion-Version` 头、鉴权、文档链接），但让代理真正用上它的三个操作 —— `provider-api-catalog`、`provider-api-docs` 和 `provider-api-request` —— 是**需要主动启用的子路径导出**。从未添加过它们的应用根本没有通往 Notion 的路径，代理会回答自己没有 Notion 工具。这一步最容易被漏掉。
...
然后为每个操作 ID 添加一个很薄的操作文件。工厂函数的所有选项都是可选的，因此每个文件只需一次调用：
...
`actions/` 中的文件会被自动发现 —— 没有需要维护的注册表。若要收窄供应商列表或通过 HTTP 暴露，请参阅 [复用 Core 的操作工厂](/docs/integrations#provider-api-factories)。

#### 3. 把 Notion 密钥放进保险库，而不是 `.env`

在 **[notion.so/my-integrations](https://www.notion.so/my-integrations)** 创建一个内部集成并复制它的密钥。然后在你的应用中打开 **Settings → API Keys & Connections**，添加一个名称恰好为 `NOTION_API_KEY` 的密钥，粘贴取值，并选择作用域：

- **Personal** —— 只有以你的身份执行的运行才能使用。
- **Workspace** —— 与组织共享。如果除你之外还有人会 @mention 代理，请选择这一项，因为 Slack 运行是以集成所有者的身份执行的，而不是以发消息的人的身份。

供应商运行时按个人 → 组织 → 工作区的顺序从加密保险库解析凭据，并且**刻意从不读取 `process.env`**，因为部署级环境变量会被多租户应用的所有用户共享。在 `.env` 里写一行 `NOTION_API_KEY` 在这里不会生效。

#### 4. 把数据库共享给集成

在 Notion 中打开目标数据库，然后通过 **••• → Connections →** 添加你的集成。跳过这一步是最常见也最容易误判的 Notion 故障：API 返回的是 **404** 而不是 403，看起来像数据库 ID 写错了。目录条目中带有这段修复说明，`provider-api-request` 会把它追加到任何 401、403 或 404 响应的 `guidance` 字段里。

数据库 ID 就在数据库 URL 里 —— 是 `?` 之前那段 32 个字符的字符串。

#### 5. 让你的代理来实现

这一步是所有设置界面都不会告诉你的：两半不需要你自己接起来，你只要提出想要的行为。在应用的代理聊天中：

> 添加一个名为 `add-notion-row` 的操作，通过供应商 API 向 Notion 数据库 `<NOTION_DATABASE_ID>` 追加一行。它应接收消息文本、Slack 作者，以及回到该 Slack 消息的链接。然后更新 `AGENTS.md`，让别人在 Slack 里 @mention 你并要求记录或保存内容时，你调用这个操作并回复新的 Notion 页面 URL。

代理应该写的是**一个操作** —— 不是 Nitro 路由，也不是浏览器里的 `fetch`：
...
- 当前版本的 Notion 把行挂在数据库内部的**数据源**上，所以先读取数据库，并用它的 `data_sources[0].id` 作为父级。直接把数据库 ID 当作父级是旧版 API 的写法。
- `executeRequest` 会返回供应商错误而不是抛出异常，所以要自己检查 `response.ok`，并把返回的 `guidance` 一起传递出去 —— “请共享数据库”的提示就在那里。
- 属性名（`Name`、`Author`、`Source`）必须与数据库的列完全一致，类型也要匹配。让代理先读取数据库的 `properties`，而不是靠猜。

`AGENTS.md` 这一半不是可选项。没有它，代理虽然有工具，但在 Slack 消息说“把这个记下来”时没有理由去用。

#### 6. 验证结果，以及失败时该查什么

在代理所在的 Slack 频道里发送 `@YourAgent 把这个记到 Notion：<SOME_TEXT>`。成功的运行会在会话串里回复新页面的 URL，并在 Notion 中出现这一行。

如果不成功：

| 你看到的现象                                              | 该查哪里                                                                                                     |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Slack 完全没有回复                                        | 提及根本没到达应用。请重新检查 [设置Slack](#slack) 中的 Request URL 和签名密钥 —— 这不是 Notion 的问题。     |
| 代理有回复，但说自己没有 Notion 工具                      | 第 2 步。供应商 API 操作从未添加到 `actions/`。                                                              |
| `notion credential not configured. Tried: NOTION_API_KEY` | 第 3 步。密钥放在了 `.env`，或者以 **Personal** 保存在了与实际执行账号不同的账号下。请改存为 **Workspace**。 |
| Notion 返回 `404`                                         | 第 4 步。数据库从未共享给该集成。                                                                            |
| Notion 就属性返回 `400`                                   | 操作里的属性名或类型与数据库的列不一致。                                                                     |
| 成功过一次，之后会话串里的回复被忽略                      | 这是预期行为。代理要处理的每条消息都必须明确提及代理 —— 请重新 @mention。                                    |
...
- **点击，而不是配置。** Dispatch 的 **管理 → 消息传递** 页面具有针对每个平台的连接按钮，并内置了 env-var 字段。
...
<Diagram id="doc-block-8ahb9q" title={"入站 Webhook 生命周期"} summary={"Webhook 仅验证、入队并返回 200。新的函数执行会耗尽队列并运行代理循环，并以 60 秒的重试作业作为安全网。"}>

```html
<div class="msg-flow">
  <div class="msg-row">
    <div class="diagram-node">
      Platform<br /><small class="diagram-muted">Slack · 邮件 · 等</small>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>
      <strong>/webhook</strong><br /><small class="diagram-muted"
        >verify signature + parse</small
      ><br /><span class="diagram-pill">INSERT 待处理任务</span>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
集成凭证首先从集成所有者的作用域密钥中解析（用户、组织、再到工作区）；部署环境变量是本地或有意单租户部署的后备方案。设置 UI 会保存作用域密钥；值绝不会返回给浏览器，也不会存入源代码。

内置运行时适配器包括 **Slack**、**Telegram**、**WhatsApp Cloud API** 和提供商 webhook 电子邮件（Resend 或 SendGrid）。在实现并注册经过验证的适配器之前，Discord 和 Microsoft Teams 不可连接。
