...
<Diagram id="doc-block-x5yenj" title={"每次运行都会为循环提供动力"} summary={"一次代理运行会产生跟踪、自动评分和反馈挂钩 - 所有这些都存储在应用程序自己的 SQL 中并显示在仪表板上。实验将流量分配给配置变体。"}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    代理运行<br /><small class="diagram-muted">production-agent.ts</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">自动捕获</span
...
### PostHog 中的反馈 {#posthog-feedback}

点赞/点踩、分类和自由文本都会发送 `$ai_feedback`。而 PostHog LLM 分析的反馈视图读取的是 `survey sent` 事件，因此请把 `POSTHOG_AI_FEEDBACK_SURVEY_ID` 设为 PostHog 从追踪的 **Feedback** 标签页创建的调查。这一个 id 就是全部配置；未设置时不会发送任何调查事件。

点赞/点踩以 PostHog 的选项序号回答调查的第一个问题（`1` 为赞，`2` 为踩），点踩之后的自由文本则回答追问，因此文本绝不会落在评分的位置。两者对同一条被评价的消息使用同一个提交 id，PostHog 正是据此把它们合并为一条回答；在追问到达之前，点踩会一直标记为未完成。
...
<Diagram id="doc-block-1ap5n7y" title={"一致哈希变体分配"} summary={"每个用户散列到一个稳定的变体，循环应用该变体的配置覆盖，并且结果以置信区间汇总每个变体。"}>

```html
<div class="exp">
  <div class="diagram-node">
    用户 ID<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">配置覆盖 A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">配置覆盖 B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    结果 per variant<br /><small class="diagram-muted"
...
通过 `defineAppConfig()` 在服务器插件中设置。每个字段还有一个部署环境变量别名——请参阅声明式配置参考。
...
通过在您自己的应用中注册 OpenTelemetry 提供程序，将跟踪发送到 Langfuse、Datadog、Grafana 或任何 OTel 兼容后端——请参阅下面的 [OpenTelemetry 范围](#otel)。

核心有意不提供任何导出器，也不保存导出端点或令牌，因此后端凭据存在于您的提供程序接线（以及保险库）中，而绝不在框架配置中。

## OpenTelemetry 跨度 {#otel}

代理循环为每次运行、模型调用和工具调用发出**实时 OpenTelemetry 范围**，因此已经运行 OTel 收集器的主机可以看到代理活动以及其分布式跟踪的其余部分。
...
- [**Actions**](/docs/actions-overview) - 在跟踪中显示为工具调用的操作
