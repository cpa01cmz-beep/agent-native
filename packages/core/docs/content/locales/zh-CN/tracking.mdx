...
`source` 可以是直接透传的操作 `ctx`，也可以是没有 `ctx` 的调用方使用的 `{ userId, anonymousId, sessionId }` 元数据对象。

浏览器会话不需要调用方自行提供：它来自环境请求上下文（`RequestContext.browserSessionId`，由操作客户端、聊天和浏览器端 `track()` 都会发送的 `X-Agent-Native-Session-Id` 请求头设置），因此无论事件由界面还是由智能体触发，都会关联到同一次访问。每个提供方都会把它映射到自己的会话字段：PostHog 使用 `$session_id`，从而把事件与会话回放关联起来；Mixpanel 和 Amplitude 使用 `session_id` 属性；Webhook 和 Agent-Native Analytics 使用顶层的 `sessionId`。对于没有浏览器的调用方（cron、CLI、MCP、A2A）该值不存在，而元数据对象中的 `sessionId` 会覆盖它。
...
export default defineAction({
  ...
  run: async ({ name, template }, ctx) => {
    ...
    // 直接透传 ctx：事件归属于 ctx.userEmail，并关联到发起调用的
    // 浏览器会话。
...
- [**Actions**](/docs/actions-overview) — 大多数跟踪调用的发起位置
- [**Server Plugins**](/docs/server-plugins) - `registerBuiltinProviders()` 在启动时在 core-routes 插件中运行
