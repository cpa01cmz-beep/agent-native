...
<Diagram id="doc-block-1j8ms62" title={"安全带拥有它的环； Agent-Native 驱动会话"} summary={"AgentHarness 底层 creates/resumes 本机会话，将其事件流式传输到正常转录本中，并在轮次之间将恢复状态保留在 SQL 中。"}>
...
@ai-sdk/harness-codex@latest`); `resolveAgentHarness`抛出一个明确的安装
...
## Codex 身份验证：代码 UI 与 Harness 适配器 {#codex-auth}

Codex 有两个界面：

- **Agent-Native 代码/桌面**在用户计算机上运行 `codex exec`，并重用
  `codex login status` 报告的本地 Codex CLI 登录。
- **`ai-sdk-harness:codex`**在底层 Harness 支持时使用 AI SDK Harness 的原生
  订阅流程。无需额外设置：凭据保留在主机上，显式 API 密钥凭据优先，
  并且无需将本地身份验证文件复制到沙箱即可传递生成的身份验证。若只使用
  AI Gateway，请将 `harnessOptions.auth` 设置为 `"ai-gateway"`；该模式不会
  读取原生订阅。

内置 `ai-sdk-harness:codex` 适配器仅支持 `permissionMode: "allow-all"`，因为这是
稳定 Codex 运行时唯一接受的模式。省略 `permissionMode` 即可使用此默认值；此
适配器会拒绝 `allow-reads` 和 `allow-edits`。
...
`listAgentHarnesses()`枚举注册的内容
一个选择器。

对于使用沙箱的内置线束，请在注册时通过 `agentOptions.sandbox` 传入沙箱提供程序。
进程重启后要重新恢复会话，提供程序必须实现 `resumeSession`；调用方提供的
`sandbox` 会话仅适用于初始创建。
...
<Diagram id="doc-block-1tdv0lu" title={"跨回合、流程和部署恢复"} summary={"每一轮都会将一个不透明的resumeState分离到SQL中；下一回合将其反馈到 createSession 而不是重播聊天记录。"}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    第 N 轮<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >不透明 · SQL harness 会话</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    第 N+1 轮<br /><small class="diagram-muted"
...
[Human Approval](/docs/actions-access-control#needs-approval) 为批准表面。
...
- [Human Approval](/docs/actions-access-control#needs-approval) — 批准表面线束运行使用。
