...
<Diagram id="doc-block-1ckqqi2" title={"添加打印菜谱；您的编码代理应用它"} summary={"agent-native 向 stdout 发出 Markdown 蓝图（对 stderr 进行诊断）；您可以将其通过管道传输到 Claude Code 或 Codex，这将使用完整的上下文编辑您的实时存储库。"}>
...
<div class="diagram-bp">
  ...
  <div class="diagram-box" data-rough>
    Markdown 蓝图<br /><small class="diagram-muted"
      >stdout · 要触碰的文件 · 规则 · 验证</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    编码 Agent<br /><small class="diagram-muted">claude · codex</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
- [**Actions**](/docs/actions-overview) - 每个蓝图构建的单一事实来源
