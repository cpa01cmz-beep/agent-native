...
<Diagram id="doc-block-2y4gqt" title={"设置清单"} summary={"只需要连接一个AI引擎。该面板会跟踪完成情况，并在完成所需的所有操作后自动隐藏。"}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span><strong>连接 AI 引擎</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>邮件投递</strong
...
<Endpoint id="doc-block-1m8aifh" title={"列出入职步骤"} method="GET" path="/_agent-native/onboarding/steps" summary={"列出所有已注册的步骤及其完成状态"} responses={[
  {
    "status": "200",
  ...
  annotations={[
    {
      lines: "5",
      label: "自动安装",
      note: "从 Nitro 插件注册 - 该框架处理渲染、完成跟踪和解除。",
    },
    {
      lines: "7",
      label: "稳定ID",
      note: "默认加载后使用相同的 `id` 重新注册会覆盖内置步骤。",
    },
    {
      lines: "12-19",
      label: "主要方法",
    ...
    },
    {
      lines: "20-26",
      label: "委托路径",
      note: '`kind: "agent-task"` 会把设置任务连同提示交给代理聊天。',
    },
    {
      lines: "28-31",
      label: "竣工检查",
      note: "`isComplete` 在服务器端运行。 OAuth 令牌位于 `oauth_tokens` 存储中 - 检查它，而不是 `process.env.GMAIL_REFRESH_TOKEN`。",
    },
  ]}
/>
...
| `database` | 没有 | 托管 PostgreSQL URL，或留空以使用本地 PGlite |
