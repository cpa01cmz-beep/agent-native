...
<Diagram id="doc-block-1w2vm3f" title={"一个电话，多个目的地"} summary={"notify() 始终写入所有者范围的收件箱行，并行扇出到每个注册通道（尽最大努力），然后在事件总线上发出 notification.sent。"}>
...
电子邮件、Slack 和 webhook 始终与收件箱一起注册。发送时优先使用每条通知元数据中的 URL 或收件人，然后回退到匹配的环境变量。如果两者均未设置，该频道在发送时不会执行任何操作。
...
<Diagram id="doc-block-10l6a1l" title={"渠道和严重性"} summary={"收件箱始终打开； webhook 需要一个环境变量；自定义频道在启动时注册。严重性驱动徽章样式并传递到每个渠道。"}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>严重程度决定徽章</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">确认和 FYI</span>
    </div>
    <div class="sev-row">
...
<Endpoint id="doc-block-yxyssw" title={"列出通知"} method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary={"列出当前用户的最近通知"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "ISO 时间戳游标 — 返回在此时间之前创建的通知，用于分页。"
  }
]} responses={[
  {
    "status": "200",
...
- **轮询集成** - 每个突变都会调用 `recordChange()`，因此使用 [`useDbSync`](/docs/client-data#usedbsync) 的模板会自动失效，无需任何额外的接线。
...
- [**Server plugins**](/docs/server-overview) — 启动时注册自定义通道
