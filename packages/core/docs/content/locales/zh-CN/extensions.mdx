---
title: "扩展"
description: "用于一次性沙盒自定义块的可选 Toolkit 能力。持久的产品行为默认应使用应用代码。"
---

# 扩展

扩展是**用于沙盒自定义块的可选 Toolkit 能力**。它们默认关闭，模板必须明确启用。Analytics 将其用于用户明确要求的一次性自定义可视化。对于持久行为，请编辑应用代码。
...
> 如需更深入的自定义，请在自己的代码库和开发流程中更改模板。只有当主机
> 有意授予 repository/workspace 写入工具时，嵌入式代理才能提供帮助；
> 扩展仍是快速、无需部署的 runtime 自定义层。
...
扩展默认关闭。要启用它们，请在 `createAgentChatPlugin()` 和
`createCoreRoutesPlugin()` 中都设置 `extensionTools: true`。前者添加面向
代理的 actions 和提示指导；后者允许通过
`POST /_agent-native/extensions` 进行经过身份验证的创建。沙盒 runtime
以及现有的读取、编辑和深层链接 routes 会继续保留以实现兼容性。

<Diagram id="doc-block-1bni74h" title={"沙盒桥"} summary={"扩展 HTML 在隔离的 iframe 中运行，仅通过一组固定的桥接助手到达主机 - 每个调用都经过范围和访问检查。"}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      宿主模板<br /><small class="diagram-muted"
        >actions，自动限定作用域的 SQL</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>，锁定域名</small
      >
    </div>
    <div class="diagram-box">
      外部 API<br /><small class="diagram-muted">仅通过 extensionFetch</small>
    </div>
  </div>
</div>
```
...
1. 打开 **Settings → Extensions**（或仅在聊天中询问），然后点击 **“新扩展程序”**。
...
<Diagram id="doc-block-eg10rt" title={"插槽将上下文推送到小部件中"} summary={"主机模板拥有命名槽；将扩展安装到其中，可以为用户当前正在查看的任何内容提供 window.slotContext 。"}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">邮件线程</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">联系人备注</span
    ><small class="diagram-muted"
...
- `dbQuery` 和 `extensionFetch` 暂时被屏蔽。
...
## 我应该使用扩展还是应用源代码流程？ {#vs-app-code}

嵌入式代理默认不编辑源代码；只有当主机有意授予 repository/workspace 写入工具时才能编辑。持久产品行为使用源代码流程，快速 runtime 自定义使用扩展。

- **从应用代码开始**：产品行为、原生 UI、布局、路由、业务逻辑或需要复用的功能都应使用代码。
- **仅当应用启用时使用自定义块**：用户必须明确要求在现有界面中添加一次性可视化或交互。它快速且无需部署，但仍是代理创建的沙盒补丁。
- **将持久的块升级为应用代码**：当一个块在原始场景之外仍有价值时，使用**升级为应用代码**，并在代码变更审核和部署前保留原始块。

|              | 扩展                     | 应用代码编辑       |
| ------------ | ------------------------ | ------------------ |
| **创建者**   | 运行时的代理（或用户）   | 用户或代码能力流程 |
| **存储在**   | 数据库                   | git 存储库         |
| **需要构建** | 否                       | 是的               |
| **需要部署** | 没有                     | 是                 |
| **范围**     | 一个用户（或与组织共享） | 整个产品，每个用户 |
| **最适合**   | 明确的一次性自定义块     | 持久的产品行为     |

经验法则：**默认使用应用代码。** 在启用扩展的应用中，一次性可视化可以是自定义块；所有仪表板都应可用的新图表类型属于模板代码。

对于应该为原生图表和表格提供数据、而不是定制小组件 UI 的自定义数据获取需求，请参见[数据程序](/docs/data-programs)——一个已保存、可刷新的 `run-code` 脚本，dashboard 面板可直接绑定到它，完全不涉及扩展 iframe。
...
- **专用网络保护** - 扩展程序无法到达内部地址。
- **需要身份验证** - 扩展程序仅针对登录用户运行，并且 `dbQuery` 调用是自动范围的。
...
- [**Actions**](/docs/actions-overview) — 扩展通过 `appAction` 调用的操作
