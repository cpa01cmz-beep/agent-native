...
Assets 是一个代理原生工作区，用于创建和管理品牌一致的媒体，尤其适合图像或视频模型具有明显优势的场景。它将上传和生成的结果组织到库和文件夹中，让团队收集产品镜头、徽标、社交图片、写实场景和其他媒体的已批准参考，以便在 Design 构图或其他地方使用，然后通过代理聊天路由生成，以便审查和完善每个资产。

<WireframeBlock id="doc-block-1ixik7e">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-1dyv8p3" title={"生成、审查、重用"} summary={"参考和提示为生成和选择会话提供信息；选定的资产进入库并通过选择器或 A2A 流出到其他应用程序。"}>

```html
<div class="diagram-assets">
  <div class="diagram-col">
    <div class="diagram-node">
      References<br /><small class="diagram-muted">logo、产品截图、风格</small>
    </div>
    <div class="diagram-node">
      提示<br /><small class="diagram-muted">聊天或 Generate 控件</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    <span class="diagram-pill accent">生成会话</span
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">已选、符合品牌的 assets</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-node">
      Picker<br /><small class="diagram-muted">iframe / MCP 应用</small>
    </div>
    <div class="diagram-node">
      A2A<br /><small class="diagram-muted">幻灯片 · 设计 · 内容</small>
    </div>
  </div>
</div>
```
...
大多数视觉工作请使用 **Design**。如果要制作落地页、广告、图表或其他构图，并且需要 HTML/CSS、精确布局、可编辑结构，或与设计工具互相导入/导出，请从 Design 开始。

当图像或视频模型更适合任务，并且结果可以作为媒体直接使用，而不需要逐块编辑时，请使用 **Assets**。这包括写实或像素风格的图像、产品镜头、社交图片和视频。如果希望生成的图像保持原样使用，手绘或 Excalidraw 风格的图表也可以是例外。

理想的工作流是从 Design 开始。当 PNG 或其他媒体素材更合适时，让 Design 代理将辅助图像生成委托给 Assets。然后可以在 Design 构图或其他应用中使用结果。
