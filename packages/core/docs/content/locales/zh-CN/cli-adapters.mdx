...
<Diagram id="doc-block-25sqgq" title={"CLI 适配器 → 注册表 → 操作面"} summary={"ShellCliAdapter 包装二进制文件； CliRegistry 收集适配器以供发现； defineAction 在代理 + UI 操作界面上公开一个调用。"}>
...
- [**Actions**](/docs/actions-overview) — 操作面 CLI 适配器通常被包裹在其中。
