...
<WireframeBlock id="doc-block-1gy0mfl">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-8gievc" title={"邮件请求如何流动"} summary={"键盘快捷键和代理提示运行相同的操作。电子邮件位于 Gmail； SQL 和 application_state 中的草稿、自动化和实时跟踪。"}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      由你驱动<br /><small class="diagram-muted">J/K/E/R 快捷键</small>
    </div>
    <div class="diagram-node">
      你向代理请求<br /><small class="diagram-muted"
        >“起草一封友好的拒绝”</small
      >
    </div>
  </div>
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Gmail<br /><small class="diagram-muted">多账户，通过 OAuth</small>
    </div>
    <div class="diagram-box">
  ...
  <div class="diagram-box">收件箱实时刷新</div>
</div>
```
...
<DataModel
  ...
  title={"邮件 SQL 表"}
  summary={
    "电子邮件本身位于 Gmail 中。 SQL 表包含 Gmail 不包含的内容：排队草稿、发送跟踪事件和 OAuth 令牌。设置和临时状态位于 settings 和 application_state 存储中。"
  }
  entities={[
    {
...
**添加代理功能。**使用 `defineAction` 在 `templates/mail/actions/` 下添加新文件。您的操作将成为代理工具、CLI 命令 (`pnpm action <name>`) 以及通过 `useActionQuery` / `useActionMutation` 键入的前端钩子表面。查看 `templates/mail/actions/star-email.ts` 作为一个简短示例，或者查看 `templates/mail/actions/manage-automations.ts` 作为具有多个子 actions 的示例。有关完整模式，请参阅 [actions](/docs/actions-overview) 文档。
...
请在自己的代码库和开发流程中完成这些更改。只有当主机有意授予 repository/workspace 写入工具时，嵌入式代理才能协助编辑源代码；否则，它通过 actions 和数据工作。请参阅 [Key Concepts](/docs/key-concepts#agent-modifies-code)。

## 后续内容

- [**模板**](/docs/cloneable-saas) — 克隆并拥有的模式，以及可作为起点的所有其他领域模板
- [**开始使用**](/docs/getting-started) — 框架基础：actions、application state 和实时同步
- [**日历**](/docs/template-calendar) — 涵盖日程安排和预订的相关领域模板
- [**自动化**](/docs/automations) — Mail 自动分类规则所基于的事件触发引擎
- [**Actions**](/docs/actions-overview) — 聊天、快捷键和自动化共同调用的 action 系统
