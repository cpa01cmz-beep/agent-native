...
<Diagram id="doc-block-tho29x" title={"客户端方向：多种来源，一种工具注册表"} summary={"配置文件、环境和运行时 UI 全部合并到 MCP 管理器中；它的工具与您的应用程序的操作一起显示为前缀并且可通过工具搜索。这是服务器方向的镜像。"}>
...
<div class="mcp-merge">
  <div class="diagram-col sources">
    ...
    <div class="diagram-box" data-rough>
      应用根 <code>mcp.config.json</code><br /><small class="diagram-muted"
        >每个应用覆盖</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      通过设置界面远程配置<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">MCP 管理器</span
    ><small class="diagram-muted">merge &middot; hot-reload</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col out">
    <div class="diagram-node">
      Agent 工具 registry<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-13gmzwb"
  title={"mcp.config.json，带注释"}
  ...
  annotations={[
    {
      lines: "3",
      label: "服务器ID",
      note: "键成为工具前缀：该服务器的工具在代理的注册表中显示为 `mcp__claude-in-chrome__*`，因此它们不会与模板的操作发生冲突。",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` 生成本地二进制文件。 Stdio 服务器旨在用于**本地开发**- 它们在边缘运行时中是无操作的。",
    },
    {
      lines: "6",
      label: "进程环境",
...
预设 ID 包括 `context7`、`sentry`、`notion`、`semgrep`、`linear`、
`atlassian`、`supabase`、`neon`、`stripe`、`cloudflare`、`gitlab`、`figma`、
`vercel`、`github`、`slack`、`asana`、`hubspot`、`intercom`、`monday`、`webflow`、`paypal`、`box`、`netlify` 和 `zapier`。

目录会区分支持通用 OAuth 流程的端点，以及需要供应商批准的客户端或预先注册的
OAuth 应用的供应商。后者仍会显示以便发现，并链接到官方设置说明；但在不支持时，
不会显示容易误导的一键连接按钮。沙盒扩展可以通过主机操作使用
`agentNative.mcp.listTools`、`agentNative.mcp.callTool` 以及
`agentNative.providerApi.catalog(...)`、`agentNative.providerApi.docs(...)`
来发现已连接的数据，但绝不会收到连接 URL、访问令牌或刷新令牌。原始的
`provider-api-request` 有意仅限主机和代理使用：如果向共享扩展开放任意方法/
路径的请求，扩展就可能变成一个经过身份验证的写入代理。因此扩展应改用
`agentNative.mcp.callTool(...)` 或专门构建的应用操作来执行范围受限的供应商
操作。

本地文件扩展必须在 `extension.json` 中明确允许 `list-mcp-tools`、
`call-mcp-tool` 以及它们使用的任何受支持的提供商操作。发现辅助函数也可通过
`agentNative.connectors.mcp` 和 `agentNative.connectors.providerApi` 使用。

### Atlassian / Jira

Atlassian 预设使用 Atlassian Rovo MCP 的 OAuth 2.1 端点
`https://mcp.atlassian.com/v1/mcp/authv2`，并提供 Jira 和 Confluence
工具。创建 Jira 工单需要具备允许写入 Jira 的 Atlassian 权限；用户仍然只能
看到其 Atlassian 帐户有权访问的项目和内容。

Atlassian 组织管理员可以控制是否允许 OAuth 客户端连接。连接之前，管理员可能
需要将应用的实际 HTTPS 源添加到 **Atlassian Administration → Rovo →
Rovo MCP server → Domains** 下的受信任域列表，并在 **Permissions** 中启用
所需的 **Read**、**Write** 和 **Search** 权限。该域必须是 Agent-Native
应用实际运行的主机；除非 `https://builder.io/**` 是部署的实际源，否则不要将其
复制到其他部署。请参阅 Atlassian 的 [Rovo MCP 设置指南](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/)
和[管理员控制说明](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/)。

目录会提供端点和设置指导。通用设置表单会存储端点和可选的加密标头，但无法取代
Atlassian 的交互式 OAuth 同意流程。当 Atlassian 要求交互式 OAuth 时，请使用
支持 OAuth 的 MCP 客户端或运行时。如果提供商或运行时支持静态 Bearer 令牌，
请遵循其批准的密钥管理流程，切勿将令牌粘贴到提示词或已提交的配置中。
...
HTTPS URL 在任何地方都被接受； plain `http://` 在开发过程中仅允许用于 `localhost`。可选的身份验证作为不记名令牌在每个请求上通过 `Authorization: Bearer …` 发送。仅支持 OAuth 的提供商（包括 Atlassian Rovo MCP）必须使用提供商的 OAuth 流程，而不能使用静态标头。
...
<Diagram id="doc-block-x58r34" title={"中心模型：一个应用程序为组织范围的 MCP 服务器提供服务"} summary={"Dispatch 拥有组织范围 MCP 服务器；消费者应用程序将它们拉取并合并为 mcp__hub_<orgId>_<name>__*。仅共享组织范围的行 - 个人凭据保持不变。"}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">组织范围 MCP 服务器</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">约每 60 秒 pull + merge</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-1xj94io" method="GET" path="/_agent-native/mcp/status" summary={"MCP 工具和入门的客户端状态"} responses={[
  {
    "status": "200",
...
- 配置错误的 `command` → 服务器被跳过，其错误出现在 `errors.<server-id>` 下的 `/_agent-native/mcp/status` 中，而其他所有服务器继续工作。
