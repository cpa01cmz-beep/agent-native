...
> **哪个工作区文档？** 此页面涵盖 **治理** - 谁在一个存储库中的多个应用程序中审查、批准和拥有内容。对于代理自定义层（代理资源：`AGENTS.md`、内存、skills、自定义代理），请参阅 [代理资源](/docs/agent-resources)；对于部署形状（一个单一存储库，许多应用程序），请参阅 [Multi-App Workspaces](/docs/multi-app-workspace)。

本指南涵盖了运行代理本机工作区的操作方面 - 如何分支、谁审查什么、如何设置代码所有权以及 Dispatch 控制平面如何适应您的治理模型。

<Diagram id="doc-block-1tiegxe" title={"两个治理平面"} summary={"Git 管理代码； Dispatch 控制运行时。它们是互补的——不要将一个复制到另一个中。"}>
...
<div class="gov">
  <div class="diagram-card">
    ...
    <div class="gov-list">
      ...
      ><span class="diagram-pill">PR 评审</span
...
并不是每个需要进行更改的人都对 git 感到满意。 [Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=workspace_management) 支持可视化分支模型，可映射到底层的 git 分支 - 对于内容和副本更改、布局调整、设计迭代以及无需开发环境的 A/B 测试非常有用。
  ...
  entries={[
    {
      ...
      note: "按变更路径自动分配 reviewers",
    },
    {
      path: ".github/labeler.yml",
      note: "按 app 自动给 PR 加标签",
    },
    {
      path: "pnpm-workspace.yaml",
      note: "Workspace 级别：宽范围 review",
    },
    {
      path: "package.json",
      note: "Workspace 级别：平台团队负责",
    },
  ]}
/>
...
| **架构迁移**                          | 平台团队+高级工程师             | 数据安全、PostgreSQL 兼容性                         |
...
资源模型和规范路径请参见[代理资源 — Global resources](/docs/agent-resources#global-resources)。

## 设置清单

对于新工作区，运行 `npx --yes @agent-native/core@latest create` 后：
...
- [ ] 为共享提供商帐户注册可重用工作区连接，然后
      ...
      该帐户
- [ ] 通过资源页面添加工作区范围的 skills、护栏说明和品牌/公司参考资源。请参阅 [代理资源](/docs/agent-resources#global-resources) 了解完整的资源模型表和推荐的入门包。
- [ ] 配置审批策略和审批人电子邮件
