...
**自动化**是代理按计划或在事件发生时执行的任务：_当 X 发生时，执行 Y_。**Scheduled** 和 **Event** 是两种触发类型，而不是两个独立产品。

框架最初将计划自动化公开为 **Recurring Jobs**。该名称、`jobs/<name>.md` 路径和 `manage-jobs` 工具会为兼容性保留。新的产品指南使用 **Automations** 作为总称；维护旧计划作业或使用组织范围及高级字段时，请参阅 [Recurring Jobs](/docs/recurring-jobs)。

<Diagram id="doc-block-15xrd2h" title={"当X发生时，做Y"} summary={"总线上触发事件，可选的自然语言条件对其进行门控，代理运行具有完整工具访问权限的自动化主体。"}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">代理运行 body</span
    ><small class="diagram-muted"
...
在完整的 Agent 页面中打开 **Automations**，查看和管理计划自动化及事件触发自动化。兼容 URL 仍为 `/agent#jobs`，因此现有链接和书签会继续工作。

<Callout tone="warning">

`manage-automations` 支持个人和组织范围的 Scheduled 与 Event 自动化。组织 Event 自动化对成员可见，但始终以创建者身份运行；管理员可以管理定义，但不能更改该执行身份。
  ...
  annotations={[
    {
      lines: "2",
      label: "没有计划任务",
      note: '事件触发器将 `schedule` 设为 `""`；cron 字段保持为空。',
    },
    {
      lines: "4-5",
      label: "触发器",
      note: "`triggerType: event` 加上 `event` 名称将此自动化订阅到总线。",
    },
    {
      lines: "6",
      label: "门",
      note: "可选的自然语言 `condition`，由 Haiku 在调度之前根据有效负载进行评估。",
    },
    {
      lines: "12",
      label: "服务器端秘密",
...
| `mode`        | `"agentic"`               | `"agentic"`  | 完整的代理循环（唯一受支持的模式）。`"deterministic"` 已被移除——它从未被实现过，设置该模式的自动化从未被触发。 |
...
发出元数据中的 `owner` 决定哪些自动化可以触发。个人和组织 Event 自动化都必须与 `createdBy` 完全匹配。
...
个人和组织自动化都使用带 `action` 参数的 `manage-automations`：
...
`manage-automations` 支持组织范围、模型覆盖和严格执行的 MCP 允许列表。`manage-jobs` 仍是旧 schedule-only 集成和交付元数据的兼容工具。
...
<Endpoint id="doc-block-vu73fx" title={"触发测试事件"} method="POST" path="/_agent-native/automations/fire-test" summary={"发出 test.event.fired 事件以验证事件触发的自动化"} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1o1fch2" title={"调度路径"} summary={"从触发事件到完成代理运行，由所有权范围和自然语言条件控制。"}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> body 作为提示、payload 作为上下文，5
      分钟超时</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-overview) - 自动化可以通过代理循环调用任何注册的操作
