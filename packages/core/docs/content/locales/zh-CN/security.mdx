...
<Diagram id="doc-block-c5ewgy" title={"层层防守"} summary={"该框架拥有大部分威胁面；您拥有两件事——标记表以确定范围和验证外部输入。"}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">由框架拥有</span
...
框架从会话创建请求上下文，并在应用读写中通过 `accessFilter()` 和 `assertAccess()` 强制执行所有权。代理 SQL 会收到按当前身份和组织过滤的临时视图。RLS 尚未启用，因此应用的直接查询仍必须使用这些辅助函数。
...
<Callout id="doc-block-1rvz2kh" tone="risk">

切勿通过字符串连接或模板文字构建 SQL。将用户输入作为 `args` 传递到 `exec` / `db-query`，或使用 Drizzle - 两者都始终参数化。 `pnpm guards` 检查在 CI 时捕获无范围和串联的查询。
...
<Diagram id="doc-block-19tnfdt" title={"范围界定管道"} summary={"代理 SQL 从不直接接触基表 - 它读取范围为当前标识的临时视图，因此裸表名称只能返回拥有的行。"}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    已登录会话<br /><small class="diagram-muted">email &middot; orgId</small>
  </div>
...
`db-query` / `db-exec` 工具拒绝模式限定的表引用（`public.<table>`）——限定名称解析为基表，并会绕过上面的临时视图。代理使用裸表名称；范围会自动应用。
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "所有者在创建时的活动组织。推动组织可见性检查。",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "私人|组织| public — 粗略默认值，默认为私有。",
        },
      ],
    },
  ]}
...
每用户/每组织凭证和保管库使用 AES-256-GCM 进行静态加密。共享工作区保管库优先使用 `WORKSPACE_SECRETS_ENCRYPTION_KEY`，然后使用由 `SECRETS_ENCRYPTION_KEY`、`A2A_SECRET` 和 `BETTER_AUTH_SECRET` 派生的旧候选密钥；轮换期间设置 `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS`，让读取操作迁移数据行。要就地加密现有明文凭证行，请运行 `pnpm action db-migrate-encrypt-credentials`（幂等、非破坏性）。
...
入站 webhook 处理程序（Resend、SendGrid、Slack、Telegram、WhatsApp、Recall.ai、Deepgram、Zoom）默认在生产中拒绝伪造请求：当缺少相应的签名秘密环境变量时，处理程序返回 401，而不是接受和分派。
...
Google Drive 的 `changes.watch` 通知不会经过这个通用适配器网关：它们是没有正文的不透明请求。因此，框架会为每个频道注册随机令牌，并在拉取更改前验证 `X-Goog-Channel-ID`、`X-Goog-Channel-Token` 和 `X-Goog-Resource-ID` 标头。这些是直接的 Drive 通知，不是 Pub/Sub 消息，也不携带 Pub/Sub OIDC bearer 令牌。
...
- [ ] 为共享多应用保管库一致设置 `WORKSPACE_SECRETS_ENCRYPTION_KEY`；应用本地加密仍使用 `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` 后备 - 请参阅 [Secrets Management](#secrets)
...
运行 `agent-native doctor`（在脚手架生成的应用中为 `pnpm doctor`）可自动检查此清单中的架构、凭据和环境变量项 — 每条发现都会标明 guard、文件和行号。请参阅 [Doctor](/docs/doctor) 文档。
...
- [**Authentication**](/docs/authentication) — 本页所依赖的身份验证模式、cookie realm 和会话细节
- [**Multi-Tenancy**](/docs/multi-tenancy) — `org_id` 范围界定背后的组织成员模型
- [**Sharing**](/docs/sharing) — 应用于可共享资源的 `ownableColumns()` 和访问守卫模式
- [**Messaging**](/docs/messaging#env-vars) — 上方清单中提到的每个集成的 webhook 签名密钥
- [**Deployment**](/docs/deployment) — 此清单假定已设置的生产环境变量
