...
模板是完整的、可分叉的代理本机应用程序，可解决实际工作流程。第一方模板使用您使用的相同框架表面构建：用于 UI 的 React 路由、通过 Drizzle 使用 PostgreSQL 数据、用于操作的 actions、用于代理行为的工作区资源以及轮询同步，以便代理和 UI 保持一致。

将 `@agent-native/core` 用于运行时契约：actions、server plugins、database helpers、app state、routing helpers 和 agent chat。将 `@agent-native/toolkit` 用于可复用的应用构建 UI 和 helpers，例如共享 primitives、toast/mobile hooks、date pickers 和简单 app shell slots。当变体承载产品行为或视觉身份时，请保留在模板本地。

在决定某项能力应放在哪一层时，请牢记这三个层次：

- **Core - 框架：**每个应用都可使用的必需运行时和数据契约。
- **Toolkit - 可选的可复用部件：**应用可以采用、组合或分叉的共享 UI 和产品系统。
- **Templates - 可选应用：**构建在 Core 上、通常使用 Toolkit 的完整领域产品，可以分叉作为起点。

Toolkit 和 Templates 都是可选的。模板并不等同于框架本身，Toolkit 也不会取代 Core。

一个好的模板：

- 使用有用的种子数据或空状态流端到端地解决一个工作流。
- 将持久状态存储在 PostgreSQL/PGlite 中，而不是 JSON 文件中。
...
聊天为您提供身份验证、持久聊天线程、PostgreSQL 支持的资源、工具、应用程序状态、actions 和轮询同步。您添加域模型和产品 UI。

如果您还没有在构建可复用的 UI 模板，请使用 [Automation-First Apps](/docs/pure-agent-apps)：定义动作，在没有浏览器的情况下运行它们，并在工作流需要持久化界面时再添加 UI。
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "server/plugins/",
      note: "启动 integrations",
    },
    {
      path: "server/routes/api/",
      note: "仅在 actions 不够时使用自定义路由",
    },
    {
      path: "shared/types.ts",
      note: "共享的 client/server 类型",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md：复杂工作流的代理指南",
    },
    {
      path: "AGENTS.md",
      note: "模板专用代理指令",
...
不要为应用程序状态添加 `data/` 目录。持久应用数据属于 PostgreSQL/PGlite，UI 通过 actions 或类型化服务器处理程序读取它。

每个模板的四个区域通过一个共享操作界面和一个 PostgreSQL/PGlite 数据库连接在一起 - 代理和 UI 是执行相同操作的平等合作伙伴：

<Diagram id="doc-block-w9lup6" title={"模板的四个区域如何连接"} summary={"UI 和代理都通过相同的操作到达 PostgreSQL/PGlite；应用程序状态和轮询同步使它们保持一致。"}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      React 界面<br /><small class="diagram-muted">app/routes · 组件</small>
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">行动</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    通过 Drizzle 使用 PostgreSQL<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">轮询ing sync</div>
</div>
```
...
## PostgreSQL中的模型数据 {#data-models}

使用框架 Drizzle 助手定义域表，让 schema 使用 PostgreSQL 和 PGlite：
...
架构更改必须是增量式的。对于新的应用自有表和列，请使用
`drizzle-kit generate` 在 `server/db/migrations/<id_name>.sql` 中
生成可审查的 SQL，并在 Node.js 部署的发布步骤中通过
`runDrizzleMigrations()` 加载。Cloudflare Workers/Pages 和其他 edge runtime 没有可部署的
文件系统。在这些环境中，请将生成的 SQL 嵌入 worker bundle 并传给
`runMigrations()`。手写的历史记录和特殊场景继续使用 `runMigrations()`。
切勿使用破坏性的 SQL、`drizzle-kit push`、表重命名或列删除。

对于应用程序读取和写入，请使用 Drizzle 的查询生成器和 `drizzle-orm` 的 PostgreSQL 兼容运算符。当 Drizzle 可以表达查询时，不要使用原始 SQL 编写产品代码，并在模板中使用 `@agent-native/core/db/schema` 的 schema 助手。
...
在添加保存用户或组织数据的架构之前，请使用 [Database](/docs/server-database) 和 [Security](/docs/security) 文档。
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "8",
      label: "工具表面",
      note: "`description` 是代理读取的内容，以决定何时将此操作作为工具调用。",
    },
    {
      lines: "9-11",
      label: "类型化契约",
      note: "一个 zod `schema` 会验证来自代理、UI、HTTP、MCP 和 A2A 的输入。",
    },
    {
      lines: "18-19",
      label: "范围写入",
      note: "从 `ctx` 标记 `ownerEmail` / `orgId`，以便该行正确地确定共享和访问检查的范围。",
    },
  ]}
/>
...
- Data lives in 通过 Drizzle 使用 PostgreSQL. Use actions for all writes; schema is additive.
...
社区模板可直接从公开 GitHub 仓库安装。请将 Agent-Native 应用放在仓库根目录：

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

如需固定特定版本，可添加 `#branch`、`#tag` 或 `#commit`：

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

CLI 只下载源代码，不会安装依赖或运行模板脚本。社区模板属于第三方代码；运行 `pnpm install` 前请检查仓库、许可证、依赖项和安装脚本。

申请目录收录时，请确保仓库根目录包含完整应用，公开仓库带有许可证和设置 README，根 `package.json` 依赖 `@agent-native/core`，并且类型检查和测试均通过。托管演示和截图为可选项。

仓库准备好后，[提交社区模板](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml)。社区条目与较小的第一方目录分开，并由作者维护。
