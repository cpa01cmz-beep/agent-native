...
**Recurring Jobs** 是 **Scheduled** 自动化的旧称。**Automations** 是 **Scheduled** 和 **Event** 的总称。`jobs/` 和 `manage-jobs` 为旧 schedule-only 集成保留。新的个人或组织工作（包括模型和 MCP 允许列表）请使用 `manage-automations` 和 `/agent#jobs` 中的 **Automations**。

</Callout>

重复性工作按时进行。要对*事件*（创建的预订、收到的电子邮件）做出反应 - 相同的 `jobs/` 文件格式加上条件 - 请参阅 [Automations](/docs/automations)。

作业位于 `jobs/<name>.md` 的 [代理资源](/docs/agent-resources) 中 — 只是一个带有 YAML frontmatter 的 Markdown 文件。无需注册，无需接线。将文件放入，框架就会拾取它。
  ...
  annotations={[
    {
      lines: "2",
      label: "当",
      note: "标准 5 字段 cron — `0 7 * * *` 是每天 07:00。",
    },
    {
      lines: "3",
      label: "暂停开关",
    ...
    },
    {
      lines: "7-12",
      label: "提示",
      note: "主体只是一个提示 - 代理在每次触发时使用其所有常规工具和代理上下文运行它。",
    },
  ]}
/>

就是这样。正文是代理在每次计划触发时运行的提示。代理可以访问其在交互式聊天中拥有的所有相同工具和代理上下文 - actions、skills、内存、连接的 MCP 服务器、子代理。
...
| `createdBy`  | 电子邮件                      | _（自动）_  | 当通过代理资源 UI 或代理创建作业时填充。                                                |
...
| `mcpTools`   | MCP 工具名称数组              | _（无）_    | 可选的显式允许列表，例如 `mcp__server__tool`；凭据仍保留在已连接的 MCP 授权中。         |

`last*` 和 `nextRun` 字段由调度程序写入。您可以阅读它们以查看历史记录，但不要手动编辑它们 - 下次运行将覆盖它们。

## 将 MCP 工具连接到作业 {#connected-mcps}

定期作业可以使用已连接到应用的远程或本地 MCP 工具。请让代理先使用 `tool-search` 发现准确名称，然后仅通过 `mcpTools` 绑定该作业所需的工具：
...
查找自上次成功运行以来的新录音。仅提取可执行的后续事项，然后使用稳定的来源和外部 ID、简短引用及录音 URL，一次调用应用受限的导入 action。不要导入完整的转录文本。
```

作业存储的是工具名称，而不是 MCP URL、访问令牌或 OAuth 刷新令牌。运行时，框架会在作业创建者或组织的请求上下文中解析已连接的授权，并且只公开允许列表中的工具。请保持列表精简，并对最终导入使用幂等的应用 action，以避免重叠运行产生重复项。有关连接和 OAuth 设置，请参阅 [MCP Clients](/docs/mcp-clients)。
...
`+` → **代理资源面板中的计划任务**。填写提示和时间表。另存为 `jobs/<slug>.md` 并在下一个匹配的刻度处开始运行。
...
<Diagram id="doc-block-1u2v87h" title={"一个调度程序勾选"} summary={"每 60 秒，调度程序就会找到到期的作业，将每个作业作为新的代理线程运行，并将结果写回作业文件。"}>
...
<div class="sched">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      >共享运行器验证身份并解析模型、MCP 允许列表、继续执行、 交付和用量</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- 在代理资源中打开 `jobs/<name>.md` — frontmatter 显示 `lastRun`、`lastStatus`、`lastError`、`nextRun`。
...
| 行动     | 参数                                                                        | 目的                                              |
| -------- | --------------------------------------------------------------------------- | ------------------------------------------------- |
| `create` | `name`、`schedule`、`instructions`（必填）； `scope`、`runAs`、`mcpTools`   | 创建新的定期作业                                  |
| `list`   | `scope`（`personal`、`shared` 或全部）                                      | 列出所有作业的状态（计划、已启用、上次/下次运行） |
| `update` | `name`（必填）； `schedule`、`instructions`、`enabled`、`runAs`、`mcpTools` | 编辑现有作业                                      |
| `delete` | `name`（必填）                                                              | 删除作业 - 始终先与用户确认                       |

**个人与共享范围。**每个自动化都位于个人或组织范围，并以创建者身份运行。管理员可以管理共享定义，但不能更改创建者身份。每次组织运行前，框架都会验证创建者仍然存在且仍是组织成员。

## 与调度包不同 {#vs-scheduling-package}

不要将重复性作业与 `@agent-native/scheduling` 混淆：

- **重复作业（本页）** — cron 计划*提示*代理在后台运行。框架级别。住在代理资源。在任何代理本机应用程序上运行。
...
- [**代理资源**](/docs/agent-resources) — 作业与 skills、内存和自定义代理一起存在
- [**Actions**](/docs/actions-overview) — 工作调用的工具
