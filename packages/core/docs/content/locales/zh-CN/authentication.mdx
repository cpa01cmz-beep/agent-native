...
- **默认：**当外发电子邮件已准备就绪时，Better Auth 优先使用 magic link，并以电子邮件/密码作为备用方式，同时支持社交提供商。设置 `AUTH_MAGIC_LINK=0` 可强制使用备用方式。
- **远程 MCP OAuth：** 适用于 MCP 主机的标准 OAuth 2.1，例如 Claude 代码和 ChatGPT 连接器。
- **自定义：**通过 `getSession` 回调带来您自己的身份验证。

<Diagram id="doc-block-146mbx9" title={"三种方式，一次会议"} summary={"浏览器访问者、MCP 主机和自定义提供程序都解析为下游作用域读取的同一 AuthSession。"}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code、ChatGPT 连接器</small>
    </div>
    <div class="diagram-card">
...
## 本地开发登录 {#local-development-sign-in}

在使用回环地址的开发服务器上，共享登录页面会优先显示 **Continue as local
dev**。它会创建或复用真实的 Better Auth 开发帐户和会话，因此使用的仍然是应用其他地方相同的已认证流程。

选择 **Show full sign in options** 可显示普通注册、密码、magic link 和社交提供商选项。如果无法创建本地会话，页面会自动显示这些选项，让你可以继续使用标准身份验证。

此便利功能仅在开发或测试环境中通过 `localhost`、`127.0.0.0/8` 或 `::1` 连接时可用。部署预览和托管生产环境会禁用此功能。它不是身份验证绕过。设置 `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` 可选择退出，让本地开发显示普通登录页面。

## 更好的身份验证（默认） {#better-auth}

默认情况下，Better Auth 支持身份验证。它提供：

- 使用 magic link 注册和登录，并以电子邮件/密码作为备用方式
...
- `POST /_agent-native/auth/magic-link` — 请求 magic link 电子邮件
...
或预览环境，并使用 `+autoz` 地址命名测试帐户
(`name+autoz@example.com`)，因此很容易识别。

## 电子邮件验证策略 {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` 只是本地和 QA 的便利设置，托管部署会忽略它。要为
任何环境（包括生产环境）声明该策略，请在配置中声明：

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

部署别名为 `AUTH_REQUIRE_EMAIL_VERIFICATION=0`。声明的值优先于
`AUTH_SKIP_EMAIL_VERIFICATION` 和各环境的默认值。

`false` 会将未经验证的地址作为登录凭据接受——任何人都可以声称拥有任何电子邮件
地址——因此仅在注册已通过其他方式受限时使用。在没有电子邮件提供商的部署上设置
`true` 则会禁用密码注册，因为验证邮件永远无法送达。
...
<Diagram id="doc-block-zb7fjc" title={"远程 MCP OAuth 握手"} summary={"具有 OAuth 功能的客户端仅从 MCP URL 引导 — 挑战、发现、动态注册，然后是 PKCE 代码交换。"}>
...
当匿名查看者点击此 URL 时，将提供框架的登录页面。成功登录（任何流程 - 令牌、电子邮件/密码或 谷歌OAuth）后，浏览器会返回 `return`。
...
普通应用页面为所有访问者使用同一个非个性化、可公开缓存的 SSR 外壳。`AppProviders` 在客户端检查会话；访问 `/dashboard` 等私有路径时，它会在挂载私有应用界面之前将匿名访问者重定向到 `/sign-in?return=%2Fdashboard`。登录后，访问者会返回 `/dashboard`。公共或 SEO 路由请使用 `isPublicPath`；通过 MCP 嵌入令牌等其他受限机制认证的界面请使用 `sessionBypass`。

该外壳的缓存时长是一个部署范围的设置——关于 `AGENT_NATIVE_SSR_CACHE`，参见 [SSR Caching](/docs/ssr-caching)。缩短或禁用缓存只会改变时长；SSR 仍会在剥离 cookie 后渲染匿名分支。

### 幕后花絮：谷歌OAuth

两个流（显式 `/sign-in` 入口点和书签路径情况）都将返回 URL 线程穿过 OAuth 状态。该状态是 HMAC 签名的，因此在运输过程中无法伪造。在回调中，返回的 URL 在重定向之前被重新验证为同源 - 因此泄漏的签名密钥仍然无法转换为开放重定向预言机。
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | 设置为 `0` 或 `1` 可为任何环境（包括生产环境）声明密码注册的验证策略；优先于 `AUTH_SKIP_EMAIL_VERIFICATION`  |
...
普通浏览器继续使用逐应用登录。自托管应用仅在使用剩余的显式 Desktop 或服务端联邦路径时需要 `AGENT_NATIVE_IDENTITY_HUB_URL`。

## 管理式部署与邀请

管理式部署请设置 `AUTH_SIGNUP=invited`、`ORG_CREATION=closed` 和 `AUTO_CREATE_DEFAULT_ORG=0`。[管理式部署指南](/docs/administered-deployments)介绍引导管理员、邀请和允许的邮箱域名。成员邀请与按域名加入是两条不同规则。
