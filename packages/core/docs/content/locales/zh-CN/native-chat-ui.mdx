...
<Diagram id="doc-block-skgt8y" title={"原生渲染路径"} summary={"操作返回 JSON；运行时匹配显式小部件判别式或 chatUI.renderer； AssistantChat 安装真实的 React 组件。没有 iframe，没有 HTML 执行。"}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    Action 运行<br /><small class="diagram-muted">返回结构化 JSON</small>
  </div>
...
<Callout id="doc-block-1gt3s7k" tone="success">

仅当操作声明 `chatUI`**或**结果携带显式已知的 `widget` 判别式时，渲染器才会接管。它从不形​​状推断任意对象，也从不从工具结果执行 HTML 或 JavaScript - 因此本机小部件不能成为注入向量。
...
渲染器请使用绝对单位而非百分比来设置尺寸。聊天记录会随消息增长，因此渲染器的容器
没有可供百分比参照的确定高度——图表柱条上的 `height: 40%` 不会报错，而是塌陷为零，
看起来就像渲染器没有画出来。请为图表及其他依赖高度的视觉元素设置像素（或 `rem`）
基准；宽度使用百分比没有问题，因为聊天记录列确实有确定的宽度。
...
<Diagram id="doc-block-zv5z9u" title={"BYO 运行时保留 Agent-Native 聊天 shell"} summary={"您的外部代理通过连接器传输标准化事件； Agent-Native 保留作曲家、成绩单、工具卡、批准和本机小部件。"}>
...
<div class="diagram-byo">
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">标准化 message-* / tool-* 事件</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
...
- [Actions](/docs/actions-overview) — 定义返回本机小部件数据的操作。
