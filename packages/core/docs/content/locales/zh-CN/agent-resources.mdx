---
title: "代理资源"
description: "Claude - 每个用户的代码级自定义 - skills、内存、指令、自定义代理、计划作业、MCP 服务器 - 由 PostgreSQL/PGlite 支持，而不是文件系统。"
---

# 代理资源

每个代理本机应用程序都附带**代理资源**：使代理成为您的自定义层。它包含团队指令 (`AGENTS.md`)、共享学习内容 (`LEARNINGS.md`)、个人结构化内存 (`memory/MEMORY.md`)、代理按需拉入的 skills、自定义子代理、计划作业和连接的 MCP 服务器 — 您期望从 Claude 代码 / Codex 设置获得的一切。

变化：**它是 PostgreSQL/PGlite 行，而不是文件系统文件。**每个用户都将自己的一套代理资源存储在数据库中。没有要启动的开发盒，没有每个用户的容器，没有要安装的文件。多租户 SaaS 可以为每个用户提供基本上免费的完全可定制的代理，因为所有这些都是行——个人内存、个人 MCP 服务器、个人 skills、个人子代理——并且共享代码库同时托管所有这些。

<Diagram id="doc-block-1729nmr" title={"Claude-Code 设置，但作为代理资源存储在 PostgreSQL/PGlite 中"} summary={"相同的定制层——指令、技能、内存、代理、作业、MCP——除了每个文件都是共享多租户数据库中的一行。"}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-Native 资源</span
    ><small class="diagram-muted">rows in one PostgreSQL/PGlite 数据库</small>
    <div class="ws-rows">
...
| Claude代码/Codex              | 代理本机资源                            |
| ----------------------------- | --------------------------------------- |
| 本地磁盘上的文件              | 共享 PostgreSQL/PGlite 数据库中的行     |
| 每个开发人员一个代码库        | 一个代码库，多个用户                    |
| 需要开发盒或容器              | 在任何无服务器/边缘主机上运行           |
| `~/.claude/` 定制             | 按用户自定义，范围为 `u:<email>:…`      |
| 每个项目 `CLAUDE.md` / skills | 每个应用 `AGENTS.md` + 内存资源         |
...
应用内资源选项卡显示所有三个范围。个人和共享/组织资源可以在那里编辑。工作区范围的资源在应用面板中是只读的，并通过 Dispatch 进行集中编辑，因此每个应用都可以看到相同的规范文件，而无需同步步骤。
...
1. 打开 **资源** 选项卡 → **共享** → `AGENTS.md`（使用 `+` 创建它 → **文件** 如果缺少）。
2. 添加一条规则，例如：
...
代理面板包括一个 **资源** 选项卡以及聊天和 CLI。它显示了所有资源的文件夹组织树、任何文本文件的内联编辑器（Markdown、JSON、YAML、纯文本）以及 `+` 菜单的类型化创建流程（文件、Skills、代理、计划任务）。用户可以浏览继承的工作区默认设置并创建/编辑/删除个人或组织资源。
...
| `mcp-servers/…`    | 工作区/共享 | 人类通过 Dispatch 或应用程序资源 | MCP配置刷新                  |

用户可以直接在“资源”选项卡中编辑这些内存文件 - 它们是常规资源。删除代理出错的行，将个人偏好保留在 `memory/MEMORY.md` 中，或将团队范围的规则推广到 `AGENTS.md` 中。
...
<DataModel
  ...
  title={"代理资源模型"}
  summary={
    "一种资源形状支持每个代理资源文件。运行时按路径和范围对其进行键控，并解析读取时的有效值。"
  }
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "用户代理资源中的单个文件 - 说明、技能、内存、代理、MCP 配置或作业。",
      fields: [
        {
          name: "path",
          type: "string",
...
Skills 是 `skills/` 路径（最好是 `skills/<name>/SKILL.md`）下的 Markdown 资源文件，为代理提供按需领域知识，在与 `/skill-name` 聊天时调用。从“资源”选项卡添加它们，或者在“代码”模式下从 `.agents/skills/` 添加它们。
...
| 功能           | 代码模式                                      | 应用模式                         |
| -------------- | --------------------------------------------- | -------------------------------- |
| @标记          | 代码库文件 + 代理资源 + 自定义代理 + 连接代理 | 代理资源 + 自定义代理 + 连接代理 |
...
工作区连接允许应用程序共享相同的提供商帐户（Slack、GitHub、HubSpot 等），而无需重复凭据。连接在 PostgreSQL/PGlite 中记录提供者身份、帐户标签、状态、范围、应用程序授权和凭据引用。秘密保留在凭证存储中；连接仅指向凭证密钥名称，例如 `SLACK_BOT_TOKEN`。
