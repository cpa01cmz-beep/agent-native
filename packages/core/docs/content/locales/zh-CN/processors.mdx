...
<Diagram id="doc-block-1zox12" title={"三个钩子在奔跑中开火的地方"} summary={"processOutputStream 监视每个块，processOutputStep 控制每个响应的工具调用，processOutputResult 在最后记录判决。任何钩子都可以通过 TripWire 中止。"}>
...
<div class="diagram-proc">
  ...
  <div class="diagram-node" data-rough>
    per model response<br /><small class="diagram-muted"
      >processOutputStep — 拦截工具调用</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
...
处理器是**配置**，不是工具，不是操作，也不是创作 DSL。处理器仅观察、改变它们自己的流范围状态和 `abort()`。它们从不定义应用程序行为、替换 actions 或出现在模型中。应用程序操作属于[actions](/docs/actions-overview)。
