...
<Diagram id="doc-block-8rv6ls" title={"两层可阻止简历上重复的副作用"} summary={"日志读取持久账本并对之前的调用进行分类；第 1 层告诉模型，第 2 层硬阻止与已完成条目匹配的重新分派写入。"}>
...
<div class="diagram-durable">
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-pill accent">第 2 层 · 硬阻塞重新派发写入</div>
  </div>
</div>
```
...
- [**Actions**](/docs/actions-overview) — `readOnly` 将读取标记为可以安全地重新运行；其他一切都被视为副作用。
