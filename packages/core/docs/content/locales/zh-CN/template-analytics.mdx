...
<WireframeBlock id="doc-block-1ejxx9a">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-11qnls8" title={"问题到图表"} summary={"代理查阅数据字典，写入 SQL，根据仓库对其进行验证，然后渲染图表或保存面板。"}>

```html
<div class="diagram-flow">
  <div class="diagram-node">简明语言<br />question</div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>写入 SQL</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill ok">Dry-run 校验</div>
    <small class="diagram-muted">BigQuery / 来源</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">图表、表格或<br />saved panel</div>
</div>
```
...
有关扩展模板的更广泛模式，请参阅 [Skills guide](/docs/skills-guide) 和 [Actions](/docs/actions-overview)。
