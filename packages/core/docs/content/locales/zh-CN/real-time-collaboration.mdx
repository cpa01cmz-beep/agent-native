---
title: "实时协作"
description: "多用户协作编辑，其中 AI 代理是一流的同行：CRDT 合并、实时呈现、SSE 快速路径和细粒度服务器端合并 - 在 PostgreSQL/PGlite 和任何主机上。"
---
...
该表由框架管理且与提供商无关（PGlite 和 Postgres 使用
相同的模式）。每行都有一个乐观并发版本列
...
| 要求                                                | 原因                                                                                                                                | 如何检查                                                                                                                                                       |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **位于公网可达主机上的 Postgres**                   | 网关从自己的网络拨号连接数据库。PGlite、`localhost`、`*.internal` 以及裸 IP 地址在PostgreSQL 连接字符串离开你的机器之前就会被拒绝。 | `DATABASE_URL` 指向 Neon 之类的托管 Postgres                                                                                                                   |
...
随后注册会按需发生，在第一个需要通道的请求上完成。应用发送自己的 PostgreSQL 连接字符串和公开
...
**自注册意味着把你的 PostgreSQL 连接字符串发送给 Builder。** Builder 会存储它，并连接到
