...
<Diagram id="doc-block-1i937ly" title={"从来源到引用的答案"} summary={"大脑将经过批准的来源摄取到原始捕获中，提取持久记忆，通过审查对其进行门控，然后才通过引用进行回答。"}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    原始捕获<br /><small class="diagram-muted">已去重、已脱敏</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted">事实 · 决策 · 流程</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">已批准、原子化</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-1n50103">
  <Screen
    surface="desktop"
    html={
...
无需矢量数据库，因此模板可在 PGlite 之间保持可移植性，
Postgres、Neon、hosted PostgreSQL 和类似主机。应用程序状态反映
当前路线、过滤器和所选 ID，以便代理始终了解当前
导航和选择。

Brain 的模式位于 `templates/brain/server/db/schema.ts` 中。九张表：
...
<Endpoint id="doc-block-uq9gat" title={"签名摄取 webhook"} summary={"剪辑和通用 transcript/capture 导入会发布带有每个源不记名令牌的 RawCapturePayload。"} method="POST" path="/api/_agent-native/brain/ingest" summary={"从 Clips 或通用源导入原始捕获"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
请在代码库开发流程中进行这些更改。只有当主机有意授予
repository/workspace 写入工具时，嵌入式代理才能协助编辑源代码；否则，
它通过 actions 和数据工作。请参阅
...
- [**代理资源**](/docs/agent-resources) — 跨应用共享资源
- [**A2A Protocol**](/docs/a2a-protocol) — 跨应用委托
- [**模板**](/docs/cloneable-saas) — 克隆并拥有的模型，以及可作为起点的所有其他领域模板
- [**入门**](/docs/getting-started) — 框架基础：actions、应用程序状态和实时同步
