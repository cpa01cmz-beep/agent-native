---
title: "临时代理"
description: "使用 <AgentPanel>、<AgentSidebar> 和 sendToAgentChat() 将代理聊天 + 代理资源安装到任何 React 应用中。"
---
...
你不需要从头构建 Agent-Native。智能体聊天、资源选项卡、CLI 终端、语音输入以及所有相关基础设施均以少量 React 组件的形式提供，可直接放入任何应用。

> **先决条件：** 服务器必须运行 `agent-chat-plugin`（它会自动安装在每个模板中）。如果您是从头开始，请参阅 [Server](/docs/server-overview)。
>
...
| `<AgentPanel>`        | 原始面板本身 - 聊天 + CLI + 代理资源选项卡         | 您想要完全控制布局或专用代理页面         |
...
<Diagram id="doc-block-jv2wjy" title={"安装型号"} summary={"<AgentSidebar> 包装您现有的布局。您的路线在主要区域中呈现；代理面板安装在它们旁边。 <AgentPanel> 是没有包装纸的同一面板。"}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        你的应用程序<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1kp2s91"
  title={"用 <AgentSidebar> 包装根布局"}
  ...
  annotations={[
    ...
    },
    {
      lines: "8-12",
      label: "启动提示",
      note: "`suggestions` 在空聊天中呈现为可点击的筹码。",
    },
    {
      lines: "13",
      label: "上下文感知芯片",
...
就是这样。用户现在在每个页面上都有一个可切换代理 - 具有聊天历史记录、代理资源选项卡、CLI 终端、语音输入和更宽的抽屉。通过 `localStorage` 重新加载，状态仍然存在。

### 道具

- **`children`** — 您的应用程序的正常布局和路线。在主区域渲染；代理面板在桌面上安装在其旁边，在移动设备上安装在其上方。
...
`<AgentPanel>` 为您提供原始选项卡（聊天/CLI/代理资源），无需侧边栏包装、折叠按钮或任何状态持久性。把它放在你想要的任何地方；您负责布局。
...
- **`isFullscreen`** — 如果您想在页面级或自定义面板中使用 Claude 样式的居中列，请设置此项。侧边栏改用更宽的抽屉。
...
将工作交给智能体的按钮，而不是通过内联 `llm()` 调用创建单独的 AI 路径。有关这一差别为何重要，请参阅[为什么要以这种方式构建应用](/docs/what-is-agent-native#why-build-apps-this-way)。对于用户发起且可见的 AI 工作，请显式打开侧边栏，以便用户可以查看、引导并在同一线程中继续：
...
- **对于用户发起且需要可见的 AI 工作**，请使用 `openSidebar: true`。仅对明确静默的后台或系统发起工作使用 `false`，并将后续操作保留在同一个 AgentSidebar 线程中。
- **`type`** — `"content"`（默认）将工作保留在使用应用工具的嵌入式代理中。`"code"` 会路由到单独的代码能力 host 以更改代码库源文件；只有当主机有意授予 repository/workspace 工具时才提供写入权限。参见 [Agent-Native 代码 UI](/docs/code-agents-ui)。
...
请将后续和修订提示保留在现有的侧边栏线程中。不要在结果旁添加第二个自由文本输入框；本地表单仅用于界面必须验证的结构化值。

当嵌入相同的路由作为MCP App时，提交
`sendToAgentChat()` 呼叫将转发至支持的聊天主机；参见
[Agent 聊天](/docs/client-agent-chat#sendtoagentchat) 用于 MCP 应用桥接行为。
...
## 选择代理聊天还是操作 {#agent-or-action}

对于 CRUD、验证、提供商读取和持久化等确定性的本地操作，请直接使用 `useActionQuery` 或 `useActionMutation`。如果用户将该流程体验为研究、分析、生成、推荐、综合或多步骤推理，请使用 `sendToAgentChat({ openSidebar: true })` 启动，并让代理编排专注的操作。实现是确定性的，并不能单独成为将类似 AI 的流程隐藏在一个不透明操作中的理由。

## 来自 UI 的类型安全 actions：`useActionMutation()` {#use-action-mutation}

当 UI 需要运行智能体工具会运行的同一操作时，请使用 [`useActionMutation`](/docs/client-data#use-action-mutation)：
...
类型安全参数来自 `defineAction()` 中的 zod 模式。完整的动作系统请参见 [Actions](/docs/actions-overview)。

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation` 与 `sendToAgentChat`。** 当用户点击确定性按钮（“Send reply”）时，直接使用 `useActionMutation` 运行操作。当工作需要智能体的推理、工具或多步骤规划时，将其交给 `sendToAgentChat`。不要从 UI 调用内联 `llm()`；这会创建单独的 AI 路径，而不是使用共享的智能体循环。
...
- [**代理资源**](/docs/agent-resources) —“代理资源”选项卡包含的内容（skills、内存、MCP 服务器、计划作业）
