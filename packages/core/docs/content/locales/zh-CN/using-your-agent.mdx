...
Agent-Native 背后的核心理念是，智能体和 UI 是**平等的伙伴**——请参阅[什么是 Agent-Native？](/docs/what-is-agent-native)了解原因。本节介绍这项承诺的另一面：当智能体停靠在应用旁时，实际与它协作是什么体验。

有一条简单的直通线。代理**看到**您正在查看的内容，您**将其引导**到您想要的内容，您可以**嵌入**它在任何地方，您可以在更合适的情况下完全**轻 UI**，并且您可以同时**共同编辑**相同的文档。其中每一个都是本节中的一个页面。

<Diagram id="doc-block-17jz6ip" title={"日复一日的循环"} summary={"使用对接代理的五种方式 - 每种方式都是本节中的一个页面。"}>
...
<div class="diagram-loop">
  ...
  <div class="diagram-card accent-card">
    <span class="diagram-pill accent">协同编辑</span
    ><small class="diagram-muted">实时，并排</small>
  </div>
</div>
```
...
智能体不是需要切换到的单独应用。凡是 Agent-Native 应用中的聊天面板，都可以停靠在任何屏幕旁——打开后，它就在你正在处理的内容旁边。应用构建者还可以连接按钮，将特定任务直接交给智能体，而不是为此构建一次性的“AI”功能。如果你是在构建应用而非使用它，请参阅[嵌入式智能体](/docs/drop-in-agent)，了解实现这一点的组件。

## 你可以去轻 UI {#ui-light}

并非每个应用程序都需要完整的仪表板。当代理*是*产品时，您可以跳过大部分自定义UI：打开应用程序，询问您想要的内容，然后让代理完成其余的工作。代理仍然有其管理界面——历史记录、代理资源、设置——但主要交互是对话而不是点击。
