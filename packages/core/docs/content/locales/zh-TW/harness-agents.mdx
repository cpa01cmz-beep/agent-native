---
title: "安全帶代理"
description: "將 Claude Code、Codex、Pi 和其他完整的編碼工具作為 Agent-Native 內的嵌入式代理執行，並具有自己的迴圈、沙箱、本機工具和可恢復的 SQL 支援的工作階段。"
...
自己的端對端迴圈，因此 Agent-Native 將其作為工作階段驅動，而不是單一
模型呼叫。

<Diagram id="doc-block-1nk0jee" title={"安全帶擁有它的環； Agent-Native 驅動工作階段"} summary={"AgentHarness 底層 creates/resumes 本機工作階段，將其事件流式傳輸到正常轉錄本中，並在輪次之間將恢復狀態保留在 SQL 中。"}>
...
`installPackage` 提示（例如`@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness`拋出一個明確的安裝
...
Agent-Native 可以充當[ACP](https://agentclientprotocol.com)（代理用戶端
...
## Codex 驗證：程式碼 UI 與 Harness 轉接器 {#codex-auth}

Codex 有兩個表面：

- **Agent-Native 程式碼/桌面**在使用者電腦上執行 `codex exec`，並重用
  `codex login status` 報告的本機 Codex CLI 登入。
- **`ai-sdk-harness:codex`**在底層 Harness 支援時使用 AI SDK Harness 的原生
  訂閱流程。無需額外設定：憑證保留在主機上，明確的 API 金鑰憑證優先，
  並且不必將本機驗證檔案複製到沙箱即可傳遞產生的驗證。若只使用 AI Gateway，
  請將 `harnessOptions.auth` 設為 `"ai-gateway"`；此模式不會讀取原生訂閱。

內建 `ai-sdk-harness:codex` 轉接器僅支援 `permissionMode: "allow-all"`，因為這是
穩定 Codex 執行階段唯一接受的模式。省略 `permissionMode` 即可使用此預設值；此
轉接器會拒絕 `allow-reads` 和 `allow-edits`。
...
`permissionMode`、`harnessOptions` 和 `agentOptions`）。使用
`listAgentHarnesses()`枚舉註冊的內容
一個選取器。

對於使用沙箱的內建線束，請在註冊時透過 `agentOptions.sandbox` 傳入沙箱提供者。
若要在程序重新啟動後重新恢復工作階段，提供者必須實作 `resumeSession`；呼叫端
提供的 `sandbox` 工作階段僅適用於初始建立。
...
是**不透明** - Agent-Native 儲存它並將其返還，但從不檢查或
解釋它。
...
當您透過 `createSession.tools` 有意設定** Agent-Native actions
...
[Human Approval](/docs/actions-access-control#needs-approval) 為核准表面。
...
- [Human Approval](/docs/actions-access-control#needs-approval) — 核准表面線束執行使用。
