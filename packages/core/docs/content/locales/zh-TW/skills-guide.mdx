...
Skills 是為代理提供特定模式與工作流程深度知識的 Markdown 檔案。與其將每條規則都塞進一個系統提示詞，不如把知識拆分成專門的檔案，僅在工作確實需要時由代理載入——既保持預設內容精簡，又讓代理能夠按需深入。
...
目前應用支援的 skills 包括 Plan 的 `visual-plan`、`visual-recap` 和 `visualize-repo`，Design 的 `visual-edit`，Assets 的 `assets`，以及用於 repository 支援的 docs/blog/MDX 編輯的 `content`。

<Callout tone="info">

**完整詳細資訊如下：** [App-backed skills — full details](#app-backed-skills-full) 中介紹了應用程式支援的 skills 的機制（清單格式、CLI 指令、市場轉接器、自動更新哈希）。

`app-skill` 套件也會產生標準 Agent Plugin 檔案。使用 `agent-native plugin import <path> --into <workspace> --yes` 匯入 Skills 和遠端 Streamable HTTP MCP 伺服器；套件標頭會被忽略，stdio 和 SSE 伺服器會回報後略過。
...
- 該指導是一次性的 - 將其放入 `AGENTS.md` 或代理記憶中
...
如果這些知識適用於在應用程式中工作的*每個人*（“總是更喜歡 CTE 而不是子查詢”），那麼它就是一項技能或共用的 `LEARNINGS.md`。如果它是關於*這個特定使用者*（“史蒂夫喜歡簡潔的答案”），它就屬於 `memory/MEMORY.md`。完整治療請參見[代理資源 — 記憶](/docs/agent-resources#memory)。

## Skills 與 SQL 資源 {#skills-vs-resources}

本頁將 skill 描述為 `.agents/skills/<name>/SKILL.md` 中的檔案。[代理資源](/docs/agent-resources#skills) 則將同一個 skill 描述為路徑為 `skills/<slug>/SKILL.md`、範圍為 workspace/shared/personal 的 SQL 資源。兩者是從不同角度描述同一概念：Code 模式直接讀寫檔案；App 模式將其儲存為資源列，因此無需檔案系統也能運作。無論採用哪種方式，都請依本頁格式編寫內容。

## 應用程式支援的 skills — 完整詳細資訊 {#app-backed-skills-full}

以下完整介紹[應用程式支援的 skills](#app-backed-skills) 背後的清單格式、CLI 指令、市場轉接器和自動更新雜湊：
...
- [**編寫代理指令**](/docs/writing-agent-instructions) — 編寫 skill 描述，並在 AGENTS.md、skills 和記憶之間拆分指引
- [**代理資源**](/docs/agent-resources#skills) — App 模式下 skills 作為 SQL 資源儲存的位置
- [**代理提及**](/docs/agent-mentions) — 在聊天中呼叫 skill 的 `/` 指令觸發器
- [**Plan 外掛與市場**](/docs/plan-plugin) — 真實應用程式支援的 skill 的端對端安裝和自動更新流程
