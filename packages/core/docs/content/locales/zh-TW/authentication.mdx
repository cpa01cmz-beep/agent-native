...
Agent-Native 應用程式使用 [Better Auth](https://better-auth.com) 進行帳戶優先設計的驗證。使用者在第一次存取時建立一個帳戶，並從第一天起就獲得真實身分。

## 概述 {#overview}

Auth 是透過 auth 伺服器外掛中的 `autoMountAuth(app)` 自動設定的。共有三種模式：

- **預設：**當外寄電子郵件已準備就緒時，Better Auth 優先使用 magic link，並以電子郵件/密碼作為備用方式，同時支援社交提供者。設定 `AUTH_MAGIC_LINK=0` 可強制使用備用方式。
- **遠端 MCP OAuth：** 適用於 MCP 主機的標準 OAuth 2.1，例如 Claude 程式碼和 ChatGPT 連線器。
- **自訂：**透過 `getSession` 回調帶來您自己的驗證。

<Diagram id="doc-block-4yfl8j" title={"三種方式，一次會議"} summary={"瀏覽器訪客、MCP 主機和自訂提供者都解析為下游作用域讀取的同一 AuthSession。"}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code、ChatGPT 連線器</small>
    </div>
    <div class="diagram-card">
...
一些標誌對此進行了調整；完整詳細資訊位於 [Environment Variables](#environment-variables) 表中：

- `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` — 使用本機開發中的普通註冊頁面而不是自動開發帳戶。
- `AUTH_DISABLED=true` — 完全跳過登入/註冊，並以一個共用使用者身分執行每個請求（僅限本機開發/預覽/示範，切勿與真實使用者一起進行正式環境）。
...
## 本機開發登入 {#local-development-sign-in}

在使用迴圈位址的開發伺服器上，共用登入頁面會優先顯示 **Continue as local
dev**。它會建立或重複使用真實的 Better Auth 開發帳戶和工作階段，因此使用的仍然是應用程式其他地方相同的驗證流程。

選擇 **Show full sign in options** 可顯示一般註冊、密碼、magic link 和社交提供者選項。如果無法建立本機工作階段，頁面會自動顯示這些選項，讓你可以繼續使用標準驗證。

此便利功能僅在開發或測試環境中，瀏覽器透過 `localhost`、`127.0.0.0/8` 或 `::1` 連線時可用。部署預覽和託管正式環境會停用此功能。這不是驗證繞過。設定 `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` 可選擇退出，讓本機開發顯示一般登入頁面。

## 更好的驗證（預設） {#better-auth}

預設情況下，Better Auth 支援驗證。它提供：

- 使用 magic link 註冊和登入，並以電子郵件/密碼作為備用方式
- 社交提供者（Google、GitHub 和 35 多個其他提供者）
- 具有角色和邀請的組織
- 用於 API 和 A2A 存取的 JWT 權杖
- 對程式化用戶端的不記名權杖支援

Better Auth 路由安裝在 `/_agent-native/auth/ba/*`。此框架還提供向後相容的端點：

- `GET /_agent-native/auth/session` — 取得目前工作階段
- `POST /_agent-native/auth/magic-link` — 要求 magic link 電子郵件
...
| 部署形狀                                 | Cookie 領域                                                                                                      |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 獨立應用                                 | 透過 slug（`APP_NAME`，或本機開發中的包名稱）隔離每個應用程式；正式環境中穩定的 `an` 前綴                        |
...
透過 [Cross-App SSO](/docs/cross-app-sso) 而不是共用 cookie。
...
設定此標誌後，電子郵件/密碼註冊不需要電子郵件
驗證和註冊驗證電子郵件未傳送。僅將其用於品質檢查
或預覽環境，並使用 `+autoz` 地址命名測試帳戶
(`name+autoz@example.com`)，因此很容易識別。

## 電子郵件驗證政策 {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` 只是本機和 QA 的便利設定，託管部署會忽略它。若要
為任何環境（包括正式環境）宣告該政策，請在設定中宣告：

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

部署別名為 `AUTH_REQUIRE_EMAIL_VERIFICATION=0`。宣告的值優先於
`AUTH_SKIP_EMAIL_VERIFICATION` 和各環境的預設值。

`false` 會將未經驗證的位址當作登入憑證接受——任何人都可以宣稱擁有任何電子郵件
位址——因此僅在註冊已透過其他方式受限時使用。在沒有電子郵件供應商的部署上設定
`true` 則會停用密碼註冊，因為驗證信永遠無法送達。

## 社交提供者 {#social-providers}

設定環境變數以啟用社交登入。 Better Auth 自動偵測它們：
...
此框架提供了內建的組織系統。這是框架自己的 `org/` 模組（由 `organizations` 和 `org_members` 表支援），而不是 Better Auth 的組織外掛，該外掛故意未註冊。每個應用程式都支援：

- 建立組織
- 邀請具有角色的成員（`owner`、`admin`、`member`）
- 切換活動組織
- 透過 `org_id` 列確定每個組織的資料範圍
...
## 遠端 MCP OAuth {#remote-mcp-oauth}

每個應用程式的 MCP 端點都可以充當標準受保護的 MCP 資源。支援 OAuth 的用戶端只能設定遠端 MCP URL：

```text
https://mail.agent-native.com/mcp
```

未經驗證的 MCP 請求返回指向 `/.well-known/oauth-protected-resource` 的 `WWW-Authenticate` 質詢。然後，用戶端發現應用程式的 OAuth 中繼資料，動態註冊公開用戶端，開啟應用程式的授權頁面，並與 PKCE 交換授權程式碼以取得存取和重新整理權杖。

<Diagram id="doc-block-ekjq74" title={"遠端 MCP OAuth 握手"} summary={"具有 OAuth 功能的用戶端僅從 MCP URL 引導 — 質詢、發現、動態註冊，然後是 PKCE 程式碼交換。"}>
...
設定時存取權杖使用 `A2A_SECRET` 進行簽名，否則使用 `BETTER_AUTH_SECRET` 進行簽名。它們攜帶簽名的使用者/組織身分和 `mcp:read`、`mcp:write` 和/或 `mcp:apps` 範圍，並且受眾綁定到確切的 MCP 資源 URL。重新整理權杖僅儲存為雜湊值並在每次重新整理時輪換。工具呼叫和 MCP Apps 資源讀取在與登入使用者相同的請求脈絡中執行；嵌入式 MCP 應用程式 iframe 永遠不會接收原始 OAuth 權杖。
...
傳遞自訂 `getSession` 回調以使用任何驗證提供者（Clerk、Auth0、Firebase 等）：
...
路由（`/_agent-native/*`）和自訂 API 路由（`/api/*`）仍然需要驗證
除非應用明確新增這些前綴
...
## 登入並傳回 URL {#sign-in-return-url}

具有**公開頁面**（共用連結、嵌入、行銷頁面）的範本通常需要一個頁面內 CTA 來要求匿名檢視者登入並將他們帶回到他們所在的頁面。此框架為此提供了一個入口點：

```
/sign-in?return=<same-origin-path>
```

當匿名檢視者點選此 URL 時，將提供框架的登入頁面。成功登入（任何流程 - 權杖、電子郵件/密碼或 GoogleOAuth）後，瀏覽器會返回 `return`。
...
一般應用程式頁面會為所有訪客使用同一個非個人化、可公開快取的 SSR 外殼。`AppProviders` 會在用戶端檢查工作階段；造訪 `/dashboard` 等私人路徑時，它會在掛載私人應用程式介面之前將匿名訪客重新導向至 `/sign-in?return=%2Fdashboard`。登入後，訪客會返回 `/dashboard`。公開或 SEO 路由請使用 `isPublicPath`；透過 MCP 內嵌權杖等其他受限機制驗證的介面請使用 `sessionBypass`。

此外殼的快取時長是一個部署範圍的設定——關於 `AGENT_NATIVE_SSR_CACHE`，參見 [SSR Caching](/docs/ssr-caching)。縮短或停用快取只會改變時長；SSR 仍會在剝離 cookie 後渲染匿名分支。

### 幕後花絮：GoogleOAuth

兩個流（顯式 `/sign-in` 入口點和書簽路徑情況）都將傳回 URL 對話串穿過 OAuth 狀態。該狀態是 HMAC 簽名的，因此在運輸過程中無法偽造。在回調中，返回的 URL 在重新導向之前被重新驗證為同來源 - 因此泄漏的簽名金鑰仍然無法轉換為開放重新導向預言機。

如果您的範本直接包裝 `/_agent-native/google/auth-url`（例如郵件和行事曆範本這樣做，以擴大範圍），則接受 `?return=<path>` 查詢並透過 `encodeOAuthState` 的選項物件形式轉發它：
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | 設定為 `0` 或 `1` 可為任何環境（包括正式環境）宣告密碼註冊的驗證政策；優先於 `AUTH_SKIP_EMAIL_VERIFICATION`          |
...
| `GITHUB_CLIENT_SECRET`                  | GitHub OAuth 秘密                                                                                                    |
...
Dispatch 的 `desktop.workspace-sso` 旗標預設關閉，且只控制可用性；驗證、簽章、nonce、來源、回呼、工作階段及撤銷檢查一律強制執行。旗標關閉或無法讀取時，Desktop 不會變更逐一應用程式登入、登出或設定。
User-Agent 中的 Canary 標記僅是可用性提示，不是遠端證明；驗證與授權皆不信任該標記。
每個 Desktop 版本都可以在裝置偏好設定 `desktopSsoEnabled` 為 true（預設值）時初始化代理程式；明確持久化的 `false` 仍代表使用者選擇退出。Dispatch 的 `desktop.workspace-sso` 旗標也必須啟用。Canary User-Agent 標記不再控制代理程式初始化，只用於識別更新頻道。
Canary 不會攔截匿名使用者前往應用程式登入頁面的導覽。使用者必須在 Desktop 設定中，為目前啟用的已註冊應用程式明確選擇工作區登入；一般應用程式登入仍是預設路徑。
一般瀏覽器繼續使用逐一應用程式登入。自我託管應用程式僅在使用剩餘的明確 Desktop 或伺服器端聯邦路徑時需要 `AGENT_NATIVE_IDENTITY_HUB_URL`。

## 管理式部署與邀請

管理式部署請設定 `AUTH_SIGNUP=invited`、`ORG_CREATION=closed` 和 `AUTO_CREATE_DEFAULT_ORG=0`。[管理式部署指南](/docs/administered-deployments)說明引導管理員、邀請和允許的電子郵件網域。成員邀請與依網域加入是不同規則。
