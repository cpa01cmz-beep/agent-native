...
**自動化**是代理依排程或在事件發生時執行的工作：_當 X 發生時，執行 Y_。**Scheduled** 和 **Event** 是兩種觸發類型，而不是兩個獨立產品。

框架最初將排程自動化公開為 **Recurring Jobs**。該名稱、`jobs/<name>.md` 路徑和 `manage-jobs` 工具會為相容性保留。新的產品指南使用 **Automations** 作為總稱；維護舊排程工作或使用組織範圍及進階欄位時，請參閱 [Recurring Jobs](/docs/recurring-jobs)。

<Diagram id="doc-block-1d1aaq0" title={"當X發生時，做Y"} summary={"總線上觸發事件，可選的自然語言條件對其進行門控，代理執行具有完整工具存取權限的自動化主體。"}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">代理執行 body</span
    ><small class="diagram-muted"
...
> “當有人使用 @builder.io 電子郵件預訂會議時，請透過 Slack 向我傳送訊息。”

代理發現可用事件，確認計畫，並為您編寫自動化。

### 從 Automations 介面

在完整的 Agent 頁面中開啟 **Automations**，檢視和管理排程自動化及事件觸發自動化。相容 URL 仍為 `/agent#jobs`，因此現有連結和書籤會繼續運作。

<Callout tone="warning">

`manage-automations` 支援個人和組織範圍的 Scheduled 與 Event 自動化。組織 Event 自動化對成員可見，但始終以建立者身分執行；管理員可以管理定義，但不能變更該執行身分。

</Callout>

第三條路徑 - 透過 `resourcePut` 手動寫入 `jobs/<name>.md` 檔案 - 與 [recurring jobs](/docs/recurring-jobs#creating) 的工作方式完全相同。對於事件驅動的自動化，您可以將下面的事件觸發 frontmatter 新增到同一檔案中。事件觸發作業設定 `schedule: ""` 並提供 `triggerType: event`、`event` 名稱和可選的 `condition`：

<AnnotatedCode
  id="doc-block-12vdn4v"
  title={"事件觸發的自動化"}
  ...
  annotations={[
    {
      lines: "2",
      label: "沒有計畫工作",
      note: '事件觸發器會將 `schedule` 設為 `""`；cron 欄位保持空白。',
    },
    {
      lines: "4-5",
      label: "觸發器",
      note: "`triggerType: event` 加上 `event` 名稱將此自動化訂閱到總線。",
    },
    {
      lines: "6",
      label: "門",
      note: "可選的自然語言 `condition`，由 Haiku 在調度之前根據有效負載進行評估。",
    },
    {
      lines: "12",
      label: "伺服器端秘密",
...
| `mode`        | `"agentic"`               | `"agentic"`  | 完整的代理迴圈（唯一受支援的模式）。`"deterministic"` 已被移除——它從未被實作過，設定該模式的自動化從未被觸發。 |
| `domain`      | 字串                      | _（可選）_   | 分組標籤（郵件、行事曆、剪輯等）                                                                               |

對於事件觸發器，`schedule`為`""`（空）；對於計畫觸發器，它帶有 cron 表達式。調度程式還寫入與調度程式相同的託管 `lastRun` / `lastStatus` / `lastError` 欄位，以及當條件評估為 false 時的 `"skipped"` 狀態。

## 事件總線 {#event-bus}

整合在模組載入時註冊事件。總線根據 [Standard Schema](https://standardschema.dev) 定義驗證有效負載並將其分派給訂閱者。

### 內建事件 {#built-in-events}
...
從代理中使用 `action=list-events` 呼叫 `manage-automations`，以檢視所有已註冊事件以及目前範本的描述和負載模式。

### 發出自訂事件 {#emitting-events}

在伺服器外掛中註冊事件型別，然後從 actions 或 webhook 處理常式發出它：
...
發出中繼資料中的 `owner` 決定哪些自動化可以觸發。個人和組織 Event 自動化都必須與 `createdBy` 完全相符。
...
- **空或缺失條件** = 無條件（始終觸發）。
- 結果透過 5 分鐘 TTL 和 500 個條目的 LRU 快取進行記憶（條件 + 負載的 SHA-256）。
- 有效負載在傳送到 Haiku 之前被截斷為 4000 個字符。
...
自動化使用 `web-request` 工具進行出站 HTTP。它支援 URL、標頭和內文中的 `${keys.NAME}` 預留位置：
...
在代理發出工具呼叫後，預留位置在**伺服器端**解析 - 原始秘密值永遠不會進入代理的脈絡。

### 參數 {#web-request-params}

| 參數         | 型別 | 預設  | 描述                                               |
| ------------ | ---- | ----- | -------------------------------------------------- |
| `url`        | 字串 | —     | 完整的 URL。可能包含 `${keys.NAME}` 引用。         |
...
金鑰是由使用者或代理建立的用於自動化使用的臨時秘密（例如 `SLACK_WEBHOOK`、`HUBSPOT_API_KEY`）。它們與註冊機密 (`registerRequiredSecret`) 的不同之處在於它們沒有範本定義的中繼資料或入門步驟。

- 透過設定 UI 或 `/_agent-native/secrets/adhoc` API 建立。
- 每個金鑰都可以有一個 **URL 允許清單**，用於限制金鑰可以傳送到哪些來源（來源層級匹配）。
- 原始值永遠不會暴露給 AI - 只有 `${keys.NAME}` 預留位置出現在代理的脈絡中。
- 分辨率從使用者範圍回退到工作區範圍，因此使用者可以覆蓋共用金鑰。

## 代理工具 {#agent-tools}

個人和組織自動化都使用帶 `action` 參數的 `manage-automations`：
...
`manage-automations` 支援組織範圍、模型覆寫和嚴格執行的 MCP 允許清單。`manage-jobs` 仍是舊 schedule-only 整合和傳遞中繼資料的相容工具。
...
<Endpoint id="doc-block-1wcqkmk" title={"觸發測試事件"} method="POST" path="/_agent-native/automations/fire-test" summary={"發出 test.event.fired 事件以驗證事件觸發的自動化"} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1ycvngf" title={"調度路徑"} summary={"從觸發事件到完成代理執行，由所有權範圍和自然語言條件控制。"}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> body 作為提示、payload 作為脈絡，5
      分鐘超時</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
1. 使用 `action=list-events` 呼叫 `manage-automations` — 找到 `calendar.booking.created`。
2. 與使用者確認計畫。
3. 使用 `action=define` 呼叫 `manage-automations`：
...
4. 自動化儲存為 `jobs/slack-on-builder-booking.md` 並立即開始監聽。

## 更多範例 {#more-examples}

### 當計畫被評論時透過 webhook 通知
...
脈絡。見[Visual Plans — Events and notifications](/docs/template-plan#events)
完整的 `plan.commented` 有效負載參考和所有四個計畫事件。

## 下一步是什麼

- [**Recurring Jobs**](/docs/recurring-jobs) - 計畫觸發的自動化重用相同的計畫程式
- [**Actions**](/docs/actions-overview) - 自動化可以透過代理迴圈呼叫任何註冊的操作
