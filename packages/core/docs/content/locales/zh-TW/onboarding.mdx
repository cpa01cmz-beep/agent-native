...
當您第一次開啟基於 Agent-Native 框架建置的應用程式時，您會看到
代理側欄中的**設定**清單。它使首次執行設定保持關閉
到代理聊天：連線人工智慧引擎，可選取將應用程式指向共用
基礎設施，僅在需要時新增提供者。

<Diagram id="doc-block-1w9qmej" title={"設定清單"} summary={"只需要連線一個 AI 引擎。該面板會跟蹤完成情況，並在完成所需的所有操作後自動隱藏。"}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span><strong>連線 AI 引擎</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>郵件投遞</strong
...
- **連線服務（一鍵點選）** — 例如*連線 Builder* 進行託管
  人工智慧閘道。點選按鈕，開啟一個視窗，您登入，視窗關閉，
...
- **開啟連結** — 某些步驟指向登入頁面或檔案。點選
  **繼續**並在新分頁中完成流程。
- **詢問代理** — 只需幾個步驟即可提供“讓代理進行設定”選項。
  點選它，代理會在聊天中接聽，引導您完成任何操作
  外部設定（建立 OAuth 憑證等）。

### 您通常會看到的內建步驟

- **連線人工智慧引擎**（必需）——唯一的強制步驟。連線
  Builder 用於一鍵託管閘道，或開啟輔助提供者金鑰
  選取並貼上您自己的 LLM 金鑰。
- **資料庫**（可選）- 當您想使用特定時設定 `DATABASE_URL`
  PostgreSQL 連線字串。
- **驗證**（可選）- 內建電子郵件/密碼帳戶的工作方式
  預設。僅當您需要這些路徑時才新增 OAuth 或存取權杖登入。
- **電子郵件傳送**（可選）- 在部署之前用於密碼重設很有用，
...
如果您點選**隱藏設定**，該瀏覽器工作階段的面板就會消失。
尚未完成的所需步驟將在下次載入時再次出現。一次
所需的一切都已完成，面板會自動隱藏 - 什麼也沒有
剩下要做的事。
...
使用者的側邊欄清單。框架處理渲染、完成
跟蹤和解雇 - 您只需聲明步驟是什麼以及如何進行
滿意。

系統是**自動安裝**的。範本不需要連線任何東西即可取得
四個內建步驟（LLM、資料庫、驗證、電子郵件）。新增特定於應用程式的
steps (Gmail, Slack, Notion, etc.), call `registerOnboardingStep()` from a
伺服器外掛。

### 自動安裝路由

所有路由均位於 `/_agent-native/onboarding/` 下：
...
| `GET /_agent-native/onboarding/dismissed`           | 讀取解雇+ allComplete標誌 |

<Endpoint id="doc-block-bxnbzn" title={"列出入職步驟"} method="GET" path="/_agent-native/onboarding/steps" summary={"列出所有已註冊的步驟及其完成狀態"} responses={[
  {
    "status": "200",
  ...
  annotations={[
    {
      lines: "5",
      label: "自動安裝",
      note: "從 Nitro 外掛註冊 - 此框架處理渲染、完成跟蹤和解除。",
    },
    {
      lines: "7",
      label: "穩定ID",
      note: "預設載入後使用相同的 `id` 重新註冊會覆蓋內建步驟。",
    },
    {
      lines: "12-19",
      label: "主要方法",
    ...
    },
    {
      lines: "20-26",
      label: "委托路徑",
      note: '`kind: "agent-task"` 會把設定工作連同提示交給代理聊天。',
    },
    {
      lines: "28-31",
      label: "竣工檢查",
      note: "`isComplete` 在伺服器端執行。 OAuth 權杖位於 `oauth_tokens` 儲存中 - 檢查它，而不是 `process.env.GMAIL_REFRESH_TOKEN`。",
    },
  ]}
/>
...
建置與外部服務（例如 Slack、Google Workspace、GitHub 或 HubSpot）互動的範本時，您應該檢查工作區是否已連線並授予該提供者與您的應用的連線。當存在中央託管連線時，這可以防止使用者在本機環境變數中複製憑證（例如 API 金鑰或重新整理權杖）。
...
請參閱 [Workspace Connections](/docs/workspace-connections) 檔案，了解連線提供者目錄方法的完整清單。

### 方法種類

| 種類               | 有效負載                                              | 用於                                   |
| ------------------ | ----------------------------------------------------- | -------------------------------------- |
| `link`             | `{ url, external? }`                                  | 將使用者傳送到 OAuth 流程或檔案頁面    |
...
| `agent-task`       | `{ prompt }`                                          | 向代理聊天傳送提示進行處理             |

`primary: true` 標誌將方法標記為其步驟的大 CTA。
...
| `database` | 沒有 | 託管 PostgreSQL URL，或留空以使用本機 PGlite |
| `auth`     | 沒有 | 內建帳戶，可選 OAuth 或存取權杖              |
| `email`    | 不   | 重新傳送或 SendGrid 用於交易電子郵件         |

任何這些都可以透過在之後使用相同的 `id` 重新註冊來覆蓋
預設載入。
...
有關步驟值儲存位置以及如何處理機密的背景資訊，
參見 [Security](/docs/security)。對於最終使用者訊息傳遞接觸點（邀請，
密碼重設）取決於**電子郵件傳送**步驟，請參閱
