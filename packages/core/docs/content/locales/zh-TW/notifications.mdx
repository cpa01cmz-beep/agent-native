...
一個功能，多個目的地。從任何伺服器端程式碼（操作、自動化、外掛）呼叫 `notify()`，事件就會出現在使用者的應用內收件箱中，並分發到每個註冊的頻道。附帶一個響鈴下拉 UI 元件，主機範本將其放入其標頭中。

通知是進入應用程式響鈴收件箱的單向警報（加上 Webhook 扇出）。要透過 Slack/電子郵件/Telegram/WhatsApp 與您的代理*交談*，請參閱 [Messaging](/docs/messaging)。
...
<Diagram id="doc-block-z9sw7h" title={"一個電話，多個目的地"} summary={"notify() 始終寫入所有者範圍的收件箱行，並行扇出到每個註冊通道（盡最大努力），然後在事件總線上發出 notification.sent。"}>
...
嚴重性驅動下拉清單中的徽章樣式，並傳遞到管道，以便它們可以根據緊急情況進行分支。

## 內建頻道 {#channels}

| 頻道      | 交付                                 | 需要                                               |
| --------- | ------------------------------------ | -------------------------------------------------- |
| `inbox`   | 持久化到`notifications`表；驅動鈴 UI | 始終開啟 - 原語的一部分。                          |
| `webhook` | POST JSON 到已設定的 URL             | `NOTIFICATIONS_WEBHOOK_URL` 環境變數在啟動時設定。 |

電子郵件、Slack 和 webhook 一律與收件匣一起註冊。傳送時會優先使用每則通知中繼資料的 URL 或收件者，然後退回使用相符的環境變數。如果兩者皆未設定，該頻道在發送時不會執行任何動作。
Webhook 通道根據所有者的臨時 [secrets](/docs/security) 解析 URL 和 `NOTIFICATIONS_WEBHOOK_AUTH` 中的 `${keys.NAME}` 引用，因此原始值永遠不會進入代理的脈絡。強制執行每鍵 URL 允許清單 - 自動化 `web-request` 工具使用相同的規則。

<Diagram id="doc-block-ff63b6" title={"管道和嚴重性"} summary={"收件箱始終開啟； webhook 需要一個環境變數；自訂頻道在啟動時註冊。嚴重性驅動徽章樣式並傳遞到每個管道。"}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>嚴重程度決定徽章</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">確認和 FYI</span>
    </div>
    <div class="sev-row">
...
const rows = await listNotifications("user@example.com", {
  unreadOnly: true,
  limit: 50,
});
const unread = await countUnread("user@example.com");
await markNotificationRead(rows[0].id, "user@example.com");
await markAllNotificationsRead("user@example.com");
await deleteNotification(rows[0].id, "user@example.com");
```

每個函式都是所有者範圍的 - 沒有跨使用者讀取，沒有跨使用者寫入。
...
透過 core-routes 外掛安裝在 `/_agent-native/notifications/*`。所有路由的範圍僅限於經過驗證的工作階段的電子郵件。
...
<Endpoint id="doc-block-te34xy" title={"列出通知"} method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary={"列出目前使用者的最近通知"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "ISO 時間戳記游標 — 傳回在此時間之前建立的通知，用於分頁。"
  }
]} responses={[
  {
    "status": "200",
...
帶有未讀徽章的響鈴圖示。點選可開啟最近通知的下拉清單。使用 shadcn 語義標記，適應主機範本的淺色/深色主題。

傳遞 `browserNotifications` 來為每個新的未讀專案觸發系統 `new Notification(...)` 快顯視窗 - 當使用者的分頁位於背景時很有用。下拉選單會呈現“啟用”提示，直到使用者授予權限；透過通知 `tag` 欄位防止每個 ID 重複。
...
| 行動   | 參數                                                                    | 目的                               |
| ------ | ----------------------------------------------------------------------- | ---------------------------------- |
| `send` | `severity`（必填）、`title`（必填）、`body`、`metadataJson`、`channels` | 向使用者的收件箱和註冊頻道傳送通知 |
| `list` | `unreadOnly`、`limit`（最多 200 個，預設 20 個）                        | 列出脈絡的最近通知                 |

自動化（請參閱 [Automations](/docs/automations)）可以透過其主體中的 `action=send` 呼叫 `manage-notifications` - 這是將外部事件轉變為使用者可見警報的規範模式。
...
- **輪詢整合** - 每個突變都會呼叫 `recordChange()`，因此使用 [`useDbSync`](/docs/client-data#usedbsync) 的範本會自動失效，無需任何額外的接線。
- **盡力扇出** — 捕獲並紀錄通道錯誤；一個失敗的通道不會阻止其他通道或收件箱寫入。
- **即發即忘** — `notify()` 在收件箱寫入完成後返回；自訂通道在背景執行。

## 下一步是什麼

- [**Automations**](/docs/automations) — `notify()` 最常見的呼叫者
- [**Security**](/docs/security) — 為 Webhook 通道提供支援的 `${keys.NAME}` 替代
- [**Server plugins**](/docs/server-overview) — 啟動時註冊自訂通道
