...
<Diagram id="doc-block-9437sa" title={"Lebenszyklus ausführen"} summary={"startRun öffnet eine laufende Zeile; updateRunProgress patcht es; completeRun verschiebt es in einen Terminalstatus und stempelt completed_at."}>
...
<Endpoint id="doc-block-1rf86fv" title={"Aktive Läufe auflisten"} method="GET" path="/_agent-native/runs" summary={"List the caller's runs. The RunsTray polls this with active=true."} auth="Session cookie (owner-scoped)" params={[
  {
    "name": "active",
    "in": "query",
    "type": "boolean",
...
- [**Daten & Sync**](/docs/client-data) – `useDbSync` für Echtzeit-Cache-Ungültigmachung
