...
> **Verwenden Sie die Dispatch-Vorlage?** All dies ist für Sie unter **Verwaltung → Nachrichten** eingerichtet. Klicken Sie, um jede Plattform zu verbinden – Sie müssen den Rest dieser Seite nicht lesen, es sei denn, Sie passen Ihre eigene Vorlage an oder erstellen sie. Siehe [Dispatch](/docs/dispatch) oder [Dispatch template reference](/docs/template-dispatch).
...
- **Aus einer Nachricht heraus andere Tools bedienen.** Eine @mention kann in einer Zeile in Notion, einem Vorgang in Jira oder einem Datensatz in HubSpot enden — siehe [@mention in Slack → Zeile in Notion](#slack-to-notion) für das vollständige Beispiel.
...
- **Kanalerwähnungen** – Starten Sie Arbeit mit einer @Erwähnung. Jede Kanalnachricht, die der Agent verarbeiten soll, muss ihn ausdrücklich erwähnen; gewöhnliche Thread-Antworten und allgemeiner Kanalverkehr rufen ihn nicht auf.
...
**Verwaltung → Messaging** die Option **Agent manifest**, laden Sie die
...
und `SLACK_SIGNING_SECRET`; klicken Sie dann unter **Verwaltung → Messaging**
...
Jede Kanalnachricht muss den Agenten ausdrücklich erwähnen; gewöhnliche
Thread-Antworten und sonstiger Kanalverkehr werden ignoriert. Der Agent lädt begrenzten Thread-Kontext samt
...
### Durchgespieltes Beispiel: @mention in Slack → Zeile in Notion {#slack-to-notion}

Sobald Slack-Erwähnungen funktionieren, lautet der häufigste nächste Wunsch sinngemäß: _„Wenn ich den Agenten in einem Thread mit @mention anspreche, soll er das Gesagte als Zeile in meine Notion-Datenbank schreiben.“_ Das funktioniert bereits — Slack-Eingang, die Notion-Provider-API und App-Aktionen sind alle eingebaut —, aber drei Teile müssen in der richtigen Reihenfolge verbunden werden, und zwei davon scheitern stillschweigend, wenn man sie überspringt.

Alles in `<ANGLE_BRACKETS>` ist ein Platzhalter. Fügen Sie niemals ein echtes Notion-Secret in Quellcode, in eine eingecheckte `.env` oder in eine Chat-Nachricht ein.

#### 1. Erwähnungen bis zur App durchreichen

Schließen Sie [Slack einrichten](#slack) ab und prüfen Sie, dass eine `@mention` in einem Kanal eine Antwort erhält, bevor Sie Notion anfassen. Zwei Verhaltensweisen sind für den Rest wichtig:

- Die Nachricht mit der `@mention` ist der Auslöser, und ihr Text ist das, was der Agent sieht.
- Jede spätere Nachricht im selben Thread muss den Agenten ausdrücklich erwähnen; eine gewöhnliche Antwort erreicht ihn nicht. Erwähnen Sie den Agenten für jede neue Anfrage erneut.

#### 2. Die Provider-API-Aktionen freischalten

Core liefert eine Notion-Provider-Voreinstellung mit (Basis-URL, `Notion-Version`-Header, Authentifizierung, Doku-Links), aber die drei Aktionen, mit denen der Agent sie nutzt — `provider-api-catalog`, `provider-api-docs` und `provider-api-request` — sind ein **optionaler Subpath-Export**. Eine App, die sie nie hinzugefügt hat, hat überhaupt keinen Weg zu Notion, und der Agent meldet, dass er kein Notion-Werkzeug hat. Diesen Schritt übersehen die meisten.

Fügen Sie eine Provider-Runtime hinzu:
...
Fügen Sie danach pro Aktions-ID eine schlanke Aktionsdatei hinzu. Alle Factory-Optionen sind optional, jede Datei kann also ein einziger Aufruf sein:
...
Dateien in `actions/` werden automatisch erkannt — es gibt keine Registry zu pflegen. Siehe [Die Core-Aktions-Factories wiederverwenden](/docs/integrations#provider-api-factories), um die Providerliste einzuschränken oder die Aktionen über HTTP anzubieten.

#### 3. Den Notion-Schlüssel in den Tresor legen, nicht in `.env`

Erstellen Sie unter **[notion.so/my-integrations](https://www.notion.so/my-integrations)** eine interne Integration und kopieren Sie deren Secret. Öffnen Sie dann in Ihrer App **Settings → API Keys & Connections**, legen Sie einen Schlüssel mit exakt dem Namen `NOTION_API_KEY` an, fügen Sie den Wert ein und wählen Sie einen Geltungsbereich:

- **Personal** — nur Läufe, die unter Ihrem Konto ausgeführt werden, können ihn verwenden.
- **Workspace** — mit der Organisation geteilt. Wählen Sie das, wenn außer Ihnen noch jemand den Agenten erwähnen wird, denn ein Slack-Lauf läuft als Eigentümer der Integration, nicht als die Person, die die Nachricht geschrieben hat.

Die Provider-Runtime löst Anmeldedaten in genau dieser Reihenfolge aus dem verschlüsselten Tresor auf — persönlich, dann Organisation, dann Workspace — und **liest bewusst niemals `process.env`**, weil eine Deployment-Umgebungsvariable von allen Nutzerinnen und Nutzern einer mandantenfähigen App geteilt würde. Eine `NOTION_API_KEY`-Zeile in `.env` wird hier nicht berücksichtigt.

#### 4. Die Datenbank für die Integration freigeben

Öffnen Sie in Notion die Zieldatenbank und fügen Sie über **••• → Connections →** Ihre Integration hinzu. Diesen Schritt zu überspringen ist der häufigste und irreführendste Notion-Fehler: Die API antwortet mit **404**, nicht 403, es sieht also nach einer falschen Datenbank-ID aus. Der Katalogeintrag enthält diesen Korrekturhinweis, und `provider-api-request` hängt ihn an das Feld `guidance` jeder 401-, 403- oder 404-Antwort an.

Die Datenbank-ID steht in der Datenbank-URL — es ist die 32 Zeichen lange Zeichenkette vor dem `?`.

#### 5. Bitten Sie Ihren Agenten, es zu bauen

Das ist der Teil, den kein Einrichtungsbildschirm verrät: Sie verdrahten die beiden Hälften nicht selbst, Sie bitten um das Verhalten. Im Agenten-Chat der App:

> Füge eine Aktion `add-notion-row` hinzu, die über die Provider-API eine Zeile an die Notion-Datenbank `<NOTION_DATABASE_ID>` anhängt. Sie soll den Nachrichtentext, den Slack-Autor und einen Link zurück zur Slack-Nachricht entgegennehmen. Aktualisiere danach `AGENTS.md`, sodass du diese Aktion aufrufst und mit der URL der neuen Notion-Seite antwortest, wenn dich jemand in Slack mit @mention bittet, etwas festzuhalten.

Was der Agent schreiben soll, ist **eine Aktion** — keine Nitro-Route und kein `fetch` aus dem Browser:
...
Drei Details in diesem Beispiel sind entscheidend:

- Aktuelle Notion-Versionen hängen Zeilen an eine **Datenquelle** innerhalb der Datenbank, lesen Sie also zuerst die Datenbank und verwenden Sie deren `data_sources[0].id` als Parent. Die Datenbank-ID direkt als Parent zu übergeben ist die Form der älteren API.
- `executeRequest` gibt Provider-Fehler zurück, statt sie zu werfen. Prüfen Sie also selbst `response.ok` und reichen Sie das zurückgegebene `guidance` weiter — dort kommt der Hinweis „Datenbank freigeben“ an.
- Die Eigenschaftsnamen (`Name`, `Author`, `Source`) müssen exakt zu den Spalten Ihrer Datenbank passen, die Typen ebenso. Lassen Sie den Agenten zuerst die `properties` der Datenbank lesen, statt zu raten.

Der `AGENTS.md`-Teil ist nicht optional. Ohne ihn hat der Agent zwar das Werkzeug, aber keinen Anlass, es zu benutzen, wenn eine Slack-Nachricht „halt das fest“ sagt.

#### 6. Prüfen — und was zu kontrollieren ist, wenn es nicht klappt

Posten Sie in einem Slack-Kanal, in dem der Agent ist: `@YourAgent halte das in Notion fest: <SOME_TEXT>`. Ein erfolgreicher Lauf antwortet im Thread mit der URL der neuen Seite, und die Zeile steht in Notion.

Wenn nicht:

| Was Sie sehen                                                              | Wo Sie nachsehen                                                                                                                                                                         |
| -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Slack antwortet überhaupt nicht                                            | Die Erwähnung hat die App nie erreicht. Prüfen Sie Request URL und Signing Secret unter [Slack einrichten](#slack) — das ist kein Notion-Problem.                                        |
| Der Agent antwortet, sagt aber, er habe kein Notion-Werkzeug               | Schritt 2. Die Provider-API-Aktionen wurden nie zu `actions/` hinzugefügt.                                                                                                               |
| `notion credential not configured. Tried: NOTION_API_KEY`                  | Schritt 3. Der Schlüssel liegt in `.env` oder wurde als **Personal** für ein anderes Konto gespeichert als das, unter dem der Lauf ausgeführt wird. Speichern Sie ihn als **Workspace**. |
| Notion antwortet mit `404`                                                 | Schritt 4. Die Datenbank wurde nie für die Integration freigegeben.                                                                                                                      |
| Notion antwortet mit `400` wegen der Eigenschaften                         | Die Eigenschaftsnamen oder -typen in der Aktion passen nicht zu den Spalten der Datenbank.                                                                                               |
| Es hat einmal funktioniert, eine spätere Antwort im Thread wurde ignoriert | Das ist erwartet. Jede Nachricht, die der Agent verarbeiten soll, muss ihn ausdrücklich erwähnen — erwähnen Sie ihn erneut.                                                              |
...
- **Klicks, nicht config.** Die Dispatch-Seite **Verwaltung → Nachrichten** verfügt über Verbindungsschaltflächen für jede Plattform mit integrierten env-var-Feldern.
