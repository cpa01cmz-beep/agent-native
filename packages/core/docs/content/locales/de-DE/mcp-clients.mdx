...
<div class="mcp-merge">
  <div class="diagram-col sources">
    ...
    <div class="diagram-box" data-rough>
      Remote über Einstellungs-UI<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-d2d2xn"
  title="mcp.config.json, kommentiert"
  ...
  annotations={[
    {
      lines: "3",
      label: "Server-ID",
      note: "Der Schlüssel wird zum Tool-Präfix: Die Tools dieses Servers werden in der Registrierung des Agenten als `mcp__claude-in-chrome__*` angezeigt, sodass sie nicht mit den Aktionen Ihrer Vorlage kollidieren können.",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` erzeugen eine lokale Binärdatei. Stdio-Server sind für die **lokale Entwicklung** gedacht – sie sind in Edge-Laufzeiten ein No-Op.",
    },
    {
      lines: "6",
      label: "Prozessumgebung",
...
Die Preset-IDs sind `context7`, `sentry`, `notion`, `semgrep`, `linear`,
`atlassian`, `supabase`, `neon`, `stripe`, `cloudflare`, `gitlab`, `figma`,
`vercel`, `github`, `slack`, `asana`, `hubspot`, `intercom`, `monday`, `webflow`, `paypal`, `box`, `netlify` und `zapier`.

Das Verzeichnis unterscheidet Endpunkte mit dem generischen OAuth-Ablauf von
Anbietern, die einen vom Anbieter genehmigten Client oder eine vorab registrierte
OAuth-Anwendung benötigen. Solche Einträge bleiben zur Entdeckung sichtbar und
verlinken auf die offiziellen Einrichtungsanweisungen, zeigen aber keinen
irreführenden Ein-Klick-Verbindungsbutton. Sandboxed Erweiterungen verwenden
`agentNative.mcp.listTools`, `agentNative.mcp.callTool` sowie
`agentNative.providerApi.catalog(...)` und `agentNative.providerApi.docs(...)`
über die Host-Aktionen, um verbundene Daten zu ermitteln; Verbindungs-URLs,
Zugriffs- und Refresh-Tokens werden ihnen niemals übergeben. Der rohe
`provider-api-request`-Aufruf bleibt bewusst dem Host und dem Agenten
vorbehalten: Stünde er beliebigen Methoden-/Pfad-Anfragen gemeinsam genutzter
Erweiterungen offen, könnte eine Erweiterung zu einem authentifizierten
Schreib-Proxy werden. Erweiterungen sollten stattdessen `agentNative.mcp.callTool(...)`
oder eine dafür vorgesehene App-Aktion für begrenzte Anbieteroperationen verwenden.

Lokale Datei-Erweiterungen müssen `list-mcp-tools`, `call-mcp-tool` und jede
unterstützte Anbieteraktion, die sie verwenden, ausdrücklich in `extension.json`
freigeben. Die Discovery-Helfer sind auch unter `agentNative.connectors.mcp`
und `agentNative.connectors.providerApi` verfügbar.

### Atlassian / Jira

Das Atlassian-Preset verwendet den OAuth-2.1-Endpunkt von Atlassian Rovo MCP,
`https://mcp.atlassian.com/v1/mcp/authv2`, und stellt Tools für Jira und
Confluence bereit. Zum Erstellen von Jira-Tickets sind Atlassian-Berechtigungen
erforderlich, die Schreibzugriffe in Jira abdecken. Benutzer sehen weiterhin
nur die Projekte und Inhalte, die ihr Atlassian-Konto zulässt.

Administratoren einer Atlassian-Organisation können steuern, ob ein OAuth-Client
eine Verbindung herstellen darf. Vor dem Verbinden muss ein Administrator
möglicherweise den tatsächlichen HTTPS-Ursprung der App unter **Atlassian
Administration → Rovo → Rovo MCP server → Domains** zur Liste der
vertrauenswürdigen Domains hinzufügen und unter **Permissions** die erforderlichen
Berechtigungen **Read**, **Write** und **Search** aktivieren. Die Domain muss der
Host sein, auf dem die agent-native-App ausgeführt wird. Übernehmen Sie
`https://builder.io/**` nicht für eine andere Bereitstellung, sofern dies nicht
deren tatsächlicher Ursprung ist. Weitere Informationen finden Sie im
[Rovo-MCP-Einrichtungsleitfaden von Atlassian](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/)
und in den [Administratorsteuerelementen](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/).

Der Katalog stellt den Endpunkt und Einrichtungshinweise bereit. Das allgemeine
Einstellungsformular speichert den Endpunkt und optionale verschlüsselte Header;
es ersetzt nicht den interaktiven OAuth-Zustimmungsablauf von Atlassian. Verwenden
Sie einen OAuth-fähigen MCP-Client oder eine entsprechende Laufzeit, wenn
Atlassian interaktives OAuth erfordert. Wenn der Anbieter oder die Laufzeit ein
statisches Bearer-Token unterstützt, folgen Sie dem dafür vorgesehenen Verfahren
zur Verwaltung von Secrets und fügen Sie Tokens niemals in Prompts oder
committete Konfigurationen ein.
...
HTTPS URLs werden überall akzeptiert; einfaches `http://` ist während der Entwicklung nur für `localhost` zulässig. Die optionale Authentifizierung erfolgt als Bearer-Token, das bei jeder Anfrage über `Authorization: Bearer …` gesendet wird. Anbieter, die ausschließlich OAuth unterstützen, darunter Atlassian Rovo MCP, erfordern anstelle eines statischen Headers den OAuth-Ablauf des Anbieters.
...
<Diagram id="doc-block-ahesy5" title="Hub-Modell: Eine App bedient MCP-Server im Organisationsbereich" summary={"Dispatch enthält die MCP-Server im Organisationsbereich; Verbraucher-Apps rufen sie ab und führen sie als mcp__hub_<orgId>_<name>__* zusammen. Es werden nur Zeilen für den Organisationsbereich freigegeben – persönliche Anmeldeinformationen bleiben erhalten."}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">MCP-Server mit Organisationsscope</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">Pull + Merge alle ~60 s</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-180d3cd" method="GET" path="/_agent-native/mcp/status" summary={"MCP Kundenstatus für Tooling und Onboarding"} responses={[
  {
    "status": "200",
...
- Ein falsch konfigurierter `command` → der Server wird übersprungen, sein Fehler erscheint in `/_agent-native/mcp/status` unter `errors.<server-id>` und alle anderen Server funktionieren weiterhin.
