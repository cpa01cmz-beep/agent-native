...
<Diagram id="doc-block-1761arz" title="Verteidigung in Schichten" summary={"Das Framework besitzt den größten Teil der Bedrohungsoberfläche. Sie besitzen zwei Dinge – das Markieren von Tabellen zur Festlegung des Gültigkeitsbereichs und die Validierung externer Eingaben."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">Framework besitzt</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">Du behältst die Kontrolle</span
    ><small class="diagram-muted"
...
Das Framework erstellt den Anfragekontext aus der Sitzung und erzwingt Eigentum mit `accessFilter()` und `assertAccess()` bei Anwendungslese- und -schreibvorgängen. Agent-SQL erhält temporäre, nach Identität und Organisation gefilterte Ansichten. RLS ist noch nicht aktiviert, daher müssen rohe Anwendungsabfragen diese Helfer weiterhin verwenden.
...
<Callout id="doc-block-umc4wb" tone="risk">

Erstellen Sie SQL niemals durch Zeichenfolgenverkettung oder Vorlagenliterale. Übergeben Sie die Benutzereingabe als `args` an `exec` / `db-query` oder verwenden Sie Drizzle – beide parametrisieren immer. Die `pnpm guards`-Prüfungen erfassen Abfragen ohne Bereichseinschränkung und verkettete Abfragen zum CI-Zeitpunkt.
...
<Diagram id="doc-block-1g8dkya" title="Die Scoping-Pipeline" summary={"Agenten-SQL berührt Basistabellen nie direkt – er liest eine temporäre Ansicht, die auf die aktuelle Identität beschränkt ist, sodass ein bloßer Tabellenname nur eigene Zeilen zurückgeben kann."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    Angemeldete Sitzung<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    Agenten-SQL<br /><small class="diagram-muted">bare table names only</small>
  </div>
</div>
```
...
Die Tools `db-query`/`db-exec` lehnen schemaqualifizierte Tabellenverweise (`public.<table>`) ab – ein qualifizierter Name wird in die Basistabelle aufgelöst und würde die obige temporäre Ansicht umgehen. Agenten verwenden bloße Tabellennamen; Der Geltungsbereich wird automatisch angewendet.
...
<DataModel
  ...
  summary={
    "Die drei Spalten, die eine Tabelle mandantenfähig und gemeinsam nutzbar machen."
  }
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Aktive Organisation des Eigentümers bei der Erstellung. Fördert Sichtbarkeitsprüfungen der Organisation.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "privat | org | public – grobe Standardeinstellung, standardmäßig privat.",
        },
      ],
    },
  ]}
...
Benutzer-/organisationsbezogene Anmeldeinformationen und der Vault werden im Ruhezustand mit AES-256-GCM verschlüsselt. Der gemeinsame Workspace-Vault bevorzugt `WORKSPACE_SECRETS_ENCRYPTION_KEY`, danach Legacy-Kandidaten aus `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET` und `BETTER_AUTH_SECRET`; während einer Rotation hält `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` alte Zeilen lesbar, bis Zugriffe sie migriert haben. Um vorhandene Klartextzeilen zu verschlüsseln, führen Sie `pnpm action db-migrate-encrypt-credentials` (idempotent, nicht destruktiv) aus.
...
Eingehende Webhook-Handler (Resend, SendGrid, Slack, Telegram, WhatsApp, Recall.ai, Deepgram, Zoom) lehnen gefälschte Anfragen standardmäßig in der Produktion ab: Wenn die entsprechende signierende geheime Umgebungsvariable fehlt, gibt der Handler 401 zurück, anstatt sie zu akzeptieren und zu versenden.
...
Google-Drive-`changes.watch`-Benachrichtigungen durchlaufen dieses allgemeine Adapter-Gate nicht. Sie sind inhaltsleere, undurchsichtige Anfragen. Daher registriert das Framework ein zufälliges Token pro Kanal und prüft die Header `X-Goog-Channel-ID`, `X-Goog-Channel-Token` und `X-Goog-Resource-ID`, bevor Änderungen abgerufen werden. Dies sind direkte Drive-Benachrichtigungen, keine Pub/Sub-Nachrichten, und sie enthalten keine Pub/Sub-OIDC-Bearer-Tokens.
...
- [ ] `WORKSPACE_SECRETS_ENCRYPTION_KEY` für gemeinsam genutzte Multi-App-Vaults konsistent gesetzt; app-lokale Verschlüsselung nutzt weiterhin `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` – siehe [Secrets Management](#secrets)
...
- [**Authentication**](/docs/authentication) — Auth-Modi, Cookie-Realms und Sitzungsdetails, auf denen diese Seite aufbaut
- [**Multi-Tenancy**](/docs/multi-tenancy) — das Org-Mitgliedschaftsmodell hinter dem `org_id`-Scoping
- [**Sharing**](/docs/sharing) — das `ownableColumns()`- und Access-Guard-Muster, angewendet auf teilbare Ressourcen
- [**Messaging**](/docs/messaging#env-vars) — die Webhook-Signaturgeheimnisse pro Integration, auf die in der obigen Checkliste verwiesen wird
- [**Deployment**](/docs/deployment) — die Produktionsumgebungsvariablen, deren Vorhandensein diese Checkliste voraussetzt
