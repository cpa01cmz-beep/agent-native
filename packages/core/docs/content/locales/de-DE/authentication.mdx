...
- **Standard:** Magic-Link zuerst in Better Auth, wenn ausgehende E-Mails bereit sind, mit E-Mail/Passwort als Fallback und sozialen Anbietern. Mit `AUTH_MAGIC_LINK=0` wird der Fallback erzwungen.
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code-, ChatGPT-Connectoren</small>
    </div>
    <div class="diagram-card">
...
## Anmeldung in der lokalen Entwicklung {#local-development-sign-in}

Auf einem Entwicklungsserver mit Loopback-Adresse zeigt die gemeinsame Anmeldeseite zuerst **Continue as local dev** an. Sie erstellt das echte Better Auth-Entwicklerkonto und die Sitzung oder verwendet sie erneut. Dadurch wird derselbe authentifizierte Ablauf wie an anderen Stellen verwendet.

Wählen Sie **Show full sign in options**, um die normalen Optionen für Registrierung, Passwort, Magic Link und soziale Anbieter anzuzeigen. Wenn die lokale Sitzung nicht erstellt werden kann, zeigt die Seite diese Optionen automatisch an, damit Sie mit der normalen Authentifizierung fortfahren können.

Diese Funktion ist nur verfügbar, wenn der Browser in der Entwicklung oder im Test über `localhost`, `127.0.0.0/8` oder `::1` verbunden ist. Sie ist für Bereitstellungsvorschauen und gehostete Produktion deaktiviert. Sie ist keine Umgehung der Authentifizierung. Setzen Sie `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1`, um sie zu deaktivieren und in der lokalen Entwicklung die normale Anmeldeseite anzuzeigen.

## Bessere Authentifizierung (Standard) {#better-auth}

Standardmäßig unterstützt Better Auth die Authentifizierung. Es bietet:

- Magic-Link-Registrierung und Anmeldung mit E-Mail/Passwort als Fallback
...
- `POST /_agent-native/auth/magic-link` – Magic-Link-E-Mail anfordern
...
oder Vorschauumgebungen und benennen Sie Testkonten mit einer `+autoz`-Adresse
(`name+autoz@example.com`), damit sie leicht zu identifizieren sind.

## Richtlinie zur E-Mail-Überprüfung {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` ist eine lokale und QA-Bequemlichkeit — eine
gehostete Bereitstellung ignoriert sie. Um die Richtlinie für jede Umgebung
festzulegen, einschließlich der Produktion, deklarieren Sie sie in der
Konfiguration:
...
Der Bereitstellungsalias lautet `AUTH_REQUIRE_EMAIL_VERIFICATION=0`. Ein
deklarierter Wert übertrifft sowohl `AUTH_SKIP_EMAIL_VERIFICATION` als auch die
Standardeinstellung der jeweiligen Umgebung.

`false` akzeptiert eine nicht überprüfte Adresse als Anmeldeinformation — jeder
kann jede E-Mail-Adresse beanspruchen — verwenden Sie es also nur dort, wo die
Anmeldung bereits anderweitig eingeschränkt ist. `true` deaktiviert auf einer
Bereitstellung ohne E-Mail-Anbieter stattdessen die Passwortanmeldung, da die
Überprüfung nie zugestellt werden könnte.
...
<Diagram id="doc-block-1q6khwn" title="Remote-Handshake MCP OAuth" summary={"Ein OAuth-fähiger Client führt einen Bootstrapping nur von MCP URL durch – Herausforderung, Erkennung, dynamische Registrierung, dann ein PKCE-Codeaustausch."}>
...
Normale App-Seiten verwenden für alle Besucher dieselbe unpersönliche, öffentlich cachebare SSR-Shell. `AppProviders` prüft die Sitzung im Client und leitet anonyme Besucher auf privaten Pfaden wie `/dashboard` zu `/sign-in?return=%2Fdashboard` weiter, bevor private App-UI gerendert wird. Nach der Anmeldung kehren sie zu `/dashboard` zurück. Verwenden Sie `isPublicPath` für öffentliche/SEO-Routen oder `sessionBypass` für Oberflächen mit einem anderen begrenzten Authentifizierungsmechanismus, etwa einem MCP-Embed-Token.

Wie lange diese Shell zwischengespeichert wird, ist eine bereitstellungsweite Einstellung – siehe [SSR-Caching](/docs/ssr-caching) für `AGENT_NATIVE_SSR_CACHE`. Das Verkürzen oder Deaktivieren des Caches ändert nur die Dauer; SSR rendert weiterhin den anonymen Zweig mit entfernten Cookies.

### Hinter den Kulissen: Google OAuth

Beide Flüsse (der explizite `/sign-in`-Einstiegspunkt und der Fall des mit Lesezeichen versehenen Pfads) führen den zurückgegebenen URL durch den OAuth-Status. Der Status ist HMAC-signiert und kann daher während der Übertragung nicht gefälscht werden. Beim Rückruf wird der zurückgegebene URL vor der Umleitung erneut als Same-Origin validiert – ein durchgesickerter Signaturschlüssel kann also immer noch nicht in ein Open-Redirect-Orakel umgewandelt werden.
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | Auf `0` oder `1` setzen, um die Überprüfungsrichtlinie für die Passwortanmeldung in jeder Umgebung festzulegen, einschließlich der Produktion; übertrifft `AUTH_SKIP_EMAIL_VERIFICATION`                                     |
...
Das standardmäßig deaktivierte Dispatch-Flag `desktop.workspace-sso` steuert nur die Verfügbarkeit; Authentifizierung, Signatur-, Nonce-, Origin-, Callback-, Sitzungs- und Widerrufsprüfungen bleiben immer verpflichtend. Ist das Flag aus oder nicht lesbar, verändert Desktop weder App-Anmeldung noch Abmeldung oder Einstellungen.
Die Canary-Kennung im User-Agent ist nur ein Verfügbarkeitshinweis und keine Remote-Attestierung; Authentifizierung und Autorisierung vertrauen ihr nicht.
Kanonische gehostete Apps erkennen Anfragen der paketierten Desktop SSO Canary ohne eine Identity-Hub-Umgebungsvariable pro App. Normale Browser verwenden die übliche Anmeldung pro App. Selbst gehostete Apps benötigen `AGENT_NATIVE_IDENTITY_HUB_URL` nur für den verbleibenden expliziten Desktop- oder serverseitigen Föderationspfad. Stabile Desktop-Builds initialisieren den Broker erst nach einer separat genehmigten späteren Veröffentlichung.
Die Canary-Version fängt anonyme App-Anmeldungen nicht ab. Benutzer müssen die Workspace-Anmeldung in den Desktop-Einstellungen für die aktive registrierte App ausdrücklich auswählen; die normale App-Anmeldung bleibt der Standardpfad.

## Verwaltete Bereitstellung und Einladungen

Für eine verwaltete Bereitstellung setzen Sie `AUTH_SIGNUP=invited`, `ORG_CREATION=closed` und `AUTO_CREATE_DEFAULT_ORG=0`. Der [Leitfaden für verwaltete Bereitstellungen](/docs/administered-deployments) beschreibt Bootstrap-Admins, Einladungen und erlaubte E-Mail-Domains. Mitgliedschaftseinladungen und Domain-Beitritt sind getrennte Regeln.
