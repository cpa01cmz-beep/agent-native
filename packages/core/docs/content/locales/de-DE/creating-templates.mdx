...
Vorlagen sind vollständige, forkbare agentennative Apps, die einen echten Arbeitsablauf lösen. Die Erstanbieter-Vorlagen werden mit derselben Framework-Oberfläche erstellt, die Sie verwenden: React-Routen für UI, PostgreSQL-Daten über Drizzle, actions für Vorgänge, Arbeitsbereichsressourcen für Agentenverhalten und Abfragesynchronisierung, damit der Agent und UI aufeinander abgestimmt bleiben.
...
Behalten Sie bei der Zuordnung einer Funktion die drei Ebenen im Blick:

- **Core - das Framework:** der erforderliche Laufzeit- und Datenvertrag, den jede App nutzen kann.
- **Toolkit - optionale wiederverwendbare Teile:** gemeinsame UI- und Produktsysteme, die Apps übernehmen, zusammensetzen oder forken können.
- **Templates - optionale Apps:** vollständige Domänenprodukte auf Core, die häufig Toolkit nutzen und als Ausgangspunkt geforkt werden können.

Toolkit und Templates sind optional. Ein Template ist nicht das Framework selbst, und Toolkit ersetzt Core nicht.

Eine gute Vorlage:

- Löst einen Workflow durchgängig, mit nützlichen Seed-Daten oder einem Leerzustandsfluss.
- Speichert den dauerhaften Zustand in PostgreSQL/PGlite, nicht in JSON-Dateien.
...
Chat bietet Ihnen Authentifizierung, dauerhafte Chat-Threads, von PostgreSQL unterstützte Ressourcen, Tools, Anwendungsstatus, actions und Abfragesynchronisierung. Sie fügen das Domänenmodell und das Produkt UI hinzu.

Wenn Sie noch keine wiederverwendbare UI-Vorlage erstellen, verwenden Sie [Automation-First Apps](/docs/pure-agent-apps): Definieren Sie Aktionen, führen Sie sie ohne Browser aus und fügen Sie später UI hinzu, wenn der Workflow eine dauerhafte Oberfläche benötigt.
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "app/root.tsx",
      note: "HTML-Shell und Provider",
    },
    {
      path: "app/routes/",
      note: "React-Router-Dateirouten",
    },
    {
      path: "app/components/",
      note: "Template-UI",
    ...
    {
      path: "app/hooks/",
      note: "UI-State- und Daten-Hooks",
    },
    {
      path: "actions/",
      note: "defineAction-Operationen: die einzige Quelle der Wahrheit",
    },
    {
    ...
    },
    {
      ...
      note: "Additive Migrationen",
    },
    {
      path: "server/plugins/",
      note: "Startup-Integrationen",
    },
    {
      path: "server/routes/api/",
      note: "Benutzerdefinierte Routen nur, wenn Aktionen nicht ausreichen",
    },
    {
      path: "shared/types.ts",
      note: "Gemeinsame Client-/Server-Typen",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: Agent-Anleitung für komplexe Workflows",
    },
    {
      path: "AGENTS.md",
      note: "Template-spezifische Agent-Anweisungen",
    },
    {
      path: "package.json",
    },
...
Fügen Sie kein `data/`-Verzeichnis für den Anwendungsstatus hinzu. Dauerhafte App-Daten gehören in PostgreSQL/PGlite, und UI liest sie über actions oder typisierte Server-Handler.

Die vier Bereiche jeder Vorlage sind über eine gemeinsame Aktionsoberfläche und eine PostgreSQL/PGlite-Datenbank miteinander verbunden – der Agent und der UI sind gleichberechtigte Partner bei denselben Vorgängen:

<Diagram id="doc-block-1pikucf" title="Wie die vier Bereiche einer Vorlage miteinander verbunden sind" summary={"Die Benutzeroberfläche und der Agent erreichen PostgreSQL/PGlite beide über dieselben Aktionen; Anwendungsstatus und Abfragesynchronisierung sorgen dafür, dass sie aufeinander abgestimmt sind."}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      React-UI<br /><small class="diagram-muted"
        >app/routes · Komponenten</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-box" data-rough>
    PostgreSQL über Drizzle<br /><small class="diagram-muted"
...
## Modelldaten in PostgreSQL {#data-models}

Definieren Sie Domänentabellen mit den Framework-Drizzle-Helfern, damit die Schemata PostgreSQL und PGlite verwenden:
...
Schemaänderungen müssen additiv sein. Verwenden Sie für neue app-eigene Tabellen
und Spalten `drizzle-kit generate`, um prüfbares SQL unter
`server/db/migrations/<id_name>.sql` zu erzeugen, und laden Sie es über
`runDrizzleMigrations()` im Release-Schritt auf Node.js-Deployments. Cloudflare
Workers/Pages und andere Edge-Runtimes haben kein bereitstellbares Dateisystem. Betten Sie dort das
generierte SQL in das Worker-Bundle ein und übergeben Sie es an
`runMigrations()`. Behalten Sie `runMigrations()` für handgeschriebene Historie
und Sonderfälle. Verwenden Sie niemals destruktives SQL, `drizzle-kit push`,
Tabellenumbenennungen oder Spaltenlöschungen.

Für App-Lese- und Schreibvorgänge verwenden Sie den Abfrage-Builder von Drizzle und die PostgreSQL-kompatiblen Operatoren von `drizzle-orm`. Schreiben Sie keinen Produktcode mit rohem SQL, wenn Drizzle die Abfrage ausdrücken kann, und verwenden Sie in Vorlagen die Schema-Helfer von `@agent-native/core/db/schema`.
...
Verwenden Sie die Dokumente [Database](/docs/server-database) und [Security](/docs/security), bevor Sie Schemas hinzufügen, die Benutzer- oder Organisationsdaten enthalten.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    {
      lines: "8",
      label: "Werkzeugoberfläche",
      note: "`description` ist das, was der Agent liest, um zu entscheiden, wann diese Aktion als Tool aufgerufen werden soll.",
    },
    {
      lines: "9-11",
      label: "Typisierter Vertrag",
      note: "Ein zod `schema` validiert Eingaben von Agent, UI, HTTP, MCP und A2A.",
    },
    {
      lines: "18-19",
      label: "Begrenztes Schreiben",
      note: "Stempeln Sie `ownerEmail` / `orgId` von `ctx`, damit die Zeile den korrekten Bereich für Freigabe- und Zugriffsprüfungen aufweist.",
    },
  ]}
/>
...
- Data lives in PostgreSQL über Drizzle. Use actions for all writes; schema is additive.
...
Community-Vorlagen werden direkt aus einem öffentlichen GitHub-Repository installiert. Die Agent-Native-App muss im Stammverzeichnis liegen:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

Mit `#branch`, `#tag` oder `#commit` kann eine bestimmte Version festgelegt werden:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

Die CLI lädt nur den Quellcode herunter. Sie installiert keine Abhängigkeiten und führt keine Skripte der Vorlage aus. Community-Vorlagen sind Drittanbieter-Code; prüfe Repository, Lizenz, Abhängigkeiten und Installationsskripte vor `pnpm install`.

Für eine Aufnahme in den Community-Katalog braucht die Vorlage eine vollständige App im Repository-Stamm, ein öffentliches Repository mit Lizenz und Setup-README, eine Abhängigkeit von `@agent-native/core` sowie erfolgreiche Typprüfungen und Tests. Eine gehostete Demo und ein Screenshot sind optional.

[Community-Vorlage einreichen](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml), wenn das Repository bereit ist. Community-Einträge bleiben vom kleineren First-Party-Katalog getrennt und werden von ihren Autoren gepflegt.
