...
<Diagram id="doc-block-fvp37q" title="Wo die drei Haken im Lauf feuern" summary={"processOutputStream überwacht jeden Block, processOutputStep steuert Tool-Aufrufe pro Antwort, processOutputResult zeichnet am Ende ein Urteil auf. Jeder Hook kann mit einem TripWire abgebrochen werden."}>
...
<div class="diagram-proc">
  ...
  <div class="diagram-node" data-rough>
    per model response<br /><small class="diagram-muted"
      >processOutputStep — Tool-Aufrufe prüfen</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    run end<br /><small class="diagram-muted"
      >processOutputResult — Urteil</small
    >
  </div>
...
Ein Prozessor ist eine **Konfiguration**, kein Werkzeug, keine Aktion und kein Authoring DSL. Prozessoren beobachten nur, verändern ihren eigenen Stream-Scope-Status und `abort()`. Sie definieren niemals das App-Verhalten, ersetzen actions oder erscheinen im Modell. App-Vorgänge gehören zu [actions](/docs/actions-overview).
