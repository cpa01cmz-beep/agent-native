...
`app-skill`-Pakete erzeugen ebenfalls standardmäßige Agent-Plugin-Dateien. Verwende `agent-native plugin import <path> --into <workspace> --yes`, um Skills und entfernte MCP-Server über Streamable HTTP zu importieren; Header werden ignoriert, stdio- und SSE-Server werden gemeldet und übersprungen.
...
- Die Anleitung ist einmalig – legen Sie sie stattdessen in `AGENTS.md` oder im Agentenspeicher ab
...
Wenn das Wissen für _jeden_ gilt, der in der App arbeitet („CTEs immer gegenüber Unterabfragen bevorzugen“), handelt es sich um eine Fertigkeit oder ein gemeinsames `LEARNINGS.md`. Wenn es um _diesen bestimmten Benutzer_ geht („Steve mag prägnante Antworten“), gehört es in `memory/MEMORY.md`. Die vollständige Behandlung finden Sie unter [Agent-Ressourcen — Memory](/docs/agent-resources#memory).

## Skills im Vergleich zu SQL-Ressourcen {#skills-vs-resources}

Diese Seite beschreibt einen Skill als Datei unter `.agents/skills/<name>/SKILL.md`. [Agent-Ressourcen](/docs/agent-resources#skills) beschreibt denselben Skill als SQL-Ressource im Pfad `skills/<slug>/SKILL.md` mit Workspace-, gemeinsamem oder persönlichem Geltungsbereich. Es ist dasselbe Konzept aus zwei Perspektiven: Im Code-Modus wird die Datei direkt gelesen und geschrieben; im App-Modus wird sie als Ressourcenzeile gespeichert, damit sie ohne Dateisystem funktioniert. Verwenden Sie in beiden Fällen das Format dieser Seite.
...
- [**Agent-Ressourcen**](/docs/agent-resources#skills) — wo Skills im App-Modus als SQL-Ressourcen gespeichert werden
