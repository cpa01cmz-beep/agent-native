---
title: "Erweiterungen"
description: "Eine optionale Toolkit-Funktion für einmalige, isolierte Custom Blocks. App-Code bleibt der Standard für dauerhaftes Produktverhalten."
---

# Erweiterungen

Erweiterungen sind eine **optionale Toolkit-Funktion für isolierte Custom Blocks**. Sie sind standardmäßig deaktiviert und müssen von der Vorlage ausdrücklich aktiviert werden. Analytics nutzt sie für ausdrücklich angeforderte, einmalige Visualisierungen. Für dauerhaftes Verhalten bearbeiten Sie den App-Code.
...
> **Erweiterungen ergänzen die Benutzeroberfläche; sie schreiben die
> Host-Benutzeroberfläche nicht um.** Eine Erweiterung wird auf einer eigenen
> Seite oder in einem benannten [Slot](#slots) dargestellt, den die Vorlage
> bereitstellt. Sie kann keine Elemente in beliebige native Komponenten
> einfügen – zum Beispiel neben jeden Teilnehmer im Kalender –, sofern dort
> kein Slot vorhanden ist. Für tiefere Anpassungen ändern Sie die Vorlage in
> Ihrem Repository und Entwicklungsablauf. Der eingebettete Agent kann nur
> helfen, wenn der Host bewusst Repository-/Workspace-Schreibwerkzeuge
> bereitstellt; Erweiterungen bleiben die schnelle Ebene ohne Bereitstellung.
...
Erweiterungen sind standardmäßig deaktiviert. Aktivieren Sie sie mit
`extensionTools: true` in `createAgentChatPlugin()` und
`createCoreRoutesPlugin()`. Das erste ergänzt agentenseitige actions und
Anweisungen; das zweite erlaubt authentifizierte Erstellung über
`POST /_agent-native/extensions`. Sandbox-Laufzeit sowie bestehende Lese-,
Bearbeitungs- und Deep-Link-Routen bleiben aus Kompatibilitätsgründen erhalten.

<Diagram id="doc-block-1yn00sz" title={"Die Sandkastenbrücke"} summary={"Die Erweiterung HTML wird in einem isolierten Iframe ausgeführt und erreicht den Host nur über einen festen Satz von Bridge-Helfern – jeder Aufruf ist umfangreich und zugriffsüberprüft."}>
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Host-Template<br /><small class="diagram-muted"
        >Actions, automatisch gescopes SQL</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>, domaingebunden</small
      >
    </div>
    <div class="diagram-box">
      Externe APIs<br /><small class="diagram-muted"
        >nur über extensionFetch</small
      >
    </div>
  </div>
</div>
...
1. Öffnen Sie **Settings → Extensions** (oder fragen Sie einfach im Chat) und klicken Sie dann auf **„Neue Erweiterung“**.
...
<Diagram id="doc-block-4kt9b8" title={"Slots übertragen Kontext in das Widget"} summary={"Die Host-Vorlage besitzt benannte Slots; Durch die Installation einer Erweiterung in einem wird window.slotContext für alles eingespeist, was der Benutzer gerade ansieht."}>
...
<div class="slot">
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Kontaktnotizen</span
    ><small class="diagram-muted"
...
- `dbQuery` und `extensionFetch` sind vorerst blockiert.
...
## Sollte ich eine Erweiterung oder den App-Quellworkflow verwenden? {#vs-app-code}

Der eingebettete Agent bearbeitet Quellcode nicht standardmäßig; das ist nur mit bewusst bereitgestellten Repository-/Workspace-Schreibwerkzeugen möglich. Nutzen Sie den Quellworkflow für dauerhaftes Produktverhalten und Erweiterungen für schnelle Laufzeitanpassungen.

- **Beginnen Sie mit App-Code** für Produktverhalten, native UI, Layout, Routen, Geschäftslogik oder alles, was wiederverwendet werden soll.
- **Verwenden Sie einen Custom Block nur, wenn die App ihn aktiviert** und der Benutzer ausdrücklich eine einmalige Visualisierung oder Interaktion in einer vorhandenen Oberfläche wünscht. Er ist schnell und ohne Bereitstellung verfügbar, bleibt aber ein isolierter, vom Agenten erstellter Patch.
- **Überführen Sie dauerhafte Blocks.** Wenn ein Block über seinen ursprünglichen Kontext hinaus nützlich wird, verwenden Sie **In App-Code überführen** und behalten Sie ihn bis zur Prüfung und Bereitstellung der Codeänderung bei.

|                                   | Erweiterung                                      | App-Code bearbeiten                 |
| --------------------------------- | ------------------------------------------------ | ----------------------------------- |
| **Erstellt von**                  | Agent (oder Benutzer) zur Laufzeit               | Benutzer oder codefähiger Workflow  |
...
| **Am besten für**                 | Ausdrückliche einmalige Custom Blocks            | Dauerhaftes Produktverhalten        |

Faustregel: **Verwenden Sie standardmäßig App-Code.** In einer App, die Erweiterungen aktiviert, kann eine einmalige Visualisierung ein Custom Block sein; ein neuer Diagrammtyp für alle Dashboards gehört in den Vorlagencode.

Für benutzerdefinierten Datenabruf, der native Diagramme und Tabellen speisen soll statt einer eigens gebauten Widget-UI, siehe [Datenprogramme](/docs/data-programs) — ein gespeichertes, aktualisierbares `run-code`-Skript, an das sich Dashboard-Panels direkt binden, ganz ohne Extension-iframe.
...
- **Authentifizierung erforderlich** – Erweiterungen werden nur für angemeldete Benutzer ausgeführt und `dbQuery`-Aufrufe werden automatisch eingeschränkt.
...
- [**Actions**](/docs/actions-overview) – die Vorgänge, die eine Erweiterung über `appAction` aufruft
