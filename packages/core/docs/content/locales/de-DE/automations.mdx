...
Eine **Automatisierung** ist eine vom Agenten ausgeführte Aufgabe, die nach Zeitplan oder bei einem Ereignis startet: _Wenn X passiert, mache Y_. **Scheduled** und **Event** sind zwei Auslösertypen, keine getrennten Produkte.

Das Framework bezeichnete geplante Automatisierungen ursprünglich als **Recurring Jobs**. Dieser Name, der Pfad `jobs/<name>.md` und das Tool `manage-jobs` bleiben kompatibel. Neue Produktanleitungen verwenden **Automations** als Oberbegriff; [Recurring Jobs](/docs/recurring-jobs) beschreibt die Pflege älterer geplanter Jobs sowie Organisationsbereich und erweiterte Felder.

<Diagram id="doc-block-1cpvlda" title="Wenn X passiert, mache Y" summary={"Ein Ereignis wird auf dem Bus ausgelöst, eine optionale Bedingung in natürlicher Sprache löst es aus und der Agent führt den Automatisierungskörper mit vollem Werkzeugzugriff aus."}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">Agent führt den Body aus</span
    ><small class="diagram-muted"
...
### Über die Automations-Oberfläche

Öffnen Sie **Automations** auf der Agent-Vollseite, um geplante und ereignisgesteuerte Automatisierungen anzuzeigen und zu verwalten. Die Kompatibilitäts-URL bleibt `/agent#jobs`, damit vorhandene Links und Lesezeichen weiter funktionieren.

<Callout tone="warning">

`manage-automations` unterstützt persönliche und Organisations-Automatisierungen für Scheduled- und Event-Trigger. Organisations-Event-Automatisierungen sind für Mitglieder sichtbar, laufen aber immer als ihr Ersteller; Administratoren können sie verwalten, ohne diese Identität zu ändern.
...
<AnnotatedCode
  id="doc-block-1p8j436"
  title="Eine ereignisgesteuerte Automatisierung"
  ...
  annotations={[
    {
      lines: "2",
      label: "Kein Cron",
      note: 'Ereignisauslöser setzen `schedule` auf `""` – das Cron-Feld bleibt leer.',
    },
    {
      lines: "4-5",
      label: "Der Auslöser",
      note: "`triggerType: event` plus der Name `event` abonniert diese Automatisierung am Bus.",
    },
    {
      lines: "6",
      label: "Tor",
      note: "Ein optionaler `condition` in natürlicher Sprache, der von Haiku vor dem Versand anhand der Nutzlast ausgewertet wird.",
    },
    {
      lines: "12",
      label: "Serverseitiges Geheimnis",
...
| `mode`        | `"agentic"`               | `"agentic"`  | Vollständige Agentenschleife (der einzige unterstützte Modus). `"deterministic"` wurde entfernt – es wurde nie implementiert, und Automatisierungen, die es festlegten, wurden nie ausgelöst. |
...
`owner` in den Emit-Metadaten bestimmt, welche Automatisierungen ausgelöst werden. Persönliche und Organisations-Event-Automatisierungen erfordern beide eine exakte Übereinstimmung mit `createdBy`.
...
Persönliche und Organisations-Automatisierungen verwenden `manage-automations` mit einem `action`-Parameter:
...
`manage-automations` unterstützt Organisationsbereich, Modellüberschreibungen und exakt durchgesetzte MCP-Zulassungslisten. `manage-jobs` bleibt das Kompatibilitäts-Tool für ältere schedule-only Integrationen und Zustellungsmetadaten.
...
<Endpoint id="doc-block-21763k" title={"Lösen Sie ein Testereignis aus"} method="POST" path="/_agent-native/automations/fire-test" summary="Geben Sie ein test.event.fired-Ereignis aus, um ereignisgesteuerte Automatisierungen zu validieren" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-5qy708" title="Der Versandpfad" summary={"Von einem ausgelösten Ereignis bis zu einem abgeschlossenen Agentenlauf, abhängig vom Besitzumfang und der Bedingung der natürlichen Sprache."}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> mit Body als Prompt, Payload als Kontext,
      5-Minuten-Timeout</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-overview) – Automatisierungen können jede registrierte Aktion über die Agentenschleife aufrufen
