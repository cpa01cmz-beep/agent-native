...
- **Subagenten** – werden mit ihrem eigenen Thread, ihrer eigenen Systemeingabeaufforderung und ihrem eigenen Toolset ausgeführt. Jeder ist einem „benutzerdefinierten Agenten“-Profil im [Agent-Ressourcen](/docs/agent-resources) zugeordnet.
...
<Diagram id="doc-block-s323ir" title="Orchestrator und Spezialisten" summary={"Der Hauptchat wird an Unteragenten delegiert, die in ihren eigenen Threads ausgeführt werden und als Inline-Chips Rückmeldung geben."}>

```html
<div class="at-orc">
  <div class="diagram-card main">
    <span class="diagram-pill accent">Hauptchat</span
  ...
  <div class="at-fan">
    ...
    <div class="at-subs">
      ...
      <div class="diagram-box">
        BigQuery-Analyse<br /><small class="diagram-muted">own tools</small>
      </div>
      <div class="diagram-box">
        E-Mail per Sprache<br /><small class="diagram-muted">own context</small>
      </div>
    </div>
  </div>
...
Benutzerdefinierte Agenten befinden sich im Arbeitsbereich von `agents/<slug>.md` – einer Markdown-Datei mit YAML-Frontmatter. Informationen zum Format finden Sie unter [Custom Agents](/docs/agent-resources#custom-agents).
...
<Diagram id="doc-block-1jbu62n" title="Was spawnTask() macht" summary={"Jeder Spawn erstellt einen Thread, behält den Status bei SQL bei und streamt Chip-Ereignisse bis zur Fertigstellung."}>
...
Subagenten werden benutzerdefinierten Agentenprofilen zugeordnet – Markdown-Dateien unter `agents/<slug>.md` im Arbeitsbereich, die im `@mention`-Dropdown-Menü angezeigt werden und als Delegierungsziele dienen. [Agent-Ressourcen — Custom Agents](/docs/agent-resources#custom-agents) besitzt das vollständige Format (Frontmatter, `tools`, `delegate-default`, Modellüberschreibungen).
...
<Diagram id="doc-block-qa08ls" title="Delegations-Tiefenschutz (Standardobergrenze 2)" summary={"Jedes Level kann bis zur Obergrenze um eins tiefer spawnen; ein Spawn darüber hinaus wird serverseitig abgelehnt."}>
...
<div class="at-depth">
  ...
  <div class="diagram-card warn">
    ...
    ><strong>Sub-Agent des Sub-Agenten</strong
...
- [**Agent-Ressourcen — Custom Agents**](/docs/agent-resources#custom-agents) – das Profilformat
- [**A2A Protocol**](/docs/a2a-protocol) – wenn der „Subagent“ vollständig in einer anderen App lebt
- [**Actions**](/docs/actions-overview) – die Tools, die ein Subagent aufruft
