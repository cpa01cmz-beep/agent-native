...
<Diagram id="doc-block-uqkhl1" title="Der native Renderpfad" summary={"Eine Aktion gibt JSON zurück; die Laufzeit entspricht einer expliziten Widget-Diskriminante oder chatUI.renderer; AssistantChat mountet eine echte React-Komponente. Kein Iframe, keine HTML-Ausführung."}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    Action läuft<br /><small class="diagram-muted"
      >gibt strukturiertes JSON zurück</small
    >
  </div>
  ...
  <div class="diagram-box">
    &lt;AssistantChat&gt;<br /><small class="diagram-muted"
      >mountet ein React-Widget</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
Dimensionieren Sie Renderer in absoluten Einheiten, nicht in Prozent. Das
Transkript wächst mit seinen Nachrichten, daher hat der Container eines Renderers
keine definite Höhe, gegen die ein Prozentwert aufgelöst werden könnte — `height: 40%`
an einem Diagrammbalken kollabiert zu null, statt einen Fehler zu werfen, was wie
ein Renderer aussieht, der nicht gezeichnet hat. Geben Sie Diagrammen und anderen
höhenabhängigen Visualisierungen eine Pixel- (oder `rem`-)Basis; Breiten in Prozent
sind in Ordnung, da die Transkriptspalte eine definite Breite hat.
...
<Diagram id="doc-block-1z0kwg9" title={"Die BYO-Laufzeit behält die Agent-Native-Chat-Shell bei"} summary={"Ihr externer Agent streamt normalisierte Ereignisse über einen Connector; Agent-Native behält den Komponisten, das Transkript, die Werkzeugkarten, Genehmigungen und native Widgets."}>

```html
<div class="diagram-byo">
  <div class="diagram-box" data-rough>
    Dein Agent<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted"
      >normalisierte message-* / tool-*-Events</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
...
- [Actions](/docs/actions-overview) – Definieren Sie die Vorgänge, die native Widget-Daten zurückgeben.
