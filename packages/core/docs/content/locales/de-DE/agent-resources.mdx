---
title: "Agent-Ressourcen"
description: "Claude-Anpassung auf Codeebene pro Benutzer – skills, Speicher, Anweisungen, benutzerdefinierte Agenten, geplante Jobs, MCP-Server – unterstützt durch PostgreSQL/PGlite, kein Dateisystem."
---

# Agent-Ressourcen

Jede agentennative App wird mit **Agent-Ressourcen** ausgeliefert: der Anpassungsebene, die den Agenten zu Ihrem macht. Es enthält Teamanweisungen (`AGENTS.md`), gemeinsame Erkenntnisse (`LEARNINGS.md`), persönliches strukturiertes Gedächtnis (`memory/MEMORY.md`), skills, das der Agent bei Bedarf einholt, benutzerdefinierte Subagenten, geplante Jobs und verbundene MCP-Server – alles, was Sie von einem Claude-Code-/Codex-Setup erwarten.

Der Clou: **Es sind PostgreSQL/PGlite-Zeilen, keine Dateisystemdateien.** Jeder Benutzer erhält seine eigenen Agent-Ressourcen, die in der Datenbank gespeichert werden. Es muss keine Entwicklungsbox gestartet werden, kein Container pro Benutzer, keine Dateien müssen gemountet werden. Ein mandantenfähiges SaaS kann jedem Benutzer praktisch kostenlos einen vollständig anpassbaren Agenten zur Verfügung stellen, da alles aus Zeilen besteht – persönlicher Speicher, persönliche MCP-Server, persönliche skills, persönliche Subagenten – und die gemeinsame Codebasis sie alle gleichzeitig hostet.

<Diagram id="doc-block-7j8xqp" title="Ein Claude-Code-Setup, aber als Agent-Ressourcen in PostgreSQL/PGlite gespeichert" summary={"Dieselbe Anpassungsebene – Anweisungen, Fähigkeiten, Speicher, Agenten, Jobs, MCP – außer dass jede Datei eine Zeile in einer gemeinsam genutzten, mandantenfähigen Datenbank ist."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-Native-Ressourcen</span
    ><small class="diagram-muted"
      >rows in one PostgreSQL/PGlite-Datenbank</small
    >
    <div class="ws-rows">
...
| Claude-Code / Codex                                | Agent-Native Ressourcen                                            |
| -------------------------------------------------- | ------------------------------------------------------------------ |
| Dateien auf Ihrer lokalen Festplatte               | Zeilen in einer gemeinsam genutzten PostgreSQL/PGlite-Datenbank    |
...
| Pro Projekt `CLAUDE.md` / skills                   | `AGENTS.md` pro App + Speicherressourcen                           |
...
1. Öffnen Sie die Registerkarte **Ressourcen** → **Freigegeben** → `AGENTS.md` (erstellen Sie sie mit `+` → **Datei**, falls fehlend).
...
## Registerkarte Ressourcen {#resources-tab}

Das Agentenfenster enthält neben Chat und CLI auch die Registerkarte **Ressourcen**. Es zeigt eine in Ordnern organisierte Baumstruktur aller Ressourcen, einen Inline-Editor für jede Textdatei (Markdown, JSON, YAML, einfacher Text) und die typisierten Erstellungsabläufe des `+`-Menüs (Dateien, Skills, Agenten, geplante Aufgaben). Benutzer können übernommene Standardeinstellungen für Arbeitsbereiche durchsuchen und persönliche oder Organisationsressourcen erstellen/bearbeiten/löschen.
...
Klicken Sie auf das `?`-Symbol in der Ressourcen-Symbolleiste, um jederzeit zu diesen Dokumenten zurückzukehren.
...
Agent-Ressourcen verfügen über zwei aktuelle Speicheroberflächen:
...
| `mcp-servers/…`    | Arbeitsbereich / freigegeben | Menschen über Dispatch oder App-Ressourcen | MCP-Konfigurationsaktualisierung                |

Benutzer können diese Speicherdateien direkt auf der Registerkarte „Ressourcen“ bearbeiten – es handelt sich um reguläre Ressourcen. Löschen Sie Zeilen, die der Agent falsch verstanden hat, behalten Sie persönliche Präferenzen in `memory/MEMORY.md` bei oder übertragen Sie teamweite Regeln in `AGENTS.md`.
...
<DataModel
  ...
  title="Das Agent-Ressourcenmodell"
  summary={
    "Eine Ressourcenform unterstützt jede Agent-Ressourcendatei. Die Laufzeit verschlüsselt es nach Pfad und Umfang und löst den effektiven Wert beim Lesen auf."
  }
  entities={[
    {
      id: "resource",
      name: "Agent-Ressource",
      note: "Eine einzelne Datei in den Agent-Ressourcen eines Benutzers – Anweisungen, Skill, Speicher, Agent, MCP Konfiguration oder Job.",
      fields: [
        {
          name: "path",
          type: "string",
...
Skills sind Markdown-Ressourcendateien unter dem `skills/`-Pfad (vorzugsweise `skills/<name>/SKILL.md`), die dem Agenten On-Demand-Domänenwissen vermitteln, das im Chat mit `/skill-name` aufgerufen wird. Fügen Sie sie über die Registerkarte „Ressourcen“ oder im Codemodus über `.agents/skills/` hinzu.
...
Agent-Ressourcen umfassen zwei Agententypen:
...
| ------------------------ | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| @-Tagging                | Codebasisdateien + Agent-Ressourcen + benutzerdefinierte Agenten + verbundene Agenten | Agent-Ressourcen + benutzerdefinierte Agenten + verbundene Agenten |
