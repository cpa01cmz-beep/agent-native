...
<Diagram id="doc-block-10yxq2x" title="Dispatch als Arbeitsbereichssteuerungsebene" summary={"Ein Posteingang, ein Tresor, ein MCP-Gateway und gemeinsam genutzte Ressourcen befinden sich vor den Domänen-Apps, die Dispatch als A2A-Peers erreichen."}>

```html
<div class="dsp-hub">
  <div class="diagram-node">
    Users &amp; external agents<br /><small class="diagram-muted"
      >Slack · E-Mail · Telegram · WhatsApp · MCP</small
    >
  </div>
  ...
  <div class="diagram-panel dsp-control" data-rough>
    ...
    <div class="dsp-caps">
      <span class="diagram-pill">Zentraler Posteingang</span
      ><span class="diagram-pill">Secret-Tresor</span
      ><span class="diagram-pill">App-übergreifende Delegation</span
      ><span class="diagram-pill">MCP gateway</span
      ><span class="diagram-pill">Workspace-Ressourcen</span>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="dsp-peers">
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **Zentraler Posteingang**         | Slack, E-Mail, Telegram, WhatsApp erreichen alle einen Agenten mit gemeinsamem Speicher + Tools          | **Verwaltung → Nachrichten** ([Messaging](/docs/messaging))                 |
...
| **Arbeitsbereichsressourcen**     | Autor skills/instructions/profiles einmal; Apps erben sie zur Laufzeit                                   | **Ressourcen** ([Agent-Ressourcen](/docs/agent-resources#global-resources)) |
...
Slack, E-Mail, Telegram und WhatsApp fließen alle in die Agentenschleife von Dispatch ein. Verbinden Sie jede Plattform einmal unter **Verwaltung → Nachrichten** und jeder Kanal erreicht denselben Agenten mit demselben Speicher und denselben Tools. Eine Slack-DM und eine E-Mail an `agent@yourcompany.com` enden als zwei Oberflächen in einem Gesprächsverlauf und nicht als zwei getrennte Bots. Siehe [Messaging](/docs/messaging) für die Anmeldeinformationen und Webhooks URLs.
...
Dispatch kann der einzelne MCP-Connector für externe Agenten sein: Fügen Sie `https://dispatch.agent-native.com/mcp` einmal in Claude, ChatGPT, Codex oder Cursor hinzu, und eine Autorisierung erreicht jede gewährte Workspace-App anstelle eines Connectors pro App. Den vollständigen Verbindungsablauf, App-Zuschüsse, OAuth und Inline-MCP-App-Vorschauen finden Sie unter [External Agents](/docs/external-agents).

<Endpoint id="doc-block-1gi4v43" method="POST" path="/mcp" summary="Einheitlicher MCP-Gateway-Endpunkt" auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
...
Siehe [Agent-Ressourcen — Global resources](/docs/agent-resources#global-resources) für die kanonische Pfadtabelle, das Starterpaket und das Override-Modell.
...
<Diagram id="doc-block-1b84or3" title={"Eine Slack-Nachricht über Dispatch"} summary={"Slack wird in SQL eingereiht, eine neue Ausführung entleert es, der Dispatch-Agent delegiert die Domänenarbeit über A2A und die Antwort landet wieder im ursprünglichen Thread. Ein 60-sekündiger Wiederholungsjob stellt alles wieder her, was während des Flugs abstürzt."}>
  ...
  <div class="dsp-row">
    ...
    <div class="diagram-box" data-rough>
      <strong>/slack/webhook</strong><br /><small class="diagram-muted"
        >prüfen + ausstehenden Task INSERTen</small
      >
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  ...
  <div class="dsp-row">
    <div class="diagram-box" data-rough>
      <strong>fresh processor</strong><br /><small class="diagram-muted"
        >Task übernehmen · Agent-Schleife starten</small
      >
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-panel center" data-rough>
      <span class="diagram-pill accent">Dispatch-Agent entscheidet</span
  ...
  <div class="dsp-row">
    <div class="diagram-box" data-rough>
      Analytics-App<br /><small class="diagram-muted"
        >A2A-Peer · führt SQL-Arbeit aus</small
      >
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
1. **Bauen Sie ein Gerüst für einen Arbeitsbereich auf, der Dispatch enthält.** Führen Sie `npx --yes @agent-native/core@latest create my-company-platform` aus und wählen Sie `dispatch` neben den gewünschten Domänenvorlagen aus. Dispatch befindet sich bei `apps/dispatch` und die restlichen Apps befinden sich daneben. Siehe [Multi-App Workspace](/docs/multi-app-workspace).
2. **Messaging verbinden.** Öffnen Sie **Verwaltung → Messaging** in Dispatch und klicken Sie auf Verbinden für Slack, E-Mail, Telegram oder WhatsApp. Die Formularfelder stimmen mit den Umgebungsvariablen im Dokument [Messaging](/docs/messaging) überein – siehe dort, was die einzelnen Plattformen benötigen.
