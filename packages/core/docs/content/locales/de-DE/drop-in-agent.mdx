---
title: "Drop-in-Agent"
description: "Mounten Sie den Agenten-Chat + Agent-Ressourcen mit <AgentPanel>, <AgentSidebar> und sendToAgentChat() in jede React-App."
---
...
Sie müssen Agent-Native nicht von Grund auf neu erstellen. Der Agenten-Chat, die Registerkarte „Agent-Ressourcen“, das CLI-Terminal, die Spracheingabe und die gesamte zugehörige Infrastruktur werden als eine Handvoll React-Komponenten geliefert, die Sie in jede App einfügen können.

> **Voraussetzung:** Auf dem Server muss `agent-chat-plugin` ausgeführt werden (es wird in jeder Vorlage automatisch gemountet). Wenn Sie bei Null anfangen, lesen Sie [Server](/docs/server-overview).
>
...
| `<AgentPanel>`        | Das Rohpanel selbst – Chat + CLI + Agent-Ressourcen-Registerkarten                                                      | Sie möchten die volle Kontrolle über das Layout oder eine dedizierte Agentenseite      |
...
<Diagram id="doc-block-rnajys" title="Das Montierungsmodell" summary={"<AgentSidebar> umschließt Ihr vorhandenes Layout. Ihre Routen werden im Hauptbereich gerendert; Das Agentenpanel wird daneben montiert. <AgentPanel> ist das gleiche Panel ohne den Verpackung."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        Ihre App<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        Agentenbereich<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-109nn9r"
  title={"Umschließen des Root-Layouts mit <AgentSidebar>"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "Starter-Eingabeaufforderungen",
      note: "`suggestions` werden als anklickbare Chips im leeren Chat gerendert.",
    },
    {
      lines: "13",
      label: "Kontextsensitive Chips",
...
Das ist es. Der Benutzer hat jetzt auf jeder Seite einen umschaltbaren Agenten – mit Chat-Verlauf, Agent-Ressourcen-Tab, CLI-Terminal, Spracheingabe und einem breiteren Drawer. Der Status bleibt über Neuladevorgänge über `localStorage` hinweg bestehen.

### Requisiten

- **`children`** – das normale Layout und die Routen Ihrer App. Im Hauptbereich gerendert; Das Agentenfeld wird auf dem Desktop daneben und auf Mobilgeräten darüber angezeigt.
...
`<AgentPanel>` bietet Ihnen die Rohregisterkarten (Chat / CLI / Agent-Ressourcen) ohne den Seitenleisten-Verpackung, die Schaltfläche zum Reduzieren oder jegliche Statuspersistenz. Platzieren Sie es, wo immer Sie wollen; Sie kümmern sich um das Layout.
...
- **`isFullscreen`** – setzen Sie diese Option für eine zentrierte Spalte im Claude-Stil auf einer Seite oder in einem benutzerdefinierten Panel. Die Seitenleiste verwendet stattdessen den breiteren Drawer.
...
Eine Schaltfläche, die Arbeit an den Agenten übergibt, statt mit einem Inline-`llm()`-Aufruf einen separaten KI-Pfad zu schaffen. Unter [Warum Apps auf diese Weise bauen?](/docs/what-is-agent-native#why-build-apps-this-way) erfahren Sie, warum dieser Unterschied wichtig ist:
...
- **Für sichtbare, vom Benutzer gestartete KI-Arbeit** verwenden Sie `openSidebar: true`. Setzen Sie `false` nur für ausdrücklich stille Hintergrund- oder systeminitiierte Arbeit und behalten Sie Folgeaktionen im selben AgentSidebar-Thread.
- **`type`** – `"content"` (Standard) behält die Arbeit im eingebetteten App-Agent, der App-Werkzeuge nutzt. `"code"` leitet für Repository-Quelländerungen zu einem separaten codefähigen Host weiter; Schreibzugriff besteht nur, wenn der Host bewusst Repository-/Workspace-Werkzeuge bereitstellt. Siehe [Agent-Native Code UI](/docs/code-agents-ui).
...
Behalten Sie Folge- und Überarbeitungseingaben im bestehenden Seitenleisten-Thread. Fügen Sie kein zweites freies Eingabefeld neben dem Ergebnis hinzu; lokale Formulare sind nur für strukturierte Werte gedacht, die die UI validieren muss.
...
[Agent-Chat](/docs/client-agent-chat#sendtoagentchat) für das MCP-App-Bridge-Verhalten.
...
## Agent-Chat oder Aktion wählen {#agent-or-action}

Verwenden Sie `useActionQuery` oder `useActionMutation` direkt für deterministische lokale Vorgänge wie CRUD, Validierung, Provider-Lesevorgänge und Persistenz. Wenn der Benutzer den Ablauf als Recherche, Analyse, Generierung, Empfehlung, Synthese oder mehrstufiges Reasoning erlebt, starten Sie ihn mit `sendToAgentChat({ openSidebar: true })` und lassen Sie den Agenten fokussierte Aktionen orchestrieren. Eine deterministische Implementierung allein ist kein Grund, einen KI-ähnlichen Ablauf in einer undurchsichtigen Aktion zu verstecken.

## Typsicheres actions vom UI: `useActionMutation()` {#use-action-mutation}

Wenn die UI denselben Vorgang ausführen muss wie ein Agent-Tool, verwenden Sie [`useActionMutation`](/docs/client-data#use-action-mutation):
...
Typsichere Argumente stammen aus dem Zod-Schema in Ihrem `defineAction()`. Das vollständige Aktionssystem finden Sie unter [Actions](/docs/actions-overview).

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation` vs. `sendToAgentChat`.** Führen Sie den Vorgang direkt mit `useActionMutation` aus, wenn der Benutzer auf eine deterministische Schaltfläche geklickt hat ("Send reply"). Übergeben Sie ihn an `sendToAgentChat`, wenn die Arbeit die Schlussfolgerungen, Tools oder mehrstufige Planung des Agenten erfordert. Rufen Sie niemals ein Inline-`llm()` aus der UI auf; dadurch entsteht ein separater KI-Pfad, statt die gemeinsame Agentenschleife zu verwenden.
...
- [**Actions**](/docs/actions-overview) – `defineAction()` und `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) – Auswahl, Navigation, Ansichtsbildschirm
- [**Agent-Ressourcen**](/docs/agent-resources) – was die Registerkarte „Agent-Ressourcen“ enthält (skills, Speicher, MCP-Server, geplante Jobs)
