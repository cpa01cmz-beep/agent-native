...
### Feedback in PostHog {#posthog-feedback}

Daumen, Kategorie und Freitext senden alle ein `$ai_feedback`. Die Feedback-Ansicht der LLM-Analytics von PostHog liest stattdessen ein `survey sent`-Ereignis. Setze deshalb `POSTHOG_AI_FEEDBACK_SURVEY_ID` auf die Umfrage, die PostHog über den Tab **Feedback** eines Trace anlegt. Diese eine ID ist die gesamte Konfiguration; ohne sie wird kein Umfrage-Ereignis gesendet.

Eine Daumenbewertung beantwortet die erste Frage der Umfrage als PostHogs Auswahlindex (`1` hoch, `2` runter), der Freitext nach einem Daumen nach unten beantwortet die Rückfrage — der Text landet also nie dort, wo die Bewertung hingehört. Beide tragen eine Submission-ID pro bewerteter Nachricht, so fügt PostHog sie zu einer Antwort zusammen, und ein Daumen nach unten gilt bis zum Eintreffen seiner Rückfrage als unvollständig.
...
<Diagram id="doc-block-5k287n" title="Zuweisung konsistenter Hash-Varianten" summary={"Jeder Benutzer hasht eine stabile Variante, die Schleife wendet die Konfigurationsüberschreibung dieser Variante an und die Ergebnisse werden pro Variante mit Konfidenzintervallen zusammengefasst."}>

```html
<div class="exp">
  <div class="diagram-node">
    Benutzer-ID<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">Konfigurations-Override A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">Konfigurations-Override B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    Ergebnisse per variant<br /><small class="diagram-muted"
...
Wird von einem Server-Plugin mit `defineAppConfig()` gesetzt. Jedes Feld hat außerdem einen Alias als Deployment-Umgebungsvariable – siehe die Referenz der deklarierten Konfiguration.
...
Senden Sie Traces an Langfuse, Datadog, Grafana oder ein beliebiges OTel-kompatibles Backend, indem Sie in Ihrer eigenen App einen OpenTelemetry-Provider registrieren – siehe [OpenTelemetry-Spans](#otel) unten.

Der Kern liefert bewusst keinen Exporter und hält weder Export-Endpunkt noch Token. Die Backend-Zugangsdaten leben daher in Ihrer Provider-Verkabelung (und im Vault), niemals in der Framework-Konfiguration.

## OpenTelemetry-Bereiche {#otel}

Die Agentenschleife gibt **Live-OpenTelemetry-Spans** für jede Ausführung, jeden Modellaufruf und jeden Tool-Aufruf aus – so sieht ein Host, der bereits einen OTel-Collector ausführt, die Agentenaktivität neben dem Rest seiner verteilten Traces.
...
- [**Actions**](/docs/actions-overview) – die Vorgänge, die als Werkzeugaufrufe in Traces erscheinen
