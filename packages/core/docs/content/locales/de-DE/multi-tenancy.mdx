...
Ein neues `npx --yes @agent-native/core@latest create`-Gerüst wird bereits geliefert mit:
...
<Diagram id="doc-block-i77hp" title="Von der Sitzung zum bereichsbezogenen SQL" summary={"Die aktive Organisation in der Sitzung wird zu AGENT_ORG_ID, die das Framework in die WHERE-Klausel jeder Abfrage einfügt."}>
...
Verwenden Sie `accessFilter()` und `assertAccess()` für serverseitige Lese- und Schreibvorgänge. Die Agent-SQL-Einschränkung erstellt temporäre gefilterte Views für eigene Tabellen. `pnpm guards` enthält die Prüfung auf nicht eingeschränkte Abfragen. RLS ist derzeit kein aktiver Durchsetzungsmechanismus und als Härtung nach 1.0 geplant. Siehe [Verwaltete Bereitstellungen](/docs/administered-deployments).
