...
> **Welches Arbeitsbereichsdokument?** Diese Seite behandelt die **Bereitstellungsform** – ein Monorepo, viele Apps, gemeinsame Authentifizierung und eine einheitliche Bereitstellung. Informationen zur Anpassungsebene des Agenten (Agent-Ressourcen: `AGENTS.md`, Speicher, skills, benutzerdefinierte Agenten) finden Sie unter [Agent-Ressourcen](/docs/agent-resources); Informationen zur Governance (wer überprüft, genehmigt und besitzt was) finden Sie unter [Workspace Governance](/docs/workspace-management).

## Standalone oder Workspace? {#standalone-or-workspace}

Wählen Sie einen Workspace, wenn ein Unternehmen mehrere interne Apps mit gemeinsamer Bereitstellungskonfiguration und Organisationskontext betreibt. Eine Standalone-App eignet sich für ein einzelnes unabhängiges Tool oder einen Prototyp. Sie ist ein unterstütztes Bereitstellungsmodell, bietet aber keine gemeinsamen Workspace-Pakete oder Root-Konfiguration. Siehe [Verwaltete Bereitstellungen](/docs/administered-deployments).
...
<FileTree
  id="doc-block-9lp73p"
  title="Ein generierter Workspace"
  entries={[
    {
      path: "package.json",
      note: "Deklariert agent-native.workspaceCore",
    },
    {
    ...
    {
      path: ".env.example",
      note: "Gemeinsame ANTHROPIC_API_KEY, A2A_SECRET, DATABASE_URL, ...",
    },
    {
    ...
    },
    {
      ...
      note: "Plugin-Overrides nur bei Bedarf",
    },
    {
      path: "packages/shared/src/client/",
      note: "Gemeinsamer React-Code nur bei Bedarf",
    },
    {
      path: "packages/shared/AGENTS.md",
      note: "Workspace-weite Anweisungen",
    },
    {
      path: "apps/mail/",
    },
...
<Diagram id="doc-block-742pxc" title={"Drei Ebenen, nach Dateinamen zusammengeführt"} summary={"Jede App löst Plugins, Skills, Aktionen und AGENTS.md zuerst aus App-Local, dann aus dem freigegebenen Paket und dann aus dem Framework-Standard auf."}>
...
Dispatch-Ressourcen sind auf **Alle Apps** (jede App erbt sie zur Laufzeit, kein Kopier- oder Synchronisierungsschritt) oder auf **Ausgewählte Apps** (je App für app-spezifischen Kontext gewährt) beschränkt. Unter [Agent-Ressourcen](/docs/agent-resources#global-resources) finden Sie die vollständige Ressourcenmodelltabelle, Pfadkonventionen und das empfohlene Starterpaket.
...
<Diagram id="doc-block-18vbd0h" title="Einheitliche Bereitstellung: ein Ursprung, ein Pfad pro App" summary={"Jede App wird hinter einem einzigen Ursprung ausgeliefert, sodass Anmeldesitzungen und App-übergreifendes A2A kostenlos sind."}>
...
- [Agent-Ressourcen](/docs/agent-resources) – die Anpassungsebene (`AGENTS.md`, `LEARNINGS.md`, persönlicher Speicher, skills, benutzerdefinierte Agenten), die jede App im Arbeitsbereich gemeinsam nutzt.
