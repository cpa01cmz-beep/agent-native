...
<Diagram id="doc-block-jraeqk" title="Die Setup-Checkliste" summary={"Es ist lediglich das Anschließen einer KI-Engine erforderlich. Das Panel verfolgt den Abschluss und wird automatisch ausgeblendet, sobald alles Erforderliche erledigt ist."}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span
    ><strong>AI-Engine verbinden</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>E-Mail-Zustellung</strong
...
<Endpoint id="doc-block-jqj1ok" title="Listen Sie die Onboarding-Schritte auf" method="GET" path="/_agent-native/onboarding/steps" summary="Listen Sie alle registrierten Schritte mit ihrem Abschlussstatus auf" responses={[
  {
    "status": "200",
...
<AnnotatedCode
  id="doc-block-7g6056"
  title="Registrieren eines benutzerdefinierten Onboarding-Schritts"
  ...
  annotations={[
    {
      lines: "5",
      label: "Automatisch montiert",
      note: "Registrieren Sie sich über ein Nitro-Plugin – das Framework übernimmt das Rendern, die Abschlussverfolgung und die Entlassung.",
    },
    {
      lines: "7",
      label: "Stabile ID",
      note: "Die erneute Registrierung bei demselben `id` nach dem Laden der Standardeinstellungen überschreibt einen integrierten Schritt.",
    },
    {
      lines: "12-19",
      label: "Primäre Methode",
      note: '`primary: true` marks the big CTA. `kind: "link"` sends the user into the OAuth flow.',
    },
    {
      lines: "20-26",
      label: "Delegierter Pfad",
      note: '`kind: "agent-task"` übergibt die Einrichtung mit einem Prompt an den Agenten-Chat.',
    },
    {
      lines: "28-31",
      label: "Abschlussprüfung",
      note: "`isComplete` läuft serverseitig. OAuth-Tokens leben im `oauth_tokens`-Store – überprüfen Sie es, nicht `process.env.GMAIL_REFRESH_TOKEN`.",
    },
  ]}
/>
...
| `database` | nein         | Gehostete PostgreSQL-URL oder leer für lokales PGlite |
