...
`source` ist entweder das `ctx` einer Action, direkt durchgereicht, oder ein Meta-Objekt `{ userId, anonymousId, sessionId }` für Aufrufer ohne `ctx`.

Die Browser-Session muss der Aufrufer nicht selbst mitgeben: Sie stammt aus dem umgebenden Request-Kontext (`RequestContext.browserSessionId`, gesetzt aus dem Header `X-Agent-Native-Session-Id`, den Action-Client, Chat und das Browser-`track()` senden). Dieselbe Sitzung wird dadurch korreliert, egal ob die Oberfläche oder der Agent das Ereignis ausgelöst hat. Jeder Anbieter bildet sie auf sein eigenes Session-Feld ab: `$session_id` bei PostHog, wodurch das Ereignis mit dem Session Replay verknüpft wird; eine `session_id`-Eigenschaft bei Mixpanel und Amplitude; ein `sessionId` auf oberster Ebene bei Webhooks und Agent-Native Analytics. Für Aufrufer ohne Browser (Cron, CLI, MCP, A2A) fehlt sie, und ein `sessionId` im Meta-Objekt überschreibt sie.
...
export default defineAction({
  ...
  run: async ({ name, template }, ctx) => {
    ...
    // ctx direkt durchreichen: Das Ereignis wird ctx.userEmail zugeordnet und
    // der Browser-Session zugeordnet, aus der der Aufruf kam.
...
- [**Actions**](/docs/actions-overview) – woher die meisten Tracking-Aufrufe kommen
- [**Server Plugins**](/docs/server-plugins) – `registerBuiltinProviders()` wird beim Start im Core-Routes-Plugin ausgeführt
