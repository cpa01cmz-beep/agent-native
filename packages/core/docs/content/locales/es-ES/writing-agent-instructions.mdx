---
title: "Instrucciones del agente de escritura y Habilidades"
...
# Instrucciones del agente de escritura y Habilidades
...
<Diagram id="doc-block-1mdys4x" title={"Tres superficies creadas + una superficie de tiempo de ejecución"} summary={"AGENTS.md y las descripciones de las herramientas se cargan en cada vuelta; carga de habilidades según demanda; application_state está escrito en vivo por su UI."}>

```html
<div class="diagram-surfaces">
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">Cada turno</span><strong>AGENTS.md</strong
    ><small class="diagram-muted"
      >el mapa: propósito, reglas base, claves de estado, índice de acciones +
      skills</small
    >
  </div>
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">Cada turno</span
    ><strong>Descripciones de herramientas</strong
    ><small class="diagram-muted"
      >guía la selección de herramientas — una frase precisa cada una</small
    >
  </div>
  <div class="diagram-card ondemand" data-rough>
    <span class="diagram-pill">Bajo demanda</span><strong>Habilidades</strong
    ><small class="diagram-muted"
      >guía profunda, cargada cuando coincide la descripción</small
    >
  </div>
  <div class="diagram-card runtime" data-rough>
    <span class="diagram-pill ok">Live</span><strong>application_state</strong
    ><small class="diagram-muted"
      >lo escribe tu UI: navegación, selección, foco</small
    >
  </div>
</div>
```
...
- **Índice Habilidades**: una lista de los skills que existen y cuándo leer cada uno.
...
- Data lives in SQL vía Drizzle. Use actions for all writes.
...
`AGENTS.md` es el estándar universal sobre el que se construye el framework, así que cualquier herramienta de codificación con IA que lea instrucciones del proyecto funciona sin configuración adicional:

- **Claude Code**: lee `CLAUDE.md` (enlazado simbólicamente desde `AGENTS.md` por la configuración de la CLI)
- **Cursor**: lee `AGENTS.md` directamente, o `.cursorrules` (la ubicación heredada de Cursor) si está presente
- **Windsurf**: lee `AGENTS.md` directamente, o `.windsurfrules` (la ubicación heredada de Windsurf) si está presente
- **Codex, Gemini y otros**: funcionan a través del panel de agente integrado
...
├── SKILL.md            # regla + camino feliz + do/don't
└── references/
    └── csv-format.md   # especificaciones de columna completa, codificaciones, casos extremos
```

Esto mantiene pequeña la superficie siempre cargada y permite escalar la profundidad sin un contexto abultado. Consulte [Habilidades Guide](/docs/skills-guide) para conocer el formato completo de las habilidades.
...
## Habilidades frente a actions {#skills-vs-actions}

Habilidades y actions son complementarios. Una habilidad es una guía que el agente lee; un
...
| Habilidades (`SKILL.md`)                 | Tú (desarrollador)                    | A pedido, cuando el agente decide que la habilidad es relevante | Instrucciones paso a paso para un patrón específico, listas de lo que se debe/no se debe hacer |
...
- **Habilidades**: instrucciones reutilizables para un patrón específico, cargadas bajo demanda. Se aplica a todos los que trabajan en la aplicación.
- **Memoria (`memory/MEMORY.md`)**: preferencias y correcciones por usuario, no orientación escrita.

## ¿Qué sigue? {#whats-next}

- [Habilidades Guide](/docs/skills-guide): el formato de archivo de habilidades, el marco skills y el skills respaldado por la aplicación.
