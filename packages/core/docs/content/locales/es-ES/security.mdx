...
<Diagram id="doc-block-1w0b1oh" title="Defensa en capas" summary={"El marco posee la mayor parte de la superficie de amenazas; usted posee dos cosas: etiquetar tablas para determinar el alcance y validar entradas externas."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">Lo controla el framework</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">Tú lo controlas</span
    ><small class="diagram-muted"
...
El marco crea el contexto de solicitud desde la sesión y aplica la propiedad con `accessFilter()` y `assertAccess()` en las lecturas y escrituras de la aplicación. El SQL del agente recibe vistas temporales filtradas para la identidad y organización activas. RLS aún no está habilitado, por lo que las consultas directas de la aplicación deben seguir usando estos asistentes.
...
<Callout id="doc-block-1qpbnrj" tone="risk">

Nunca construyas SQL mediante concatenación de cadenas o literales de plantilla. Pase la entrada del usuario como `args` a `exec` / `db-query`, o use Drizzle; ambos siempre parametrizan. Las comprobaciones `pnpm guards` detectan consultas concatenadas y sin ámbito en el momento de CI.
...
<Diagram id="doc-block-1qtj4kn" title="El canal de alcance" summary={"El agente SQL nunca toca las tablas base directamente: lee una vista temporal cuyo ámbito es la identidad actual, por lo que un nombre de tabla simple solo puede devolver filas propias."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    Sesión iniciada<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    SQL del agente<br /><small class="diagram-muted"
...
Las herramientas `db-query` / `db-exec` rechazan referencias a tablas calificadas por esquema (`public.<table>`): un nombre calificado se resuelve en la tabla base y omitiría la vista temporal anterior. Los agentes utilizan nombres de tablas simples; el alcance se aplica automáticamente.
...
<DataModel
  id="doc-block-1e2bdss"
  title="What ownableColumns() adds"
  summary="Las tres columnas que hacen que una tabla sea compatible con los inquilinos y se pueda compartir."
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Organización activa del propietario en el momento de la creación. Impulsa las comprobaciones de visibilidad de la organización.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "privado | organización | público: valor predeterminado aproximado, el valor predeterminado es privado.",
        },
      ],
    },
  ]}
...
Las credenciales por usuario/organización y la bóveda se cifran en reposo con AES-256-GCM. La bóveda compartida del espacio de trabajo prefiere `WORKSPACE_SECRETS_ENCRYPTION_KEY`, seguida de candidatos heredados derivados de `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET` y `BETTER_AUTH_SECRET`; durante una rotación, `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` mantiene legibles las filas antiguas hasta migrarlas. Para cifrar filas de credenciales de texto plano existentes, ejecute `pnpm action db-migrate-encrypt-credentials` (idempotente, no destructivo).
...
Los controladores de webhooks entrantes (Resend, SendGrid, Slack, Telegram, WhatsApp, Recall.ai, Deepgram, Zoom) rechazan solicitudes falsificadas de forma predeterminada en producción: cuando falta la var de entorno secreta de firma correspondiente, el controlador devuelve 401 en lugar de aceptar y enviar.
...
Las notificaciones `changes.watch` de Google Drive no pasan por esa puerta de enlace de adaptadores genérica: son solicitudes opacas sin cuerpo. Por eso el framework registra un token aleatorio por canal y verifica las cabeceras `X-Goog-Channel-ID`, `X-Goog-Channel-Token` y `X-Goog-Resource-ID` antes de obtener los cambios. Son notificaciones directas de Drive, no mensajes de Pub/Sub, y no contienen tokens OIDC de Pub/Sub.
...
- [ ] `WORKSPACE_SECRETS_ENCRYPTION_KEY` establecido de forma consistente para bóvedas compartidas multiapp; el cifrado local conserva los respaldos `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET`: consulte [Secrets Management](#secrets)
