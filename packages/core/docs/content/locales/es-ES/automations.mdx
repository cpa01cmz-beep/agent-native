...
Una **automatización** es una tarea que el agente ejecuta según un horario o cuando ocurre un evento: _cuando sucede X, haz Y_. **Scheduled** y **Event** son dos tipos de activador, no productos separados.

El framework exponía originalmente las automatizaciones programadas como **Recurring Jobs**. Ese nombre, la ruta `jobs/<name>.md` y la herramienta `manage-jobs` se mantienen por compatibilidad. La nueva guía usa **Automations** como concepto general; consulte [Recurring Jobs](/docs/recurring-jobs) para mantener un trabajo programado antiguo o usar el ámbito de organización y campos avanzados.

<Diagram id="doc-block-1kgnnem" title="Cuando suceda X, haz Y" summary={"Se activa un evento en el autobús, una condición opcional de lenguaje natural lo bloquea y el agente ejecuta el cuerpo de automatización con acceso completo a las herramientas."}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">El agente ejecuta el cuerpo</span
    ><small class="diagram-muted"
...
### Desde la interfaz de Automations

Abra **Automations** en la página completa del agente para ver y administrar automatizaciones programadas y activadas por eventos. La URL de compatibilidad sigue siendo `/agent#jobs`, por lo que los enlaces y marcadores existentes continúan funcionando.

<Callout tone="warning">

`manage-automations` admite automatizaciones personales y de organización para activadores Scheduled y Event. Las automatizaciones de eventos de una organización son visibles para sus miembros, pero siempre se ejecutan como su creador; los administradores pueden gestionarlas sin cambiar esa identidad.
...
<AnnotatedCode
  id="doc-block-1y316w7"
  title={"Una automatización activada por eventos"}
  ...
  annotations={[
    {
      lines: "2",
      label: "Sin cron",
      note: 'Los disparadores de evento establecen `schedule` en `""`; el campo cron queda vacío.',
    },
    {
      lines: "4-5",
      label: "El gatillo",
      note: "`triggerType: event` más el nombre `event` suscribe esta automatización al bus.",
    },
    {
      lines: "6",
      label: "Puerta",
      note: "Un `condition` opcional en lenguaje natural, evaluado por Haiku con respecto a la carga útil antes del envío.",
    },
    {
      lines: "12",
      label: "Secreto del lado del servidor",
...
| `mode`        | `"agentic"`               | `"agentic"`    | Bucle de agente completo (el único modo compatible). `"deterministic"` fue eliminado: nunca se implementó, y las automatizaciones que lo configuraban nunca se ejecutaban. |
...
El `owner` de los metadatos de emisión determina qué automatizaciones se activan. Las automatizaciones de eventos personales y de organización requieren una coincidencia exacta con `createdBy`.
...
Las automatizaciones personales y de organización usan `manage-automations` con un parámetro `action`:
...
`manage-automations` admite ámbito de organización, anulaciones de modelo y listas permitidas de MCP que se aplican de forma estricta. `manage-jobs` se mantiene para integraciones antiguas solo programadas y metadatos de entrega.
...
<Endpoint id="doc-block-91d4v1" title="Disparar un evento de prueba" method="POST" path="/_agent-native/automations/fire-test" summary="Emitir un evento test.event.fired para validar automatizaciones activadas por eventos" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-lh7yi0" title="La ruta de despacho" summary={"Desde un evento activado hasta una ejecución de agente completa, controlado por el alcance de propiedad y la condición del lenguaje natural."}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> con body como prompt, payload como contexto y
      timeout de 5 minutos</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-overview): las automatizaciones pueden invocar cualquier acción registrada a través del bucle del agente
