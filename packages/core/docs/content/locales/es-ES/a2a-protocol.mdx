...
<Diagram id="doc-block-1aru0jq" title="Un agente le entrega el trabajo a otro" summary={"Un agente de correo descubre la tarjeta del agente de análisis, envía un mensaje JSON-RPC y recupera una tarea completada."}>

```html
<div class="diagram-handoff">
  <div class="diagram-card">
    <strong>Agente de Mail</strong
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-pill ok">tarea · completada</div>
  </div>
  <div class="diagram-card" data-rough>
    <strong>Agente de Analytics</strong
    ><small class="diagram-muted"
      >ejecuta run-query y devuelve el resultado</small
    >
  </div>
</div>
```
...
<Endpoint id="doc-block-189x3y0" title="Endpoint A2A principal" summary={"Todos los métodos JSON-RPC son POSTed aquí. Se muestra message/send."} method="POST" path="/_agent-native/a2a" summary={"Envía un mensaje y espera a que se complete la tarea."} auth="JWT bearer signed with A2A_SECRET (or legacy apiKeyEnv static token)" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-1hp5wot" title={"Ciclo de vida de tareas asíncronas sin servidor"} summary={"async:true vuelve a funcionar en milisegundos, luego una nueva ejecución ejecuta el bucle del agente mientras la persona que llama sondea."} caption="A recurring sweeper re-claims any task left in flight if the function execution dies mid-run.">
...
<div class="diagram-async">
  ...
  <div class="diagram-panel">
    ...
    ><small class="diagram-muted">~milisegundos</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>
    POST autoactivado /_agent-native/a2a/_process-task<br /><small
      class="diagram-muted"
      >token HMAC · ejecución nueva · timeout completo</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
...
<AnnotatedCode
  id="doc-block-pf4y3d"
  title="Mensaje A2A con partes escritas"
  language="json"
  code={
  ...
  annotations={[
    {
      lines: "4",
      label: "text part",
      note: "Instrucciones sencillas en lenguaje natural que el agente lee.",
    },
    {
      lines: "5",
      label: "data part",
      note: "Argumentos JSON estructurados, por ejemplo un rango de fechas, pasados junto con el prompt.",
    },
    {
      lines: "6-9",
      label: "file part",
      note: "Adjunte un archivo por nombre, `mimeType` y base64 `bytes`.",
    },
  ]}
/>
...
## Conexiones de agentes alojados {#hosted-agent-connections}

Un agente alojado puede participar en A2A cuando expone un punto de conexión A2A
compatible. Un proveedor de modelos o un SDK de agentes por sí solo no es un
par A2A. Mantenga la interfaz, las acciones y la base de datos de la aplicación
en su host habitual y conecte el entorno del agente mediante un adaptador o su
punto de conexión A2A.

Una conexión alojada tiene metadatos del punto de conexión y un descriptor de
autenticación. Guarde estos metadatos con el recurso del agente remoto o el
registro de conexión. Nunca incluya un token ni un secreto de cliente en el
manifiesto:
...
`url` es el punto de conexión de invocación o la URL base. `cardUrl` es opcional
cuando el descubrimiento usa `/.well-known/agent-card.json`. Úselo cuando el
proveedor publique su tarjeta en otra ruta. El cliente lee la versión del
protocolo en la tarjeta del agente. Si la tarjeta del proveedor no la incluye,
pase `protocolVersion` directamente a `A2AClient`. `auth` describe cómo el
código del servidor obtiene un token de portador. Los descriptores admitidos son `bearer`
con un `credentialRef` almacenado en la bóveda, o `oauth-client-credentials`
con `tokenUrl`, `clientId`, `clientSecretRef` y `scope`. `credentialRef` y
`clientSecretRef` son referencias a secretos, nunca los valores de los secretos.

### A2A con portador genérico {#generic-bearer-a2a}

Use el cliente estándar cuando el par acepte `Authorization: Bearer ...`,
sirva una tarjeta compatible y acepte JSON-RPC de Agent-Native:
...
Resuelva `token` en el servidor mediante la conexión del espacio de trabajo o
la bóveda. No lo envíe desde código del navegador, no lo escriba en un
manifiesto ni lo incluya en un prompt. Una clave API estática solo es una
credencial de transporte para el servicio remoto. No establece la identidad del
llamador de Agent-Native ni transporta `approvedActions`.

### Microsoft Foundry {#microsoft-foundry}

[Los agentes alojados de Azure AI Foundry](https://learn.microsoft.com/en-us/azure/foundry/agents/overview)
ejecutan su código o contenedor de agente detrás de un punto de conexión, una
identidad, escalado, estado de sesión y observabilidad administrados. Este
modelo aloja el entorno de ejecución del agente. Mantenga la interfaz, las
acciones y el despliegue PostgreSQL de Agent-Native en el host de la aplicación
y llame al punto de conexión de Foundry desde código del servidor.

Foundry puede exponer un punto de conexión A2A mediante [Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/enable-agent-to-agent-endpoint).
Requiere autenticación de Microsoft Entra y el llamador necesita permiso
Foundry Agent Consumer. La versión 1.0 es el punto de conexión JSON-RPC de GA;
use la URL de tarjeta `.../agents/{agent}/endpoint/protocols/a2a/agentCard/v1.0` cuando esté disponible. La versión
0.3 es el punto de conexión de vista previa que se obtiene cuando no se
selecciona una versión. Pase `protocolVersion` a `A2AClient` solo cuando la
tarjeta omita la versión. Foundry v1 no ofrece transmisión SSE, por lo que el
cliente usa `message/send`. El proveedor de tokens debe obtener y renovar un
token de acceso de Entra. `A2A_SECRET` y una clave API estática no satisfacen la
autenticación de Foundry.

Para las credenciales de cliente de una entidad de servicio, solicita un token
de Entra con el ámbito `https://ai.azure.com/.default` y asigna al llamador el
rol **Foundry Agent Consumer** en el ámbito del proyecto o del agente. Guarda el
secreto de cliente en el vault y haz referencia a él en el manifiesto:
...
[Gemini Enterprise puede registrar agentes A2A externos](https://docs.cloud.google.com/gemini/enterprise/docs/register-and-manage-an-a2a-agent),
y sus asistentes gestionados exponen un punto de conexión JSON-RPC estándar de
A2A. El punto de conexión está bajo el recurso del asistente, por ejemplo
`.../assistants/default_assistant/agents/{id}/a2a`, mientras que la tarjeta del
agente puede publicarse en una URL personalizada. Use el cliente de portador
genérico y pase esa URL de tarjeta:
...
El llamador necesita un token de portador OAuth de Google con el permiso
`discoveryengine.assist`. El registro personalizado de agentes A2A es Pre-GA,
así que confirme que está disponible en el proyecto de destino antes de
mostrarlo en el selector del espacio de trabajo. Consulte [invocar un agente de Gemini Enterprise](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)
para conocer la URL del recurso y los requisitos de IAM.

### Anthropic Managed Agents {#anthropic-managed-agents}

Anthropic Managed Agents no expone un punto de conexión A2A entrante. Use el
adaptador del framework cuando Agent-Native deba ser propietario de la
superficie A2A, el estado del flujo de trabajo y las aprobaciones, mientras el
agente alojado sea propietario de su sesión, contexto y herramientas. El
adaptador crea una sesión de Managed Agents, envía eventos `user.message` y lee
el flujo de eventos de la sesión. Un evento bloqueante `agent.tool_use` o
`agent.mcp_tool_use` se convierte en una tarea A2A con estado `input-required` y
en un evento de ejecución `approval-request`. Reanude la sesión con
confirmaciones estructuradas y explícitas en los metadatos devueltos; nunca
infiera una aprobación a partir de un mensaje:
...
`credentialRef` es una referencia de la bóveda a la clave de API de Anthropic.
El adaptador la resuelve dentro del ámbito del usuario y la organización del
llamador y envía los encabezados requeridos `anthropic-beta:
managed-agents-2026-04-01` y `anthropic-version: 2023-06-01`. Registre
`anthropic-managed-agents` en el catálogo de conexiones del espacio de trabajo
para reutilizar las concesiones de conexión existentes por aplicación. El
adaptador se monta en el punto de extensión normal `A2AConfig.handler`:
...
// pase `handler` en la A2AConfig que se entrega a mountA2A()
```
...
## Actividad del agente delegado {#delegated-agent-activity}

Los pares de Agent-Native pueden adjuntar a los mensajes de estado de la tarea
una parte `data` opcional con `kind: "agent-native/agent-activity"`. Contiene
una instantánea acotada de los resúmenes de razonamiento visibles para el
usuario, los nombres y estados de las herramientas, el tiempo transcurrido y el
texto progresivo de la respuesta. La aplicación que llama muestra estas
actualizaciones como una ejecución de agente anidada y contrae el historial de
trabajo cuando termina la tarea delegada.

La parte de actividad nunca incluye entradas ni resultados de herramientas,
credenciales o razonamiento oculto del proveedor. Trátala como datos de
presentación no fiables, nunca como identidad, autorización, aprobación,
enrutamiento o validación de artefactos. Los pares A2A externos pueden omitir la
extensión; su estado normal y su respuesta final siguen apareciendo en la misma
vista de agente delegado.
...
### Sincronización de secretos de la organización {#organization-secret-sync}

La configuración **Organización → Autenticación entre aplicaciones** administra el secreto con ámbito de organización que autentica la delegación entre aplicaciones implementadas por separado en la misma organización. No es una configuración de inicio de sesión de usuarios ni de Cross-App SSO. El secreto complementa el `A2A_SECRET` de la implementación: un JWT de A2A con una reclamación `org_domain` se puede verificar con el secreto de la organización correspondiente.

El propietario de la organización debe asignar a cada aplicación participante el mismo dominio de correo permitido y el mismo secreto con ámbito de organización:

1. En la primera aplicación, genere o revele el secreto en **Organización → Autenticación entre aplicaciones**.
2. En cada aplicación adicional, elija **Pegar secreto de otra aplicación** y pegue ese valor. Este paso manual es necesario para la primera conexión: una aplicación que aún no tiene el secreto no puede verificar una solicitud de sincronización remota.
3. Cuando las aplicaciones compartan el secreto, use **Sincronizar con aplicaciones** para enviar futuras actualizaciones a las aplicaciones conectadas y detectadas. El resultado informa de cualquier aplicación que no se pudo actualizar.

Al **Regenerar** el secreto, la interfaz de configuración lo sincroniza de inmediato y firma la actualización con el secreto anterior para que las aplicaciones ya conectadas puedan verificar el cambio. Si una aplicación pierde esa sincronización, pegue allí el nuevo secreto manualmente y vuelva a sincronizar. Trate este secreto como cualquier otra credencial: no lo incluya en código del cliente, registros ni configuraciones de clientes de terceros.
