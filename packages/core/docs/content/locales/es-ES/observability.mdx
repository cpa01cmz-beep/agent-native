...
<Diagram id="doc-block-w82uj9" title="Cada carrera alimenta el bucle" summary={"La ejecución de un agente produce un seguimiento, puntuaciones automatizadas y un enlace de retroalimentación, todo almacenado en el SQL de la aplicación y que aparece en el panel. Los experimentos dividen el tráfico entre variantes de configuración."}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    Ejecución del agente<br /><small class="diagram-muted"
      >production-agent.ts</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Capturado automáticamente</span
...
### Comentarios en PostHog {#posthog-feedback}

Los pulgares, las categorías y el texto libre emiten todos `$ai_feedback`. En cambio, la vista de comentarios del análisis de LLM de PostHog lee un evento `survey sent`, así que asigna a `POSTHOG_AI_FEEDBACK_SURVEY_ID` la encuesta que PostHog crea desde la pestaña **Feedback** de un seguimiento. Ese único id es toda la configuración; sin él no se envía ningún evento de encuesta.

Un voto con el pulgar responde a la primera pregunta de la encuesta con el índice de opción de PostHog (`1` arriba, `2` abajo), y el texto libre tras un pulgar hacia abajo responde a la repregunta, de modo que el texto nunca cae donde va la valoración. Ambos llevan un mismo id de envío por mensaje valorado, que es como PostHog los une en una sola respuesta, y un pulgar hacia abajo queda marcado como incompleto hasta que llega su repregunta.
...
<Diagram id="doc-block-16c50bi" title={"Asignación de variante de hash consistente"} summary={"Cada usuario aplica un hash a una variante estable, el bucle aplica la anulación de configuración de esa variante y los resultados se acumulan por variante con intervalos de confianza."}>

```html
<div class="exp">
  <div class="diagram-node">
    ID de usuario<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">sobrescritura de configuración A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">sobrescritura de configuración B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
Se establece desde un complemento de servidor con `defineAppConfig()`. Cada campo también tiene un alias de variable de entorno de despliegue: consulta la referencia de configuración declarada.
...
Envíe seguimientos a Langfuse, Datadog, Grafana o cualquier backend compatible con OTel registrando un proveedor de OpenTelemetry en su propia aplicación: consulte [Extensiones de OpenTelemetry](#otel) más abajo.

El núcleo no incluye ningún exportador ni almacena un punto final o token de exportación, por lo que las credenciales del backend viven en la configuración de su proveedor (y en la bóveda), nunca en la configuración del marco.

## Extensiones de OpenTelemetry {#otel}

El bucle del agente emite **intervalos de OpenTelemetry en vivo** para cada ejecución, llamada de modelo y llamada de herramienta, de modo que un host que ya ejecuta un recopilador OTel ve la actividad del agente junto con el resto de sus seguimientos distribuidos.
...
- [**Actions**](/docs/actions-overview): las operaciones que aparecen como llamadas a herramientas en los seguimientos
