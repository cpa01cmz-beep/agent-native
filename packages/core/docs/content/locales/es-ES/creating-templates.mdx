...
Ten presentes las tres capas al decidir dónde pertenece una capacidad:

- **Core - el framework:** el contrato obligatorio de runtime y datos que cualquier app puede usar.
- **Toolkit - piezas reutilizables opcionales:** UI y sistemas de producto compartidos que las apps pueden adoptar, componer o bifurcar.
- **Templates - apps opcionales:** productos de dominio completos construidos sobre Core, que suelen usar Toolkit y pueden bifurcarse como punto de partida.

Toolkit y Templates son opcionales. Un template no es el framework, y Toolkit no sustituye a Core.

Una buena plantilla:

- Resuelve un flujo de trabajo de principio a fin, con datos iniciales útiles o un flujo de estado vacío.
- Almacena el estado duradero en PostgreSQL/PGlite, no en archivos JSON.
...
Chat le brinda autenticación, hilos de chat duraderos, recursos respaldados por PostgreSQL, herramientas, estado de la aplicación, actions y sincronización de sondeo. Agregas el modelo de dominio y el producto UI.

Si todavía no está creando una plantilla de UI reutilizable, use [Aplicaciones orientadas a la automatización](/docs/pure-agent-apps): defina acciones, ejecútelas sin un navegador y añada UI más adelante cuando el flujo de trabajo necesite una superficie duradera.
...
<FileTree
  id="doc-block-1qe28n0"
  title="Estructura del proyecto de plantilla"
  entries={[
    {
      path: "app/",
      note: "Frontend React",
    ...
    {
      path: "app/root.tsx",
      note: "Shell HTML y proveedores",
    },
    {
      path: "app/routes/",
      note: "Rutas de archivo de React Router",
    },
    {
      path: "app/components/",
      note: "UI de la plantilla",
    },
    {
      path: "app/hooks/",
      note: "Hooks de estado y datos de la UI",
    },
    {
      path: "actions/",
      note: "Operaciones defineAction: la única fuente de verdad",
    },
    {
      path: "server/db/schema.ts",
      note: "Esquema Drizzle",
    },
    {
      path: "server/plugins/db.ts",
      note: "Migraciones aditivas",
    },
    {
      path: "server/plugins/",
      note: "Integraciones de arranque",
    },
    {
      path: "server/routes/api/",
      note: "Rutas personalizadas solo cuando las actions no bastan",
    },
    {
      path: "shared/types.ts",
      note: "Tipos compartidos cliente/servidor",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: guía del agente para flujos de trabajo complejos",
    },
    {
      path: "AGENTS.md",
      note: "Instrucciones de agente específicas de la plantilla",
    },
    {
      path: "package.json",
    },
...
No agregue un directorio `data/` para el estado de la aplicación. Los datos duraderos de la aplicación pertenecen a PostgreSQL/PGlite y el UI los lee a través de actions o controladores de servidor escritos.

Las cuatro áreas de cada plantilla se conectan a través de una superficie de acción compartida y una base de datos PostgreSQL/PGlite: el agente y el UI son socios iguales en las mismas operaciones:

<Diagram id="doc-block-1reooa2" title={"Cómo se conectan las cuatro áreas de una plantilla"} summary={"Tanto la UI como el agente llegan a PostgreSQL/PGlite mediante las mismas acciones; el estado de la aplicación y la sincronización del sondeo los mantienen alineados."}>
...
<div class="diagram-tmpl">
  ...
  <div class="diagram-box" data-rough>
    PostgreSQL vía Drizzle<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">Sondeoing sync</div>
</div>
```
...
## Datos del modelo en PostgreSQL {#data-models}

Defina tablas de dominio con los asistentes del marco Drizzle para que los esquemas utilicen PostgreSQL y PGlite:
...
Los cambios de esquema deben ser aditivos. Para nuevas tablas y columnas propias de
la aplicación, use `drizzle-kit generate` para crear SQL revisable en
`server/db/migrations/<id_name>.sql` y cárguelo con
`runDrizzleMigrations()` desde el paso de release en despliegues Node.js.
Cloudflare Workers/Pages y otros runtimes edge no tienen un sistema de archivos desplegable. Allí,
integre el SQL generado en el bundle del worker y páselo a `runMigrations()`.
Mantenga `runMigrations()` para el historial escrito a mano y casos especiales.
Nunca utilice SQL destructivo, `drizzle-kit push`, cambios de nombre de tablas ni
eliminación de columnas.

Para lecturas y escrituras de aplicaciones, utilice el generador de consultas de Drizzle y los operadores compatibles con PostgreSQL de `drizzle-orm`. No escriba código de producto con SQL sin formato cuando Drizzle pueda expresar la consulta y use en las plantillas los ayudantes de esquema de `@agent-native/core/db/schema`.
...
Utilice los documentos [Database](/docs/server-database) y [Security](/docs/security) antes de agregar esquemas que contengan datos de usuarios u organizaciones.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    {
      lines: "8",
      label: "Superficie de la herramienta",
      note: "`description` es lo que lee el agente para decidir cuándo llamar a esta acción como herramienta.",
    },
    {
      lines: "9-11",
      label: "Contrato tipado",
      note: "Un zod `schema` valida la entrada del agente, la UI, HTTP, MCP y A2A.",
    },
    {
      lines: "18-19",
      label: "escritura con alcance",
      note: "Sello `ownerEmail` / `orgId` de `ctx` para que la fila tenga el alcance correcto para compartir y verificar el acceso.",
    },
  ]}
/>
...
- Data lives in PostgreSQL vía Drizzle. Use actions for all writes; schema is additive.
...
Las plantillas de la comunidad se instalan directamente desde un repositorio público de GitHub. Coloca la aplicación Agent-Native en la raíz:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

Añade `#branch`, `#tag` o `#commit` para fijar una revisión concreta:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

La CLI solo descarga el código fuente; no instala dependencias ni ejecuta scripts de la plantilla. Las plantillas de la comunidad son código de terceros, así que revisa el repositorio, la licencia, las dependencias y los scripts antes de ejecutar `pnpm install`.

Para aparecer en el catálogo, incluye una aplicación completa en la raíz, un repositorio público con licencia y README de configuración, una dependencia de `@agent-native/core`, y comprobaciones de tipos y pruebas correctas. La demo alojada y la captura de pantalla son opcionales.

[Envía una plantilla de la comunidad](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml) cuando el repositorio esté listo. Estas plantillas permanecen separadas del catálogo propio y las mantienen sus autores.
