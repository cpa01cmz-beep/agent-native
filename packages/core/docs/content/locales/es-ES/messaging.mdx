...
- **Menciones de canales**: use una @mención para iniciar el trabajo. Cada mensaje del canal que deba procesarse debe mencionar explícitamente al agente; las respuestas normales del hilo y la conversación general del canal no lo invocan.
...
OAuth administrado es la configuración recomendada. En **Administración →
...
`SLACK_SIGNING_SECRET`; después, en **Administración → Mensajería**, pulse
...
Cada mensaje del canal debe mencionar explícitamente al agente; las respuestas
normales del hilo y el resto de la conversación del canal no lo invocan. Se carga un contexto acotado
...
- Cada mensaje posterior en el mismo hilo debe mencionar explícitamente al agente; una respuesta normal no llega a él. Vuelve a hacer una @mention para cada solicitud nueva.
...
| Funcionó una vez y luego se ignoró una respuesta posterior del hilo | Es lo esperado. Cada mensaje que deba procesar el agente debe mencionarlo explícitamente: vuelve a hacer una @mention.                                |
...
- **Clics, no configuración.** La página **Administración → Mensajería** de Dispatch tiene botones de conexión para cada plataforma con los campos env-var integrados.
