...
<Diagram id="doc-block-dzm02u" title={"Un arnés es dueño de su bucle; Agent-Native dirige la sesión"} summary={"El AgentHarness sustrato creates/resumes la sesión nativa, transmite sus eventos a la transcripción normal y persiste resumeState en SQL entre turnos."}>
...
Pista `installPackage` (por ejemplo `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` genera una instalación clara
...
## Autenticación de Codex: interfaz de código frente a adaptadores de arnés {#codex-auth}

Hay dos superficies de Codex:

- **Agent-Native Código/Escritorio** ejecuta `codex exec` en la máquina del usuario
  y reutiliza el inicio de sesión local de Codex CLI indicado por `codex login status`.
- **`ai-sdk-harness:codex`** usa el flujo de suscripción nativa de AI SDK Harness
  cuando el arnés subyacente lo admite. No se necesita ninguna configuración adicional:
  las credenciales permanecen en el host, las credenciales explícitas de API tienen
  prioridad y la autenticación resultante se conecta al sandbox sin copiar el archivo
  de autenticación local. Establece `harnessOptions.auth` en `"ai-gateway"` para usar
  solo AI Gateway; ese modo nunca lee una suscripción nativa.

El adaptador integrado `ai-sdk-harness:codex` solo admite
`permissionMode: "allow-all"`, porque es el único modo aceptado por el tiempo de
ejecución estable de Codex. Omite `permissionMode` para usar ese valor
predeterminado; este adaptador rechaza `allow-reads` y `allow-edits`.
...
`permissionMode`, `harnessOptions` y `agentOptions`). Utilice `listAgentHarnesses()` para enumerar para qué está registrado
un recolector.

Para los arneses integrados con sandbox, pasa un proveedor de sandbox mediante
`agentOptions.sandbox` al registrarlos. El proveedor debe implementar
`resumeSession` para rehidratar después de reiniciar el proceso; una sesión
`sandbox` propiedad del llamador solo cubre la creación inicial.
...
<Diagram id="doc-block-11yarik" title={"Reanudación en turnos, procesos y despliegues"} summary={"Cada turno separa un resumeState opaco en SQL; el siguiente turno lo devuelve a createSession en lugar de reproducir el historial de chat."}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    Turno N<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >opaco · sesión de arnés SQL</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    Turno N+1<br /><small class="diagram-muted"
...
[Human Approval](/docs/actions-access-control#needs-approval) para la superficie de aprobación.
...
[Native Interfaz de chat](/docs/native-chat-ui).
...
- [Native Interfaz de chat](/docs/native-chat-ui): pon tu propio agente detrás del chat UI con `AgentChatRuntime`.
...
- [Human Approval](/docs/actions-access-control#needs-approval): uso del arnés de superficie de aprobación.
