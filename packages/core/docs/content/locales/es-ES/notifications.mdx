...
<Diagram id="doc-block-jlcfmm" title="Una llamada, muchos destinos" summary={"notify() siempre escribe la fila de la bandeja de entrada del propietario, se distribuye en cada canal registrado en paralelo (mejor esfuerzo) y luego emite notificación.sent en el bus de eventos."}>
...
El correo electrónico, Slack y el webhook siempre se registran junto con la bandeja de entrada. La entrega prefiere la URL o los destinatarios de los metadatos de la notificación y, a continuación, recurre a la variable de entorno correspondiente. Si no se configura ninguno, ese canal no hace nada durante la emisión.
...
<Diagram id="doc-block-cbsg3f" title="Canales y gravedad." summary={"la bandeja de entrada siempre está activada; el webhook necesita una var env; Los canales personalizados se registran al inicio. La gravedad impulsa el estilo de la insignia y se transmite a todos los canales."}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>La gravedad determina la insignia</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">confirmaciones, FYI</span>
    </div>
    <div class="sev-row">
...
<Endpoint id="doc-block-1wnwsyu" title="Lista de notificaciones" method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary="Listar notificaciones recientes para el usuario actual" auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "Cursor de marca de tiempo ISO: devuelve las notificaciones creadas antes de este momento para la paginación."
  }
]} responses={[
  {
    "status": "200",
...
- **Integración de encuesta**: cada mutación llama a `recordChange()`, por lo que las plantillas que utilizan [`useDbSync`](/docs/client-data#usedbsync) se invalidan automáticamente sin ningún cableado adicional.
...
- [**Server plugins**](/docs/server-overview): donde se registran los canales personalizados al inicio
