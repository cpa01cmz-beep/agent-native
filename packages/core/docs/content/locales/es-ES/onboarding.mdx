...
<Diagram id="doc-block-evcyhz" title={"La lista de verificación de configuración"} summary={"Solo se requiere conectar un motor de IA. El panel realiza un seguimiento de la finalización y se oculta automáticamente una vez que se hace todo lo necesario."}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span
    ><strong>Conectar un motor de IA</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>Entrega de correo</strong
...
<Endpoint id="doc-block-1c52min" title={"Enumerar los pasos de incorporación"} method="GET" path="/_agent-native/onboarding/steps" summary={"Enumere todos los pasos registrados con su estado de finalización"} responses={[
  {
    "status": "200",
...
<AnnotatedCode
  id="doc-block-1yi0wy2"
  title={"Registrar un paso de incorporación personalizado"}
  ...
  annotations={[
    {
      lines: "5",
      label: "Montado automáticamente",
      note: "Regístrese desde un plugin Nitro: el marco maneja la renderización, el seguimiento de finalización y el descarte.",
    },
    {
      lines: "7",
      label: "Identificación estable",
      note: "Volver a registrarse con el mismo `id` después de la carga predeterminada anula un paso integrado.",
    },
    {
      lines: "12-19",
      label: "método primario",
      note: '`primary: true` marks the big CTA. `kind: "link"` sends the user into the OAuth flow.',
    },
    {
      lines: "20-26",
      label: "Ruta del delegado",
      note: '`kind: "agent-task"` entrega la configuración al chat del agente con un prompt.',
    },
    {
      lines: "28-31",
      label: "Verificación de finalización",
      note: "`isComplete` se ejecuta en el lado del servidor. Los tokens OAuth se encuentran en la tienda `oauth_tokens`; compruébalo, no en `process.env.GMAIL_REFRESH_TOKEN`.",
    },
  ]}
/>
...
| `database`     | no          | URL de PostgreSQL alojado o vacío para PGlite local       |
