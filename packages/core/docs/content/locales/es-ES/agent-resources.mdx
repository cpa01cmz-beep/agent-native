---
title: "Recursos del Agente"
description: "Claude: personalización a nivel de código por usuario (skills, memoria, instrucciones, agentes personalizados, trabajos programados, servidores MCP) respaldados por PostgreSQL/PGlite, no por un sistema de archivos."
---

# Recursos del Agente

Cada aplicación nativa del agente viene con **recursos del agente**: la capa de personalización que hace suyo al agente. Contiene instrucciones de equipo (`AGENTS.md`), aprendizajes compartidos (`LEARNINGS.md`), memoria estructurada personal (`memory/MEMORY.md`), skills que el agente utiliza según demanda, subagentes personalizados, trabajos programados y servidores MCP conectados: todo lo que esperarías de una configuración de código Claude/Codex.

El giro: **son filas PostgreSQL/PGlite, no archivos del sistema de archivos.** Cada usuario obtiene su propio conjunto de recursos del agente almacenado en la base de datos. No hay una caja de desarrollo que activar, ni un contenedor por usuario, ni archivos que montar. Un SaaS multiinquilino puede brindar a cada usuario un agente totalmente personalizable de forma esencialmente gratuita, porque todo son filas (memoria personal, servidores MCP personales, skills personal, subagentes personales) y el código base compartido los aloja todos a la vez.

<Diagram id="doc-block-1v2thml" title="Una configuración de Claude-Code, pero almacenada como recursos del agente en PostgreSQL/PGlite" summary={"La misma capa de personalización (instrucciones, habilidades, memoria, agentes, trabajos, MCP) excepto que cada archivo es una fila en una base de datos compartida de múltiples inquilinos."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Recursos nativos del agente</span
    ><small class="diagram-muted"
      >rows in one base de datos PostgreSQL/PGlite</small
    >
    <div class="ws-rows">
    ...
    <small class="diagram-muted"
      >una base de código, muchos usuarios, con alcance
...
| Código Claude / Codex                           | Recursos nativos del agente                                         |
| ----------------------------------------------- | ------------------------------------------------------------------- |
| Archivos en su disco local                      | Filas en una base de datos PostgreSQL/PGlite compartida             |
...
| `CLAUDE.md` / skills por proyecto               | `AGENTS.md` por aplicación + recursos de memoria                    |
| Configuración MCP en un archivo JSON            | Configuración MCP en JSON _o_ la configuración UI, según el alcance |

Las mismas capacidades. Economía diferente. Consulte [Aplicaciones](/docs/cloneable-saas) para saber por qué esto es importante para SaaS.
...
<Diagram id="doc-block-pmpd5a" title="Tres alcances, un archivo efectivo" summary={"El tiempo de ejecución resuelve la misma ruta en los ámbitos del espacio de trabajo, la aplicación y el personal durante la lectura: gana el ámbito más específico."}>
...
<div class="ws-stack">
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >valores predeterminados de empresa desde Dispatch</small
    ><code>context/brand.md</code>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Organización / aplicación</span
...
1. Abra la pestaña **Recursos** → **Compartido** → `AGENTS.md` (créelo con `+` → **Archivo** si falta).
...
<Callout id="doc-block-3em4co" tone="info">

Sin reinicio, sin redistribución. `AGENTS.md` se lee al comienzo de cada turno, por lo que una edición que guardes ahora cambia el comportamiento del agente en el siguiente mensaje.
...
Haga clic en el icono `?` en la barra de herramientas de Recursos para volver a estos documentos en cualquier momento.
...
Los recursos del agente tienen dos superficies de memoria actuales:
...
| `mcp-servers/…`    | Espacio de trabajo/compartido | Humanos a través de Dispatch o recursos de la aplicación | Actualización de configuración MCP                 |

Los usuarios pueden editar estos archivos de memoria directamente en la pestaña Recursos; son recursos regulares. Elimine líneas en las que el agente se equivocó, mantenga las preferencias personales en `memory/MEMORY.md` o promueva reglas para todo el equipo en `AGENTS.md`.
...
<DataModel
  id="doc-block-1mx0tv0"
  title="El modelo de recursos del agente"
  summary={
    "Una forma de recurso respalda cada archivo de recursos del agente. El tiempo de ejecución lo clave por ruta y alcance y resuelve el valor efectivo en la lectura."
  }
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "Un único archivo en los recursos del agente de un usuario: instrucciones, habilidad, memoria, agente, configuración MCP o trabajo.",
      fields: [
        {
          name: "path",
          type: "string",
        ...
        },
        {
          ...
          note: "¿En qué nivel vive esta fila?",
        },
        {
          name: "owner",
          type: "string",
        ...
        },
        {
          name: "content",
          type: "text",
          note: "Markdown / JSON / YAML cuerpo",
        },
      ],
    },
  ]}
...
Skills son archivos de recursos Markdown en la ruta `skills/` (preferiblemente `skills/<name>/SKILL.md`) que brindan al agente conocimiento del dominio bajo demanda, invocados en el chat con `/skill-name`. Agréguelos desde la pestaña Recursos o, en modo Código, desde `.agents/skills/`.
...
<AnnotatedCode
  id="doc-block-sr7bz7"
  title="Un perfil de agente personalizado"
  ...
  annotations={[
    {
      ...
      note: "`name` es lo que aparece en el menú desplegable `@` y en lo que delega el agente principal.",
    },
    {
      lines: "3-4",
      label: "Cuando delegar",
      note: "`description` es lo que lee el orquestador para decidir que este perfil se ajusta a una tarea.",
    },
    {
      lines: "5",
      label: "modelo",
      note: "`inherit` reutiliza el modelo del agente principal. Anular sólo cuando el perfil claramente necesite uno diferente.",
    },
    {
      lines: "6",
      note: "`tools: inherit` por ahora: el campo está reservado para futuras políticas de herramientas por agente.",
    },
  ]}
/>
...
Los recursos del agente incluyen dos tipos de agentes:
...
## Etiquetas @ y comandos / {#at-tagging}

Escribe `@` en la entrada del chat para hacer referencia a un agente, archivo o recurso, o `/` para invocar un skill. Ambos abren un menú desplegable en el cursor y añaden un chip en línea al seleccionar un elemento. Consulta [Menciones de agentes](/docs/agent-mentions) para ver toda la mecánica (enrutamiento, `MentionProvider` y selección de subagentes) y la [Guía de Skills](/docs/skills-guide) para saber cómo se crean y descubren los skills.
...
| ---------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| @ etiquetado                 | Archivos de código base + recursos del agente + agentes personalizados + agentes conectados | Recursos del agente + agentes personalizados + agentes conectados |
...
- [**Guía de Skills**](/docs/skills-guide) — crear y definir el alcance de los archivos de skill referenciados desde `skills/`
- [**Menciones de agentes**](/docs/agent-mentions) — mecánica completa de las menciones `@` y los comandos `/`
- [**Trabajos recurrentes**](/docs/recurring-jobs) — programar los prompts creados a partir de tareas programadas
- [**Gobernanza de Workspace**](/docs/workspace-management) — quién revisa, aprueba y posee los cambios del workspace
- [**Workspaces de varias aplicaciones**](/docs/multi-app-workspace) — la forma de despliegue que abarca un workspace
- [**Conexiones de Workspace**](/docs/workspace-connections) — compartir credenciales de proveedores entre aplicaciones
