...
description: "Monte los recursos del agente y el chat del agente en cualquier aplicación React con <AgentPanel>, <AgentSidebar> y sendToAgentChat()."
---
...
No es necesario crear un agente nativo desde cero. El chat del agente, la pestaña de recursos del agente, el terminal CLI, la entrada de voz y toda la infraestructura relacionada se envían como un puñado de componentes React que se pueden colocar en cualquier aplicación.

> **Requisito previo:** el servidor debe ejecutar `agent-chat-plugin` (se monta automáticamente en cada plantilla). Si estás empezando desde cero, consulta [Server](/docs/server-overview).
>
...
| `<AgentPanel>`        | El panel sin formato en sí: chat + CLI + pestañas de recursos del agente                                    | Quieres control total sobre el diseño o una página de agente dedicada                |
...
<Diagram id="doc-block-5dwazo" title="El modelo de montaje" summary={"<AgentSidebar> ajusta su diseño existente. Tus rutas se muestran en el área principal; el panel del agente se monta junto a ellos. <AgentPanel> es el mismo panel sin el contenedor."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        Tu aplicación<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        Panel del agente<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1dtgx5y"
  title={"Ajustando el diseño raíz con <AgentSidebar>"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "Indicaciones de inicio",
      note: "`suggestions` se renderiza como chips en los que se puede hacer clic en el chat vacío.",
    },
    {
      lines: "13",
      label: "Chips sensibles al contexto",
      note: '`dynamicSuggestions` merges screen-aware prompts (e.g. "Summarize this selection") with your static ones. On by default.',
    },
    {
      lines: "18-20",
      label: "Botón de alternar",
...
Eso es todo. El usuario ahora tiene un agente alternable en cada página, con historial de chat, pestaña de recursos del agente, terminal CLI, entrada de voz y un panel más ancho. El estado persiste entre recargas a través de `localStorage`.

### Accesorios

- **`children`**: el diseño y las rutas normales de tu aplicación. Representado en el área principal; el panel del agente se monta junto a él en el escritorio y encima en el móvil.
...
`<AgentPanel>` le ofrece pestañas sin formato (Chat / CLI / Recursos del Agente) sin el envoltorio de la barra lateral, el botón contraer ni ninguna persistencia de estado. Ponlo donde quieras; tú manejas el diseño.
...
- **`isFullscreen`**: actívelo si desea una columna centrada al estilo Claude en una superficie de página o panel personalizado. La barra lateral usa el panel más ancho.
...
Un botón que entrega trabajo al agente en lugar de crear una ruta de IA independiente con una llamada `llm()` en línea. Consulta [Por qué crear aplicaciones de esta forma](/docs/what-is-agent-native#why-build-apps-this-way) para entender por qué importa esta distinción:
...
- **Para trabajo de IA visible iniciado por el usuario**, usa `openSidebar: true`. Usa `false` solo para trabajo de fondo o iniciado por el sistema que sea explícitamente silencioso, y mantén los seguimientos en el mismo hilo de AgentSidebar.
- **`type`**: `"content"` (predeterminado) mantiene el trabajo en el agente integrado, que usa herramientas de la aplicación. `"code"` lo envía a un host separado con capacidad de código para cambios en el repositorio; la escritura solo está disponible si el host concede intencionalmente herramientas de repository/workspace. Consulte [Agent-Native Código UI](/docs/code-agents-ui).
...
Mantén los mensajes de seguimiento y revisión en el hilo existente de la barra lateral. No añadas un segundo campo de texto libre junto al resultado; usa formularios locales solo para valores estructurados que la interfaz deba validar.
...
[Chat del Agent](/docs/client-agent-chat#sendtoagentchat) para el comportamiento del puente de la aplicación MCP.
...
## Elegir entre el chat del agente y una acción {#agent-or-action}

Usa `useActionQuery` o `useActionMutation` directamente para operaciones locales deterministas como CRUD, validación, lecturas del proveedor y persistencia. Si el usuario vive el flujo como investigación, análisis, generación, recomendación, síntesis u otro razonamiento de varios pasos, inícialo con `sendToAgentChat({ openSidebar: true })` y deja que el agente coordine acciones enfocadas. Que la implementación sea determinista no basta para ocultar un flujo parecido a la IA dentro de una acción opaca.

## actions con seguridad tipográfica desde UI: `useActionMutation()` {#use-action-mutation}

Cuando la UI necesita ejecutar la misma operación que ejecutaría una herramienta de agente, usa [`useActionMutation`](/docs/client-data#use-action-mutation):
...
Los argumentos de tipo seguro provienen del esquema zod en su `defineAction()`. Consulte [Actions](/docs/actions-overview) para conocer el sistema de acción completo.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` y `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — selección, navegación, visualización de pantalla
- [**Recursos del Agente**](/docs/agent-resources): qué contiene la pestaña Recursos del Agente (skills, memoria, servidores MCP, trabajos programados)
