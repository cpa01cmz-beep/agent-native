...
Ya se envía un andamio `npx --yes @agent-native/core@latest create` nuevo con:
...
Si está evaluando un agente nativo para un CRM, un rastreador de proyectos, una bandeja de entrada de soporte o cualquier herramienta de equipo, la base multiinquilino ya está ahí. Todas las plantillas propias son multiinquilino; consulte [Plantillas](/docs/cloneable-saas) para ver la lista.

<Diagram id="doc-block-3yhq1v" title={"Membresía y aislamiento de la organización"} summary={"Los usuarios se unen a organizaciones como owner/admin/member. Cada fila poseíble lleva el org_id del inquilino propietario y ninguna fila cruza el límite."}>
...
<Diagram id="doc-block-1fl0ytr" title={"De la sesión al ámbito SQL"} summary={"La organización activa en la sesión se convierte en AGENT_ORG_ID, que el marco integra en la cláusula WHERE de cada consulta."}>
...
- [**Security — Data Scoping**](/docs/security#data-scoping) — aislamiento de nivel SQL, contrato `ownableColumns()` y protecciones de acceso
- [**Multi-App Workspace**](/docs/multi-app-workspace) — aloja múltiples aplicaciones Agent-Native en un monorepo con autenticación compartida y RBAC

## Modelo de aislamiento

Usa `accessFilter()` y `assertAccess()` para las lecturas y escrituras del servidor. El aislamiento de SQL del agente crea vistas temporales filtradas para las tablas con propietario. `pnpm guards` incluye la comprobación de consultas sin ámbito. RLS no es actualmente una capa activa y está prevista como refuerzo posterior a 1.0. Consulta [Despliegues administrados](/docs/administered-deployments).
