...
- **Predeterminado:** Magic link como flujo principal de Better Auth cuando el correo saliente está listo, con correo electrónico/contraseña como alternativa y proveedores sociales. Establezca `AUTH_MAGIC_LINK=0` para forzar la alternativa.
...
<Diagram id="doc-block-1lj059p" title={"Tres entradas, una sesión"} summary={"Los visitantes del navegador, los hosts MCP y los proveedores personalizados se resuelven en la misma sesión de autenticación que lee el alcance descendente."}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">conectores de Claude Code y ChatGPT</small>
    </div>
    <div class="diagram-card">
...
## Inicio de sesión en desarrollo local {#local-development-sign-in}

En un servidor de desarrollo con una dirección de bucle local, la página compartida de inicio de sesión muestra primero **Continue as local dev**. Crea o reutiliza la cuenta y la sesión de desarrollo reales de Better Auth, por lo que sigue el mismo flujo autenticado que se usa en el resto de la aplicación.

Selecciona **Show full sign in options** para mostrar las opciones normales de registro, contraseña, magic link y proveedores sociales. Si no se puede crear la sesión local, la página muestra esas opciones automáticamente para que puedas continuar con la autenticación estándar.

Esta comodidad solo está disponible cuando el navegador se conecta mediante `localhost`, `127.0.0.0/8` o `::1` en desarrollo o pruebas. Está desactivada en las vistas previas de despliegue y en la producción alojada. No es una omisión de autenticación. Establece `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` para no usarla y mostrar la página de inicio de sesión normal durante el desarrollo local.

## Mejor autenticación (predeterminada) {#better-auth}

De forma predeterminada, Better Auth potencia la autenticación. Proporciona:

- Registro e inicio de sesión mediante magic link, con correo electrónico/contraseña como alternativa
...
- `POST /_agent-native/auth/magic-link` — solicitar un correo con magic link
...
o obtener una vista previa de los entornos y nombrar cuentas de prueba con una dirección `+autoz`
(`name+autoz@example.com`) para que sean fáciles de identificar.

## Política de verificación de correo electrónico {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` es una comodidad local y de control de calidad:
una implementación alojada la ignora. Para declarar la política en cualquier
entorno, incluida la producción, decláresela en la configuración:

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

El alias de implementación es `AUTH_REQUIRE_EMAIL_VERIFICATION=0`. Un valor
declarado prevalece tanto sobre `AUTH_SKIP_EMAIL_VERIFICATION` como sobre el
valor predeterminado de cada entorno.

`false` acepta una dirección no verificada como credencial de inicio de sesión
—cualquiera puede reclamar cualquier correo electrónico—, así que úselo solo
donde el registro ya esté restringido de otra manera. `true` en una
implementación sin proveedor de correo electrónico deshabilita el registro con
contraseña, porque la verificación nunca podría entregarse.
...
<Diagram id="doc-block-1roe6ix" title={"Apretón de manos remoto MCP OAuth"} summary={"Un cliente compatible con OAuth arranca solo desde MCP URL: desafío, descubrimiento, registro dinámico y luego un intercambio de código PKCE."}>
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | Establezca en `0` o `1` para declarar la política de verificación del registro con contraseña en cualquier entorno, incluida la producción; prevalece sobre `AUTH_SKIP_EMAIL_VERIFICATION`                                  |
...
Las apps alojadas canónicas reconocen las solicitudes de Desktop SSO Canary empaquetado sin configuración del centro de identidad por app. Los navegadores normales usan el inicio de sesión habitual de cada app. Las apps autoalojadas solo requieren `AGENT_NATIVE_IDENTITY_HUB_URL` para la ruta explícita restante de Desktop o la federación del servidor. Las compilaciones estables de Desktop no inicializan el intermediario hasta que una versión posterior se apruebe por separado.
...
## Despliegue administrado e invitaciones

Para un despliegue administrado, configura `AUTH_SIGNUP=invited`, `ORG_CREATION=closed` y `AUTO_CREATE_DEFAULT_ORG=0`. La [guía de despliegues administrados](/docs/administered-deployments) explica los administradores de inicio, las invitaciones y los dominios permitidos. Las invitaciones de membresía y la unión por dominio son reglas distintas.
