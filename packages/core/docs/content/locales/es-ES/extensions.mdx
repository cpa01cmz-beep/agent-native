---
title: "Extensiones"
description: "Una capacidad opcional del toolkit para bloques personalizados puntuales y aislados. El código de la aplicación sigue siendo la opción predeterminada para el comportamiento duradero."
---

# Extensiones

Las extensiones son una **capacidad opcional del toolkit para bloques personalizados aislados**. Están desactivadas de forma predeterminada; una plantilla debe habilitarlas explícitamente. Analytics las utiliza para visualizaciones personalizadas puntuales solicitadas expresamente. Para un comportamiento duradero, edita el código de la aplicación.
...
> **Las extensiones añaden interfaz; no reescriben la interfaz de la aplicación
> anfitriona.** Una extensión se muestra en su propia página o dentro de una
> [ranura](#slots) con nombre que expone la plantilla. No puede insertar
> elementos en componentes nativos arbitrarios —por ejemplo, junto a cada
> asistente en Calendario— a menos que la plantilla exponga una ranura allí.
> Para una personalización más profunda, cambia la plantilla en tu repositorio
> y flujo de desarrollo. El agente integrado solo puede ayudar si el host
> concede intencionalmente herramientas de escritura de repository/workspace;
> las extensiones siguen siendo la capa rápida y sin despliegues.
...
Las extensiones están desactivadas de forma predeterminada. Para habilitarlas,
establece `extensionTools: true` tanto en `createAgentChatPlugin()` como en
`createCoreRoutesPlugin()`. El primero añade las actions y la guía para el
agente; el segundo permite la creación autenticada mediante
`POST /_agent-native/extensions`. El runtime aislado y las rutas existentes de
lectura, edición y enlaces profundos se conservan por compatibilidad.

<Diagram id="doc-block-916xvd" title="El puente de la caja de arena" summary={"La extensión HTML se ejecuta en un iframe aislado y llega al host solo a través de un conjunto fijo de ayudas de puente: cada llamada tiene un alcance y un acceso verificado."}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Plantilla host<br /><small class="diagram-muted"
        >acciones, SQL con alcance automático</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>, bloqueado al dominio</small
      >
    </div>
    <div class="diagram-box">
      APIs externas<br /><small class="diagram-muted"
        >solo mediante extensionFetch</small
      >
    </div>
  </div>
</div>
...
1. Abra **Settings → Extensions** (o simplemente pregunte en el chat) y haga clic en **"Nueva extensión"**.
...
<Diagram id="doc-block-1uwb6po" title="Las ranuras introducen contexto en el widget" summary={"La plantilla de host posee espacios con nombre; instalar una extensión en uno le proporciona window.slotContext para lo que el usuario esté viendo actualmente."}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">Hilo de correo</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Notas de contacto</span
    ><small class="diagram-muted"
...
- `dbQuery` y `extensionFetch` están bloqueados por ahora.
...
## ¿Debo usar una extensión o el flujo de código fuente? {#vs-app-code}

El agente integrado no edita el código fuente de forma predeterminada; solo puede hacerlo si el host concede intencionalmente herramientas de escritura de repository/workspace. Usa el flujo de código fuente para el comportamiento duradero y las extensiones para personalización rápida en runtime.

- **Empieza con el código de la aplicación** para el comportamiento del producto, la UI nativa, el layout, las rutas, la lógica de negocio o cualquier cosa que deba reutilizarse.
- **Usa un bloque personalizado solo si la aplicación lo habilita** y el usuario pide explícitamente una visualización o interacción puntual en una superficie existente. Es rápido y no requiere despliegue, pero sigue siendo un parche aislado creado por el agente.
- **Promueve los bloques duraderos.** Si un bloque resulta útil más allá de su contexto original, usa **Promover a código de aplicación** y mantenlo activo hasta que el cambio de código se revise y despliegue.

|                                 | Extensión                                     | Edición del código de la aplicación     |
| ------------------------------- | --------------------------------------------- | --------------------------------------- |
| **Creado por**                  | Agente (o usuario) en tiempo de ejecución     | Usuario o flujo con capacidad de código |
...
| **Mejor para**                  | Bloques personalizados puntuales              | Comportamiento duradero del producto    |

Regla general: **usa el código de la aplicación de forma predeterminada.** En una aplicación que habilita extensiones, una visualización puntual puede ser un bloque personalizado; un nuevo tipo de gráfico que deba estar disponible en todos los dashboards pertenece al código de la plantilla.

Para la obtención de datos personalizada que deba alimentar gráficos y tablas nativos en lugar de una UI de widget a medida, vea [Programas de datos](/docs/data-programs) — un script `run-code` almacenado y actualizable al que los paneles de dashboard se vinculan directamente, sin ningún iframe de extensión de por medio.
...
- **Se requiere autenticación**: las extensiones solo se ejecutan para usuarios que han iniciado sesión y las llamadas `dbQuery` tienen un alcance automático.
...
- [**Actions**](/docs/actions-overview): las operaciones que llama una extensión a través de `appAction`
