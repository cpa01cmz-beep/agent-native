---
title: "المراسلة"
description: "تحدث إلى وكيلك عبر Slack أو Microsoft Teams أو تفاعلات Discord أو البريد الإلكتروني أو Telegram أو WhatsApp."
---

# المراسلة

قم بتوصيل وكيلك بـ Slack أو Microsoft Teams أو تفاعلات Discord أو البريد الإلكتروني أو Telegram أو WhatsApp. إنه الوكيل نفسه بنفس الذاكرة والأدوات وسجل المحادثات.

> **نطاق الدعم بدقة:** يتحقق Teams من رموز JWT الرسمية لـ Bot Framework ويرد في المحادثة الأصلية. يدعم Discord أوامر الشرطة المائلة عبر تفاعلات HTTP الموقعة بـ Ed25519 مع إقرار مؤجل؛ ولا يستقبل رسائل الخادم العادية أو الرسائل المباشرة من دون تشغيل Gateway. يحتفظ Telegram بـ `message_thread_id` ويستخدم WhatsApp قيمة `wamid` الواردة للردود السياقية.
>
> يتطلب Teams القيم `MICROSOFT_TEAMS_APP_ID` و`MICROSOFT_TEAMS_APP_PASSWORD` و`MICROSOFT_TEAMS_ALLOWED_TENANT_IDS`. ويتطلب Discord القيم `DISCORD_APPLICATION_ID` و`DISCORD_PUBLIC_KEY`. كما يتطلب Telegram القيمة `TELEGRAM_WEBHOOK_SECRET` ويتطلب WhatsApp القيمة `WHATSAPP_APP_SECRET` بالإضافة إلى بيانات الاعتماد الحالية.

> **هل تستخدم قالب الإرسال؟** تم إعداد كل هذا لك في **الإدارة → المراسلة**. انقر لربط كل نظام أساسي — لن تحتاج إلى قراءة بقية هذه الصفحة إلا إذا كنت تقوم بتخصيص القالب الخاص بك أو إنشائه. راجع [Dispatch](/docs/dispatch) أو [Dispatch template reference](/docs/template-dispatch).
...
- **نفّذ إجراءات في أدوات أخرى انطلاقًا من رسالة.** يمكن أن ينتهي `@mention` بصف في Notion أو مهمة في Jira أو سجل في HubSpot — راجع [`@mention` في Slack ← صف في Notion](#slack-to-notion) للمثال الكامل.
- للحصول على تنبيهات أحادية الاتجاه داخل التطبيق (رمز الجرس، webhooks)، راجع [Notifications](/docs/notifications).

<Diagram id="doc-block-13wwymq" title={"العديد من القنوات، وكيل واحد"} summary={"تنضم كل منصة إلى نفس حلقة الوكيل ونفس سجل سلاسل الرسائل SQL - لذلك تواصل رسالة مباشرة Slack وبريد إلكتروني نفس المحادثة."}>
...
<div class="msg-fanin">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">حلقة وكيل واحدة</span
    ><small class="diagram-muted">نفس الذاكرة · نفس الأدوات</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    سجل محادثة SQL واحد<br /><small class="diagram-muted"
...
- **إشارات القناة** — استخدم @mention لبدء العمل. يجب أن تتضمن كل رسالة قناة تريد من الوكيل معالجتها إشارة صريحة؛ لا تستدعيه الردود العادية في السلاسل أو الدردشة العامة.
- **الرسائل المباشرة** — الرسائل الخاصة المقبولة هي محادثات سرية، وتتطلب عضوًا موثّقًا في المؤسسة أو هوية Slack مرتبطة.
- **الهوية** — يمكن للرسائل الخاصة ربط بريد Slack الموثق بالمستخدم نفسه في التطبيق؛ تستخدم القنوات المُدارة هوية الخدمة والسياسة الخاصة بها.
...
### الإعداد المُدار عبر Add to Slack

OAuth المُدار هو الإعداد الموصى به. من **الإدارة → المراسلة**، اختر
**Agent manifest** لتنزيل ملف `agent-native-slack-manifest.json` الخاص بمضيفك،
ثم استورده من **App Manifest** في Slack. يفعّل الملف تجربة `agent_view` الحالية
وعلامة تبويب **Messages** قابلة للكتابة للرسائل المباشرة. تحقّق من هذه النطاقات:

لا يتضمن البيان أيقونة التطبيق. ضمن **Basic Information → Display Information**،
ارفع أيقونة Agent-Native كي تتطابق قائمة Agents مع هوية المنتج. يمكنك استخدام
`icon-512.svg` العام للتطبيق كمصدر وتحويله إلى تنسيق رفع يدعمه Slack عند الحاجة.
...
اضبط OAuth Redirect URL على
`https://your-app.example.com/_agent-native/integrations/slack/oauth/callback`،
وEvent Subscriptions Request URL على
`https://your-app.example.com/_agent-native/integrations/slack/webhook`،
وInteractivity Request URL على
`https://your-app.example.com/_agent-native/integrations/slack/interactions`.
اشترك في `app_home_opened` و`app_context_changed` و`app_mention`
و`message.im`.
بعد إعداد `SLACK_CLIENT_ID` و`SLACK_CLIENT_SECRET`
و`SLACK_SIGNING_SECRET`، افتح **الإدارة → المراسلة** وانقر **Add to Slack**.
يُخزّن كل تثبيت رمزًا مشفرًا منفصلًا، وتفصل مفاتيح المحادثة بين مساحة العمل
والتطبيق والقناة وسلسلة الرسائل.

أعد تثبيت تطبيق Slack بعد تغيير البيان أو النطاقات أو الأحداث أو Agent view أو
علامة تبويب Messages. يستخدم `app_home_opened` لاكتشاف فتح الرسائل المباشرة،
ويضيف `app_context_changed` سياق العرض الحالي، ويسلّم `message.im` الرسائل الخاصة.
قد يخفي مسؤولو Slack الوكلاء المثبتين عن المؤسسة، كما يمكن لكل مستخدم إخفاؤهم من
الشريط العلوي. إذا نجح التثبيت ولم يظهر الوكيل، فتحقق من عناصر التحكم هذه أو افتحه
من **Tools → Apps**.

لا تُقبل الرسائل الخاصة الشخصية في Slack إلا من عضو موثّق في مؤسسة Agent-Native
أو من هوية Slack مرتبطة مسبقًا بذلك العضو. أما سياسات وميزانيات هوية الخدمة الخاصة
بالقناة فهي منفصلة.

يجب أن تتضمن كل رسالة قناة إشارة صريحة إلى الوكيل؛ ولا تستدعيه الردود العادية
في السلاسل أو محادثات القناة العامة. يحمّل الوكيل سياقًا محدودًا مع التفاعلات
ومراجع الملفات فقط، ويعرض خطة وتقدمًا مباشرًا مع أزرار الإلغاء والموافقة/الرفض.
الرسائل الخاصة تستخدم هوية المستخدم الشخصية، بينما تستخدم القنوات المُدارة هوية
خدمة صريحة وسياسات للضيوف وSlack Connect والنموذج الافتراضي. ذاكرة القناة صريحة
ومعزولة ولا تُحفظ تلقائيًا. تحتفظ الإجراءات الدورية التي تُنشأ من القناة بوجهة
Slack، وتفرض ميزانيات يومية أو شهرية للمؤسسة أو الطالب أو القناة مع سجل تدقيق كامل.
يتيح `assistant:write` أحداث سياق Agent View، بينما يتيح `chat:write` الردود
وتحديثات الحالة المباشرة.

### مثال عملي كامل: `@mention` في Slack ← صف في Notion {#slack-to-notion}

بعد أن تعمل إشارات Slack، يكون الطلب التالي الأكثر شيوعًا صيغة من صيغ _«عندما أشير إلى الوكيل بـ `@mention` داخل سلسلة رسائل، خذ ما كتبته وأضفه صفًا في قاعدة بيانات Notion الخاصة بي»_. هذا ممكن بالفعل — استقبال Slack، وواجهة مزوّد Notion، وإجراءات التطبيق كلها مدمجة — لكنه يتطلب ربط ثلاثة أجزاء بالترتيب الصحيح، واثنان منها يفشلان بصمت إذا تجاوزتهما.

كل ما يظهر بين `<ANGLE_BRACKETS>` أدناه هو عنصر نائب. لا تلصق أبدًا سرًا حقيقيًا من Notion في الشيفرة المصدرية أو في ملف `.env` مُودَع في المستودع أو في رسالة دردشة.

#### 1. اجعل الإشارات تصل إلى التطبيق

أكمل [إعداد Slack](#slack) وتأكد من أن `@mention` في قناة يحصل على رد قبل أن تبدأ بـ Notion. هناك سلوكان مهمان لبقية المثال:

- الرسالة التي تحتوي على `@mention` هي الاستدعاء، ونصها هو ما يراه الوكيل.
- يجب أن تتضمن كل رسالة لاحقة في السلسلة إشارة صريحة إلى الوكيل؛ فالردود العادية اللاحقة لا تصل إليه. أعد استخدام `@mention` لكل طلب جديد.

#### 2. أتِح إجراءات واجهة المزوّد

يأتي Core بإعداد مسبق لمزوّد Notion (عنوان URL الأساسي، وترويسة `Notion-Version`، والمصادقة، وروابط التوثيق)، لكن الإجراءات الثلاثة التي تتيح للوكيل استخدامه — `provider-api-catalog` و`provider-api-docs` و`provider-api-request` — هي **تصدير مسار فرعي اختياري**. التطبيق الذي لم يضِفها مطلقًا لا يملك أي طريق إلى Notion، وسيقول الوكيل إنه لا يملك أداة Notion. هذه هي الخطوة التي يفوّتها معظم الناس.

أضِف بيئة تشغيل المزوّد:
...
ثم أضِف ملف إجراء رفيعًا لكل معرّف إجراء. جميع خيارات المصنع اختيارية، لذا يكفي في كل ملف استدعاء واحد:
...
تُكتشف الملفات الموجودة في `actions/` تلقائيًا — لا يوجد سجل يحتاج إلى تعديل. راجع [إعادة استخدام مصانع الإجراءات في Core](/docs/integrations#provider-api-factories) لتضييق قائمة المزوّدين أو إتاحتها عبر HTTP.

#### 3. ضع مفتاح Notion في الخزنة لا في `.env`

أنشئ تكاملًا داخليًا على **[notion.so/my-integrations](https://www.notion.so/my-integrations)** وانسخ سرّه. ثم في تطبيقك افتح **Settings → API Keys & Connections**، وأضِف مفتاحًا باسم `NOTION_API_KEY` تمامًا، والصق القيمة، واختر النطاق:

- **Personal** — لا يستطيع استخدامه إلا التشغيلات التي تجري باسمك.
- **Workspace** — مشترك مع المؤسسة. اختر هذا إذا كان غيرك سيشير إلى الوكيل، لأن تشغيل Slack يجري باسم مالك التكامل لا باسم من كتب الرسالة.

تحلّ بيئة تشغيل المزوّد بيانات الاعتماد من الخزنة المشفّرة بهذا الترتيب — شخصي، ثم مؤسسة، ثم مساحة عمل — و**لا تقرأ `process.env` إطلاقًا وعن قصد**، لأن متغيّر بيئة على مستوى النشر سيكون مشتركًا بين جميع مستخدمي تطبيق متعدد المستأجرين. لن يُؤخذ سطر `NOTION_API_KEY` في `.env` بعين الاعتبار هنا.

#### 4. شارك قاعدة البيانات مع التكامل

في Notion، افتح قاعدة البيانات المستهدفة ثم أضِف تكاملك من **••• → Connections →**. تخطّي هذه الخطوة هو أكثر أعطال Notion شيوعًا وأكثرها تضليلًا: تُجيب الواجهة بـ **404** لا 403، فيبدو الأمر وكأن معرّف قاعدة البيانات خاطئ. يحمل مدخل الكتالوج نصّ المعالجة هذا، ويضيفه `provider-api-request` إلى حقل `guidance` في أي استجابة 401 أو 403 أو 404.

تجد معرّف قاعدة البيانات في عنوان URL الخاص بها — وهو السلسلة المكوّنة من 32 حرفًا قبل `?`.

#### 5. اطلب من وكيلك أن يبنيه

هذا هو الجزء الذي لا تخبرك به أي شاشة إعداد: لست أنت من يربط النصفين، بل تطلب السلوك المطلوب. في دردشة الوكيل داخل التطبيق:

> أضِف إجراءً باسم `add-notion-row` يضيف صفًا إلى قاعدة بيانات Notion `<NOTION_DATABASE_ID>` عبر واجهة المزوّد. يجب أن يستقبل نص الرسالة، وكاتبها في Slack، ورابطًا يعود إلى رسالة Slack. ثم حدِّث `AGENTS.md` بحيث تستدعي هذا الإجراء وترد بعنوان صفحة Notion الجديدة عندما يشير إليك أحد بـ `@mention` في Slack ويطلب تسجيل شيء أو حفظه.

ما ينبغي أن يكتبه الوكيل هو **إجراء واحد** — لا مسار Nitro، ولا `fetch` من المتصفح:
...
ثلاث تفاصيل في هذا المثال أساسية:

- ترتبط الصفوف في إصدارات Notion الحالية بـ**مصدر بيانات** داخل قاعدة البيانات، لذا اقرأ قاعدة البيانات أولًا واستخدم `data_sources[0].id` الخاص بها كأب. تمرير معرّف قاعدة البيانات مباشرةً كأب هو صيغة الواجهة القديمة.
- يعيد `executeRequest` أخطاء المزوّد بدل رميها، لذا تحقّق بنفسك من `response.ok` ومرّر قيمة `guidance` المعادة — فهناك تصل تلميحة «شارك قاعدة البيانات».
- يجب أن تطابق أسماء الخصائص (`Name` و`Author` و`Source`) أعمدة قاعدة بياناتك تمامًا، وأن تطابق الأنواع أيضًا. اجعل الوكيل يقرأ `properties` الخاصة بقاعدة البيانات أولًا بدل التخمين.

الجزء الخاص بـ `AGENTS.md` ليس اختياريًا. من دونه يملك الوكيل الأداة لكن لا سبب لديه لاستخدامها حين تقول رسالة في Slack «سجّل هذا».

#### 6. تأكّد من نجاحه، وماذا تفحص عند الفشل

في قناة Slack التي يوجد فيها الوكيل، اكتب `@YourAgent سجّل هذا في Notion: <SOME_TEXT>`. التشغيل الناجح يرد داخل السلسلة بعنوان الصفحة الجديدة، ويظهر الصف في Notion.

عندما لا ينجح:

| ما تراه                                                   | أين تبحث                                                                                                                            |
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| لا يرد Slack إطلاقًا                                      | لم تصل الإشارة إلى التطبيق أصلًا. أعِد فحص Request URL وسر التوقيع في [إعداد Slack](#slack) — هذه ليست مشكلة Notion.                |
| يرد الوكيل لكنه يقول إنه لا يملك أداة Notion              | الخطوة 2. لم تُضَف إجراءات واجهة المزوّد إلى `actions/` مطلقًا.                                                                     |
| `notion credential not configured. Tried: NOTION_API_KEY` | الخطوة 3. المفتاح موجود في `.env`، أو محفوظ بنطاق **Personal** لحساب غير الحساب الذي يجري التشغيل باسمه. احفظه بنطاق **Workspace**. |
| يرد Notion بـ `404`                                       | الخطوة 4. لم تُشارَك قاعدة البيانات مع التكامل.                                                                                     |
| يرد Notion بـ `400` بخصوص الخصائص                         | أسماء الخصائص أو أنواعها في الإجراء لا تطابق أعمدة قاعدة البيانات.                                                                  |
| نجح مرة، ثم جرى تجاهل رد لاحق في السلسلة                  | هذا متوقّع. يجب أن تتضمن كل رسالة تريد من الوكيل معالجتها إشارة صريحة — أعد استخدام `@mention`.                                     |
...
- **النقرات، وليس التكوين.** تحتوي صفحة **الإدارة → المراسلة** في Dispatch على أزرار اتصال لكل نظام أساسي مع حقول env-var المضمنة فيها.
...
<Diagram id="doc-block-db7fc7" title={"دورة حياة خطاف الويب الوارد"} summary={"يقوم خطاف الويب فقط بالتحقق من الرقم 200 وإدراجه في قائمة الانتظار وإرجاعه. يؤدي تنفيذ وظيفة جديدة إلى استنزاف قائمة الانتظار وتشغيل حلقة الوكيل، مع إعادة المحاولة لمدة 60 ثانية كشبكة أمان."}>

```html
<div class="msg-flow">
  <div class="msg-row">
    <div class="diagram-node">
      Platform<br /><small class="diagram-muted">Slack · البريد · إلخ</small>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>
      <strong>/webhook</strong><br /><small class="diagram-muted"
        >verify signature + parse</small
      ><br /><span class="diagram-pill">INSERT مهمة معلقة</span>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<Endpoint id="doc-block-of3dmy" method="POST" path="/_agent-native/integrations/slack/webhook" summary={"Slack الأحداث API خطاف الويب الوارد"} auth={"HMAC-SHA256 of the raw body using `SLACK_SIGNING_SECRET`, checked against the `X-Slack-Signature` header. In production also gated by `SLACK_ALLOWED_TEAM_IDS` / `SLACK_ALLOWED_API_APP_IDS`."} params={[
  {
...
يتم حل بيانات اعتماد التكامل أولاً من الأسرار ذات النطاق لمالك التكامل (المستخدم ثم المؤسسة ثم مساحة العمل)؛ وتكون متغيرات بيئة النشر بديلاً للتثبيت المحلي أو أحادي المستأجر المقصود. تحفظ واجهة الإعداد أسراراً ذات نطاق؛ ولا تُعاد القيم أبداً إلى المتصفح أو تُخزَّن في كود المصدر.

محولات وقت التشغيل المضمنة هي **Slack** و**Telegram** و**WhatsApp Cloud API** والبريد الإلكتروني عبر webhooks لموفر الخدمة (Resend أو SendGrid). لا يمكن توصيل Discord أو Microsoft Teams حتى يتم تنفيذ محول تم التحقق منه وتسجيله.
