...
- **الافتراضي:** روابط تسجيل الدخول السحرية أولاً في Better Auth عندما يكون البريد الصادر جاهزًا، مع استخدام البريد الإلكتروني/كلمة المرور كخيار احتياطي ومقدمي الخدمات الاجتماعية. اضبط `AUTH_MAGIC_LINK=0` لفرض الخيار الاحتياطي.
...
<Diagram id="doc-block-5y2l0x" title={"ثلاث طرق في جلسة واحدة"} summary={"يقوم كل زوار المستعرض ومضيفي MCP والموفرين المخصصين بالتوصل إلى نفس AuthSession الذي يقرأه النطاق النهائي."}>
...
<div class="auth-modes">
  <div class="diagram-col">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >email/password &middot; جوجل &middot; جيثب</small
      >
    </div>
    <div class="diagram-card">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >موظف &middot; مصادقة0 &middot; Firebase</small
      >
    </div>
  </div>
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted"
      >بريد إلكتروني &middot; orgId &middot; orgRole</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
## تسجيل الدخول في التطوير المحلي {#local-development-sign-in}

على خادم تطوير يستخدم عنوان loopback، تعرض صفحة تسجيل الدخول المشتركة **Continue as local dev** أولًا. تنشئ حساب تطوير وجلسة حقيقيين في Better Auth أو تعيد استخدامهما، لذلك تستخدم مسار المصادقة نفسه المستخدم في بقية التطبيق.

اختر **Show full sign in options** لعرض خيارات التسجيل العادية وكلمة المرور والرابط السحري ومقدمي الخدمات الاجتماعية. إذا تعذر إنشاء الجلسة المحلية، تعرض الصفحة هذه الخيارات تلقائيًا حتى تتمكن من متابعة المصادقة القياسية.

تتوفر هذه الميزة فقط عندما يتصل المتصفح عبر `localhost` أو `127.0.0.0/8` أو `::1` في بيئة التطوير أو الاختبار. ويتم تعطيلها في معاينات النشر والإنتاج المستضاف. إنها ليست تجاوزًا للمصادقة. اضبط `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` لإلغاء الاشتراك وعرض صفحة تسجيل الدخول العادية في التطوير المحلي.

## مصادقة أفضل (افتراضي) {#better-auth}

بشكل افتراضي، تقوم Better Auth بتشغيل المصادقة. ويقدم:

- التسجيل عبر رابط سحري وتسجيل الدخول، مع البريد الإلكتروني/كلمة المرور كخيار احتياطي
...
- `POST /_agent-native/auth/magic-link` — طلب رسالة بريد إلكتروني تحتوي على رابط سحري
...
أو معاينة البيئات، وتسمية حسابات الاختبار بعنوان `+autoz`
(`name+autoz@example.com`) حتى يسهل التعرف عليها.

## سياسة التحقق من البريد الإلكتروني {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` هو وسيلة راحة محلية ولضمان الجودة — وتتجاهله أي
عملية نشر مُستضافة. لتحديد السياسة لأي بيئة، بما في ذلك الإنتاج، صرِّح بها في
الإعدادات:
...
الاسم المستعار للنشر هو `AUTH_REQUIRE_EMAIL_VERIFICATION=0`. القيمة المُصرَّح بها
تتفوق على `AUTH_SKIP_EMAIL_VERIFICATION` وعلى الافتراضي الخاص بكل بيئة معًا.

تقبل القيمة `false` عنوانًا غير مُتحقق منه كبيانات اعتماد لتسجيل الدخول — إذ
يمكن لأي شخص المطالبة بأي بريد إلكتروني — لذا استخدمها فقط حيث يكون الاشتراك
مقيدًا بالفعل بطريقة أخرى. أما `true` في عملية نشر بلا مزوّد بريد إلكتروني فتعطل
الاشتراك بكلمة مرور بدلًا من ذلك، لأن رسالة التحقق لن يمكن تسليمها أبدًا.
...
<div class="mcp-flow">
  <div class="diagram-node">
    1 &middot; طلب MCP<br /><small class="diagram-muted">لا رمز</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node warn">
    2 &middot; تحدي 401<br /><small class="diagram-muted"
      >WWW-Authenticate</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    3 &middot; اكتشف البيانات الوصفية<br /><small class="diagram-muted"
      >.well-known</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    4 &middot; تسجيل العميل<br /><small class="diagram-muted"
      >ديناميكية، عامة</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    5 &middot; تفويض + MCP<br /><small class="diagram-muted">تبادل الكود</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node ok">
    6 &middot; الوصول + التحديث<br /><small class="diagram-muted"
...
تستخدم صفحات التطبيق العادية غلاف SSR واحدًا عامًا وقابلاً للتخزين المؤقت لجميع الزوار. يتحقق `AppProviders` من الجلسة على العميل؛ وفي مسار خاص مثل `/dashboard` يعيد توجيه الزائر المجهول إلى `/sign-in?return=%2Fdashboard` قبل عرض واجهة التطبيق الخاصة. بعد تسجيل الدخول يعود الزائر إلى `/dashboard`. استخدم `isPublicPath` للمسارات العامة أو مسارات SEO، أو `sessionBypass` لسطح تتم مصادقته بآلية محددة أخرى مثل رمز تضمين MCP.

مدة تخزين هذا الغلاف مؤقتًا هي إعداد على مستوى عملية النشر بأكملها — انظر [SSR Caching](/docs/ssr-caching) لـ `AGENT_NATIVE_SSR_CACHE`. تقصير التخزين المؤقت أو تعطيله يغيّر المدة فقط؛ لا يزال SSR يعرض الفرع المجهول مع إزالة ملفات تعريف الارتباط.

### خلف الكواليس: Google OAuth

يربط كلا التدفقين (نقطة الدخول `/sign-in` الصريحة وحالة المسار ذي الإشارة المرجعية) الإرجاع URL خلال حالة OAuth. الولاية موقعة بـ HMAC، لذا لا يمكن تزويرها أثناء النقل. في رد الاتصال، تتم إعادة التحقق من صحة الإرجاع URL باعتباره نفس الأصل قبل إعادة التوجيه - لذلك لا يزال من غير الممكن تحويل مفتاح التوقيع المسرب إلى أوراكل إعادة توجيه مفتوحة.
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | عيِّنه على `0` أو `1` لتحديد سياسة التحقق لاشتراكات كلمة المرور في أي بيئة، بما في ذلك الإنتاج؛ يتفوق على `AUTH_SKIP_EMAIL_VERIFICATION`                                 |
...
تتحكم علامة Dispatch الافتراضية المتوقفة `desktop.workspace-sso` في الإتاحة فقط؛ وتظل فحوص المصادقة والتوقيع وnonce والأصل والاستدعاء والجلسة والإلغاء إلزامية دائمًا. عند توقف العلامة أو تعذر قراءتها، لا يغيّر Desktop تسجيل الدخول أو الخروج لكل تطبيق ولا إعداداته.
علامة وكيل مستخدم Canary هي تلميح للإتاحة وليست إثباتًا عن بُعد؛ ولا تعتمد عليها المصادقة أو التفويض.
تتعرف التطبيقات المستضافة الأساسية على طلبات Desktop SSO Canary المجمعة من دون إعداد بيئة مركز هوية لكل تطبيق. وتبقى المتصفحات العادية على تسجيل الدخول المعتاد لكل تطبيق. تحتاج التطبيقات ذاتية الاستضافة إلى `AGENT_NATIVE_IDENTITY_HUB_URL` فقط لمسار Desktop الصريح أو اتحاد الخادم المتبقي. لا تبدأ إصدارات Desktop المستقرة الوسيط قبل الموافقة على إصدار لاحق بشكل منفصل.
لا يعترض إصدار Canary تنقّل تسجيل الدخول المجهول للتطبيق. يجب على المستخدم اختيار تسجيل الدخول إلى مساحة العمل صراحةً من إعدادات سطح المكتب للتطبيق المسجّل النشط؛ ويظل تسجيل الدخول العادي للتطبيق هو المسار الافتراضي.

## النشر المُدار وتسجيل الدعوات

للنشر المُدار، اضبط `AUTH_SIGNUP=invited` و`ORG_CREATION=closed` و`AUTO_CREATE_DEFAULT_ORG=0`. يشرح [دليل النشر المُدار](/docs/administered-deployments) حسابات مسؤولي التهيئة، والدعوات، والنطاقات المسموح بها. دعوة العضوية وسياسة الانضمام عبر النطاق قاعدتان منفصلتان.
