...
<Diagram id="doc-block-139f5ge" title={"من المصدر إلى الإجابة المذكورة"} summary={"يستوعب الدماغ المصادر المعتمدة في لقطات أولية، ويقطر الذاكرة الدائمة، ويمررها عبر المراجعة، وعندها فقط يجيب على الاستشهادات."}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    اللقطات الخام<br /><small class="diagram-muted">مزال التكرار، منقح</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted">حقائق · قرارات · عمليات</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">معتمد، ذري</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-1uh7c1t">
  <Screen
    surface="desktop"
    html={
...
لا توجد متطلبات لقاعدة بيانات متجهة، لذا يظل القالب قابلاً للنقل عبر PGlite،
Postgres، وNeon، وhosted PostgreSQL، والمضيفون المشابهون. تعكس حالة التطبيق
...
<DataModel
  ...
  entities={[
    ...
    },
    {
      ...
      note: "الحمولات الأولية التي تم استيعابها",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "قائمة انتظار التقطير في الخلفية",
      fields: [
        {
          name: "operation",
          type: "text",
...
<Endpoint id="doc-block-1ahk6lb" title={"Webhook إدخال موقّع"} summary={"تنشر Clips وعمليات استيراد transcript/capture العامة RawCapturePayload مع رمز حامل لكل مصدر."} method="POST" path="/api/_agent-native/brain/ingest" summary={"قم باستيراد لقطة أولية من Clips أو مصدر عام"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
أجرِ هذه التغييرات في مسار تطوير المستودع. يمكن للوكيل المضمّن المساعدة في
تحرير المصدر فقط عندما يمنحه المضيف عمدًا أدوات كتابة repository/workspace؛
وبخلاف ذلك يعمل من خلال actions والبيانات. انظر
...
- [**موارد الوكيل**](/docs/agent-resources) — الموارد المشتركة عبر التطبيقات
