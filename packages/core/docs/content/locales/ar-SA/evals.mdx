...
<Diagram id="doc-block-1xedr0l" title={"من المدخلات الثابتة لنشر البوابة"} summary={"يقوم العداء فعليًا بتشغيل حلقة الوكيل في كل حالة، ويسجل المخرجات، ويخرج من الصفر إذا انخفض أي مسجل عن العتبة - مما يجعله بوابة CI قابلة للإسقاط."}>
...
<div class="eval-flow">
  ...
  <div class="diagram-node">
    تشغيل حلقة الوكيل<br /><small class="diagram-muted"
...
<Diagram id="doc-block-aa66rm" title={"خط أنابيب الهداف ذو الأربع خطوات"} summary={"المعالجة المسبقة والتحليل الافتراضي للهوية؛ مطلوب فقط generatorScore. يمكن للتحليل تشغيل JS عادي أو استدعاء القاضي LLM عبر ctx."}>
...
agent-native eval billing            # فقط الملفات التي يحتوي مسارها على "billing"
...
- [**Actions**](/docs/actions-overview) — الأدوات/actions التي تظهر في `toolCalls`
