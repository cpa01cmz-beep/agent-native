...
<Diagram id="doc-block-e8kdpo" title={"تشغيل دورة الحياة"} summary={"startRun يفتح صفًا قيد التشغيل؛ updateRunProgress يصححه؛ completeRun ينقله إلى حالة طرفية واحدة ويختم completed_at."}>
...
<Endpoint id="doc-block-aqgfrg" title={"قائمة عمليات التشغيل النشطة"} method="GET" path="/_agent-native/runs" summary={"List the caller's runs. The RunsTray polls this with active=true."} auth="Session cookie (owner-scoped)" params={[
  {
    "name": "active",
    "in": "query",
    "type": "boolean",
...
- [**البيانات والمزامنة**](/docs/client-data) — `useDbSync` لإبطال ذاكرة التخزين المؤقت في الوقت الفعلي
