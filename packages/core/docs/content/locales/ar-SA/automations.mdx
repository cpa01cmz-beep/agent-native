...
**الأتمتة** هي مهمة ينفذها الوكيل عند حلول موعد أو وقوع حدث: _عندما يحدث X، افعل Y_. **Scheduled** و**Event** نوعان من المشغلات، وليسا منتجين منفصلين.

كان إطار العمل يعرض الأتمتة المجدولة سابقًا باسم **Recurring Jobs**. يبقى هذا الاسم ومسار `jobs/<name>.md` وأداة `manage-jobs` للتوافق. الإرشادات الجديدة تستخدم **Automations** كمفهوم شامل؛ راجع [Recurring Jobs](/docs/recurring-jobs) عند صيانة مهمة مجدولة قديمة أو استخدام نطاق المؤسسة والحقول المتقدمة.
...
<div class="auto-flow">
  ...
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >يتحقق هايكو: &ldquo;ينتهي البريد الإلكتروني بـ @builder.io&rdquo;</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      >الإجراءات &middot; web-request &middot; MCP &middot; sub-agents</small
    >
  </div>
</div>
```
...
### من واجهة Automations

افتح **Automations** في صفحة الوكيل الكاملة لعرض الأتمتة المجدولة والمشغلة بالأحداث وإدارتها. يبقى عنوان التوافق `/agent#jobs` حتى تستمر الروابط والإشارات المرجعية الحالية في العمل.

<Callout tone="warning">

تدعم أداة `manage-automations` الأتمتة الشخصية وأتمتة المؤسسة لكل من المشغلات المجدولة والمشغلة بالأحداث. تكون أتمتة أحداث المؤسسة مرئية للأعضاء، لكنها تعمل دائمًا بهوية منشئها؛ ويمكن للمسؤولين إدارتها من دون تغيير تلك الهوية.
...
<AnnotatedCode
  id="doc-block-1nt3xht"
  title={"أتمتة ناجمة عن الأحداث"}
...
| `mode`        | `"agentic"`               | `"agentic"`  | حلقة الوكيل الكاملة (الوضع الوحيد المدعوم). تمت إزالة `"deterministic"` — لم يتم تنفيذه قط، وعمليات الأتمتة التي تحدده لم تُطلق أبدًا. |
...
يحدد `owner` في بيانات تعريف الحدث الأتمتة التي يمكن تشغيلها. تتطلب أتمتة الأحداث الشخصية وأتمتة المؤسسة تطابقًا تامًا مع هوية `createdBy`.
...
تستخدم عمليات الأتمتة الشخصية وأتمتة المؤسسة أداة `manage-automations` مع معلمة `action`:
...
تدعم `manage-automations` نطاق المؤسسة وتجاوزات النموذج وقوائم السماح الدقيقة لأدوات MCP. وتبقى `manage-jobs` أداة توافق للتكاملات المجدولة القديمة وبيانات التسليم.
...
<Endpoint id="doc-block-1jlxsh5" title={"إطلاق حدث اختباري"} method="POST" path="/_agent-native/automations/fire-test" summary={"قم بإصدار حدث test.event.fired للتحقق من صحة العمليات التلقائية التي يتم تشغيلها بواسطة الحدث"} responses={[
  {
    "status": "200",
...
<div class="disp">
  ...
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >مطابقة <code>createdBy</code> مع مالك الحدث والتحقق من عضوية
      المؤسسة</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >Haiku yes/no على الحمولة &mdash; خطأ شنيع &rarr;
...
- [**Actions**](/docs/actions-overview) — يمكن للأتمتة استدعاء أي إجراء مسجل عبر حلقة الوكيل
