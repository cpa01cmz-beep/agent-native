...
<Diagram id="doc-block-608ry9" title={"طائرتان للحكم"} summary={"Git يحكم التعليمات البرمجية؛ Dispatch يحكم وقت التشغيل. إنهما متكاملان، فلا تنسخ أحدهما داخل الآخر."}>

```html
<div class="gov">
  <div class="diagram-card">
    <span class="diagram-pill accent">Git وGitHub</span
    ><strong>حوكمة الكود</strong>
    <div class="gov-list">
      <span class="diagram-pill">CODEOWNERS</span
      ><span class="diagram-pill">branch protection</span
      ><span class="diagram-pill">مراجعة PR</span
...
ليس كل من يحتاج إلى إجراء تغييرات يشعر بالارتياح تجاه git. يدعم [Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=workspace_management) نموذج التفرع المرئي الذي يعين فروع git الموجودة تحت الغطاء - وهو مفيد لتغييرات المحتوى والنسخ، وتعديلات التخطيط، وتكرارات التصميم، واختبار A/B بدون بيئة تطوير.
...
<FileTree
  id="doc-block-1mdjyq3"
  title={"إعدادات الحوكمة في repo"}
  entries={[
    {
      path: ".github/CODEOWNERS",
      note: "يعيّن reviewers تلقائياً حسب المسار المتغير",
    },
    {
      path: ".github/labeler.yml",
      note: "يضيف labels تلقائياً إلى PRs حسب app",
    },
    {
      path: "pnpm-workspace.yaml",
      note: "على مستوى workspace: review واسعة",
    },
    {
      path: "package.json",
      note: "على مستوى workspace: يملكه فريق platform",
    },
  ]}
/>
...
| **عمليات ترحيل المخطط**              | فريق المنصة + مهندس أول                     | أمان البيانات، توافق PostgreSQL                                |
...
للاطلاع على نموذج المورد والمسارات الأساسية، راجع [موارد الوكيل — Global resources](/docs/agent-resources#global-resources).

## قائمة التحقق من الإعداد

للحصول على مساحة عمل جديدة، بعد تشغيل `npx --yes @agent-native/core@latest create`:
...
- [ ] قم بتسجيل اتصالات مساحة العمل القابلة لإعادة الاستخدام لحسابات الموفر المشتركة، ثم
      ...
      هذا الحساب
- [ ] قم بإضافة skills على مستوى مساحة العمل وتعليمات الدرابزين والموارد المرجعية للعلامة التجارية/الشركة عبر صفحة الموارد. راجع [موارد الوكيل](/docs/agent-resources#global-resources) للحصول على جدول نموذج الموارد الكامل وحزمة البداية الموصى بها.
