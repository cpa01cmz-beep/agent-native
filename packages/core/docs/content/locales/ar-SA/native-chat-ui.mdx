...
<Diagram id="doc-block-c8ctoa" title={"مسار العرض الأصلي"} summary={"يقوم الإجراء بإرجاع JSON؛ يطابق وقت التشغيل أداة تمييز صريحة أو chatUI.renderer؛ يقوم AssistantChat بتثبيت مكون React حقيقي. لا يوجد إطار iframe ولا يوجد تنفيذ HTML."}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    الإجراء يعمل<br /><small class="diagram-muted">يعيد JSON منظما</small>
  </div>
...
<Callout id="doc-block-14mzarh" tone="success">

يتولى العارض المسؤولية فقط عندما يعلن الإجراء `chatUI` **أو** تحمل النتيجة تمييزًا صريحًا معروفًا `widget`. لا يستنتج أبدًا شكلًا لكائنات عشوائية ولا ينفذ أبدًا HTML أو JavaScript من نتائج الأداة - لذا لا يمكن أن تصبح الأداة الأصلية متجهًا للحقن.
...
قم بتحديد أبعاد الـ renderers بوحدات مطلقة وليس بنسب مئوية. ينمو سجل المحادثة
ليلائم رسائله، لذا لا يملك حاوي الـ renderer ارتفاعًا محددًا تُحسب النسبة المئوية
بالنسبة إليه — فتنهار `height: 40%` على عمود الرسم البياني إلى صفر بدلاً من أن تُصدر
خطأ، وهو ما يبدو كأن الـ renderer فشل في الرسم. امنح الرسوم البيانية وغيرها من
العناصر المعتمدة على الارتفاع أساسًا بالبكسل (أو `rem`)؛ أما العروض فلا بأس بها
كنسب مئوية لأن عمود المحادثة يملك عرضًا محددًا.
...
<Diagram id="doc-block-1qei37v" title={"يحتفظ وقت تشغيل BYO بقشرة الدردشة Agent-Native"} summary={"يقوم وكيلك الخارجي ببث الأحداث التي تمت تسويتها من خلال موصل؛ Agent-Native يحتفظ بالمؤلف والنص وبطاقات الأدوات والموافقات والأدوات الأصلية."}>

```html
<div class="diagram-byo">
  <div class="diagram-box" data-rough>
    وكيلك<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">أحداث message-* / tool-* موحدة</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
...
- [Actions](/docs/actions-overview) — حدد العمليات التي تُرجع بيانات عنصر واجهة المستخدم الأصلية.
