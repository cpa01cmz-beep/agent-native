...
<Diagram id="doc-block-3en0hq" title={"الدفاع في طبقات"} summary={"يمتلك الإطار معظم سطح التهديد؛ أنت تمتلك شيئين - وضع علامات على الجداول لتحديد نطاق المدخلات الخارجية والتحقق من صحتها."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">يمتلكه الإطار</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">أنت تملك التحكم</span
    ><small class="diagram-muted"
...
يُنشئ الإطار سياق الطلب من الجلسة ويطبق الملكية عبر `accessFilter()` و`assertAccess()` على قراءات وكتابات التطبيق. يتلقى SQL الخاص بالوكيل طرق عرض مؤقتة مفلترة للهوية والمنظمة النشطة؛ لم يتم تمكين RLS بعد، لذلك يجب أن تستخدم استعلامات التطبيق هذه المساعدات.
...
<Callout id="doc-block-1jrymve" tone="risk">

لا تقم أبدًا بإنشاء SQL عن طريق تسلسل السلسلة أو القيم الحرفية للقالب. قم بتمرير إدخال المستخدم كـ `args` إلى `exec` / `db-query`، أو استخدم Drizzle — كلاهما يحددان المعلمات دائمًا. تقوم عمليات التحقق `pnpm guards` بالتقاط الاستعلامات غير ذات النطاق والمتسلسلة في وقت CI.
...
<Diagram id="doc-block-u1dtvr" title={"خط أنابيب النطاق"} summary={"لا يلمس الوكيل SQL الجداول الأساسية مباشرة أبدًا - فهو يقرأ من خلال عرض مؤقت محدد للهوية الحالية، لذلك يمكن لاسم الجدول المجرد إرجاع الصفوف المملوكة فقط."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    جلسة مسجلة الدخول<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    SQL الوكيل<br /><small class="diagram-muted">bare table names only</small>
  </div>
</div>
```
...
ترفض أدوات `db-query` / `db-exec` مراجع الجدول المؤهلة للمخطط (`public.<table>`) - يتم تحليل الاسم المؤهل إلى الجدول الأساسي وسيتجاوز العرض المؤقت أعلاه. يستخدم الوكلاء أسماء الجداول العارية؛ يتم تطبيق النطاق تلقائيًا.
...
<DataModel
  ...
  summary={"الأعمدة الثلاثة التي تجعل الجدول مدركًا للمستأجر وقابلاً للمشاركة."}
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "مؤسسة المالك النشطة عند الإنشاء. يقود عمليات التحقق من رؤية المؤسسة.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "خاص | منظمة | عام - افتراضي خشن، افتراضي خاص.",
        },
      ],
    },
  ]}
...
يتم تشفير بيانات الاعتماد لكل مستخدم/لكل مؤسسة والمخزن باستخدام AES-256-GCM. تفضل خزنة مساحة العمل `WORKSPACE_SECRETS_ENCRYPTION_KEY`، ثم المفاتيح القديمة المشتقة من `SECRETS_ENCRYPTION_KEY` و`A2A_SECRET` و`BETTER_AUTH_SECRET`؛ أثناء التدوير استخدم `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` حتى تُرحّل القراءات الصفوف. لتشفير صفوف بيانات الاعتماد النصية الموجودة، شغّل `pnpm action db-migrate-encrypt-credentials` (غير فعال وغير مدمر).
...
ترفض معالجات خطاف الويب الوارد (Resend، وSendGrid، وSlack، وTelegram، وWhatsApp، وRecall.ai، وDeepgram، وZoom) الطلبات المزيفة بشكل افتراضي في الإنتاج: عندما يكون env var السري للتوقيع مفقودًا، يقوم المعالج بإرجاع 401 بدلاً من القبول والإرسال.
...
لا تمر إشعارات `changes.watch` في Google Drive عبر بوابة المحول العامة؛ فهي طلبات مبهمة بلا نص. لذلك يسجل الإطار رمزًا عشوائيًا لكل قناة ويتحقق من رؤوس `X-Goog-Channel-ID` و`X-Goog-Channel-Token` و`X-Goog-Resource-ID` قبل جلب التغييرات. هذه إشعارات مباشرة من Drive وليست رسائل Pub/Sub، ولا تحمل رموز OIDC الخاصة بـ Pub/Sub.
...
- [ ] عيّن `WORKSPACE_SECRETS_ENCRYPTION_KEY` باستمرار للخزن المشتركة متعددة التطبيقات؛ تبقى مفاتيح `SECRETS_ENCRYPTION_KEY` و`BETTER_AUTH_SECRET` بدائل للتشفير المحلي - راجع [Secrets Management](#secrets)
...
- [**Authentication**](/docs/authentication) — أوضاع المصادقة، ونطاقات الكوكيز (cookie)، وتفاصيل الجلسة التي تُبنى عليها هذه الصفحة
- [**Multi-Tenancy**](/docs/multi-tenancy) — نموذج عضوية المؤسسة وراء تحديد النطاق بـ `org_id`
- [**Sharing**](/docs/sharing) — نمط `ownableColumns()` وحارس الوصول المُطبَّق على الموارد القابلة للمشاركة
- [**Messaging**](/docs/messaging#env-vars) — أسرار توقيع webhook لكل تكامل، المُشار إليها في قائمة التحقق أعلاه
- [**Deployment**](/docs/deployment) — متغيرات بيئة الإنتاج التي تفترض قائمة التحقق هذه أنها مُعيَّنة
