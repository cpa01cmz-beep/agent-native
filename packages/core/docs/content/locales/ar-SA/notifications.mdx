...
<Diagram id="doc-block-5nzjwo" title={"مكالمة واحدة، وجهات عديدة"} summary={"يقوم notify() دائمًا بكتابة صف البريد الوارد الخاص بنطاق المالك, وتوجيهه إلى كل قناة مسجلة بالتوازي (أفضل جهد)، ثم إرسال الإشعارات المرسلة في ناقل الأحداث."}>
...
تُسجَّل قنوات البريد الإلكتروني وSlack والـ webhook دائمًا إلى جانب صندوق الوارد. يفضّل التسليم عنوان URL أو المستلمين المحددين في بيانات الإشعار، ثم يعود إلى متغير البيئة المطابق. وإذا لم يُضبط أي منهما، فلا تنفّذ تلك القناة شيئًا عند الإرسال.
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>تحدد الخطورة الشارة</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">تأكيدات، FYI</span>
    </div>
    <div class="sev-row">
...
- **تكامل الاستقصاء** — تستدعي كل طفرة `recordChange()`, لذا يتم إبطال صحة القوالب التي تستخدم [`useDbSync`](/docs/client-data#usedbsync) تلقائيًا دون أي أسلاك إضافية.
...
- [**Server plugins**](/docs/server-overview) — حيث يتم تسجيل القنوات المخصصة عند بدء التشغيل
