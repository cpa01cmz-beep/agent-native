...
<Diagram id="doc-block-5xxioz" title={"توجيه العميل: العديد من المصادر وسجل أداة واحد"} summary={"يتم دمج ملفات التكوين وenv وواجهة المستخدم الخاصة بوقت التشغيل في مدير MCP؛ تظهر أدواته مسبوقة وقابلة للبحث عن الأدوات جنبًا إلى جنب مع إجراءات تطبيقك. هذه هي مرآة اتجاه الخادم."}>
  ...
  <div class="diagram-col sources">
    <div class="diagram-box" data-rough>
      Workspace <code>mcp.config.json</code><br /><small class="diagram-muted"
        >مشترك بين التطبيقات</small
      >
    </div>
    <div class="diagram-box" data-rough>
      جذر التطبيق <code>mcp.config.json</code><br /><small class="diagram-muted"
        >تجاوز لكل تطبيق</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      بعيد عبر واجهة الإعدادات<br /><small class="diagram-muted"
  ...
  annotations={[
    {
      lines: "3",
      label: "معرف الخادم",
      note: "يصبح المفتاح هو بادئة الأداة: تظهر أدوات هذا الخادم كـ `mcp__claude-in-chrome__*` في سجل الوكيل، بحيث يمكنها 't collide with your template's الإجراءات.",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` يُنتج ثنائيًا محليًا. خوادم Stdio مخصصة **للتطوير المحلي** — وهي محظورة في أوقات تشغيل الحافة.",
    },
    {
      lines: "6",
      label: "بيئة العملية",
...
معرّفات الإعدادات المسبقة هي `context7` و`sentry` و`notion` و`semgrep` و`linear`
و`atlassian` و`supabase` و`neon` و`stripe` و`cloudflare` و`gitlab` و`figma`
و`vercel` و`github` و`slack` و`asana` و`hubspot` و`intercom` و`monday` و`webflow` و`paypal` و`box` و`netlify` و`zapier`.

يميّز الدليل بين نقاط النهاية التي تدعم تدفق OAuth العام والمزوّدين الذين
يتطلبون عميلاً معتمداً من المزوّد أو تطبيق OAuth مسجلاً مسبقاً. تبقى هذه
الإدخالات متاحة للاكتشاف وترتبط بتعليمات الإعداد الرسمية، لكنها لا تعرض زر
اتصال بنقرة واحدة عندما لا يكون ذلك مدعوماً. يمكن للإضافات المعزولة استخدام
`agentNative.mcp.listTools` و`agentNative.mcp.callTool` و
`agentNative.providerApi.catalog(...)` و`agentNative.providerApi.docs(...)`
عبر إجراءات المضيف لاكتشاف البيانات المتصلة؛ ولا تتلقى عناوين الاتصال أو رموز
الوصول أو رموز التحديث أبدًا. أما طلب `provider-api-request` الخام فمقصور
عمدًا على المضيف والوكيل: فتعريض طلبات حرة بأي طريقة أو مسار للإضافات
المشتركة قد يحوّل الإضافة إلى وكيل كتابة مُصادَق عليه، لذا يجب أن تستخدم
الإضافات `agentNative.mcp.callTool(...)` أو إجراء تطبيق مخصص لهذا الغرض
لتنفيذ عمليات مزوّد محدودة النطاق بدلاً من ذلك.

يجب أن تسمح الإضافات المحلية صراحةً بـ `list-mcp-tools` و`call-mcp-tool` وأي
إجراء مزوّد مدعوم تستخدمه في `extension.json`. تتوفر أدوات الاكتشاف أيضًا عبر
`agentNative.connectors.mcp` و`agentNative.connectors.providerApi`.

### Atlassian / Jira

يستخدم إعداد Atlassian المسبق نقطة نهاية OAuth 2.1 الخاصة بـ Atlassian Rovo MCP،
`https://mcp.atlassian.com/v1/mcp/authv2`، ويوفّر أدوات Jira وConfluence.
يتطلب إنشاء تذاكر Jira أذونات Atlassian التي تشمل عمليات الكتابة في Jira؛
ولا يرى المستخدم سوى المشاريع والمحتوى المسموح بهما لحسابه على Atlassian.

يمكن لمسؤولي مؤسسات Atlassian التحكم في السماح لعميل OAuth بالاتصال.
قبل الاتصال، قد يحتاج المسؤول إلى إضافة أصل HTTPS الفعلي للتطبيق إلى قائمة
النطاقات الموثوقة ضمن **Atlassian Administration → Rovo →
Rovo MCP server → Domains**، وتمكين أذونات **Read** و**Write** و**Search**
المطلوبة ضمن **Permissions**. يجب أن يكون النطاق هو المضيف الذي يعمل عليه
تطبيق agent-native؛ لا تنسخ `https://builder.io/**` إلى عملية نشر أخرى إلا إذا
كان ذلك هو أصلها الفعلي. راجع [دليل إعداد Rovo MCP من Atlassian](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/)
و[عناصر تحكم المسؤول](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/).

يوفّر الكتالوج نقطة النهاية وإرشادات الإعداد. يخزّن نموذج الإعدادات العام نقطة
النهاية والرؤوس المشفّرة الاختيارية؛ ولا يحل محل تدفق موافقة OAuth التفاعلي
الخاص بـ Atlassian. استخدم عميل MCP أو بيئة تشغيل تدعم OAuth عندما يتطلب
Atlassian مصادقة OAuth التفاعلية. إذا كان الموفّر أو بيئة التشغيل يدعم رمز
Bearer ثابتًا، فاتبع مسار إدارة الأسرار المعتمد لديه، ولا تلصق الرموز أبدًا
في المطالبات أو الإعدادات الملتزم بها.
...
يتم قبول HTTPS URL في كل مكان؛ يُسمح فقط بـ `http://` العادي لـ `localhost` أثناء التطوير. يتم إدخال المصادقة الاختيارية كرمز مميز لحامل يتم إرساله عبر `Authorization: Bearer …` عند كل طلب. يتطلب الموفّرون الذين يدعمون OAuth فقط، بما في ذلك Atlassian Rovo MCP، تدفق OAuth الخاص بالموفّر بدلاً من رأس ثابت.
...
<Diagram id="doc-block-19vcxxd" title={"نموذج Hub: يخدم تطبيق واحد خوادم MCP ذات النطاق التنظيمي"} summary={"Dispatch يحمل خوادم MCP ذات النطاق التنظيمي؛ تقوم تطبيقات المستهلك بسحبها ودمجها كـ mcp__hub_<orgId>_<name>__*. تتم مشاركة صفوف نطاق المؤسسة فقط، وتظل بيانات الاعتماد الشخصية في مكانها."}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">خوادم MCP على نطاق المؤسسة</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">سحب + دمج كل ~60 ثانية</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-1cvykad" method="GET" path="/_agent-native/mcp/status" summary={"MCP حالة العميل للأدوات والإعداد"} responses={[
  {
    "status": "200",
...
- تم تكوين `command` بشكل خاطئ → تم تخطي الخادم، ويظهر خطأه في `/_agent-native/mcp/status` ضمن `errors.<server-id>`، ويستمر كل خادم آخر في العمل.
