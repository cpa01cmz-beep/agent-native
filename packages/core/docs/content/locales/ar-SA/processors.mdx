...
<Diagram id="doc-block-18d4lyv" title={"حيث تطلق الخطافات الثلاثة أثناء الركض"} summary={"يراقب processOutputStream كل قطعة، وتستدعي أداة البوابات processOutputStep كل استجابة، ويسجل processOutputResult حكمًا في النهاية. يمكن إحباط أي خطاف باستخدام TripWire."}>
...
<div class="diagram-proc">
  ...
  <div class="diagram-node" data-rough>
    per model response<br /><small class="diagram-muted"
      >processOutputStep — يضبط استدعاءات الأدوات</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    run end<br /><small class="diagram-muted"
      >processOutputResult — الحكم</small
    >
  </div>
...
المعالج هو **تكوين**، وليس أداة، وليس إجراءً، وليس تأليفًا DSL. تقوم المعالجات فقط بمراقبة وتعديل حالة نطاق الدفق الخاصة بها و`abort()`. ولا تحدد أبدًا سلوك التطبيق، أو تستبدل actions, أو تظهر للنموذج. تنتمي عمليات التطبيق إلى [actions](/docs/actions-overview).
