...
description: "تحرير تعاوني متعدد المستخدمين حيث يكون وكيل الذكاء الاصطناعي نظيرًا من الدرجة الأولى: دمج CRDT، والتواجد المباشر، والمسار السريع SSE، والدمج الدقيق من جانب الخادم - على PostgreSQL/PGlite وأي مضيف."
---
...
- **يعمل في كل مكان** — PostgreSQL/PGlite.
...
الجدول مُدار بواسطة إطار العمل ولا يعتمد على الموفر (يستخدم PGlite وPostgres
...
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Postgres على مضيف يمكن الوصول إليه علنًا**                | تتصل البوابة بقاعدة البيانات من شبكتها الخاصة. أما PGlite و`localhost` و`*.internal` وعناوين IP المجردة فتُرفض قبل أن تغادر سلسلة اتصال PostgreSQL جهازك أصلًا. | أن يشير `DATABASE_URL` إلى Postgres مُدار مثل Neon                                                                                                                                            |
...
اتصال PostgreSQL والأصل العام الخاصين به، فيستعيد معرّف قناة وسرّ توقيع، ثم يصدر رموز
...
**التسجيل الذاتي يعني إرسال سلسلة اتصال PostgreSQL إلى Builder.** يخزّنها Builder
