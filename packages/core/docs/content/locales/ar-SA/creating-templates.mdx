...
النماذج عبارة عن تطبيقات متكاملة ومتشعبة للوكلاء الأصليين تعمل على حل سير العمل الحقيقي. تم إنشاء قوالب الطرف الأول بنفس سطح إطار العمل الذي تستخدمه: مسارات React لـ UI، وبيانات PostgreSQL عبر Drizzle، وactions للعمليات، وموارد مساحة العمل لسلوك الوكيل، ومزامنة الاستقصاء بحيث يظل الوكيل وUI متوازيين.
...
ضع الطبقات الثلاث في الاعتبار عند تحديد مكان الإمكانية:

- **Core - الإطار:** عقد وقت التشغيل والبيانات الأساسي الذي يمكن لكل تطبيق استخدامه.
- **Toolkit - أجزاء اختيارية قابلة لإعادة الاستخدام:** واجهة مستخدم وأنظمة منتج مشتركة يمكن للتطبيقات اعتمادها أو تركيبها أو تفريعها.
- **Templates - تطبيقات اختيارية:** منتجات مجال كاملة مبنية على Core، وغالبًا ما تستخدم Toolkit، ويمكن تفريعها كنقاط بداية.

Toolkit وTemplates اختياريان. القالب ليس الإطار نفسه، وToolkit لا يحل محل Core.

نموذج جيد:

- يعمل على حل سير عمل واحد من البداية إلى النهاية، باستخدام بيانات أولية مفيدة أو تدفق في حالة فارغة.
- يخزن الحالة الدائمة في PostgreSQL/PGlite، وليس ملفات JSON.
...
تمنحك الدردشة مصادقة وسلاسل محادثات متينة وموارد مدعومة بـ PostgreSQL وأدوات وحالة التطبيق وactions ومزامنة الاستقصاء. يمكنك إضافة نموذج المجال والمنتج UI.

إذا لم تكن تنشئ قالب واجهة مستخدم قابلاً لإعادة الاستخدام بعد، فاستخدم [Automation-First Apps](/docs/pure-agent-apps): عرّف الإجراءات، وشغّلها من دون متصفح، وأضف واجهة المستخدم لاحقاً عندما يحتاج سير العمل إلى سطح دائم.
...
<FileTree
  id="doc-block-15r9qt6"
  title={"بنية مشروع ال template"}
  entries={[
    {
      path: "app/",
      note: "واجهة React",
    ...
    {
      path: "app/root.tsx",
      note: "HTML shell و providers",
    },
    {
      path: "app/routes/",
      note: "مسارات ملفات React Router",
    },
    {
      path: "app/components/",
      note: "UI الخاصة بال template",
    },
    {
      path: "app/hooks/",
      note: "hooks لحالة UI والبيانات",
    },
    {
      path: "actions/",
      note: "عمليات defineAction: المصدر الوحيد للحقيقة",
    },
    {
    ...
    },
    {
      ...
      note: "migrations إضافية",
    },
    {
      path: "server/plugins/",
      note: "تكاملات بدء التشغيل",
    },
    {
      path: "server/routes/api/",
      note: "routes مخصصة فقط عندما لا تكفي actions",
    },
    {
      path: "shared/types.ts",
      note: "client/server types مشتركة",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: إرشادات agent لل workflows المعقدة",
    },
    {
      path: "AGENTS.md",
      note: "تعليمات agent خاصة بال template",
    },
    {
      path: "package.json",
    },
...
لا تقم بإضافة دليل `data/` لحالة التطبيق. تنتمي بيانات التطبيق الدائمة إلى PostgreSQL/PGlite، ويقرأها UI من خلال actions أو معالجات الخادم المكتوبة.

تربط المناطق الأربع لكل قالب معًا من خلال سطح عمل مشترك واحد وقاعدة بيانات PostgreSQL/PGlite واحدة - الوكيل وUI شريكان متساويان في نفس العمليات:

<Diagram id="doc-block-7npsk8" title={"كيف تتصل المناطق الأربع للقالب"} summary={"يصل كل من واجهة المستخدم والوكيل إلى PostgreSQL/PGlite من خلال نفس الإجراءات؛ حالة التطبيق ومزامنة الاستقصاء تحافظ على توافقهما."}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      واجهة React<br /><small class="diagram-muted"
        >app/routes · المكونات</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">الإجراءات</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    PostgreSQL عبر Drizzle<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">مزامنة بالاستقصاء</div>
</div>
```
...
## بيانات النموذج في PostgreSQL {#data-models}

حدد جداول المجال باستخدام مساعدات Drizzle في إطار العمل بحيث تستخدم المخططات PostgreSQL وPGlite:
...
يجب أن تكون تغييرات المخطط إضافية. بالنسبة إلى الجداول والأعمدة الجديدة الخاصة
بالتطبيق، استخدم `drizzle-kit generate` لإنشاء SQL قابل للمراجعة ضمن
`server/db/migrations/<id_name>.sql`، ثم حمّله عبر
`runDrizzleMigrations()` من خطوة الإصدار في عمليات نشر Node.js. لا يوفر
Cloudflare Workers/Pages وبيئات edge الأخرى لا توفر نظام ملفات قابلًا للنشر، لذا ضمّن SQL المُنشأ في
حزمة العامل ومرّره إلى `runMigrations()` هناك. احتفظ بـ `runMigrations()` للتاريخ
المكتوب يدويًا والحالات الخاصة. لا تستخدم مطلقًا SQL أو `drizzle-kit push` أو
إعادة تسمية الجداول أو إسقاط الأعمدة.

لقراءة التطبيق وكتابته، استخدم منشئ الاستعلامات والمشغلات المحمولة Drizzle من `drizzle-orm`. لا تكتب رمز المنتج باستخدام SQL الخام عندما يتمكن Drizzle من التعبير عن الاستعلام، واستخدم مساعدات المخطط من `@agent-native/core/db/schema` في القوالب.
...
استخدم مستندات [Database](/docs/server-database) و[Security](/docs/security) قبل إضافة المخططات التي تحتوي على بيانات المستخدم أو بيانات المؤسسة.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "8",
      label: "سطح الأداة",
      note: "`description` هو ما يقرأه الوكيل ليقرر متى يتم استدعاء هذا الإجراء كأداة.",
    },
    {
      lines: "9-11",
      label: "عقد typed",
      note: "يقوم zod `schema` واحد بالتحقق من الإدخال من agent وواجهة المستخدم وHTTP وMCP وA2A.",
    },
    {
      lines: "18-19",
      label: "كتابة محددة النطاق",
      note: "ختم `ownerEmail` / `orgId` من `ctx` بحيث يتم تحديد نطاق الصف بشكل صحيح لعمليات التحقق من المشاركة والوصول.",
    },
  ]}
/>
...
- Data lives in PostgreSQL عبر Drizzle. Use actions for all writes; schema is additive.
...
تُثبَّت قوالب المجتمع مباشرة من مستودع GitHub عام. ضع تطبيق Agent-Native في جذر المستودع:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

أضف `#branch` أو `#tag` أو `#commit` لتثبيت مراجعة محددة:

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

تنزّل CLI المصدر فقط، ولا تثبّت الاعتماديات أو تشغّل برامج القالب. قوالب المجتمع شيفرة من جهات خارجية؛ راجع المستودع والترخيص والاعتماديات وبرامج التثبيت قبل تشغيل `pnpm install`.

للإدراج في الكتالوج، ضمّن تطبيقًا كاملًا في الجذر، ومستودعًا عامًا بترخيص وملف README للإعداد، واعتمادًا على `@agent-native/core`، وفحوص أنواع واختبارات ناجحة. العرض المستضاف ولقطة الشاشة اختياريان.

[أرسل قالب مجتمع](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml) عندما يصبح المستودع جاهزًا. تبقى إدراجات المجتمع منفصلة عن الكتالوج الرسمي ويصونها مؤلفوها.
