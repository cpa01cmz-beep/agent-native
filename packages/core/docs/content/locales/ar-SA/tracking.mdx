...
‏`source` هو إمّا `ctx` الخاص بالإجراء يُمرَّر كما هو، أو كائن بيانات وصفية `{ userId, anonymousId, sessionId }` للمستدعين الذين لا يملكون `ctx`.

‏جلسة المتصفح ليست شيئًا على المستدعي تمريره: فهي تأتي من سياق الطلب المحيط (`RequestContext.browserSessionId`، الذي يُضبط من ترويسة `X-Agent-Native-Session-Id` التي يرسلها عميل الإجراءات والدردشة ودالة `track()` في المتصفح)، فتُربط الزيارة نفسها سواء أطلقت الواجهة الحدث أم الوكيل. ويعيّنها كل مزوّد إلى حقل الجلسة الخاص به: `$session_id` في PostHog، وهو ما يربط الحدث بإعادة تشغيل الجلسة؛ وخاصية `session_id` في Mixpanel وAmplitude؛ و`sessionId` على المستوى الأعلى في الويب هوك وAgent-Native Analytics. وهي غائبة للمستدعين بلا متصفح (cron وCLI وMCP وA2A)، ويتجاوزها `sessionId` الممرَّر في كائن البيانات الوصفية.
...
export default defineAction({
  ...
  run: async ({ name, template }, ctx) => {
    ...
    // مرِّر ctx كما هو: يُنسب الحدث إلى ctx.userEmail ويرتبط بجلسة المتصفح
    // التي جاء منها الاستدعاء.
...
- [**Actions**](/docs/actions-overview) — حيث تنشأ معظم مكالمات التتبع
- [**Server Plugins**](/docs/server-plugins) — يعمل `registerBuiltinProviders()` في البرنامج الإضافي للمسارات الأساسية عند بدء التشغيل
