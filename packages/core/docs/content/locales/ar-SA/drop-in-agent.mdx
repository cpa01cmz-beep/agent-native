---
title: "وكيل الدخول"
description: "قم بتثبيت دردشة الوكيل + موارد الوكيل في أي تطبيق React باستخدام <AgentPanel>، و<AgentSidebar>، وsendToAgentChat()."
---
...
لست بحاجة إلى إنشاء وكيل أصلي من البداية. يتم شحن دردشة الوكيل وعلامة تبويب موارد الوكيل ومحطة CLI والإدخال الصوتي وجميع البنية التحتية ذات الصلة كمجموعة من مكونات React التي تضعها في أي تطبيق.

> **المتطلبات الأساسية:** يجب أن يقوم الخادم بتشغيل `agent-chat-plugin` (يتم تركيبه تلقائيًا في كل قالب). إذا كنت تبدأ من الصفر، فراجع [Server](/docs/server-overview).
>
...
| `<AgentPanel>`        | اللوحة الأولية نفسها — الدردشة + CLI + علامات تبويب موارد الوكيل                 | تريد التحكم الكامل في التخطيط، أو صفحة وكيل مخصصة        |
...
<Diagram id="doc-block-aqmizx" title={"نموذج جبل"} summary={"<AgentSidebar> يغلف تخطيطك الحالي. يتم عرض مساراتك في المنطقة الرئيسية؛ يتم تركيب لوحة الوكيل بجانبهم. <AgentPanel> هي نفس اللوحة بدون الغلاف."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        تطبيقك<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        لوحة الوكيل<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1nt1prb"
  title={"تغليف تخطيط الجذر بـ <AgentSidebar>"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "يبدأ تشغيل المطالبات",
      note: "يتم عرض `suggestions` كشرائح قابلة للنقر في الدردشة الفارغة.",
    },
    {
      lines: "13",
      label: "شرائح تدرك السياق",
...
هذا كل شيء. أصبح لدى المستخدم الآن وكيل قابل للتبديل في كل صفحة - مع سجل الدردشة، وعلامة تبويب موارد الوكيل، ومحطة CLI، والإدخال الصوتي، ولوحة أوسع. تستمر الحالة عبر عمليات إعادة التحميل عبر `localStorage`.

### الدعائم

- **`children`** — التخطيط والمسارات العادية لتطبيقك. المقدمة في المنطقة الرئيسية. يتم تثبيت لوحة الوكيل بجانبها على سطح المكتب وفوقها على الهاتف المحمول.
...
يمنحك `<AgentPanel>` علامات التبويب الأولية (الدردشة / CLI / موارد الوكيل) بدون غلاف الشريط الجانبي، أو زر الطي، أو أي ثبات للحالة. ضعها أينما تريد؛ يمكنك التعامل مع التخطيط.
...
- **`isFullscreen`** — اضبط هذا الخيار إذا كنت تريد عمودًا مركزيًا بنمط Claude لسطح صفحة أو لوحة مخصصة. يستخدم الشريط الجانبي اللوحة الأوسع بدلًا من ذلك.
...
زر يسلّم العمل إلى الوكيل بدلاً من إنشاء مسار AI منفصل عبر استدعاء `llm()` مضمن. راجع [لماذا نبني التطبيقات بهذه الطريقة](/docs/what-is-agent-native#why-build-apps-this-way) لمعرفة أهمية هذا الفرق:
...
- **للعمل المرئي الذي يبدأه المستخدم ويستخدم الذكاء الاصطناعي**، استخدم `openSidebar: true`. استخدم `false` فقط للعمل الصامت المحدد في الخلفية أو الذي يبدأه النظام، وأبقِ المتابعات في سلسلة AgentSidebar نفسها.
- **`type`** — يحتفظ `"content"` (افتراضي) بالعمل في وكيل التطبيق المضمن الذي يعمل من خلال أدوات التطبيق. يوجه `"code"` إلى مضيف منفصل قادر على التعامل مع الكود لإجراء تغييرات مصدر المستودع؛ ولا تتوفر الكتابة إلا عندما يمنح المضيف عمدًا أدوات repository/workspace. راجع [رمز Agent-Native UI](/docs/code-agents-ui).
...
أبقِ مطالبات المتابعة والمراجعة في سلسلة الشريط الجانبي الحالية. لا تضف حقل إدخال حرًا ثانيًا بجانب النتيجة؛ استخدم نموذجًا محليًا فقط للقيم المنظمة التي يجب على الواجهة التحقق منها.
...
[محادثة الوكيل](/docs/client-agent-chat#sendtoagentchat) لسلوك جسر تطبيق MCP.
...
## اختيار دردشة الوكيل مقابل الإجراء {#agent-or-action}

استخدم `useActionQuery` أو `useActionMutation` مباشرة للعمليات المحلية الحتمية مثل CRUD والتحقق وقراءة موفر البيانات والحفظ. إذا اختبر المستخدم سير العمل على أنه بحث أو تحليل أو إنشاء أو توصية أو تركيب أو استدلال متعدد الخطوات، فابدأه باستخدام `sendToAgentChat({ openSidebar: true })` ودع الوكيل ينسق الإجراءات المركزة. لا يكفي كون التنفيذ حتميًا لإخفاء سير عمل يشبه الذكاء الاصطناعي داخل إجراء واحد مبهم.

## Typesafe actions من UI: `useActionMutation()` {#use-action-mutation}

عندما تحتاج UI إلى تشغيل العملية نفسها التي قد تشغّلها أداة وكيل، استخدم [`useActionMutation`](/docs/client-data#use-action-mutation):
...
تأتي وسائط النوع الآمن من مخطط zod في `defineAction()`. راجع [Actions](/docs/actions-overview) للتعرف على نظام العمل الكامل.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` و`useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — الاختيار، التنقل، شاشة العرض
- [**موارد الوكيل**](/docs/agent-resources) — ما تحتويه علامة تبويب موارد الوكيل (skills، الذاكرة، خوادم MCP، المهام المجدولة)
