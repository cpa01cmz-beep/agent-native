...
تُصدر حزم `app-skill` أيضًا ملفات Agent Plugin قياسية. استخدم `agent-native plugin import <path> --into <workspace> --yes` لاستيراد Skills وMCP البعيدة عبر Streamable HTTP؛ يتم تجاهل الرؤوس، بينما يتم الإبلاغ عن خوادم stdio وSSE وتخطيها.
...
- التوجيه يتم لمرة واحدة - ضعه في `AGENTS.md` أو ذاكرة الوكيل بدلاً من ذلك
...
إذا كانت المعرفة تنطبق على _جميع_ الأشخاص\_ الذين يعملون في التطبيق ("يفضلون دائمًا CTEs على الاستعلامات الفرعية")، فهي مهارة أو `LEARNINGS.md` مشتركة. إذا كان الأمر يتعلق بهذا المستخدم تحديدًا ("يحب ستيف الإجابات المختصرة")، فهو ينتمي إلى `memory/MEMORY.md`. راجع [موارد الوكيل — الذاكرة](/docs/agent-resources#memory) للحصول على العلاج الكامل.

## المهارات مقابل موارد SQL {#skills-vs-resources}

تعرض هذه الصفحة المهارة كملف في `.agents/skills/<name>/SKILL.md`. وتعرض [موارد الوكيل](/docs/agent-resources#skills) المهارة نفسها كمورد SQL في المسار `skills/<slug>/SKILL.md`، بنطاق مساحة العمل أو النطاق المشترك أو الشخصي. وهما المفهوم نفسه من زاويتين: يقرأ وضع Code الملف ويكتبه مباشرةً؛ بينما يخزّنه وضع App كسجل مورد ليعمل من دون نظام ملفات. اكتب المحتوى بتنسيق هذه الصفحة في الحالتين.
...
- [**موارد الوكيل**](/docs/agent-resources#skills) — موضع المهارات كموارد SQL في وضع App
