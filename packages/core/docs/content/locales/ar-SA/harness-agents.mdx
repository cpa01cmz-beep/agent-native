...
<Diagram id="doc-block-1q7je8a" title={"يمتلك الحزام حلقته. Agent-Native يقود الجلسة"} summary={"تقوم الركيزة AgentHarness creates/resumes بالجلسة الأصلية، بتدفق أحداثها إلى النص العادي، وتستمر في حالة الاستئناف في SQL بين المنعطفات."}>
...
تلميح `installPackage` (على سبيل المثال `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` يُجري تثبيتًا واضحًا
...
## مصادقة Codex: الكود UI مقابل محولات أدوات الحماية {#codex-auth}

هناك سطحان لـ Codex:

- **Agent-Native Code / Desktop** يقوم بتشغيل `codex exec` على جهاز المستخدم
  ويعيد استخدام تسجيل دخول Codex CLI المحلي الذي يبلّغ عنه `codex login status`.
- **`ai-sdk-harness:codex`** يستخدم تدفق اشتراك AI SDK Harness الأصلي حيثما كان
  مدعومًا. لا يلزم إعداد إضافي: تبقى بيانات الاعتماد على المضيف، وتأتي بيانات
  اعتماد مفتاح API الصريحة أولاً، ويتم تمرير المصادقة الناتجة إلى صندوق الحماية
  دون نسخ ملف المصادقة المحلي. عيّن `harnessOptions.auth` إلى `"ai-gateway"`
  لاستخدام AI Gateway فقط؛ هذا الوضع لا يقرأ اشتراكًا أصليًا.

يدعم محول `ai-sdk-harness:codex` المضمّن فقط
`permissionMode: "allow-all"`، لأن هذا هو الوضع الوحيد المقبول في إصدار Codex
المستقر. اترك `permissionMode` دون تحديد لاستخدام هذا الإعداد الافتراضي؛ يتم
رفض `allow-reads` و`allow-edits` لهذا المحول.
...
`permissionMode`، و`harnessOptions`، و`agentOptions`). استخدم
`listAgentHarnesses()` لتعداد ما تم تسجيله من أجله منتقي.

بالنسبة إلى أحزمة Agent-Native المضمنة التي تستخدم صندوق حماية، مرر موفر صندوق
الحماية عبر `agentOptions.sandbox` عند تسجيلها. يجب أن يطبق الموفر
`resumeSession` لإعادة تهيئة الجلسة بعد إعادة تشغيل العملية؛ جلسة `sandbox` التي
يملكها المستدعي تغطي الإنشاء الأولي فقط.
...
<Diagram id="doc-block-23lsu4" title={"الاستئناف عبر المنعطفات والعمليات والنشر"} summary={"يقوم كل دور بفصل حالة السيرة الذاتية المبهمة إلى SQL؛ المنعطف التالي يعيده مرة أخرى إلى createSession بدلاً من إعادة تشغيل سجل الدردشة."}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    الدور N<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >معتم · جلسة حاضنة SQL</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    الدور N+1<br /><small class="diagram-muted"
...
[Human Approval](/docs/actions-access-control#needs-approval) لسطح الموافقة.
...
- [Human Approval](/docs/actions-access-control#needs-approval) — يتم استخدام مجموعة أحزمة السطح المعتمدة.
