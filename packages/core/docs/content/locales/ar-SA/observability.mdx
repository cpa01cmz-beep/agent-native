...
<Diagram id="doc-block-17pwj3a" title={"كل شوط يغذي الحلقة"} summary={"ينتج عن تشغيل وكيل واحد تتبعًا ونتائج تلقائية وخطافًا للتعليقات - يتم تخزين كل ذلك في SQL الخاص بالتطبيق ويظهر على لوحة المعلومات. تقوم التجارب بتقسيم حركة المرور عبر متغيرات التكوين."}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    تشغيل الوكيل<br /><small class="diagram-muted">production-agent.ts</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">يلتقط تلقائيا</span
...
### التعليقات في PostHog {#posthog-feedback}

كل تعليق — الإبهام أو الفئة أو النص الحر — يُرسل حدث `$ai_feedback`. أما عرض التعليقات في تحليلات LLM لدى PostHog فيقرأ حدث `survey sent` بدلاً من ذلك، لذا اضبط `POSTHOG_AI_FEEDBACK_SURVEY_ID` على الاستطلاع الذي ينشئه PostHog من تبويب **Feedback** في أي تتبّع. هذا المعرّف وحده هو كل ما يلزم من إعداد، وبدونه لا يُرسل أي حدث استطلاع.

يجيب تصويت الإبهام عن السؤال الأول في الاستطلاع برقم الخيار في PostHog (`1` لأعلى، `2` لأسفل)، بينما يجيب النص الحر بعد الإبهام لأسفل عن سؤال المتابعة، فلا يقع النص أبدًا في موضع التقييم. ويحمل الاثنان معرّف إرسال واحدًا لكل رسالة مُقيَّمة، وهو ما يجعل PostHog يدمجهما في إجابة واحدة، ويظل الإبهام لأسفل غير مكتمل حتى تصل متابعته.
...
<Diagram id="doc-block-1vqpr2h" title={"تعيين متغير التجزئة المتسق"} summary={"يقوم كل مستخدم بالتجزئة إلى متغير ثابت، وتطبق الحلقة تجاوز تكوين هذا المتغير، ويتم عرض النتائج لكل متغير مع فواصل الثقة."}>

```html
<div class="exp">
  <div class="diagram-node">
    معرف المستخدم<br /><small class="diagram-muted">تجزئة متسقة</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">تجاوز التكوين أ</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">تجاوز التكوين ب</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    النتائج لكل متغير<br /><small class="diagram-muted"
...
يتم الضبط من مكوّن إضافي للخادم باستخدام `defineAppConfig()`. لكل حقل أيضًا اسم مستعار لمتغير بيئة النشر — راجع مرجع التكوين المُعلَن.
...
أرسل التتبعات إلى Langfuse أو Datadog أو Grafana أو أي واجهة خلفية متوافقة مع OTel عن طريق تسجيل موفّر OpenTelemetry في تطبيقك — راجع [امتدادات OpenTelemetry](#otel) أدناه.

لا يشحن النواة أي مُصدِّر ولا تحتفظ بأي نقطة نهاية تصدير أو رمز مميز، لذا فإن بيانات اعتماد الواجهة الخلفية توجد في إعداد الموفّر لديك (وفي الخزنة)، وليس في تكوين إطار العمل.

## امتدادات القياس عن بعد المفتوحة {#otel}

تصدر حلقة الوكيل **امتدادات OpenTelemetry المباشرة** لكل تشغيل واستدعاء نموذج واستدعاء أداة — بحيث يرى المضيف الذي يقوم بالفعل بتشغيل مجمع OTel نشاط الوكيل جنبًا إلى جنب مع بقية آثاره الموزعة.
...
- [**Actions**](/docs/actions-overview) — العمليات التي تظهر كاستدعاءات أداة في التتبع
