...
チャットでエージェントがこのアクションを実行すると、組み込みの`data-chart`レンダラーが構造化された結果をトランスクリプト内のグラフに変換します。レンダラーファイルや`app/root.tsx`へのインポートを追加する必要はありません。カスタムのネイティブコンポーネントについては、[Native Chat UI](/docs/native-chat-ui)を参照してください。
