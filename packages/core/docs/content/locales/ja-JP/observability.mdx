...
<Diagram id="doc-block-1utex2x" title={"すべての実行がループにフィードされます"} summary={"エージェントを 1 回実行すると、トレース、自動スコア、フィードバック フックが生成され、これらはすべてアプリ独自の SQL に保存され、ダッシュボードに表示されます。実験では、構成バリアント間でトラフィックを分割しました。"}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    エージェント実行<br /><small class="diagram-muted"
      >production-agent.ts</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">自動で取得</span
...
### PostHog でのフィードバック {#posthog-feedback}

サムズ、カテゴリ、自由記述はいずれも `$ai_feedback` を送信します。一方、PostHog の LLM 分析のフィードバック表示が読むのは `survey sent` イベントです。トレースの **Feedback** タブから PostHog が作成するサーベイを `POSTHOG_AI_FEEDBACK_SURVEY_ID` に設定してください。この ID ひとつが設定のすべてで、未設定ならサーベイイベントは送信されません。

サムズの投票は、サーベイの最初の質問に PostHog の選択肢番号（`1` が高評価、`2` が低評価）で答えます。サムズダウンに続く自由記述は追加質問への回答なので、テキストが評価の位置に入ることはありません。両者は評価対象のメッセージごとに同じ送信 ID を持ち、PostHog はそれを手がかりに 1 件の回答としてまとめます。サムズダウンは追加回答が届くまで未完了のままです。
...
<Diagram id="doc-block-1fo2rr2" title={"一貫したハッシュバリアントの割り当て"} summary={"各ユーザーは安定したバリアントにハッシュし、ループはそのバリアントの設定オーバーライドを適用し、結果は信頼区間を使用してバリアントごとにロールアップされます。"}>

```html
<div class="exp">
  <div class="diagram-node">
    ユーザーID<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">設定オーバーライドA</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">設定オーバーライドB</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
`defineAppConfig()` を使用してサーバー プラグインから設定します。各フィールドにはデプロイ環境変数のエイリアスもあります。宣言された構成リファレンスを参照してください。
...
独自のアプリで OpenTelemetry プロバイダーを登録することで、Langfuse、Datadog、Grafana、または任意の OTel 互換バックエンドにトレースを送信します。以下の [OpenTelemetry スパン](#otel) を参照してください。

コアは意図的にエクスポーターを同梱せず、エクスポート エンドポイントやトークンも保持しません。そのため、バックエンドの資格情報はフレームワーク構成ではなく、プロバイダーの配線 (およびボールト) に存在します。

## OpenTelemetry スパン {#otel}

エージェント ループは、実行、モデル呼び出し、ツール呼び出しごとに **ライブ OpenTelemetry スパン** を出力します。そのため、すでに OTel コレクターを実行しているホストは、残りの分散トレースと一緒にエージェント アクティビティを確認できます。
...
- [**Actions**](/docs/actions-overview) — トレース内のツール呼び出しとして表示される操作
