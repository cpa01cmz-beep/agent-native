...
**自動化** は、スケジュールまたはイベント発生時にエージェントが実行するタスクです: _X が発生したら Y を実行します_。**Scheduled** と **Event** は 2 つのトリガー種別であり、別々の製品ではありません。

フレームワークは当初、スケジュールされた自動化を **Recurring Jobs** として公開していました。この名称、`jobs/<name>.md` パス、`manage-jobs` ツールは互換性のために残ります。新しいガイドでは **Automations** を包括的な名称として使用します。既存のスケジュールジョブ、組織スコープ、高度なフィールドについては [Recurring Jobs](/docs/recurring-jobs) を参照してください。

<Diagram id="doc-block-1f6019n" title={"Xが起こったらYをする"} summary={"イベントがバス上で発生し、オプションの自然言語条件によってイベントがゲートされ、エージェントが完全なツール アクセス権でオートメーション本体を実行します。"}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">エージェントがbodyを実行</span
    ><small class="diagram-muted"
...
フルページの Agent 画面で **Automations** を開き、スケジュールおよびイベントトリガーの自動化を表示・管理します。互換 URL は引き続き `/agent#jobs` なので、既存のリンクやブックマークは動作します。

<Callout tone="warning">

`manage-automations` は Scheduled と Event の両方で個人および組織の自動化をサポートします。組織の Event 自動化はメンバーに表示されますが、常に作成者として実行されます。管理者はその実行IDを変更せずに管理できます。
  ...
  annotations={[
    {
      lines: "2",
      label: "クロンなし",
      note: 'イベントトリガーは `schedule` を `""` に設定し、cron フィールドは空のままにします。',
    },
    {
      lines: "4-5",
      label: "トリガー",
      note: "`triggerType: event` に `event` という名前を追加すると、このオートメーションがバスにサブスクライブされます。",
    },
    {
      lines: "6",
      label: "ゲート",
      note: "オプションの自然言語 `condition` 。ディスパッチ前にペイロードに対して Haiku によって評価されます。",
    },
    {
      lines: "12",
      label: "サーバー側の秘密",
...
イベントメタデータの `owner` が起動可能な自動化を決めます。個人と組織の Event 自動化はいずれも `createdBy` と完全に一致する必要があります。
...
個人および組織の自動化では、`action` パラメータ付きの `manage-automations` を使用します。
...
`manage-automations` は組織スコープ、モデルオーバーライド、厳密に適用される MCP 許可リストをサポートします。`manage-jobs` は既存のスケジュール専用連携と配信メタデータ向けの互換ツールとして残ります。
...
<Endpoint id="doc-block-g6juno" title={"テストイベントを起動する"} method="POST" path="/_agent-native/automations/fire-test" summary={"test.event.fired イベントを発行して、イベントトリガーのオートメーションを検証します。"} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1p6nwla" title={"ディスパッチパス"} summary={"起動されたイベントから完了したエージェントの実行まで、所有権の範囲と自然言語の条件によって制御されます。"}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code>
      bodyをプロンプト、payloadをコンテキストにして5分タイムアウト</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-overview) — オートメーションはエージェント ループ経由で登録されたアクションを呼び出すことができます
