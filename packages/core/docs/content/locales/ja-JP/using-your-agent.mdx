...
Agent-Nativeの背後にある決定的な考え方は、エージェントと UI が **対等なパートナーである**ということです。理由については、[Agent-Nativeとは？](/docs/what-is-agent-native) を参照してください。このセクションは、その約束の残り半分、つまり、エージェントがアプリの隣にドッキングされたときに実際にエージェントを操作するとどのような感じになるかについて説明します。
...
<Diagram id="doc-block-xvffge" title={"日々のループ"} summary={"ドッキングされたエージェントを操作する 5 つの方法 - それぞれがこのセクションのページです。"}>
...
<div class="diagram-loop">
  ...
  <div class="diagram-card accent-card">
    <span class="diagram-pill accent">共同編集</span
    ><small class="diagram-muted">ライブで並列</small>
  </div>
</div>
```
...
すべてのアプリに完全なダッシュボードが必要なわけではありません。エージェントが製品である場合、カスタム UI のほとんどをスキップできます。アプリを開いて必要なものを尋ね、残りはエージェントに任せます。エージェントには履歴、エージェント リソース、設定などの管理画面がまだありますが、主な操作はクリックではなく会話です。
