---
title: "拡張機能"
description: "単発のサンドボックス化されたカスタムブロック向けの任意の Toolkit 機能。永続的な製品動作にはアプリコードを使うのが既定です。"
---

# 拡張機能

拡張機能は、**サンドボックス化されたカスタムブロック向けの任意の Toolkit 機能**です。既定では無効で、テンプレートが明示的に有効にする必要があります。Analytics では、明示的に依頼された単発のカスタム可視化に使用します。永続的な動作にはアプリコードを編集してください。
...
> 公開されていない限りできません。より深いカスタマイズは、独自のリポジトリと
> 開発ワークフローでテンプレートを変更します。埋め込みエージェントが支援できるのは、
> ホストが repository/workspace の書き込みツールを意図的に付与した場合だけです。
> 拡張機能はデプロイ不要ですばやく追加できるレイヤーです。
...
拡張機能は既定で無効です。有効にするには、
`createAgentChatPlugin()` と `createCoreRoutesPlugin()` の両方で
`extensionTools: true` を設定します。前者はエージェント向けの actions
とガイダンスを追加し、後者は `POST /_agent-native/extensions` による
認証済みの作成を許可します。サンドボックス runtime と既存の読み取り、
編集、ディープリンク routes は互換性のため維持されます。

<Diagram id="doc-block-co8nb2" title={"サンドボックスブリッジ"} summary={"拡張機能 HTML は分離された iframe で実行され、ブリッジ ヘルパーの固定セットを介してのみホストに到達します。すべての呼び出しはスコープ設定され、アクセス チェックされます。"}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      ホストテンプレート<br /><small class="diagram-muted"
        >actions、自動スコープSQL</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>、ドメイン固定</small
      >
    </div>
    <div class="diagram-box">
      外部API<br /><small class="diagram-muted">extensionFetchのみ</small>
    </div>
  </div>
</div>
```
...
1. **Settings → Extensions** を開き (またはチャットで質問し)、[新しい拡張機能] をクリックします。
...
<Diagram id="doc-block-nxjr28" title={"スロットはコンテキストをウィジェットにプッシュします"} summary={"ホスト テンプレートは名前付きスロットを所有します。拡張機能を拡張機能にインストールすると、ユーザーが現在表示しているものに対して window.slotContext がフィードされます。"}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">メールスレッド</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">連絡先メモ</span
    ><small class="diagram-muted"
...
- `dbQuery` と `extensionFetch` は現在ブロックされています。
...
## 拡張機能とアプリのソースワークフローのどちらを使うべきですか？ {#vs-app-code}

埋め込みエージェントはデフォルトではソースコードを編集しません。ホストが repository/workspace の書き込みツールを意図的に付与した場合に限り編集できます。永続的な製品動作にはソースワークフローを、すばやい runtime カスタマイズには拡張機能を使います。

- **アプリコードから始めます。** 製品動作、ネイティブ UI、レイアウト、ルート、ビジネスロジック、または広く再利用する機能はコードで実装します。
- **カスタムブロックはアプリが有効にした場合のみ使用します。** ユーザーが既存の面で単発の可視化や操作を明示的に求めた場合に限ります。高速でデプロイ不要ですが、エージェントが作成したサンドボックスパッチです。
- **永続的なブロックはアプリコードに昇格させます。** 元の用途を超えて役立つ場合は **アプリコードに昇格** を使い、コード変更のレビューとデプロイまでは元のブロックを維持します。

|                        | 延長                                  | アプリコードの編集                   |
| ---------------------- | ------------------------------------- | ------------------------------------ |
| **作成者**             | 実行時のエージェント (またはユーザー) | ユーザーまたはコード対応ワークフロー |
| **保存場所**           | データベース                          | git リポジトリ                       |
| **ビルドが必要です**   | いいえ                                | はい                                 |
| **デプロイが必要です** | いいえ                                | はい                                 |
| **範囲**               | 1 人のユーザー (または組織と共有)     | 製品全体、すべてのユーザー           |
| **こんな方に最適**     | 明示的な単発カスタムブロック          | 永続的な製品動作                     |

経験則: **既定ではアプリコードを使います。** 拡張機能を有効にしたアプリでは、単発の可視化をカスタムブロックにできます。すべてのダッシュボードで使う新しいチャート種類はテンプレートコードに属します。

専用ウィジェット UI ではなくネイティブなチャートやテーブルにデータを供給すべきカスタムデータ取得については、[データプログラム](/docs/data-programs) を参照してください——拡張機能の iframe を一切介さずに dashboard パネルが直接バインドする、保存済みで更新可能な `run-code` スクリプトです。
...
- **認証が必要** — 拡張機能はログイン ユーザーに対してのみ実行され、`dbQuery` 呼び出しは自動スコープされます。
...
- [**Actions**](/docs/actions-overview) — 拡張機能が `appAction` 経由で呼び出す操作
