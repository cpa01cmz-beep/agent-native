...
<Diagram id="doc-block-ku3gn8" title={"多層防御"} summary={"フレームワークは脅威の表面の大部分を所有します。所有するものは 2 つあります。1 つは外部入力のスコープ設定と検証のためのテーブルのタグ付けです。"}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">フレームワークが所有</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">自分で所有</span
    ><small class="diagram-muted"
...
フレームワークはセッションからリクエスト コンテキストを作成し、アプリの読み書きに `accessFilter()` と `assertAccess()` で所有権を適用します。エージェント SQL には、アクティブな ID と組織でフィルタリングされた一時ビューが渡されます。RLS はまだ有効ではないため、アプリの直接クエリでは引き続きこれらのヘルパーを使用してください。
...
<Callout id="doc-block-ifb9lp" tone="risk">

文字列の連結またはテンプレート リテラルによって SQL を構築しないでください。ユーザー入力を `args` として `exec` / `db-query` に渡すか、Drizzle を使用します。どちらも常にパラメーター化されます。 `pnpm guards` チェックは、CI 時にスコープのないクエリと連結されたクエリをキャッチします。
...
<Diagram id="doc-block-1hp97f6" title={"スコーピングパイプライン"} summary={"エージェント SQL はベース テーブルに直接触れることはありません。現在の ID をスコープとする一時ビューを介して読み取ります。そのため、ベア テーブル名は所有されている行のみを返すことができます。"}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    サインイン済みセッション<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    エージェントSQL<br /><small class="diagram-muted"
...
`db-query` / `db-exec` ツールは、スキーマ修飾されたテーブル参照 (`public.<table>`) を拒否します。修飾名はベース テーブルに解決され、上記の一時ビューをバイパスします。エージェントは裸のテーブル名を使用します。スコープは自動的に適用されます。
...
<DataModel
  id="doc-block-1u3up31"
  title="What ownableColumns() adds"
  summary={"テーブルをテナント認識し、共有可能にする 3 つの列。"}
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "作成時の所有者のアクティブな組織。組織の可視性チェックを推進します。",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "プライベート |組織 | public — 大まかなデフォルト、デフォルトはプライベートです。",
        },
      ],
    },
  ]}
...
ユーザーごと/組織ごとの資格情報とボールトは AES-256-GCM で保存時に暗号化されます。共有ワークスペース Vault は `WORKSPACE_SECRETS_ENCRYPTION_KEY` を優先し、`SECRETS_ENCRYPTION_KEY`、`A2A_SECRET`、`BETTER_AUTH_SECRET` 由来の旧候補を続けて試します。ローテーション中は `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` を設定し、読み取りで行を移行します。既存の平文行を暗号化するには `pnpm action db-migrate-encrypt-credentials` (冪等、非破壊) を実行します。
...
受信 Webhook ハンドラー (Resend、SendGrid、Slack、Telegram、WhatsApp、Recall.ai、Deepgram、Zoom) は、運用環境ではデフォルトで偽装リクエストを拒否します。対応する署名シークレット環境変数が欠落している場合、ハンドラーは受け入れてディスパッチする代わりに 401 を返します。
...
Google Drive の `changes.watch` 通知は、この一般的なアダプター ゲートを通りません。本文のない不透明なリクエストであるため、フレームワークはチャンネルごとにランダムなトークンを登録し、変更を取得する前に `X-Goog-Channel-ID`、`X-Goog-Channel-Token`、`X-Goog-Resource-ID` ヘッダーを検証します。これは Pub/Sub メッセージではなく Drive からの直接通知で、Pub/Sub OIDC ベアラートークンは含まれません。
...
- [ ] 共有マルチアプリ Vault では `WORKSPACE_SECRETS_ENCRYPTION_KEY` を同じ値に設定し、アプリ固有の暗号化には `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` のフォールバックを使用 — [Secrets Management](#secrets) を参照
