...
<Diagram id="doc-block-1kd4cmf" title={"ネイティブのレンダーパス"} summary={"アクションは JSON を返します。ランタイムは明示的なウィジェット判別式または chatUI.renderer と一致します。 AssistantChat は実際の React コンポーネントをマウントします。 iframe も HTML 実行もありません。"}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    Actionが実行<br /><small class="diagram-muted">構造化JSONを返す</small>
  </div>
...
<Callout id="doc-block-u5ci3s" tone="success">

レンダラーは、アクションが `chatUI`**または**結果に明示的な既知の `widget` 判別式が含まれていると宣言した場合にのみ引き継ぎます。任意のオブジェクトを形状推論したり、ツールの結果から HTML や JavaScript を実行したりすることはありません。そのため、ネイティブ ウィジェットがインジェクション ベクトルになることはありません。
...
renderer のサイズは割合ではなく絶対単位で指定してください。トランスクリプトは
メッセージに合わせて伸びるため、renderer のコンテナには割合を解決するための確定した
高さがありません。チャートバーの `height: 40%` はエラーにならず 0 に潰れるので、
renderer が描画に失敗したように見えます。チャートなど高さに依存するビジュアルには
ピクセル（または `rem`）を基準として与えてください。トランスクリプトの列は確定した
幅を持つため、幅は割合でも問題ありません。
...
<Diagram id="doc-block-nv1xto" title={"BYO ランタイムは Agent-Native チャット シェルを保持します"} summary={"外部エージェントは、コネクタを介して正規化されたイベントをストリーミングします。 Agent-Native は、コンポーザー、トランスクリプト、ツール カード、承認、およびネイティブ ウィジェットを保持します。"}>

```html
<div class="diagram-byo">
  <div class="diagram-box" data-rough>
    あなたのエージェント<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">正規化されたmessage-* / tool-*イベント</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
...
- [Actions](/docs/actions-overview) — ネイティブ ウィジェット データを返す操作を定義します。
