...
description: "AI エージェントがファーストクラスのピアであるマルチユーザー共同編集: CRDT マージ、ライブ プレゼンス、SSE 高速パス、および詳細なサーバー側マージ - PostgreSQL/PGlite および任意のホスト上で。"
---
...
テーブルはフレームワークによって管理され、プロバイダーに依存しません (PGlite および Postgres は使用します
...
| 要件                                                         | 理由                                                                                                                                                                         | 確認方法                                                                                                                                                                                          |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **公開到達可能なホスト上の Postgres**                        | ゲートウェイは自身のネットワークからデータベースに接続します。PGlite、`localhost`、`*.internal`、生の IP アドレスは、PostgreSQL 接続文字列がマシンから出る前に拒否されます。 | `DATABASE_URL` が Neon などのマネージド Postgres を指していること                                                                                                                                 |
...
アプリは PostgreSQL 接続文字列と公開オリジンを送信し、チャネル ID と署名シークレットを受け
...
**自己登録とは、Builder に PostgreSQL 接続文字列を送ることです。** Builder は
