...
<Diagram id="doc-block-1iwsjf9" title={"出典から引用された回答まで"} summary={"Brain は、承認された情報源を生のキャプチャに取り込み、耐久性のある記憶を抽出し、レビューを通じてそれをゲートし、その後のみ引用で回答します。"}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    未加工キャプチャ<br /><small class="diagram-muted"
      >重複排除済み、編集済み</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted">事実 · 決定 · プロセス</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">承認済み、アトミック</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-tzkwfd">
  <Screen
    surface="desktop"
    html={
...
ベクター データベース要件がないため、テンプレートは PGlite 全体で移植可能です。
Postgres、Neon、hosted PostgreSQL、および同様のホスト。アプリケーションの状態は
...
<Endpoint id="doc-block-lxwucy" title={"署名付き取り込み Webhook"} summary={"クリップと汎用の transcript/capture インポートは、ソースごとのベアラー トークンを含む RawCapturePayload をポストします。"} method="POST" path="/api/_agent-native/brain/ingest" summary={"クリップまたは汎用ソースから生のキャプチャをインポートする"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
これらの変更はリポジトリの開発ワークフローで行います。埋め込み
エージェントがソース編集を支援できるのは、ホストが
repository/workspace の書き込みツールを意図的に付与した場合だけです。
それ以外では actions とデータを通じて動作します。参照
...
- [**エージェント リソース**](/docs/agent-resources) — アプリ間でリソースを共有
