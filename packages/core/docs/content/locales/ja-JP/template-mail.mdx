...
<WireframeBlock id="doc-block-w7d4bl">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-ql3kld" title={"メールリクエストの流れ"} summary={"キーボード ショートカットとエージェント プロンプトは同じアクションを実行します。電子メールは Gmail にあります。ドラフト、自動化、追跡は SQL と application_state で実行されます。"}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      あなたが操作<br /><small class="diagram-muted"
        >J/K/E/R ショートカット</small
      >
    </div>
    <div class="diagram-node">
      エージェントに依頼<br /><small class="diagram-muted"
        >"丁寧な断り文を下書き"</small
      >
    </div>
  </div>
...
<DataModel
  ...
  title={"メール SQL テーブル"}
  summary={
    "メール自体は Gmail 内に存在します。 SQL テーブルには、キューに入れられた下書き、送信追跡イベント、OAuth トークンなど、Gmail にはないものが保持されます。設定と一時的な状態は、settings ストアと application_state ストアに存在します。"
  }
  entities={[
    {
...
これらの変更は、独自のリポジトリと開発ワークフローで行います。埋め込みエージェントがソース編集を支援できるのは、ホストが repository/workspace の書き込みツールを意図的に付与した場合だけです。それ以外では actions とデータを通じて動作します。[Key Concepts](/docs/key-concepts#agent-modifies-code) を参照してください。
...
- [**Actions**](/docs/actions-overview) — チャット、ショートカット、自動化が呼び出す action システム
