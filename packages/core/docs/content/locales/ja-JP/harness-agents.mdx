...
<Diagram id="doc-block-1hwvd6l" title={"ハーネスはそのループを所有します。 Agent-Native がセッションを駆動します"} summary={"AgentHarness 基板 creates/resumes はネイティブ セッションであり、そのイベントを通常のトランスクリプトにストリーミングし、ターン間で SQL のresumeState を保持します。"}>
...
`installPackage` ヒント (例: `@ai-sdk/harness@latest
@ai-sdk/harness-codex@latest`); `resolveAgentHarness` はクリア インストールをスローします
...
## Codex 認証: コード UI とハーネス アダプター {#codex-auth}

Codex サーフェスは 2 つあります。

- **Agent-Native コード / デスクトップ** はユーザーのマシン上で `codex exec` を実行し、
  `codex login status` が報告するローカル Codex CLI ログインを再利用します。
- **`ai-sdk-harness:codex`** は、基盤となるハーネスがサポートする場合、AI SDK Harness
  のネイティブ サブスクリプション フローを使用します。追加設定は不要です。
  認証情報はホストに残り、明示的な API キー認証情報が優先され、ローカル認証ファイルを
  コピーせずに結果の認証がサンドボックスに渡されます。AI Gateway のみを使用するには
  `harnessOptions.auth` を `"ai-gateway"` に設定します。このモードはネイティブ
  サブスクリプションを読み取りません。

組み込みの `ai-sdk-harness:codex` アダプターがサポートするのは
`permissionMode: "allow-all"` のみです。安定版 Codex ランタイムで受け付けられる
唯一のモードだからです。デフォルトを使うには `permissionMode` を省略してください。
このアダプターでは `allow-reads` と `allow-edits` は拒否されます。
...
`permissionMode`、`harnessOptions`、`agentOptions`)。`listAgentHarnesses()` を使用して、
登録されているものを列挙します。
ピッカー。

サンドボックスを使用する組み込みハーネスでは、登録時に
`agentOptions.sandbox` を通じてサンドボックスプロバイダーを渡します。
プロセス再起動後の再ハイドレーションには、プロバイダーが `resumeSession`
を実装している必要があります。呼び出し元が渡す `sandbox` セッションは
初回作成だけを対象とします。
...
<Diagram id="doc-block-17mj1y9" title={"ターン、プロセス、デプロイをまたいで再開"} summary={"各ターンは、不透明なresumeStateをSQLに切り離します。次のターンでは、チャット履歴を再生する代わりに、それが createSession にフィードバックされます。"}>

```html
<div class="diagram-resume">
  <div class="diagram-node" data-rough>
    ターンN<br /><small class="diagram-muted">streamTurn</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    detach &rarr; resumeState<br /><small class="diagram-muted"
      >不透明 · SQL harnessセッション</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    ターンN+1<br /><small class="diagram-muted"
...
承認画面の場合は [Human Approval](/docs/actions-access-control#needs-approval)。
...
- [Human Approval](/docs/actions-access-control#needs-approval) — 承認サーフェス ハーネス実行が使用します。
