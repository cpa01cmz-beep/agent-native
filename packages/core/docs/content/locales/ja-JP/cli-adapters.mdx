...
<Diagram id="doc-block-tjhxpy" title={"CLI アダプタ → レジストリ → アクション サーフェス"} summary={"ShellCliAdapter はバイナリをラップします。 CliRegistry は検出用のアダプターを収集します。 defineAction は、エージェント + UI アクション サーフェス上で 1 つの呼び出しを公開します。"}>
...
- [**Actions**](/docs/actions-overview) — 通常、CLI アダプターがラップされるアクション サーフェス。
