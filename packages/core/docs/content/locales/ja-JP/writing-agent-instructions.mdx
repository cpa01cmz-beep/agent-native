...
<Diagram id="doc-block-1aqsfxk" title={"3 つの作成済みサーフェス + 1 つのランタイム サーフェス"} summary={"AGENTS.md とツールの説明は毎ターンロードされます。スキルはオンデマンドでロードされます。 application_state は UI によってライブで書き込まれます。"}>

```html
<div class="diagram-surfaces">
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">毎ターン</span><strong>AGENTS.md</strong
    ><small class="diagram-muted"
      >地図: 目的、基本ルール、状態キー、action + skills索引</small
    >
  </div>
  <div class="diagram-card always" data-rough>
    <span class="diagram-pill accent">毎ターン</span><strong>ツール説明</strong
    ><small class="diagram-muted"
      >ツール選択を導く — それぞれ1つの正確な文</small
    >
  </div>
  <div class="diagram-card ondemand" data-rough>
    <span class="diagram-pill">オンデマンド</span><strong>スキル</strong
    ><small class="diagram-muted"
      >詳細な手順、説明が一致したときに読み込み</small
    >
  </div>
  <div class="diagram-card runtime" data-rough>
    <span class="diagram-pill ok">Live</span><strong>application_state</strong
    ><small class="diagram-muted"
      >UIが書き込む: ナビゲーション、選択、フォーカス</small
    >
  </div>
</div>
```
...
- **スキル インデックス** — 存在する skills のリストと、それぞれをいつ読み取るか。
...
- Data lives in Drizzle 経由の SQL. Use actions for all writes.
...
`AGENTS.md` はこのフレームワークが基盤とするユニバーサル標準であるため、プロジェクトの指示を読み取る AI コーディングツールであれば、どれでもそのまま動作します。

- **Claude Code**: `CLAUDE.md` を読み取ります（CLI セットアップにより `AGENTS.md` からシンボリックリンクされます）
- **Cursor**: `AGENTS.md` を直接読み取るか、存在する場合は `.cursorrules`（Cursor のレガシーな配置場所）を読み取ります
- **Windsurf**: `AGENTS.md` を直接読み取るか、存在する場合は `.windsurfrules`（Windsurf のレガシーな配置場所）を読み取ります
- **Codex、Gemini、その他**: 組み込みのエージェントパネル経由で動作します
- **Builder.io**: ビジュアル編集とコラボレーションを備えたクラウドホスト型エージェントです
...
├── SKILL.md            # ルール + ハッピー パス + do/don't
└── references/
    └── csv-format.md   # 完全な列仕様、エンコーディング、エッジケース
```

これにより、常に読み込まれるサーフェスが小さく保たれ、コンテキストを肥大化させることなく深度をスケールできるようになります。完全なスキル形式については、[スキル Guide](/docs/skills-guide) を参照してください。
...
## スキル 対 actions {#skills-vs-actions}

スキル と actions は相補的です。スキルとは、エージェントが読むガイダンスです。
...
| スキル (`SKILL.md`)              | あなた（開発者）   | エージェントがスキルが関連していると判断した場合のオンデマンド | 特定のパターンのステップバイステップのハウツー、やるべきこと/やらないことのリスト |
...
- **スキル** — オンデマンドでロードされる、特定のパターンの再利用可能なハウツー。アプリで作業している全員に適用されます。
- **メモリ (`memory/MEMORY.md`)** — ユーザーごとの設定と修正。作成されたガイダンスではありません。

## 次は何ですか {#whats-next}

- [スキル Guide](/docs/skills-guide) — スキル ファイル形式、フレームワーク skills、およびアプリベースの skills。
