...
<Diagram id="doc-block-fghvxh" title={"1 つの運営ストア上の 3 つのレイヤー"} summary={"CLI、Desktop、および共有 UI は、同じファイルベースの実行ストアおよびエグゼキューター上の異なるサーフェスです。ホストは CodeAgentsHost コントラクトを通じてそれを適応させます。"}>
...
<div class="diagram-layers">
  <div class="diagram-row">
    <div class="diagram-card" data-rough>
      ...
      ><small class="diagram-muted">開始 · 再開 · 状態 · 停止</small>
    </div>
    <div class="diagram-card" data-rough>
      <span class="diagram-pill">Desktop</span
      ><small class="diagram-muted"
        >ネイティブ端末 · webviews · ディープリンク</small
      >
    </div>
    <div class="diagram-card" data-rough>
      <span class="diagram-pill accent">共有d UI</span
  ...
  <div class="diagram-box" data-rough>
    ファイルベースのrun store + 実行器<br /><small class="diagram-muted"
...
[Harness Agents](/docs/harness-agents);このアダプターは、ローカル認証ファイルを
サンドボックスにコピーせず、サポートされているネイティブ サブスクリプション認証情報を
ホスト上で解決するようになりました。
...
<Diagram id="doc-block-1fqcjia" title={"リモート Dispatch は送信専用です"} summary={"モバイルがデスクトップと直接通信することはありません。 Desktop は Dispatch をロングポーリングし、コマンドを要求し、ローカル実行ストアを駆動し、結果をミラーリングします。"}>

```html
<div class="diagram-remote">
  <div class="diagram-node" data-rough>
    モバイル / Telegram<br /><small class="diagram-muted"
      >/code · セッション</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Dispatchリレー<br /><small class="diagram-muted"
      >ホスト · 実行 · トランスクリプト</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&larr;</div>
  <div class="diagram-node" data-rough>
    Desktop<br /><small class="diagram-muted"
      >ロングポーリング · クレーム · run storeを駆動</small
    >
  </div>
</div>
```
...
<Endpoint id="doc-block-1i1x5xz" title={"デスクトップのクレームキューにある作業"} method="POST" path="/_agent-native/integrations/remote/poll" summary={"デスクトップはリレーをロングポーリングして、キューに入れられたコマンドを要求します"} auth="Desktop device token" responses={[
  {
    "status": "200",
