...
<Diagram id="doc-block-ohy1i8" title={"セットアップチェックリスト"} summary={"AI エンジンの接続のみが必要です。パネルは完了を追跡し、必要な作業がすべて完了すると自動的に非表示になります。"}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span
    ><strong>AIエンジンを接続</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>メール配信</strong
...
<Endpoint id="doc-block-u62ecz" title={"オンボーディング手順をリストする"} method="GET" path="/_agent-native/onboarding/steps" summary={"登録されているすべてのステップとその完了ステータスをリストします。"} responses={[
  {
    "status": "200",
  ...
  annotations={[
    {
      lines: "5",
      label: "自動実装",
      note: "Nitro プラグインから登録します。フレームワークはレンダリング、完了追跡、および終了を処理します。",
    },
    {
      lines: "7",
      label: "安定したID",
      note: "デフォルトのロード後に同じ `id` で再登録すると、組み込みのステップがオーバーライドされます。",
    },
    {
      lines: "12-19",
      label: "主な方法",
    ...
    },
    {
      lines: "20-26",
      label: "代理パス",
      note: '`kind: "agent-task"` は、プロンプト付きでセットアップをエージェントチャットに渡します。',
    },
    {
      lines: "28-31",
      label: "完了チェック",
      note: "`isComplete` はサーバー側で実行されます。 OAuth トークンは `oauth_tokens` ストアに存在します。`process.env.GMAIL_REFRESH_TOKEN` ではなく、ストアを確認してください。",
    },
  ]}
/>
...
| `database` | いいえ | ホスト型 PostgreSQL URL、またはローカル PGlite 用に空欄        |
