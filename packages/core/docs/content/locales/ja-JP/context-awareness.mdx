...
<Diagram id="doc-block-12zdzol" title={"あなたが見ているものをエージェントはどう見るか"} summary={"UI は軽量の状態キーを書き込みます。ビュースクリーンはそれらを実際の記録に統合します。エージェントは、UI を移動するために「navigate back」を書き込むことができます。"}>

```html
<div class="diagram-ctx">
  <div class="diagram-card col">
    <span class="diagram-pill">UIが書き込む</span>
    <div class="diagram-node">
      navigation<br /><small class="diagram-muted">ビュー、開いているID</small>
    </div>
    <div class="diagram-node">
      __url__<br /><small class="diagram-muted">shareable filters</small>
    </div>
    <div class="diagram-node">
      selection<br /><small class="diagram-muted">行、ブロック、図形</small>
    </div>
  </div>
...
<AnnotatedCode
  id="doc-block-1s7nvvi"
  title={"ビュースクリーン — エージェントの目"}
  ...
  annotations={[
    {
      lines: "10-11",
      label: "工具表面",
      note: "エージェントはこの説明を読んで、`view-screen` を呼び出して現在の UI を確認できることを知ります。",
    },
    {
      lines: "13",
      label: "http: false",
      note: "内部アクション — HTTP では公開されません。ブラウザではなく、エージェントと `pnpm action` がそれを呼び出します。",
    },
    {
      lines: "15-16",
      label: "読み取り状態",
      note: "UI が作成した軽量の `navigation` キーと `selection` キーを取得します。",
    },
    {
      lines: "23-37",
      label: "水分補給",
      note: "これらの ID を SQL から直接**新鮮**レコードに変換するため、エージェントは動作する前にライブ オブジェクトを確認します。",
    },
  ]}
/>
...
<Callout id="doc-block-p59d9d" tone="info">

**`navigation` と `selection` は小さくしてください。**レコード全体ではなく、ID と短いラベルを保存します。 `view-screen` はオンデマンドで信頼できる情報源を取得するため、古い状態や大量の状態がエージェントに届くことはありません。
...
`app/lib/tab-id.ts` からインポートされる `TAB_ID` は、フレームワークの `getBrowserTabId()` の値です。ページローカルの WebMCP 呼び出しは同じ id を `X-Agent-Native-Browser-Tab` で送信するため、サーバーはタブごとの `navigation`/`selection` 状態を、エージェントまたはユーザーが実際に操作しているタブへと正確に解決します — 同期を保つべき第二のタブ方式は存在しません。
...
<Diagram id="doc-block-1hujrtk" title={"ソースのタグ付けにより自己再フェッチのジッターが阻止される"} summary={"タブは、独自の TAB_ID がスタンプされた同期イベントを無視しますが、エージェントと他のタブの書き込みには引き続き反応します。"}>

```html
<div class="diagram-jitter">
  <div class="diagram-node">
    このタブが書き込む<br /><small class="diagram-muted"
      >X-Request-Source: TAB_ID</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    サーバーがソースを保存<br />on the event
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
    <div class="diagram-pill warn">source == TAB_ID &rarr; ignored</div>
    <small class="diagram-muted">再取得なし、ちらつきなし</small>
    <div class="diagram-pill ok">agent / other tab &rarr; applied</div>
    <small class="diagram-muted">UI がライブ更新</small>
  </div>
</div>
```
