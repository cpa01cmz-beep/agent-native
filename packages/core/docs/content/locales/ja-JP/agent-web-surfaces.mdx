...
<Diagram id="doc-block-1xzjykc" title={"パブリックルートが公開するもの"} summary={"パブリック ルートは、エージェントに優しい表現に展開されます。ルートの読み取りはツールの呼び出しとは別のものです。ツールへのアクセスはオプトインのままです。"}>

```html
<div class="diagram-web">
  <div class="diagram-box" data-rough>
    公開ルート<br /><small class="diagram-muted"
  ...
  <div class="diagram-col gate">
    <span class="diagram-pill warn">ツールは非公開のまま</span
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "5-7",
      label: "安全性を自己説明する",
      note: "読み取り専用としてマークし、認証が必要かどうかを宣言し、重要かどうかフラグを立てます。ポリシーで明示的に許可されていない限り、パブリック MCP は consequential/write アクションを除外します。",
    },
  ]}
/>
...
- [**Actions**](/docs/actions-overview) — actions をパブリック エージェント プロトコルにオプトインする方法
