...
<Diagram id="doc-block-16znyzf" title={"あるエージェントが別のエージェントに仕事を引き渡す"} summary={"メール エージェントは分析エージェントのカードを検出し、JSON-RPC メッセージを送信し、完了したタスクを返します。"}>

```html
<div class="diagram-handoff">
  <div class="diagram-card">
    <strong>Mail エージェント</strong
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-pill ok">タスク · 完了</div>
  </div>
  <div class="diagram-card" data-rough>
    <strong>Analytics エージェント</strong
    ><small class="diagram-muted">run-queryを実行し結果を返す</small>
  </div>
</div>
```
...
<Endpoint id="doc-block-1a5kbes" title={"プライマリ A2A エンドポイント"} summary={"ここではすべての JSON ～ RPC メソッドが POSTed です。 message/send が表示されます。"} method="POST" path="/_agent-native/a2a" summary={"メッセージを送信し、タスクが完了するまで待ちます"} auth="JWT bearer signed with A2A_SECRET (or legacy apiKeyEnv static token)" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-1kw2hn0" title={"サーバーレスでの非同期タスクのライフサイクル"} summary={"async:true はミリ秒単位で動作を返し、その後、呼び出し元がポーリングしている間、新たな実行によってエージェント ループが実行されます。"} caption="A recurring sweeper re-claims any task left in flight if the function execution dies mid-run.">
...
<div class="diagram-async">
  ...
  <div class="diagram-panel">
    ...
    ><small class="diagram-muted">約ミリ秒</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>
    自己起動 POST /_agent-native/a2a/_process-task<br /><small
      class="diagram-muted"
      >HMAC token · 新規実行 · フルタイムアウト</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
  ...
  annotations={[
    {
      lines: "4",
      label: "text part",
      note: "エージェントが読む単純な自然言語の指示。",
    },
    {
      lines: "5",
      label: "data part",
      note: "構造化された JSON 引数 - e.g。日付範囲 — プロンプトと一緒に渡されます。",
    },
    {
      lines: "6-9",
      label: "file part",
      note: "`mimeType` および Base64 `bytes` という名前でファイルを添付します。",
    },
  ]}
/>
...
## ホスト型エージェント接続 {#hosted-agent-connections}

ホスト型エージェントは、互換性のある A2A エンドポイントを公開すれば
A2A に参加できます。モデルプロバイダーやエージェント SDK だけでは
A2A ピアにはなりません。アプリの UI、アクション、データベースは通常
のアプリホストに置き、アダプターまたはエージェント自身の A2A エンド
ポイントでランタイムに接続します。

ホスト型接続には、エンドポイントのメタデータと認証記述子があります。
このメタデータは remote-agent リソースまたは接続レコードと一緒に管理
します。マニフェストにトークンやクライアントシークレットを入れない
でください。
...
`url` は呼び出しエンドポイントまたはベース URL です。検出に
`/.well-known/agent-card.json` を使う場合、`cardUrl` は省略できます。
プロバイダーが別の場所でカードを公開する場合に指定します。クライアント
はエージェントカードからプロトコルバージョンを読み取ります。カードに
バージョンがない場合は、`protocolVersion` を `A2AClient` に直接渡します。
`auth` はサーバー側コードが bearer トークンを取得する方法を示します。
対応する記述子は、Vault に保存した `credentialRef` を
使う `bearer`、または `tokenUrl`、`clientId`、`clientSecretRef`、`scope`
を持つ `oauth-client-credentials` です。`credentialRef` と
`clientSecretRef` はシークレットへの参照であり、値そのものではありま
せん。

### 汎用 bearer A2A {#generic-bearer-a2a}

ピアが `Authorization: Bearer ...` を受け入れ、互換性のあるカードを提供
し、Agent-Native JSON-RPC を受け入れる場合は標準クライアントを使います。
...
`token` はワークスペース接続または Vault からサーバー側で解決します。
ブラウザーコードから送信したり、マニフェストやプロンプトに書き込ん
だりしないでください。静的 API キーはリモートサービスの転送用認証
情報にすぎず、Agent-Native の呼び出し元 ID や `approvedActions` を運び
ません。

### Microsoft Foundry {#microsoft-foundry}

[Azure AI Foundry のホスト型エージェント](https://learn.microsoft.com/en-us/azure/foundry/agents/overview)
は、エージェントコードまたはコンテナーを、管理されたエンドポイント、
ID、スケーリング、セッション状態、可観測性の背後で実行します。これは
エージェントランタイムのホスティングモデルです。Agent-Native の UI、
アクション、PostgreSQL のデプロイはアプリホストに置き、サーバー側
コードから Foundry エンドポイントを呼び出します。

Foundry は [Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/enable-agent-to-agent-endpoint)
を通じて A2A エンドポイントを公開できます。Microsoft Entra 認証が
必要で、呼び出し元には Foundry Agent Consumer 権限が必要です。エンド
ポイントが Agent-Native JSON-RPC を話す場合は、プロバイダーの v0.3
カードとエンドポイントに `cardUrl` と `protocolVersion` を指定します。
トークンプロバイダーは Entra アクセストークンを取得して更新する必要
があります。`A2A_SECRET` や静的 API キーでは Foundry の認証を満たせ
ません。

### Gemini Enterprise {#gemini-enterprise}

[Gemini Enterprise は外部 A2A エージェントを登録できます](https://docs.cloud.google.com/gemini/enterprise/docs/register-and-manage-an-a2a-agent)。
Agent Registry はプロキシエンドポイントも公開できます。その HTTP+JSON
バインディングはプロバイダー固有で、検出は `GET {url}/v1/card`、呼び
出しは `POST {url}/v1/message:send`、ストリーミングは
`POST {url}/v1/message:stream` を使います。リクエストには Google OAuth
または ADC bearer トークンと Google Cloud IAM 権限が必要です。詳しくは
[Registry A2A エンドポイントでエージェントを呼び出す](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)
を参照してください。

これらのパスは Agent-Native JSON-RPC のパス
(`/.well-known/agent-card.json` と `/_agent-native/a2a`) とは異なります。
ワイヤー形式を変換し、Google の認証情報を取得して更新するサーバー側
アダプターを追加してからエンドポイントを登録してください。ターゲット
が標準 A2A エンドポイントを別に公開している場合は、汎用 bearer の案内
を使います。Gemini Enterprise の登録トラフィックは Agent Gateway の
ポリシーを通らないため、ターゲット独自の認証と認可ポリシーを適用して
ください。

### Anthropic Managed Agents {#anthropic-managed-agents}

Anthropic Managed Agents はインバウンド A2A エンドポイントを提供しません。Agent-Native が A2A の境界、ワークフロー状態、承認を管理し、ホストされたエージェントがセッション、コンテキスト、ツールを管理する場合は、フレームワークアダプターを使用します。アダプターは Managed Agents セッションを作成し、`user.message` イベントを送り、セッションのイベントストリームを読み取ります。ブロッキングする `agent.tool_use` または `agent.mcp_tool_use` イベントは、A2A の `input-required` タスクと `approval-request` ランタイムイベントになります。返されたメタデータに明示的な構造化確認を指定して再開してください。メッセージから承認を推測しないでください。
...
`credentialRef` は Vault に保存された Anthropic API キーへの参照です。アダプターは呼び出し元のユーザーと組織のスコープでこれを解決し、必須の `anthropic-beta: managed-agents-2026-04-01` と `anthropic-version: 2023-06-01` ヘッダーを送信します。アプリごとに既存の接続許可を再利用するには、ワークスペース接続カタログに `anthropic-managed-agents` を登録します。アダプターは通常の `A2AConfig.handler` seam にマウントします。
...
## 委任エージェントのアクティビティ {#delegated-agent-activity}

Agent-Native ピアは、タスクのステータスメッセージに
`kind: "agent-native/agent-activity"` を持つ任意の `data` パートを添付できます。
このパートには、ユーザーに表示される推論の要約、ツール名と完了状態、
経過時間、段階的な応答テキストが制限付きスナップショットとして含まれます。
呼び出し元のアプリは更新を入れ子のエージェント実行として表示し、委任タスクが
完了すると作業履歴を折りたたみます。

アクティビティパートにツール入力、ツール結果、認証情報、非公開のプロバイダー
推論が含まれることはありません。これは信頼できない表示データとして扱い、
本人確認、認可、承認、ルーティング、成果物の検証には使用しないでください。
外部の A2A ピアはこの拡張を省略できます。その場合も通常のステータスと最終応答は
同じ委任エージェント表示にレンダリングされます。
...
### 組織シークレットの同期 {#organization-secret-sync}

**組織 → アプリ間認証** の設定は、同じ組織で別々にデプロイされたアプリ間の委任を認証する組織スコープのシークレットを管理します。これはユーザーのサインインや Cross-App SSO の設定ではありません。このシークレットはデプロイの `A2A_SECRET` を補完します。`org_domain` クレームを持つ A2A JWT は、対応する組織のシークレットで検証できます。

組織の所有者は、参加するすべてのアプリに同じ許可済みメールドメインと同じ組織スコープのシークレットを設定する必要があります。

1. 最初のアプリで、**組織 → アプリ間認証** からシークレットを生成または表示します。
2. 追加の各アプリで **別のアプリからシークレットを貼り付ける** を選び、その値を貼り付けます。最初の接続にはこの手動手順が必要です。まだシークレットを持たないアプリは、リモート同期リクエストを検証できません。
3. アプリがシークレットを共有したら、**アプリに同期** を使って接続済み・検出済みアプリに今後の更新を送信します。結果には更新できなかったアプリが表示されます。

シークレットを**再生成**すると、設定 UI はすぐに同期し、すでに接続されているアプリが変更を検証できるよう以前のシークレットで更新に署名します。アプリがこの同期に失敗した場合は、そこに新しいシークレットを手動で貼り付けてから、もう一度同期してください。このシークレットは他の認証情報と同様に扱い、クライアントコード、ログ、またはサードパーティ製クライアントの設定に入れないでください。
