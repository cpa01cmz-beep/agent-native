---
title: "ドロップインエージェント"
description: "<AgentPanel>、<AgentSidebar>、sendToAgentChat() を使用して、エージェント チャット + エージェント リソースを React アプリにマウントします。"
---
...
エージェントネイティブを最初から構築する必要はありません。エージェント チャット、エージェント リソース タブ、CLI ターミナル、音声入力、およびすべての関連インフラストラクチャは、任意のアプリにドロップできる少数の React コンポーネントとして出荷されます。

> **前提条件:** サーバーは `agent-chat-plugin` を実行している必要があります (すべてのテンプレートで自動マウントされます)。最初から始める場合は、[Server](/docs/server-overview) を参照してください。
>
...
| `<AgentPanel>`        | 生のパネル自体 — チャット + CLI + エージェント リソース タブ                                         | レイアウトを完全に制御したい、または専用のエージェント ページが必要      |
...
<Diagram id="doc-block-h9v4c1" title={"マウントモデル"} summary={"<AgentSidebar> は、既存のレイアウトをラップします。ルートはメイン領域にレンダリングされます。エージェントパネルはその横に取り付けられます。 <AgentPanel> は、ラッパーを除いた同じパネルです。"}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        あなたのアプリ<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        エージェントパネル<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-15t8j07"
  title={"<AgentSidebar> によるルート レイアウトのラップ"}
  ...
  annotations={[
    ...
    },
    {
      lines: "8-12",
      label: "スタータープロンプト",
      note: "`suggestions` は空のチャット上でクリック可能なチップとしてレンダリングされます。",
    },
    {
      lines: "13",
      label: "コンテキスト認識チップ",
...
それだけです。ユーザーは、チャット履歴、エージェント リソース タブ、CLI ターミナル、音声入力、より広いドロワーを備えた、すべてのページに切り替え可能なエージェントを表示できるようになりました。状態は `localStorage` 経由でリロードしても維持されます。

### 小道具

- **`children`** — アプリの通常のレイアウトとルート。メインエリアにレンダリングされます。エージェント パネルは、デスクトップでは横に、モバイルではその上にマウントされます。
...
`<AgentPanel>` は、サイドバー ラッパー、折りたたみボタン、または状態の永続性のない生のタブ (チャット / CLI / エージェント リソース) を提供します。好きな場所に置いてください。レイアウトを扱うのはあなたです。
...
- **`isFullscreen`** — ページまたはカスタムパネルで Claude スタイルの中央揃えの列を使用する場合に設定します。サイドバーではより広いドロワーを使用します。
...
インライン`llm()`呼び出しで別のAI経路を作る代わりに、エージェントへ作業を引き渡すボタンです。この違いが重要な理由は、[なぜこの方法でアプリを構築するのか](/docs/what-is-agent-native#why-build-apps-this-way)を参照してください。
...
- **ユーザーが開始する表示可能な AI 作業**には `openSidebar: true` を使用します。`false` は明示的にサイレントなバックグラウンドまたはシステム起動の作業にだけ使用し、フォローアップは同じ AgentSidebar スレッドに残します。
- **`type`** — `"content"` (デフォルト) は、アプリツールを使う埋め込みエージェントで作業を保持します。`"code"` はリポジトリのソース変更用の別のコード対応ホストにルーティングします。書き込みは、ホストが repository/workspace ツールを意図的に付与した場合に限り利用できます。[Agent-Native コード UI](/docs/code-agents-ui) を参照してください。
...
フォローアップや修正のプロンプトは既存のサイドバー スレッドに残します。結果の横に 2 つ目の自由入力欄を追加しないでください。ローカルフォームは UI で検証する構造化された値にだけ使用します。

同じルートが MCP アプリとして埋め込まれている場合、送信されます
`sendToAgentChat()` 通話は、サポートされている場合はホスト チャットに転送されます。
MCP アプリ ブリッジ動作の [Agent チャット](/docs/client-agent-chat#sendtoagentchat)。
...
## エージェント チャットとアクションの選択 {#agent-or-action}

CRUD、検証、プロバイダーの読み取り、永続化などの決定的なローカル操作には、`useActionQuery` または `useActionMutation` を直接使用します。ユーザーがそのワークフローを調査、分析、生成、推奨、合成、または複数ステップの推論として体験する場合は、`sendToAgentChat({ openSidebar: true })` で開始し、エージェントに焦点を絞ったアクションを調整させます。実装が決定的であることだけを理由に、AI に見えるワークフローを 1 つの不透明なアクションに隠さないでください。

## UI からのタイプセーフ actions: `useActionMutation()` {#use-action-mutation}

UIがエージェントツールと同じ操作を実行する必要がある場合は、[`useActionMutation`](/docs/client-data#use-action-mutation)を使用します。
...
タイプセーフな引数は、`defineAction()` の zod スキーマから取得されます。フルアクションシステムについては[Actions](/docs/actions-overview)を参照してください。

<Callout id="doc-block-1757zi5" tone="decision">

**`useActionMutation` と `sendToAgentChat`。** ユーザーが決定的なボタン（"Send reply"）をクリックした場合は、`useActionMutation`で操作を直接実行します。作業にエージェントの推論、ツール、または複数ステップの計画が必要な場合は、`sendToAgentChat`に渡します。UIからインライン`llm()`を呼び出さないでください。共有エージェントループを使わない別のAI経路が作られてしまいます。
...
- [**Actions**](/docs/actions-overview) — `defineAction()` および `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — 選択、ナビゲーション、画面表示
- [**エージェント リソース**](/docs/agent-resources) — [エージェント リソース] タブに含まれるもの (skills、メモリ、MCP サーバー、スケジュールされたジョブ)
