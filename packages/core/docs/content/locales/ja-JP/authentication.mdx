...
- **デフォルト:** 送信メールの準備ができている場合は Better Auth の magic link を優先し、電子メール/パスワードをフォールバックとしてソーシャルプロバイダーとともに使用します。フォールバックを強制するには `AUTH_MAGIC_LINK=0` を設定します。
- **リモート MCP OAuth:** Claude コードや ChatGPT コネクタなどの MCP ホスト用の標準 OAuth 2.1。
- **カスタム:** `getSession` コールバック経由で独自の認証を取得します。

<Diagram id="doc-block-76smlp" title={"1 回のセッションで 3 つの方法で"} summary={"ブラウザーの訪問者、MCP ホスト、およびカスタム プロバイダーはすべて、ダウンストリーム スコーピングが読み取る同じ AuthSession に解決されます。"}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">Claude Code、ChatGPTコネクタ</small>
    </div>
    <div class="diagram-card">
...
## ローカル開発のサインイン {#local-development-sign-in}

ループバックの開発サーバーでは、共有サインインページに **Continue as local dev** が最初に表示されます。実際の Better Auth 開発アカウントとセッションを作成または再利用するため、アプリの他の場所と同じ認証済みの経路を使います。

**Show full sign in options** を選ぶと、通常のサインアップ、パスワード、magic link、ソーシャルプロバイダーの選択肢が表示されます。ローカルセッションを作成できない場合は、標準認証を続けられるように、ページがこれらの選択肢を自動的に表示します。

この機能は、開発またはテスト中にブラウザが `localhost`、`127.0.0.0/8`、`::1` を通じて接続した場合のみ利用できます。デプロイプレビューとホストされた本番環境では無効になります。これは認証のバイパスではありません。`AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` を設定すると、この機能を無効にして、ローカル開発で通常のサインインページを表示できます。

## より良い認証 (デフォルト) {#better-auth}

デフォルトでは、Better Auth が認証を強化します。以下を提供します:

- magic link による登録とログイン（電子メール/パスワードをフォールバックとして使用）
...
- `POST /_agent-native/auth/magic-link` — magic link メールをリクエスト
...
またはプレビュー環境で、`+autoz` アドレスを持つテスト アカウントに名前を付けます
(`name+autoz@example.com`) なので、簡単に識別できます。

## メール検証ポリシー {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` はローカルおよび QA 向けの利便性であり、ホスト
された環境では無視されます。本番環境を含むあらゆる環境でポリシーを明示するには、
設定で宣言します:
...
デプロイ用のエイリアスは `AUTH_REQUIRE_EMAIL_VERIFICATION=0` です。宣言された値
は `AUTH_SKIP_EMAIL_VERIFICATION` と環境ごとの既定値の両方より優先されます。

`false` は未検証のアドレスをログイン資格情報として受け入れます。誰でも任意のメール
アドレスを主張できるため、サインアップが別の方法ですでに制限されている場合にのみ
使用してください。メールプロバイダーのないデプロイで `true` を設定した場合は、検証
を配信できないため、代わりにパスワードによるサインアップが無効になります。
...
<Diagram id="doc-block-1fsvafh" title={"リモート MCP OAuth ハンドシェイク"} summary={"OAuth 対応クライアントは、MCP URL だけからブートストラップし、チャレンジ、検出、動的登録、その後 PKCE コード交換を行います。"}>
...
通常のアプリページは、すべての訪問者に対して同一の非個人化された公開キャッシュ可能な SSR シェルを使用します。`AppProviders` がクライアントでセッションを確認し、`/dashboard` のような非公開パスでは、非公開 UI をマウントする前に匿名ユーザーを `/sign-in?return=%2Fdashboard` へリダイレクトします。サインイン後は `/dashboard` に戻ります。公開/SEO ルートには `isPublicPath` を、MCP 埋め込みトークンなど別の限定された認証方式を使う画面には `sessionBypass` を指定します。

そのシェルをどれだけキャッシュするかはデプロイメント全体の設定です — `AGENT_NATIVE_SSR_CACHE` については [SSR Caching](/docs/ssr-caching) を参照してください。キャッシュを短縮または無効化しても変わるのは期間だけであり、SSR は引き続き Cookie を取り除いた匿名ブランチをレンダリングします。

### 舞台裏: Google OAuth

両方のフロー (明示的な `/sign-in` エントリポイントとブックマークされたパスのケース) は、OAuth 状態を通じてリターン URL をスレッド化します。状態はHMAC署名されているため、輸送中に偽造することはできません。コールバックでは、戻り値 URL はリダイレクトの前に同一オリジンとして再検証されます。そのため、漏洩した署名キーをオープン リダイレクト オラクルに変えることはできません。
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | `0` または `1` を設定すると、本番環境を含むあらゆる環境でパスワードサインアップの検証ポリシーを宣言できます。`AUTH_SKIP_EMAIL_VERIFICATION` より優先されます                                                       |
...
既定で無効な Dispatch の `desktop.workspace-sso` フラグは利用可否だけを制御し、認証、署名、nonce、オリジン、コールバック、セッション、失効の検証は常に必須です。フラグが無効または読み取れない場合、Desktop はアプリごとのサインイン、サインアウト、設定を変更しません。
User-Agent の Canary マーカーは利用可否のヒントにすぎず、リモートアテステーションではありません。認証と認可はこれを信頼しません。
正規のホスト型アプリは、アプリごとの ID ハブ環境設定なしでパッケージ版 Desktop SSO Canary のリクエストを認識します。通常のブラウザは通常のアプリ別サインインを使います。セルフホスト型アプリで `AGENT_NATIVE_IDENTITY_HUB_URL` が必要なのは、残る明示的な Desktop 経路またはサーバー側フェデレーション経路だけです。安定版 Desktop は、後続リリースが別途承認されるまでブローカーを初期化しません。
Canary は匿名ユーザーのアプリサインイン画面への遷移を横取りしません。ユーザーは、アクティブな登録済みアプリについて Desktop の設定からワークスペースサインインを明示的に選択する必要があり、通常のアプリサインインが既定の経路のままです。

## 管理デプロイと招待

管理デプロイでは `AUTH_SIGNUP=invited`、`ORG_CREATION=closed`、`AUTO_CREATE_DEFAULT_ORG=0` を設定します。[管理デプロイガイド](/docs/administered-deployments)でブートストラップ管理者、招待、許可ドメインを確認してください。メンバー招待とドメインによる参加は別のルールです。
