...
テンプレートは、実際のワークフローを解決する完全なフォーク可能なエージェント ネイティブ アプリです。ファーストパーティ テンプレートは、使用するのと同じフレームワーク サーフェスを使用して構築されます。UI の React ルート、Drizzle 経由の PostgreSQL データ、操作の actions、エージェントの動作のワークスペース リソース、およびエージェントと UI の連携を維持するためのポーリング同期です。
...
機能の置き場所を決めるときは、3 つのレイヤーを意識してください。

- **Core - フレームワーク:** すべてのアプリが利用できる必須のランタイムとデータ契約。
- **Toolkit - オプションの再利用可能な部品:** アプリが採用、構成、フォークできる共通 UI とプロダクトシステム。
- **Templates - オプションのアプリ:** Core 上に構築され、Toolkit をよく利用する完全なドメイン製品。出発点としてフォークできます。

Toolkit と Templates はオプションです。Template 自体がフレームワークなのではなく、Toolkit が Core を置き換えるわけでもありません。

良いテンプレート:

- 有用なシード データまたは空の状態のフローを使用して、1 つのワークフローをエンドツーエンドで解決します。
- 永続的な状態は、JSON ファイルではなく、PostgreSQL/PGlite に保存されます。
...
チャットでは、認証、耐久性のあるチャット スレッド、PostgreSQL を利用したリソース、ツール、アプリケーションの状態、actions、ポーリング同期が提供されます。ドメイン モデルと製品 UI を追加します。

再利用可能なUIテンプレートをまだ構築していない場合は、[Automation-First Apps](/docs/pure-agent-apps)を使用してください。アクションを定義し、ブラウザなしで実行して、ワークフローに永続的なサーフェスが必要になったらUIを追加します。
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "app/root.tsx",
      note: "HTML shell と providers",
    },
    {
      path: "app/routes/",
      note: "React Router のファイルルート",
    },
    {
      path: "app/components/",
      note: "テンプレート UI",
    ...
    {
      path: "app/hooks/",
      note: "UI 状態とデータ hooks",
    },
    {
      path: "actions/",
      note: "defineAction 操作: 唯一の source of truth",
    },
    {
    ...
    {
      path: "server/plugins/",
      note: "起動時 integrations",
    },
    {
      path: "server/routes/api/",
      note: "actions だけでは足りない場合のみ custom routes",
    },
    {
      path: "shared/types.ts",
      note: "共有 client/server 型",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md: 複雑な workflow 向けのエージェントガイド",
    },
    {
      path: "AGENTS.md",
      note: "テンプレート固有のエージェント指示",
    },
    {
      path: "package.json",
    },
...
アプリケーション状態に `data/` ディレクトリを追加しないでください。永続的なアプリ データは PostgreSQL/PGlite に属し、UI は actions または型指定されたサーバー ハンドラーを通じてそれを読み取ります。

各テンプレートの 4 つの領域は、1 つの共有アクション サーフェスと 1 つの PostgreSQL/PGlite データベースを介して連携します。エージェントと UI は、同じ操作に関して同等のパートナーです。

<Diagram id="doc-block-1e4ddix" title={"テンプレートの 4 つの領域がどのように接続されるか"} summary={"UI とエージェントは両方とも同じアクションを通じて PostgreSQL/PGlite に到達します。アプリケーションの状態とポーリング同期により、それらの整合性が維持されます。"}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      React UI画面<br /><small class="diagram-muted"
        >app/routes · コンポーネント</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">アクション</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Drizzle 経由の PostgreSQL<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">ポーリングing sync</div>
</div>
```
...
## PostgreSQL のモデル データ {#data-models}

フレームワーク Drizzle ヘルパーを使用してドメイン テーブルを定義し、スキーマが PostgreSQL と PGlite を使用するようにします。
...
スキーマの変更は追加的である必要があります。新しいアプリ固有のテーブルや
列には `drizzle-kit generate` を使って
`server/db/migrations/<id_name>.sql` にレビュー可能な SQL を生成し、Node.js
デプロイのリリース手順から `runDrizzleMigrations()` で読み込みます。
Cloudflare Workers/Pages と他の edge ランタイムにはデプロイ可能なファイルシステムがないため、
生成した SQL を worker bundle に埋め込み、`runMigrations()` に渡します。手書きの
履歴や特殊なケースには `runMigrations()` を使います。破壊的な SQL、
`drizzle-kit push`、テーブル名の変更、または列の削除は決して使用しないでください。

アプリの読み取りと書き込みには、Drizzle のクエリ ビルダーと `drizzle-orm` の PostgreSQL 対応オペレーターを使用します。Drizzle でクエリを表現できる場合は、生の SQL を使用して製品コードを記述せず、テンプレートでは `@agent-native/core/db/schema` のスキーマ ヘルパーを使用してください。
...
ユーザーまたは組織のデータを保持するスキーマを追加する前に、[Database](/docs/server-database) ドキュメントと [Security](/docs/security) ドキュメントを使用してください。
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "8",
      label: "工具表面",
      note: "`description` は、このアクションをいつツールとして呼び出すかを決定するためにエージェントが読み取るものです。",
    },
    {
      lines: "9-11",
      label: "型付き契約",
      note: "1つの zod `schema` が、エージェント、UI、HTTP、MCP、A2A からの入力を検証します。",
    },
    {
      lines: "18-19",
      label: "スコープ付き書き込み",
      note: "`ctx` から `ownerEmail` / `orgId` をスタンプして、行の共有およびアクセス チェックの範囲が正しく設定されるようにします。",
    },
  ]}
/>
...
- Data lives in Drizzle 経由の PostgreSQL. Use actions for all writes; schema is additive.
...
コミュニティテンプレートは公開 GitHub リポジトリから直接インストールします。Agent-Native アプリをリポジトリのルートに配置してください。

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

特定のリビジョンを固定するには `#branch`、`#tag`、または `#commit` を追加します。

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

CLI はソースだけをダウンロードし、依存関係のインストールやテンプレートのスクリプト実行は行いません。コミュニティテンプレートは第三者のコードです。`pnpm install` の前にリポジトリ、ライセンス、依存関係、インストールスクリプトを確認してください。

カタログ掲載には、ルートにある完成したアプリ、ライセンスとセットアップ README のある公開リポジトリ、`@agent-native/core` への依存、成功する型チェックとテストが必要です。ホスト版デモとスクリーンショットは任意です。

リポジトリの準備ができたら[コミュニティテンプレートを申請](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml)してください。コミュニティ掲載は公式カタログと分けられ、作者が管理します。
