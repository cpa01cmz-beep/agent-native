...
> **どのワークスペース ドキュメントですか?** このページでは、**デプロイ形態** (1 つのモノリポジトリ、多数のアプリ、共有認証、および統合デプロイ) について説明します。エージェントのカスタマイズ レイヤー (**エージェント リソース**: `AGENTS.md`、メモリ、skills、カスタム エージェント) については、「[エージェント リソース](/docs/agent-resources)」を参照してください。ガバナンス (誰が何を審査、承認、所有するか) については、[Workspace Governance](/docs/workspace-management) を参照してください。

## スタンドアロンかワークスペースか {#standalone-or-workspace}

会社が複数の社内アプリを運用し、デプロイ設定と組織コンテキストを共有する場合はワークスペースを選びます。独立した単一ツールやプロトタイプにはスタンドアロンアプリを選びます。スタンドアロンもサポートされるデプロイ形態ですが、共有ワークスペースパッケージやルート設定は含まれません。[管理デプロイ](/docs/administered-deployments)を参照してください。
...
<FileTree
  id="doc-block-y17o35"
  title={"生成された workspace"}
  entries={[
    {
      path: "package.json",
      note: "agent-native.workspaceCore を宣言",
    },
    {
    ...
    {
      path: ".env.example",
      note: "共有の ANTHROPIC_API_KEY、A2A_SECRET、DATABASE_URL、...",
    },
    {
    ...
    },
    {
      ...
      note: "必要な場合のみ plugin overrides",
    },
    {
      path: "packages/shared/src/client/",
      note: "必要な場合のみ共有 React コード",
    },
    {
      path: "packages/shared/AGENTS.md",
      note: "workspace 全体の指示",
    },
    {
      path: "apps/mail/",
    },
...
<Diagram id="doc-block-j5uf48" title={"3 つのレイヤー、ファイル名ごとに結合"} summary={"各アプリは、最初にアプリローカルからプラグイン、スキル、アクション、AGENTS.md を解決し、次に共有パッケージ、次にフレームワークのデフォルトを解決します。"}>
...
ディスパッチ リソースのスコープは、**すべてのアプリ** (すべてのアプリが実行時に継承し、コピーや同期手順はありません) または **選択されたアプリ** (アプリ固有のコンテキストに対してアプリごとに付与されます) です。完全なリソース モデル テーブル、パス規則、推奨スターター パックについては、[エージェント リソース](/docs/agent-resources#global-resources) を参照してください。
...
<Diagram id="doc-block-yfp58s" title={"統合デプロイ: アプリごとに 1 つのオリジン、1 つのパス"} summary={"すべてのアプリは単一のオリジンで出荷されるため、ログイン セッションとアプリ間の A2A は無料です。"}>
...
- [エージェント リソース](/docs/agent-resources) — ワークスペース内のすべてのアプリが共有するカスタマイズ レイヤー (`AGENTS.md`、`LEARNINGS.md`、個人メモリ、skills、カスタム エージェント)。
