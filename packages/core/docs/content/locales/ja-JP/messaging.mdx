...
> **ディスパッチ テンプレートを使用しますか?** これらすべては **[管理] → [メッセージング]** で設定されます。クリックして各プラットフォームに接続します。独自のテンプレートをカスタマイズまたは構築する場合を除き、このページの残りの部分を読む必要はありません。 [Dispatch](/docs/dispatch) または [Dispatch template reference](/docs/template-dispatch) を参照してください。
...
- **メッセージから他のツールを操作。** @mention の結果を Notion の行、Jira の課題、HubSpot のレコードにできます。完全な手順は [Slack の @mention → Notion の行](#slack-to-notion) を参照してください。
- 一方向のアプリ内アラート (ベルのアイコン、webhooks) については、[Notifications](/docs/notifications) を参照してください。

<Diagram id="doc-block-1cagm52" title={"多くのチャネル、1 人のエージェント"} summary={"すべてのプラットフォームは同じエージェント ループと同じ SQL スレッド履歴にファンインするため、SlackのDM と電子メールは同じ会話を継続します。"}>
...
<div class="msg-fanin">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">1 つのエージェントループ</span
    ><small class="diagram-muted">同じメモリ · 同じツール</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    1 つの SQL スレッド履歴<br /><small class="diagram-muted"
...
- **チャンネルメンション** — 作業開始時は @mention します。エージェントに処理させる各チャンネルメッセージには明示的なメンションが必要です。通常のスレッド返信やチャンネルの一般的な会話では起動しません。
...
管理対象 OAuth が推奨設定です。**管理 → Messaging** で **Agent manifest** を選択し、
...
`app_home_opened`、`app_context_changed`、`app_mention`、`message.im` を購読します。
`SLACK_CLIENT_ID`、`SLACK_CLIENT_SECRET`、`SLACK_SIGNING_SECRET`
を設定したら、**管理 → Messaging** で **Add to Slack** を選択します。各インストールの
...
各チャンネルメッセージにはエージェントへの明示的なメンションが必要です。通常の
スレッド返信やチャンネルの一般的な会話ではエージェントを起動しません。制限された文脈にはリアクションとファイル参照が含まれ、
...
### 実践例: Slack の @mention から Notion の行へ {#slack-to-notion}

Slack のメンションが動くようになると、次に必ず出てくる要望が _「スレッドでエージェントに @mention したら、その内容を Notion のデータベースに行として追加してほしい」_ です。これはすでに実現できます。Slack の受信、Notion のプロバイダー API、アプリのアクションはすべて組み込み済みです。ただし 3 つの部品を正しい順序でつなぐ必要があり、そのうち 2 つは飛ばしても何も言わずに失敗します。

以下の `<ANGLE_BRACKETS>` はすべてプレースホルダーです。実際の Notion のシークレットをソース、コミットされる `.env`、チャットのメッセージに貼り付けないでください。

#### 1. メンションをアプリに届ける

Notion に手を付ける前に [Slack をセットアップ](#slack) を完了し、チャンネルでの `@mention` に返信が返ることを確認してください。以降の手順では次の 2 点が重要です。

- `@mention` を含むメッセージが起動のきっかけであり、そのテキストがエージェントに渡ります。
- 同じスレッドで後から送る各メッセージにも、エージェントへの明示的なメンションが必要です。通常の返信は届かないため、新しい依頼ごとに改めて @mention してください。

#### 2. プロバイダー API アクションを公開する

Core には Notion プロバイダーのプリセット (ベース URL、`Notion-Version` ヘッダー、認証、ドキュメントリンク) が同梱されていますが、エージェントがそれを使うための 3 つのアクション — `provider-api-catalog`、`provider-api-docs`、`provider-api-request` — は**オプトインのサブパスエクスポート**です。これらを追加していないアプリには Notion への経路がまったくなく、エージェントは Notion のツールがないと答えます。ここが最も見落とされやすい手順です。
...
続いてアクション ID ごとに薄いアクションファイルを追加します。ファクトリーのオプションはすべて省略可能なので、各ファイルは 1 回の呼び出しで済みます。
...
`actions/` 内のファイルは自動的に検出されるため、編集すべきレジストリはありません。プロバイダーを絞り込んだり HTTP で公開したりする場合は [Core のアクションファクトリーを再利用する](/docs/integrations#provider-api-factories) を参照してください。

#### 3. Notion のキーは `.env` ではなく保管庫に入れる

**[notion.so/my-integrations](https://www.notion.so/my-integrations)** で内部インテグレーションを作成し、シークレットをコピーします。次にアプリで **Settings → API Keys & Connections** を開き、名前をちょうど `NOTION_API_KEY` としたキーを追加して値を貼り付け、スコープを選びます。

- **Personal** — あなたとして実行される処理だけが使用できます。
- **Workspace** — 組織で共有されます。あなた以外の人がエージェントを @mention する場合はこちらを選んでください。Slack の実行はメッセージを書いた人ではなくインテグレーションの所有者として動くためです。

プロバイダーランタイムは暗号化された保管庫から、個人 → 組織 → ワークスペースの順に資格情報を解決し、**意図的に `process.env` を一切読みません**。デプロイ時の環境変数はマルチテナントアプリの全ユーザーで共有されてしまうためです。`.env` に `NOTION_API_KEY` の行を書いても、ここでは読み取られません。

#### 4. データベースをインテグレーションと共有する

Notion で対象のデータベースを開き、**••• → Connections →** からインテグレーションを追加します。これを飛ばすのが Notion で最も多く、最も紛らわしい失敗です。API は 403 ではなく **404** を返すため、データベース ID が間違っているように見えます。カタログのエントリーにはこの対処テキストが含まれており、`provider-api-request` は 401・403・404 いずれの応答でもそれを `guidance` フィールドに追加します。

データベース ID はデータベースの URL にあります。`?` の直前にある 32 文字の文字列です。

#### 5. エージェントに作ってもらう

どの設定画面にも書かれていないのがこの部分です。2 つの半分を自分でつなぐのではなく、望む振る舞いを依頼します。アプリのエージェントチャットで次のように伝えます。

> プロバイダー API を使って Notion データベース `<NOTION_DATABASE_ID>` に行を追加する `add-notion-row` というアクションを作ってください。メッセージ本文、Slack の投稿者、Slack メッセージへのリンクを受け取れるようにしてください。そのうえで `AGENTS.md` を更新し、Slack で @mention されて記録や保存を頼まれたらそのアクションを呼び出し、新しい Notion ページの URL を返信するようにしてください。

エージェントが書くべきなのは**1 つのアクション**です。Nitro のルートでも、ブラウザーからの `fetch` でもありません。
...
このサンプルで重要な点は 3 つあります。

- 現行の Notion では行はデータベース内の**データソース**にぶら下がるため、まずデータベースを読み、その `data_sources[0].id` を親として使います。データベース ID をそのまま親に渡すのは旧 API の形式です。
- `executeRequest` はプロバイダーのエラーを送出せず戻り値として返すため、`response.ok` を自分で確認し、返ってきた `guidance` をそのまま伝えてください。「データベースを共有してください」というヒントはそこに届きます。
- プロパティ名 (`Name`、`Author`、`Source`) はデータベースの列と完全に一致している必要があり、型も一致させる必要があります。推測させず、まずデータベースの `properties` をエージェントに読ませてください。

`AGENTS.md` の更新は省略できません。それがないと、エージェントはツールを持っていても、Slack のメッセージで「これを記録して」と言われたときに使う理由がありません。

#### 6. 動作確認と、うまくいかないときの確認先

エージェントが参加している Slack チャンネルで `@YourAgent これを Notion に記録して: <SOME_TEXT>` と投稿します。成功した実行はスレッド内に新しいページの URL を返信し、Notion に行が追加されます。

うまくいかない場合:

| 見えている症状                                            | 確認する場所                                                                                                                                            |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Slack がまったく返信しない                                | メンションがアプリに届いていません。[Slack をセットアップ](#slack) の Request URL と署名シークレットを確認してください。Notion 側の問題ではありません。 |
| エージェントは返信するが Notion のツールがないと言う      | 手順 2。プロバイダー API アクションが `actions/` に追加されていません。                                                                                 |
| `notion credential not configured. Tried: NOTION_API_KEY` | 手順 3。キーが `.env` にあるか、実行者とは別のアカウントの **Personal** として保存されています。**Workspace** で保存してください。                      |
| Notion が `404` を返す                                    | 手順 4。データベースがインテグレーションと共有されていません。                                                                                          |
| Notion がプロパティについて `400` を返す                  | アクション内のプロパティ名または型がデータベースの列と一致していません。                                                                                |
| 一度は成功したが、その後のスレッド返信が無視された        | 想定どおりの動作です。エージェントに処理させる各メッセージには明示的なメンションが必要です。改めて @mention してください。                              |
...
- **設定ではなくクリックです。** Dispatch の **[管理] → [メッセージング]** ページには、env-var フィールドが組み込まれたすべてのプラットフォームの接続ボタンがあります。
...
<Diagram id="doc-block-utrp4l" title={"インバウンド Webhook ライフサイクル"} summary={"Webhook は検証し、キューに入れ、200 を返すだけです。新しい関数を実行するとキューが空になり、セーフティ ネットとして 60 秒の再試行ジョブを使用してエージェント ループが実行されます。"}>

```html
<div class="msg-flow">
  <div class="msg-row">
    <div class="diagram-node">
      Platform<br /><small class="diagram-muted">Slack · メール · など</small>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>
      <strong>/webhook</strong><br /><small class="diagram-muted"
        >verify signature + parse</small
      ><br /><span class="diagram-pill">INSERT 保留中タスク</span>
    </div>
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<Endpoint id="doc-block-11qicag" method="POST" path="/_agent-native/integrations/slack/webhook" summary={"Slack イベント API インバウンド Webhook"} auth={"HMAC-SHA256 of the raw body using `SLACK_SIGNING_SECRET`, checked against the `X-Slack-Signature` header. In production also gated by `SLACK_ALLOWED_TEAM_IDS` / `SLACK_ALLOWED_API_APP_IDS`."} params={[
  {
