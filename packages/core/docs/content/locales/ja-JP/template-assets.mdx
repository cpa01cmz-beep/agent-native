...
Assets は、ブランドと一貫したメディアを作成および管理するためのエージェントネイティブのワークスペースです。特に画像モデルや動画モデルに明確な強みがある場合に向いています。アップロードと生成された結果をライブラリとフォルダーに整理し、チームが製品ショット、ロゴ、ソーシャル画像、フォトリアルなシーンなどの承認済みの参照素材を集めて、Design のコンポジションや別の場所で使えるようにします。生成はエージェントチャットを通じて行われるため、すべてのアセットをレビューして調整できます。

<WireframeBlock id="doc-block-5g124l">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-o95vnt" title={"生成、レビュー、再利用"} summary={"参照とプロンプトにより、セッションの生成と選択が行われます。選択したアセットはライブラリに配置され、ピッカーまたは A2A を介して他のアプリに流出します。"}>
...
<div class="diagram-assets">
  <div class="diagram-col">
    ...
    <div class="diagram-node">
      プロンプト<br /><small class="diagram-muted"
        >チャットまたは Generate コントロール</small
      >
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    <span class="diagram-pill accent">生成セッション</span
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">選択済みでブランド一貫性のある assets</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-node">
      Picker<br /><small class="diagram-muted">iframe / MCP アプリ</small>
    </div>
    <div class="diagram-node">
      A2A<br /><small class="diagram-muted"
        >スライド · デザイン · コンテンツ</small
      >
    </div>
  </div>
</div>
...
## Assets と Design の使い分け

ほとんどのビジュアル制作には **Design** を使います。ランディングページ、広告、図、その他のコンポジションで、HTML/CSS、正確なレイアウト、編集可能な構造、デザインツールとのインポート/エクスポートが必要な場合は、Design から始めてください。

画像または動画モデルの方が適していて、結果を細かく編集するのではなくメディアとしてそのまま使える場合は **Assets** を使います。フォトリアルまたはピクセルベースの画像、製品ショット、ソーシャル画像、動画などがこれに当たります。生成した画像をそのまま使いたい場合は、手描き風や Excalidraw 風の図が例外になることもあります。

想定されるワークフローでは Design から始め、PNG などのメディア素材が適している場合に Design エージェントが補助画像の生成を Assets に委譲します。結果は Design のコンポジションや別のアプリで使えます。
