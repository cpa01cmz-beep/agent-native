...
Un nouvel échafaudage `npx --yes @agent-native/core@latest create` est déjà livré avec :
...
Si vous évaluez un agent natif pour un CRM, un outil de suivi de projet, une boîte de réception d'assistance ou tout autre outil d'équipe, la base multi-tenant est déjà là. Tous les modèles propriétaires sont multi-tenants – voir [Modèles](/docs/cloneable-saas) pour la liste.

<Diagram id="doc-block-1dto6fl" title={"Appartenance à une organisation et isolement"} summary={"Les utilisateurs rejoignent les organisations en tant que owner/admin/member. Chaque ligne propriétaire porte le org_id du locataire qui en est propriétaire, et aucune ligne ne traverse la limite."}>
...
<Diagram id="doc-block-1bfayqu" title={"De la session à la portée SQL"} summary={"L'organisation active sur la session devient AGENT_ORG_ID, que le framework intègre dans la clause WHERE de chaque requête."}>
...
- [**Authentication**](/docs/authentication#organizations) — sessions, fournisseurs sociaux et surface de gestion de l’organisation
- [**Security — Data Scoping**](/docs/security#data-scoping) — isolation au niveau SQL, contrat `ownableColumns()` et protections d’accès
- [**Multi-App Workspace**](/docs/multi-app-workspace) — hébergement de plusieurs applications Agent-Native dans un monorepo avec authentification partagée et RBAC

## Modèle d’isolation

Utilisez `accessFilter()` et `assertAccess()` pour les lectures et écritures côté serveur. Le cloisonnement SQL de l’agent crée des vues temporaires filtrées pour les tables détenues. `pnpm guards` inclut le contrôle des requêtes non cloisonnées. RLS ne fait pas actuellement partie des contrôles actifs. Son ajout est prévu après la version 1.0. Consultez [Déploiements administrés](/docs/administered-deployments).
