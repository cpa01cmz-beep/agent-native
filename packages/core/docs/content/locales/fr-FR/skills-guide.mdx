...
Les skills sont des fichiers Markdown qui apportent à l'agent une connaissance approfondie de modèles et de workflows précis. Au lieu d'entasser toutes les règles dans une seule invite système, vous répartissez les connaissances dans des fichiers ciblés que l'agent ne charge que lorsqu'une tâche en a réellement besoin. Le contexte par défaut reste ainsi réduit, tout en permettant à l'agent d'aller en profondeur à la demande.
...
Les skills basées sur des apps incluent aujourd'hui `visual-plan`, `visual-recap` et `visualize-repo` depuis Plan, `visual-edit` depuis Design, `assets` depuis Assets et `content` pour l'édition docs/blog/MDX adossée au dépôt.
...
Les paquets `app-skill` produisent également les fichiers Agent Plugin standards. Utilisez `agent-native plugin import <path> --into <workspace> --yes` pour importer les Skills et les serveurs MCP distants via Streamable HTTP ; les en-têtes sont ignorés et les serveurs stdio et SSE sont signalés puis ignorés.
...
- Le guide est unique : placez-le plutôt dans `AGENTS.md` ou dans la mémoire de l'agent
...
Si les connaissances s'appliquent à _tout le monde_ travaillant dans l'application (« préférez toujours les CTE aux sous-requêtes »), il s'agit d'une compétence ou d'un `LEARNINGS.md` partagé. S'il s'agit de _cet utilisateur particulier_ ("Steve aime les réponses concises"), cela appartient à `memory/MEMORY.md`. Voir [Ressources de l'agent — Mémoire](/docs/agent-resources#memory) pour le traitement complet.

## Skills et ressources SQL {#skills-vs-resources}

Cette page présente un skill comme un fichier situé dans `.agents/skills/<name>/SKILL.md`. [Ressources de l'agent](/docs/agent-resources#skills) présente ce même skill comme une ressource SQL au chemin `skills/<slug>/SKILL.md`, de portée workspace, partagée ou personnelle. Il s'agit du même concept vu sous deux angles : le mode Code lit et écrit directement le fichier ; le mode App le stocke sous forme de ligne de ressource afin qu'il fonctionne sans système de fichiers. Dans les deux cas, rédigez le contenu au format décrit sur cette page.

## skills basé sur une application — tous les détails {#app-backed-skills-full}

Le format du manifeste, les commandes CLI, les adaptateurs de marketplace et le hachage de mise à jour automatique qui sous-tendent les [skills basés sur une application](#app-backed-skills), en détail :
...
- [**Rédiger les instructions de l'agent**](/docs/writing-agent-instructions) — formuler les descriptions des skills et répartir les consignes entre AGENTS.md, les skills et la mémoire
- [**Ressources de l'agent**](/docs/agent-resources#skills) — où les skills résident sous forme de ressources SQL en mode App
- [**Mentions d'agents**](/docs/agent-mentions) — le déclencheur de commande `/` qui invoque un skill dans le chat
- [**Plugin et marketplace Plan**](/docs/plan-plugin) — installation et mise à jour automatique de bout en bout d'un véritable skill basé sur une application
