---
title: "Ressources de l'agent"
description: "Personnalisation au niveau du code Claude par utilisateur – skills, mémoire, instructions, agents personnalisés, tâches planifiées, serveurs MCP – soutenue par PostgreSQL/PGlite, et non par un système de fichiers."
---

# Ressources de l'agent

Chaque application native d'agent est livrée avec des **ressources de l'agent** : la couche de personnalisation qui fait de l'agent le vôtre. Il contient des instructions d'équipe (`AGENTS.md`), des apprentissages partagés (`LEARNINGS.md`), une mémoire structurée personnelle (`memory/MEMORY.md`), un skills que l'agent extrait à la demande, des sous-agents personnalisés, des tâches planifiées et des serveurs MCP connectés – tout ce que vous attendez d'une configuration Claude Code / Codex.

Le twist : **il s'agit de lignes PostgreSQL/PGlite, pas de fichiers du système de fichiers.** Chaque utilisateur obtient son propre ensemble de ressources de l'agent stocké dans la base de données. Il n'y a pas de boîte de développement à démarrer, pas de conteneur par utilisateur, pas de fichiers à monter. Un SaaS multi-locataires peut offrir à chaque utilisateur un agent entièrement personnalisable et essentiellement gratuit, car il s'agit uniquement de lignes (mémoire personnelle, serveurs MCP personnels, skills personnels, sous-agents personnels) et la base de code partagée les héberge toutes en même temps.

<Diagram id="doc-block-bkcqdw" title={"Une configuration Claude-Code, mais stockée sous forme de ressources de l'agent dans PostgreSQL/PGlite"} summary={"La même couche de personnalisation (instructions, compétences, mémoire, agents, tâches, MCP) sauf que chaque fichier est une ligne dans une base de données mutualisée partagée."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Ressources agent-native</span
    ><small class="diagram-muted"
      >rows in one base de données PostgreSQL/PGlite</small
    >
    <div class="ws-rows">
    ...
    <small class="diagram-muted"
      >une base de code, plusieurs utilisateurs, avec périmètres
...
| Code Claude / Codex                                  | Ressources natives de l'agent                                 |
| ---------------------------------------------------- | ------------------------------------------------------------- |
| Fichiers sur votre disque local                      | Lignes dans une base de données PostgreSQL/PGlite partagée    |
...
| `CLAUDE.md` / skills par projet                      | `AGENTS.md` par application + ressources de mémoire           |
| Configuration MCP dans un fichier JSON               | Configuration MCP dans JSON _ou_ les paramètres UI, par scope |

Mêmes capacités. Une économie différente. Consultez [Applications](/docs/cloneable-saas) pour savoir pourquoi cela est important pour le SaaS.
...
<Diagram id="doc-block-zk5bz1" title="Trois scopes, un fichier efficace" summary={"Le runtime résout le même chemin dans l’espace de travail, l’application et les étendues personnelles en lecture : l’étendue la plus spécifique l’emporte."}>
...
<div class="ws-stack">
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >valeurs par défaut d’entreprise depuis Dispatch</small
    ><code>context/brand.md</code>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Organisation / application</span
...
1. Ouvrez l'onglet **Ressources** → **Partagé** → `AGENTS.md` (créez-le avec `+` → **Fichier** s'il est manquant).
...
<Callout id="doc-block-1aa9qfd" tone="info">

Pas de redémarrage, pas de redéploiement. `AGENTS.md` est lu au début de chaque tour, donc une modification que vous enregistrez modifie désormais le comportement de l'agent au message suivant.
...
## Onglet Ressources {#resources-tab}

Le panneau de l'agent comprend un onglet **Ressources** à côté de Chat et CLI. Il affiche une arborescence organisée en dossiers de toutes les ressources, un éditeur en ligne pour n'importe quel fichier texte (Markdown, JSON, YAML, texte brut) et les flux de création typés du menu `+` (Fichiers, Skills, Agents, Tâches planifiées). Les utilisateurs peuvent parcourir les paramètres par défaut hérités de l'espace de travail et créer/modifier/supprimer des ressources personnelles ou d'organisation.
...
Cliquez sur l'icône `?` dans la barre d'outils Ressources pour revenir à ces documents à tout moment.
...
Les ressources de l'agent comportent deux surfaces de mémoire actuelles :
...
| `mcp-servers/…`    | Espace de travail / partagé | Humains via Dispatch ou les ressources de l'application | Actualisation de la configuration MCP   |

Les utilisateurs peuvent modifier ces fichiers de mémoire directement dans l'onglet Ressources : ce sont des ressources standard. Supprimez les lignes sur lesquelles l'agent s'est trompé, conservez les préférences personnelles dans `memory/MEMORY.md` ou promouvez les règles à l'échelle de l'équipe dans `AGENTS.md`.
...
<DataModel
  id="doc-block-c2uuu2"
  title={"Le modèle de ressources de l'agent"}
  summary={
    "Une forme de ressource soutient chaque fichier de ressources de l’agent. Le runtime le classe par chemin et portée et résout la valeur effective lors de la lecture."
  }
  entities={[
    {
      id: "resource",
      name: "agent resource",
      note: "Un seul fichier dans les ressources de l'agent d'un utilisateur : instructions, compétence, mémoire, agent, configuration MCP ou tâche.",
      fields: [
        {
          name: "path",
          type: "string",
        ...
        },
        {
          ...
          note: "À quel niveau se trouve cette rangée",
        },
        {
          name: "owner",
          type: "string",
...
Skills sont des fichiers de ressources Markdown sous le chemin `skills/` (de préférence `skills/<name>/SKILL.md`) qui donnent à l'agent des connaissances sur le domaine à la demande, invoquées dans une conversation avec `/skill-name`. Ajoutez-les depuis l'onglet Ressources ou, en mode Code, depuis `.agents/skills/`.
...
<AnnotatedCode
  id="doc-block-qfr8wr"
  title={"Un profil d'agent personnalisé"}
  ...
  annotations={[
    {
      ...
      note: "`name` est ce qui apparaît dans la liste déroulante `@` et à quoi l'agent principal délègue.",
    },
    {
      lines: "3-4",
      label: "Quand déléguer",
      note: "`description` est ce que l'orchestrateur lit pour décider que ce profil correspond à une tâche.",
    },
    {
      lines: "5",
      label: "Modèle",
      note: "`inherit` réutilise le modèle de l'agent principal. Remplacez uniquement lorsque le profil en a clairement besoin d’un autre.",
    },
    {
      lines: "6",
      note: "`tools: inherit` pour l'instant — le champ est réservé aux futures politiques d'outils par agent.",
    },
  ]}
/>
...
Les ressources de l'agent comprennent deux types d'agents :
...
## Mentions @ et commandes / {#at-tagging}

Saisissez `@` dans le champ de chat pour référencer un agent, un fichier ou une ressource, ou `/` pour invoquer un skill. Dans les deux cas, un menu s'ouvre au niveau du curseur et ajoute une puce intégrée lorsque vous sélectionnez un élément. Consultez [Mentions d'agents](/docs/agent-mentions) pour le fonctionnement complet (routage, `MentionProvider`, sélection de sous-agents) et le [Guide Skills](/docs/skills-guide) pour savoir comment les skills sont rédigés et découverts.
...
| ----------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| @marquage                     | Fichiers de base de code + ressources de l'agent + agents personnalisés + agents connectés | Ressources de l'agent + agents personnalisés + agents connectés |
...
- [**Guide Skills**](/docs/skills-guide) — rédiger et définir la portée des fichiers de skill référencés depuis `skills/`
- [**Mentions d'agents**](/docs/agent-mentions) — fonctionnement complet des mentions `@` et des commandes `/`
- [**Tâches récurrentes**](/docs/recurring-jobs) — planifier les invites créées à partir des tâches planifiées
- [**Gouvernance du Workspace**](/docs/workspace-management) — qui examine, approuve et possède les modifications du workspace
- [**Workspaces multi-applications**](/docs/multi-app-workspace) — la forme de déploiement couverte par un workspace
- [**Connexions du Workspace**](/docs/workspace-connections) — partager les identifiants des fournisseurs entre les applications
