...
## Liens Content privés {#private-content-links}

Un lien de partage Content privé peut identifier le document sans en exposer le contenu. Ajoutez l’URL `/mcp` de la même instance Content comme serveur MCP distant dans votre agent, authentifiez-vous avec votre compte Agent-Native, activez la connexion ou ses outils, puis relancez la demande initiale. La page `/mcp/connect` fournit les options de configuration, mais son ouverture seule n’installe pas la connexion.

Ce parcours fonctionne avec tout hôte compatible MCP, notamment Claude, ChatGPT, Claude Code, Codex, Cursor, Antigravity, OpenCode et GitHub Copilot / VS Code. Les écrans et invites d’authentification varient selon l’hôte. Antigravity gère les serveurs distants dans **Settings → Customizations → Installed MCP Servers** ou dans sa configuration MCP ; la commande `connect` d’Agent-Native ne configure pas encore Antigravity.

L’authentification identifie le compte connecté sans élargir ses autorisations. Un document public reste lisible depuis son lien de partage. Un document privé n’est lisible via MCP que si ce compte y a déjà accès. Si `get-document` renvoie « introuvable » après la connexion, cette réponse ne distingue pas une autorisation manquante d’un identifiant de document obsolète, supprimé ou invalide. Vérifiez le lien et l’accès du compte connecté avant de modifier les autorisations.

La réinstallation du connecteur ou la modification du lien de partage public ne résout pas un refus d’autorisation confirmé du compte connecté.
