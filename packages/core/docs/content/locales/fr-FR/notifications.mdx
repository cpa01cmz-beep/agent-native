...
<Diagram id="doc-block-188vju" title="Un appel, plusieurs destinations" summary={"notify() écrit toujours la ligne de la boîte de réception réservée au propriétaire, est diffusée en parallèle sur chaque canal enregistré (au mieux), puis émet notification.sent sur le bus d'événements."}>
...
L'e-mail, Slack et le webhook sont toujours enregistrés avec la boîte de réception. La livraison privilégie l'URL ou les destinataires des métadonnées de la notification, puis utilise la variable d'environnement correspondante. Si aucun des deux n'est défini, ce canal ne fait rien lors de l'émission.
...
<Diagram id="doc-block-1c0wbur" title={"Canaux et gravité"} summary={"la boîte de réception est toujours activée ; le webhook a besoin d'une variable d'environnement ; les canaux personnalisés s'enregistrent au démarrage. La gravité détermine le style du badge et est transmise à tous les canaux."}>
...
<div class="diagram-channels">
  ...
  <div class="diagram-panel col" data-rough>
    <strong>La gravité détermine le badge</strong>
    <div class="sev-row">
      <span class="diagram-pill">info</span
      ><span class="diagram-muted">confirmations, infos</span>
    </div>
    <div class="sev-row">
...
<Endpoint id="doc-block-1wwo0xz" title="Liste des notifications" method="GET" path={"/_agent-native/notifications?unread=true&limit=50&before=ISO"} summary={"Répertorier les notifications récentes pour l'utilisateur actuel"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
  ...
  },
  {
    ...
    "type": "string",
    "required": false,
    "description": "Curseur d'horodatage ISO — renvoie les notifications créées avant cette date pour la pagination."
  }
]} responses={[
  {
    "status": "200",
...
- **Intégration de sondage** — chaque mutation appelle `recordChange()` afin que les modèles utilisant [`useDbSync`](/docs/client-data#usedbsync) s'invalident automatiquement sans aucun câblage supplémentaire.
...
- [**Server plugins**](/docs/server-overview) — où les canaux personnalisés sont enregistrés au démarrage
