...
<Diagram id="doc-block-czfekg" title={"La liste de contrôle de configuration"} summary={"Seule la connexion d’un moteur AI est requise. Le panneau suit l'achèvement et se cache automatiquement une fois que tout ce qui est requis est fait."}>

```html
<div class="ob">
  <div class="diagram-card">
    <span class="diagram-pill warn">required</span
    ><strong>Connecter un moteur IA</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill">optional</span><strong>Livraison e-mail</strong
...
<Endpoint id="doc-block-16x29ch" title={"Répertorier les étapes d'intégration"} method="GET" path="/_agent-native/onboarding/steps" summary={"Répertoriez toutes les étapes enregistrées avec leur statut d'achèvement"} responses={[
  {
    "status": "200",
...
<AnnotatedCode
  id="doc-block-1v14h41"
  title={"Enregistrer une étape d'intégration personnalisée"}
  ...
  annotations={[
    {
      lines: "5",
      label: "Monté automatiquement",
      note: "Inscrivez-vous à partir d'un plugin Nitro — le framework gère le rendu, le suivi de l'achèvement et le licenciement.",
    },
    {
      lines: "7",
      label: "Identifiant stable",
      note: "La réinscription avec le même `id` après le chargement des valeurs par défaut remplace une étape intégrée.",
    },
    {
      lines: "12-19",
    ...
    {
      lines: "28-31",
      label: "Contrôle d'achèvement",
      note: "`isComplete` s'exécute côté serveur. Les jetons OAuth vivent dans la boutique `oauth_tokens` – vérifiez-le, pas `process.env.GMAIL_REFRESH_TOKEN`.",
    },
  ]}
/>
...
| `database` | non         | URL PostgreSQL hébergée ou vide pour PGlite local     |
