...
<WireframeBlock id="doc-block-19zibl7">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-8lk6c4" title={"Comment se déroule une demande de courrier"} summary={"Les raccourcis clavier et les invites des agents exécutent les mêmes actions. L'e-mail réside dans Gmail ; les brouillons, les automatisations et le suivi en direct dans SQL et application_state."}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      Vous pilotez<br /><small class="diagram-muted">raccourcis J/K/E/R</small>
    </div>
    <div class="diagram-node">
      Vous demandez à l’agent<br /><small class="diagram-muted"
        >"rédige un refus amical"</small
      >
    </div>
  </div>
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Gmail<br /><small class="diagram-muted">multi-compte, via OAuth</small>
    </div>
    <div class="diagram-box">
      SQL + application_state<br /><small class="diagram-muted"
        >brouillons · automatisations · suivi</small
      >
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">La boîte de réception s’actualise en direct</div>
</div>
```
...
<DataModel
  id="doc-block-zz1dhg"
  title="Tableaux Mail SQL"
  summary={
    "L'e-mail lui-même réside dans Gmail. Les tables SQL contiennent ce que Gmail ne contient pas : les brouillons en file d'attente, les événements de suivi d'envoi et les jetons OAuth. Les paramètres et l’état éphémère se trouvent dans les magasins settings et application_state."
  }
  entities={[
    {
...
**Ajout d'une fonctionnalité d'agent.** Ajoutez un nouveau fichier sous `templates/mail/actions/` à l'aide de `defineAction`. Votre action devient un outil d'agent, une commande CLI (`pnpm action <name>`) et une surface de hook frontal typée via `useActionQuery` / `useActionMutation`. Regardez `templates/mail/actions/star-email.ts` pour un court exemple ou `templates/mail/actions/manage-automations.ts` pour un exemple avec plusieurs sous-actions. Consultez la documentation [actions](/docs/actions-overview) pour le modèle complet.
...
Effectuez ces modifications dans votre dépôt et votre flux de développement. L'agent intégré ne peut participer aux modifications de source que si l'hôte accorde intentionnellement des outils d'écriture repository/workspace ; sinon, il agit via les actions et les données. Voir [Key Concepts](/docs/key-concepts#agent-modifies-code).

## Et ensuite

- [**Modèles**](/docs/cloneable-saas) — le modèle à cloner et à posséder, ainsi que tous les autres modèles de domaine pour démarrer
...
- [**Calendrier**](/docs/template-calendar) — un modèle de domaine associé pour la planification et les réservations
- [**Automatisations**](/docs/automations) — le moteur déclenché par des événements sur lequel reposent les règles de tri automatique de Mail
- [**Actions**](/docs/actions-overview) — le système d’actions appelé par le chat, les raccourcis et les automatisations
