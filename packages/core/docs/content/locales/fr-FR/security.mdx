...
<Diagram id="doc-block-1jaem9n" title={"Défense en couches"} summary={"Le framework possède la majeure partie de la surface des menaces ; vous possédez deux choses : le marquage des tables pour la portée et la validation des entrées externes."}>

```html
<div class="sec-layers">
  <div class="diagram-card free">
    <span class="diagram-pill ok">Possédé par le framework</span
  ...
  <div class="diagram-card you">
    <span class="diagram-pill warn">Vous en gardez le contrôle</span
    ><small class="diagram-muted"
...
Le framework crée le contexte de requête depuis la session et applique la propriété avec `accessFilter()` et `assertAccess()` aux lectures et écritures de l’application. Le SQL de l’agent reçoit des vues temporaires filtrées pour l’identité et l’organisation actives. RLS n’est pas encore activé ; les requêtes applicatives directes doivent donc continuer à utiliser ces assistants.
...
<Callout id="doc-block-1iizpox" tone="risk">

Ne construisez jamais SQL par concaténation de chaînes ou par littéraux de modèle. Transmettez l'entrée utilisateur comme `args` à `exec` / `db-query`, ou utilisez Drizzle - les deux sont toujours paramétrés. Les vérifications `pnpm guards` détectent les requêtes non ciblées et concaténées au moment CI.
...
<Diagram id="doc-block-u1hl1o" title="Le pipeline de cadrage" summary={"L'agent SQL ne touche jamais directement les tables de base : il lit une vue temporaire limitée à l'identité actuelle, de sorte qu'un nom de table nu ne peut renvoyer que les lignes possédées."}>

```html
<div class="scope-pipe">
  <div class="diagram-node">
    Session connectée<br /><small class="diagram-muted"
  ...
  <div class="diagram-node ok">
    SQL de l’agent<br /><small class="diagram-muted"
...
Les outils `db-query` / `db-exec` rejettent les références de table qualifiées par le schéma (`public.<table>`) : un nom qualifié est résolu en table de base et contournerait la vue temporaire ci-dessus. Les agents utilisent des noms de table nus ; la portée est appliquée automatiquement.
...
<DataModel
  id="doc-block-19f5mow"
  title="What ownableColumns() adds"
  summary="Les trois colonnes qui rendent une table sensible aux locataires et partageable."
  entities={[
    {
      id: "ownable",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Organisation active du propriétaire au moment de la création. Pilote les contrôles de visibilité de l’organisation.",
        },
        {
          name: "visibility",
          type: "enum",
          nullable: false,
          note: "privé | org | public – valeur par défaut grossière, par défaut privée.",
        },
      ],
    },
  ]}
...
Les informations d’identification par utilisateur/organisation et le coffre sont chiffrés au repos avec AES-256-GCM. Le coffre partagé de l’espace de travail préfère `WORKSPACE_SECRETS_ENCRYPTION_KEY`, puis les candidats hérités dérivés de `SECRETS_ENCRYPTION_KEY`, `A2A_SECRET` et `BETTER_AUTH_SECRET` ; pendant une rotation, `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` garde les anciennes lignes lisibles jusqu’à leur migration. Pour chiffrer les lignes en clair existantes, exécutez `pnpm action db-migrate-encrypt-credentials` (idempotent, non destructif).
...
Les gestionnaires de webhooks entrants (Resend, SendGrid, Slack, Telegram, WhatsApp, Recall.ai, Deepgram, Zoom) refusent les requêtes falsifiées par défaut en production : lorsque la variable d'environnement du secret de signature correspondante est manquante, le gestionnaire renvoie 401 au lieu d'accepter et de distribuer.
...
Les notifications `changes.watch` de Google Drive ne passent pas par cette passerelle d'adaptateur générique : ce sont des requêtes opaques sans corps. Le framework enregistre donc un jeton aléatoire par canal et vérifie les en-têtes `X-Goog-Channel-ID`, `X-Goog-Channel-Token` et `X-Goog-Resource-ID` avant de récupérer les changements. Il s'agit de notifications directes de Drive, pas de messages Pub/Sub, et elles ne contiennent pas de jetons OIDC bearer Pub/Sub.
...
- [ ] `WORKSPACE_SECRETS_ENCRYPTION_KEY` défini de façon cohérente pour les coffres multi-apps partagés ; le chiffrement local conserve les replis `SECRETS_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET` — voir [Secrets Management](#secrets)
