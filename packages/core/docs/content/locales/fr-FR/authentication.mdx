...
- **Par défaut :** Lien magique en priorité dans Better Auth lorsque l'e-mail sortant est prêt, avec e-mail/mot de passe en secours et fournisseurs sociaux. Définissez `AUTH_MAGIC_LINK=0` pour forcer le mode de secours.
...
<Diagram id="doc-block-16302hf" title={"Trois façons d'entrer, une seule séance"} summary={"Les visiteurs du navigateur, les hôtes MCP et les fournisseurs personnalisés résolvent tous la même AuthSession que celle lue par la portée en aval."}>
...
<div class="auth-modes">
  <div class="diagram-col">
    ...
    <div class="diagram-card">
      ...
      ><small class="diagram-muted">connecteurs Claude Code et ChatGPT</small>
    </div>
    <div class="diagram-card">
...
## Connexion en développement local {#local-development-sign-in}

Sur un serveur de développement utilisant une adresse de bouclage, la page de connexion partagée affiche d'abord **Continue as local dev**. Elle crée ou réutilise le véritable compte et la session de développement Better Auth, selon le même parcours authentifié que le reste de l'application.

Choisissez **Show full sign in options** pour afficher les options normales d'inscription, de mot de passe, de lien magique et de fournisseurs sociaux. Si la session locale ne peut pas être créée, la page affiche automatiquement ces options afin que vous puissiez continuer avec l'authentification standard.

Cette facilité est disponible uniquement lorsque le navigateur se connecte via `localhost`, `127.0.0.0/8` ou `::1` en développement ou lors des tests. Elle est désactivée pour les aperçus de déploiement et la production hébergée. Il ne s'agit pas d'un contournement de l'authentification. Définissez `AGENT_NATIVE_DISABLE_AUTO_DEV_ACCOUNT=1` pour la désactiver et afficher la page de connexion normale en développement local.

## Meilleure authentification (par défaut) {#better-auth}

Par défaut, Better Auth gère l'authentification. Il fournit :

- Inscription et connexion par lien magique, avec e-mail/mot de passe en secours
...
- `POST /_agent-native/auth/magic-link` — demander un e-mail avec lien magique
...
ou prévisualisez les environnements et nommez les comptes de test avec une adresse `+autoz`
(`name+autoz@example.com`) pour qu'ils soient faciles à identifier.

## Politique de vérification des e-mails {#email-verification-policy}

`AUTH_SKIP_EMAIL_VERIFICATION` est une commodité locale et de contrôle qualité :
un déploiement hébergé l'ignore. Pour définir la politique dans n'importe quel
environnement, production comprise, déclarez-la dans la configuration :

```ts
defineAppConfig({ auth: { requireEmailVerification: false } });
```

L'alias de déploiement est `AUTH_REQUIRE_EMAIL_VERIFICATION=0`. Une valeur
déclarée l'emporte à la fois sur `AUTH_SKIP_EMAIL_VERIFICATION` et sur la
valeur par défaut de chaque environnement.

`false` accepte une adresse non vérifiée comme identifiant de connexion —
n'importe qui peut revendiquer n'importe quel e-mail — ne l'utilisez donc que
là où l'inscription est déjà restreinte autrement. `true` sur un déploiement
sans fournisseur d'e-mail désactive plutôt l'inscription par mot de passe, car
la vérification ne pourrait jamais être délivrée.
...
<Diagram id="doc-block-10nnoa3" title={"Prise de contact à distance MCP OAuth"} summary={"Un client compatible OAuth démarre uniquement à partir du MCP URL : défi, découverte, enregistrement dynamique, puis échange de code PKCE."}>
...
```
/sign-in?return=<same-origin-path>
```
...
function SignInCta() {
  const onClick = () => {
    ...
    window.location.href = "/sign-in?return=" + encodeURIComponent(ret);
  };
...
| `AUTH_REQUIRE_EMAIL_VERIFICATION`       | Défini sur `0` ou `1` pour déclarer la politique de vérification de l'inscription par mot de passe dans n'importe quel environnement, production comprise ; l'emporte sur `AUTH_SKIP_EMAIL_VERIFICATION`                          |
...
Le flag Dispatch `desktop.workspace-sso`, désactivé par défaut, ne contrôle que la disponibilité ; les contrôles d’authentification, de signature, de nonce, d’origine, de callback, de session et de révocation restent toujours obligatoires. Si le flag est désactivé ou illisible, Desktop ne modifie ni la connexion, ni la déconnexion, ni les réglages propres à chaque application.
Le marqueur Canary de l’agent utilisateur est seulement un indice de disponibilité, pas une attestation distante ; l’authentification et l’autorisation ne lui font pas confiance.
Les navigateurs ordinaires restent sur la connexion propre à chaque application. Les applications auto-hébergées utilisent `AGENT_NATIVE_IDENTITY_HUB_URL` uniquement pour la voie Desktop explicite ou la fédération côté serveur restante.
Chaque version de Desktop peut initialiser le courtier lorsque la préférence par appareil `desktopSsoEnabled` est vraie (valeur par défaut) ; une valeur persistée explicitement à `false` reste une désactivation volontaire. Le flag Dispatch `desktop.workspace-sso` doit également être activé. Le marqueur Canary de l’agent utilisateur n’est plus un contrôle d’initialisation ; il identifie uniquement le canal de mise à jour.
La version Canary n’intercepte pas la navigation de connexion anonyme des applications. L’utilisateur doit choisir explicitement la connexion à l’espace de travail dans les paramètres de Desktop pour l’application enregistrée active ; la connexion normale de l’application reste le parcours par défaut.

## Déploiement administré et invitations

Pour un déploiement administré, définissez `AUTH_SIGNUP=invited`, `ORG_CREATION=closed` et `AUTO_CREATE_DEFAULT_ORG=0`. Le [guide des déploiements administrés](/docs/administered-deployments) décrit les administrateurs d’amorçage, les invitations et les domaines autorisés. Les invitations d’adhésion et l’accès par domaine sont deux règles distinctes.
