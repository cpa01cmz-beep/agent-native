...
Gardez les trois couches à l’esprit pour choisir où placer une capacité :

- **Core - le framework :** le contrat d’exécution et de données requis que toute app peut utiliser.
- **Toolkit - éléments réutilisables facultatifs :** UI et systèmes produit partagés que les apps peuvent adopter, composer ou forker.
- **Templates - apps facultatives :** produits métier complets bâtis sur Core, souvent avec Toolkit, que l’on peut forker comme point de départ.

Toolkit et Templates sont facultatifs. Un template n’est pas le framework lui-même, et Toolkit ne remplace pas Core.

Un bon modèle :

- Résout un flux de travail de bout en bout, avec des données de départ utiles ou un flux à état vide.
- Stocke l'état durable dans PostgreSQL/PGlite, pas dans les fichiers JSON.
...
```bash
npx --yes @agent-native/core@latest create my-template --template chat --standalone
```
...
Chat vous offre une authentification, des fils de discussion durables, des ressources soutenues par PostgreSQL, des outils, l'état de l'application, actions et la synchronisation des sondages. Vous ajoutez le modèle de domaine et le produit UI.

Si vous ne créez pas encore de modèle UI réutilisable, utilisez [Automation-First Apps](/docs/pure-agent-apps) : définissez des actions, exécutez-les sans navigateur et ajoutez l’interface plus tard lorsque le workflow a besoin d’une surface durable.
...
<FileTree
  id="doc-block-ixpw0s"
  title={"Structure du projet de modèle"}
  entries={[
    {
      path: "app/",
      note: "Frontend React",
    ...
    {
      path: "app/root.tsx",
      note: "Shell HTML et providers",
    },
    {
      path: "app/routes/",
      note: "Routes fichier React Router",
    },
    {
      path: "app/components/",
      note: "UI du modèle",
    ...
    {
      path: "app/hooks/",
      note: "Hooks d'état et de données de l'UI",
    },
    {
      path: "actions/",
      note: "Opérations defineAction : la source de vérité unique",
    },
    {
    ...
    },
    {
      ...
      note: "Migrations additives",
    },
    {
      path: "server/plugins/",
      note: "Intégrations au démarrage",
    },
    {
      path: "server/routes/api/",
      note: "Routes personnalisées seulement quand les actions ne suffisent pas",
    },
    {
      path: "shared/types.ts",
      note: "Types client/serveur partagés",
    },
    {
      path: ".agents/skills/",
      note: "<skill>/SKILL.md : consignes agent pour les workflows complexes",
    },
    {
      path: "AGENTS.md",
      note: "Instructions agent propres au modèle",
    },
    {
      path: "package.json",
    },
...
N'ajoutez pas de répertoire `data/` pour l'état de l'application. Les données d'application durables appartiennent à PostgreSQL/PGlite, et le UI les lit via actions ou des gestionnaires de serveur typés.

Les quatre zones de chaque modèle s'articulent via une surface d'action partagée et une base de données PostgreSQL/PGlite : l'agent et le UI sont des partenaires égaux pour les mêmes opérations :

<Diagram id="doc-block-1gl364t" title={"Comment les quatre zones d'un modèle se connectent"} summary={"L'interface utilisateur et l'agent atteignent tous deux PostgreSQL/PGlite via les mêmes actions ; l'état de l'application et la synchronisation des interrogations les maintiennent alignés."}>

```html
<div class="diagram-tmpl">
  <div class="diagram-col">
    <div class="diagram-node">
      UI React<br /><small class="diagram-muted">app/routes · composants</small>
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Opérations</span
    ><small class="diagram-muted">defineAction()</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    PostgreSQL avec Drizzle<br /><small class="diagram-muted"
      >additive schema</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-pill ok">Sondageing sync</div>
</div>
```
...
## Données du modèle dans PostgreSQL {#data-models}

Définissez des tables de domaines avec les assistants du framework Drizzle afin que les schémas utilisent PostgreSQL et PGlite :
...
Les modifications du schéma doivent être additives. Pour les nouvelles tables et
colonnes propres à l'application, utilisez `drizzle-kit generate` pour créer du
SQL révisable dans `server/db/migrations/<id_name>.sql`, puis chargez-le
avec `runDrizzleMigrations()` depuis l'étape de release sur les déploiements
Node.js. Cloudflare Workers/Pages et les autres runtimes edge n'ont pas de système de fichiers
déployable. Intégrez-y le SQL généré dans le bundle du worker et transmettez-le à
`runMigrations()`. Conservez `runMigrations()` pour l'historique écrit à la main
et les cas particuliers. N'utilisez jamais de SQL destructeur, `drizzle-kit push`,
de renommage de tables ou de suppression de colonnes.

Pour les lectures et écritures d'applications, utilisez le générateur de requêtes de Drizzle et les opérateurs compatibles avec PostgreSQL de `drizzle-orm`. N'écrivez pas de code produit avec SQL brut lorsque Drizzle peut exprimer la requête, et utilisez les assistants de schéma de `@agent-native/core/db/schema` dans les modèles.
...
Utilisez les documents [Database](/docs/server-database) et [Security](/docs/security) avant d'ajouter des schémas contenant des données utilisateur ou organisationnelles.
...
<AnnotatedCode
  ...
  annotations={[
    ...
    {
      lines: "8",
      label: "Surface de l'outil",
      note: "`description` est ce que l'agent lit pour décider quand appeler cette action en tant qu'outil.",
    },
    {
      lines: "9-11",
      label: "Contrat typé",
      note: "Un zod `schema` valide les entrées de l’agent, de l’UI, de HTTP, de MCP et d’A2A.",
    },
    {
      lines: "18-19",
      label: "Écriture étendue",
      note: "Tamponnez `ownerEmail` / `orgId` à partir de `ctx` afin que la ligne soit correctement délimitée pour les contrôles de partage et d'accès.",
    },
  ]}
/>
...
- Data lives in PostgreSQL avec Drizzle. Use actions for all writes; schema is additive.
...
Les modèles de la communauté s’installent directement depuis un dépôt GitHub public. Placez l’application Agent-Native à la racine du dépôt :

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo
```

Ajoutez `#branch`, `#tag` ou `#commit` pour épingler une révision précise :

```bash
npx --yes @agent-native/core@latest create my-app --template community:owner/repo#v1.0.0
```

La CLI télécharge uniquement le code source. Elle n’installe pas les dépendances et n’exécute pas les scripts du modèle. Les modèles communautaires sont du code tiers : vérifiez le dépôt, la licence, les dépendances et les scripts avant d’exécuter `pnpm install`.

Pour figurer au catalogue, fournissez une application complète à la racine, un dépôt public avec une licence et un README de configuration, une dépendance à `@agent-native/core`, ainsi que des vérifications de types et des tests réussis. Une démo hébergée et une capture d’écran sont facultatives.

[Proposez un modèle communautaire](https://github.com/BuilderIO/agent-native/issues/new?template=community-template.yml) lorsque le dépôt est prêt. Ces modèles restent séparés du catalogue officiel et sont maintenus par leurs auteurs.
