...
<Diagram id="doc-block-verg77" title={"Exécuter le cycle de vie"} summary={"startRun ouvre une ligne en cours d'exécution ; updateRunProgress le corrige ; completeRun le déplace vers un état de terminal et tamponne completed_at."}>
...
<Endpoint id="doc-block-fltdqp" title={"Répertorier les exécutions actives"} method="GET" path="/_agent-native/runs" summary={"List the caller's runs. The RunsTray polls this with active=true."} auth="Session cookie (owner-scoped)" params={[
  {
    "name": "active",
    "in": "query",
    "type": "boolean",
...
- [**Données et synchronisation**](/docs/client-data) — `useDbSync` pour l'invalidation du cache en temps réel
