...
<Diagram id="doc-block-8ofslt" title="Le chemin de rendu natif" summary={"Une action renvoie JSON ; le runtime correspond à un discriminant de widget explicite ou chatUI.renderer ; AssistantChat monte un véritable composant React. Pas d'iframe, pas d'exécution HTML."}>

```html
<div class="diagram-render">
  <div class="diagram-node">
    L’action s’exécute<br /><small class="diagram-muted"
      >renvoie du JSON structuré</small
    >
  </div>
...
Dimensionnez les renderers en unités absolues, pas en pourcentages. La transcription
grandit pour s'adapter à ses messages : le conteneur d'un renderer n'a donc pas de
hauteur définie permettant de résoudre un pourcentage — `height: 40%` sur la barre
d'un graphique s'effondre à zéro au lieu de lever une erreur, ce qui donne l'impression
d'un renderer qui n'a pas su se dessiner. Donnez aux graphiques et autres visuels
dépendant de la hauteur une base en pixels (ou en `rem`) ; les largeurs en pourcentage
conviennent, car la colonne de transcription a bien une largeur définie.
...
<Diagram id="doc-block-5i7siq" title="Le runtime BYO conserve le shell de discussion Agent-Native" summary={"Votre agent externe diffuse des événements normalisés via un connecteur ; Agent-Native conserve le compositeur, la transcription, les fiches outils, les approbations et les widgets natifs."}>

```html
<div class="diagram-byo">
  <div class="diagram-box" data-rough>
    Votre agent<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted"
      >événements message-* / tool-* normalisés</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card col">
...
- [Actions](/docs/actions-overview) : définissez les opérations qui renvoient les données natives du widget.
