...
Une **automatisation** est une tâche exécutée par l'agent selon un calendrier ou lorsqu'un événement survient : _quand X se produit, faites Y_. **Scheduled** et **Event** sont deux types de déclencheurs, pas des produits distincts.

Le framework présentait à l'origine les automatisations planifiées comme des **Recurring Jobs**. Ce nom, le chemin `jobs/<name>.md` et l'outil `manage-jobs` restent disponibles pour compatibilité. La nouvelle documentation utilise **Automations** comme terme générique ; consultez [Recurring Jobs](/docs/recurring-jobs) pour maintenir une ancienne tâche planifiée ou utiliser la portée organisation et les champs avancés.

<Diagram id="doc-block-1vqdjiw" title="Quand X se produit, faites Y" summary={"Un événement se déclenche sur le bus, une condition facultative en langage naturel le contrôle et l'agent exécute le corps d'automatisation avec un accès complet aux outils."}>
...
<div class="auto-flow">
  ...
  <div class="diagram-card accent">
    <span class="diagram-pill accent">L’agent exécute le body</span
    ><small class="diagram-muted"
...
### Depuis l'interface Automations

Ouvrez **Automations** dans la page Agent complète pour afficher et gérer les automatisations planifiées et déclenchées par événement. L'URL de compatibilité reste `/agent#jobs`, afin que les liens et favoris existants continuent de fonctionner.

<Callout tone="warning">

`manage-automations` prend en charge les automatisations personnelles et d'organisation pour les déclencheurs Scheduled et Event. Les automatisations d'événement d'une organisation sont visibles par ses membres, mais s'exécutent toujours en tant que leur créateur ; les administrateurs peuvent les gérer sans changer cette identité.
...
<AnnotatedCode
  id="doc-block-105qlke"
  title={"Une automatisation déclenchée par un événement"}
  ...
  annotations={[
    ...
    },
    {
      ...
      note: "`triggerType: event` plus le nom `event` abonne cet automatisme au bus.",
    },
    {
      lines: "6",
      label: "Porte",
      note: "Un `condition` en langage naturel facultatif, évalué par Haiku par rapport à la charge utile avant l'expédition.",
    },
    {
      lines: "12",
      label: "Secret côté serveur",
...
Le `owner` des métadonnées d'émission détermine quelles automatisations se déclenchent. Les automatisations d'événement personnelles et d'organisation exigent une correspondance exacte avec `createdBy`.
...
Les automatisations personnelles et d'organisation utilisent `manage-automations` avec un paramètre `action` :
...
`manage-automations` prend en charge la portée organisation, les remplacements de modèle et les listes d'autorisation MCP strictement appliquées. `manage-jobs` reste disponible pour les anciennes intégrations uniquement planifiées et les métadonnées de livraison.
...
<Endpoint id="doc-block-f9fp1y" title={"Déclencher un événement test"} method="POST" path="/_agent-native/automations/fire-test" summary={"Émettez un événement test.event.fired pour valider les automatisations déclenchées par un événement"} responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-aws59c" title={"Le chemin d'expédition"} summary={"D'un événement déclenché à une exécution d'agent terminée, en fonction de l'étendue de la propriété et de la condition du langage naturel."}>
...
<div class="disp">
  ...
  <div class="diagram-card accent">
    ...
    ><small class="diagram-muted"
      ><code>runAgentLoop</code> avec body comme prompt, payload comme contexte,
...
- [**Actions**](/docs/actions-overview) — les automatisations peuvent appeler n'importe quelle action enregistrée via la boucle d'agent
