...
<Diagram id="doc-block-x2jw9s" title="Chaque course alimente la boucle" summary={"Une exécution d'agent produit une trace, des scores automatisés et un feedback hook, le tout stocké dans le propre SQL de l'application et affiché sur le tableau de bord. Les expériences répartissent le trafic entre les variantes de configuration."}>

```html
<div class="obs-loop">
  <div class="diagram-node">
    Exécution agent<br /><small class="diagram-muted"
      >production-agent.ts</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Capturé automatiquement</span
...
### Retours dans PostHog {#posthog-feedback}

Pouces, catégories et texte libre émettent tous `$ai_feedback`. La vue des retours de l'analytique LLM de PostHog lit en revanche un événement `survey sent` : définissez donc `POSTHOG_AI_FEEDBACK_SURVEY_ID` sur le sondage que PostHog crée depuis l'onglet **Feedback** d'une trace. Cet unique identifiant constitue toute la configuration ; sans lui, aucun événement de sondage n'est envoyé.

Un vote au pouce répond à la première question du sondage par l'indice de choix de PostHog (`1` en haut, `2` en bas), et le texte libre qui suit un pouce vers le bas répond à la relance : le texte n'atterrit donc jamais à la place de la note. Les deux portent un même identifiant de soumission par message noté, ce qui permet à PostHog de les réunir en une seule réponse, et un pouce vers le bas reste marqué incomplet jusqu'à l'arrivée de sa relance.
...
<Diagram id="doc-block-1cfpj8h" title={"Attribution de variantes de hachage cohérentes"} summary={"Chaque utilisateur hache une variante stable, la boucle applique le remplacement de configuration de cette variante et les résultats sont cumulés par variante avec des intervalles de confiance."}>

```html
<div class="exp">
  <div class="diagram-node">
    ID utilisateur<br /><small class="diagram-muted">consistent hash</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-card">
      <span class="diagram-pill">control &middot; 50%</span
      ><small class="diagram-muted">surcharge de config A</small>
    </div>
    <div class="diagram-card">
      <span class="diagram-pill accent">treatment &middot; 50%</span
      ><small class="diagram-muted">surcharge de config B</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    Résultats per variant<br /><small class="diagram-muted"
...
Défini depuis un plugin serveur avec `defineAppConfig()`. Chaque champ possède également un alias de variable d'environnement de déploiement — voir la référence de configuration déclarée.
...
Envoyez les traces vers Langfuse, Datadog, Grafana ou tout backend compatible OTel en enregistrant un fournisseur OpenTelemetry dans votre propre application — voir [Spans OpenTelemetry](#otel) ci-dessous.

Le noyau ne fournit délibérément aucun exportateur et ne conserve ni point de terminaison d'export ni jeton : les identifiants du backend vivent donc dans votre câblage de fournisseur (et dans le coffre-fort), jamais dans la configuration du framework.

## Étendues OpenTelemetry {#otel}

La boucle d'agent émet des **spans OpenTelemetry en direct** pour chaque exécution, appel de modèle et appel d'outil — de sorte qu'un hôte qui exécute déjà un collecteur OTel voit l'activité de l'agent aux côtés du reste de ses traces distribuées.
...
- [**Actions**](/docs/actions-overview) — les opérations qui apparaissent sous forme d'appels d'outils dans les traces
