...
description: "Montez le chat et les ressources de l'agent dans n'importe quelle application React avec <AgentPanel>, <AgentSidebar> et sendToAgentChat()."
---
...
Vous n'avez pas besoin de créer un agent natif à partir de zéro. Le chat de l'agent, l'onglet Ressources de l'agent, le terminal CLI, la saisie vocale et toute l'infrastructure associée sont livrés sous la forme d'une poignée de composants React que vous déposez dans n'importe quelle application.

> **Prérequis :** le serveur doit exécuter `agent-chat-plugin` (il se monte automatiquement dans chaque modèle). Si vous partez de zéro, voir [Server](/docs/server-overview).
>
...
| `<AgentPanel>`        | Le panneau brut lui-même – chat + CLI + onglets Ressources de l'agent                                                   | Vous souhaitez un contrôle total sur la mise en page ou une page d'agent dédiée         |
...
<Diagram id="doc-block-11zxhl1" title={"Le modèle de monture"} summary={"<AgentSidebar> enveloppe votre mise en page existante. Vos itinéraires s'affichent dans la zone principale ; le panneau d'agent se monte à côté d'eux. <AgentPanel> est le même panneau sans le wrapper."}>
  ...
  <div class="diagram-box sidebar" data-rough>
    ...
    <div class="inner">
      <div class="diagram-node main">
        Votre application<br /><small class="diagram-muted"
          >children: header + &lt;Outlet/&gt;</small
        >
      </div>
      <div class="diagram-node panel">
        Panneau de l’agent<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-2xqn6p"
  title={"Encapsuler la disposition racine avec <AgentSidebar>"}
  ...
  annotations={[
    ...
    {
      lines: "8-12",
      label: "Invites de démarrage",
      note: "`suggestions` s’affiche sous forme de puces cliquables sur le chat vide.",
    },
    {
      lines: "13",
      label: "Puces contextuelles",
...
C'est tout. L'utilisateur dispose désormais d'un agent commutable sur chaque page – avec historique des discussions, onglet ressources de l'agent, terminal CLI, saisie vocale et un panneau élargi. L'état persiste lors des rechargements via `localStorage`.

### Accessoires

- **`children`** : présentation et itinéraires normaux de votre application. Rendu dans la zone principale ; le panneau d'agent se monte à côté sur le bureau et par-dessus sur mobile.
...
`<AgentPanel>` vous donne les onglets bruts (Chat / CLI / Ressources de l'agent) sans le wrapper de la barre latérale, le bouton de réduction ou toute persistance d'état. Placez-le où vous voulez ; vous gérez la mise en page.
...
- **`isFullscreen`** – activez cette option pour une colonne centrée de style Claude sur une surface de page ou un panneau personnalisé. La barre latérale utilise plutôt le panneau élargi.
...
Un bouton qui confie le travail à l’agent au lieu de créer un chemin IA séparé avec un appel `llm()` en ligne. Consultez [Pourquoi créer des applications ainsi](/docs/what-is-agent-native#why-build-apps-this-way) pour comprendre cette distinction :
...
- **Pour un travail d'IA visible lancé par l'utilisateur**, utilisez `openSidebar: true`. Utilisez `false` uniquement pour un travail d'arrière-plan ou lancé par le système explicitement silencieux, et gardez les suivis dans le même fil AgentSidebar.
- **`type`** — `"content"` (par défaut) conserve le travail dans l'agent intégré, qui utilise les outils de l'application. `"code"` l'achemine vers un hôte distinct capable de modifier le dépôt ; l'écriture n'est disponible que si l'hôte accorde intentionnellement des outils repository/workspace. Voir [Code Agent-Native UI](/docs/code-agents-ui).
...
Gardez les messages de suivi et de révision dans le fil existant de la barre latérale. N'ajoutez pas un second champ de texte libre à côté du résultat; utilisez des formulaires locaux uniquement pour les valeurs structurées que l'interface doit valider.
...
[Chat de l’Agent](/docs/client-agent-chat#sendtoagentchat) pour le comportement du pont de l'application MCP.
...
## Choisir entre le chat de l'agent et une action {#agent-or-action}

Utilisez directement `useActionQuery` ou `useActionMutation` pour les opérations locales déterministes comme le CRUD, la validation, les lectures de fournisseurs et la persistance. Si l'utilisateur vit le flux comme une recherche, une analyse, une génération, une recommandation, une synthèse ou un raisonnement en plusieurs étapes, démarrez-le avec `sendToAgentChat({ openSidebar: true })` et laissez l'agent orchestrer des actions ciblées. Une implémentation déterministe ne suffit pas à justifier de cacher un flux de type IA dans une action opaque.

## Typesafe actions du UI : `useActionMutation()` {#use-action-mutation}

Lorsque l’UI doit exécuter la même opération qu’un outil d’agent exécuterait, utilisez [`useActionMutation`](/docs/client-data#use-action-mutation) :
...
Les arguments de type sécurisé proviennent du schéma zod de votre `defineAction()`. Voir [Actions](/docs/actions-overview) pour le système d'action complet.
...
- [**Actions**](/docs/actions-overview) — `defineAction()` et `useActionMutation()`
- [**Context Awareness**](/docs/context-awareness) — sélection, navigation, écran d'affichage
- [**Ressources de l'agent**](/docs/agent-resources) : ce que contient l'onglet Ressources de l'agent (skills, mémoire, serveurs MCP, tâches planifiées)
