...
description: "Un partage familier et cohérent, intégré au framework. Chaque ressource créée par l’utilisateur — documents, tableaux de bord, conceptions, présentations, clips, enregistrements et formulaires — bénéficie du même modèle privé par défaut avec une interface de partage cohérente."
---

# Partage et confidentialité

Chaque ressource qu'un utilisateur crée dans une application native d'agent (un document, un tableau de bord, une conception, une présentation, un enregistrement d'écran, une transcription de réunion, un formulaire, un lien de réservation) est **privée par défaut pour le créateur**. Les autres personnes ne le voient que lorsque le créateur le partage explicitement ou modifie sa visibilité en `org` ou `public`.

C'est un modèle de partage familier partout : le même bouton de partage, la même boîte de dialogue, le même modèle de visibilité à trois niveaux et les mêmes autorisations par utilisateur et par organisation, dans chaque modèle, sans réinvention par application.
...
<Diagram id="doc-block-i7p9cb" title={"Visibilité, s'élargissant vers l'extérieur"} summary={"Une visibilité grossière sur la ressource donne le ton ; les attributions d'actions explicites dans le tableau associé ajoutent des personnes nommées en haut."}>
...
- **Commentateur** – lecture et ajout de commentaires, sans pouvoir modifier la ressource ni gérer les partages.
...
- **Applications de données personnelles** comme le courrier : conçues pour l'utilisateur. Il n'y a pas de concept « partager ma boîte de réception ».
...
- Une case à cocher `Notifier les personnes` pour les autorisations individuelles par e-mail.
...
<DataModel
  id="doc-block-1rkh7kr"
  title="Tableau des partages de ressources et de compagnons"
  summary={
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: false,
          note: "La seule source de vérité pour la propriété.",
        },
        {
          name: "org_id",
          type: "text",
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          name: "role",
          type: "enum",
          note: "spectateur | commentateur | éditeur | administrateur",
        },
        {
          name: "created_by",
          type: "text",
...
- [**Sécurité — Portée des données**](/docs/security#data-scoping) — le modèle de filtre d'accès et de propriété sur lequel repose le partage
- [**Authentification**](/docs/authentication) — les sessions, les organisations et la manière dont l'identité alimente le contexte de la requête
- [**Extensions**](/docs/extensions#sharing) — le partage dans la surface de mini-application en bac à sable
- [**Création de modèles**](/docs/creating-templates) — l'intégration de `ownableColumns` au schéma d'un nouveau modèle
