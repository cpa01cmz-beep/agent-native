...
<Diagram id="doc-block-1ud68rc" title={"De la source à la réponse citée"} summary={"Le cerveau ingère les sources approuvées dans des captures brutes, distille une mémoire durable, la contrôle et répond ensuite seulement par des citations."}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box" data-rough>
    Captures brutes<br /><small class="diagram-muted"
      >dédupliqué, expurgé</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Distill<br /><small class="diagram-muted"
      >faits · décisions · processus</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
  ...
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted">approuvé, atomique</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<WireframeBlock id="doc-block-rrj2hq">
  <Screen
    surface="desktop"
    html={
...
aucune exigence de base de données vectorielles, le modèle reste donc portable sur PGlite,
Postgres, Neon, hosted PostgreSQL et hôtes similaires. L'état de l'application reflète le
itinéraire actuel, filtres et ID sélectionnés afin que l'agent connaisse toujours l'itinéraire actuel
navigation et sélection.

Le schéma du cerveau réside dans `templates/brain/server/db/schema.ts`. Neuf tableaux :
...
<DataModel
  id="doc-block-1gx8o4"
  title={"Modèle de données Brain"}
  summary={
    "Les connecteurs produisent des captures brutes ; la distillation les transforme en connaissances révisables ; les propositions contrôlent les entrées sensibles. Les exécutions de synchronisation et la file d’ingestion suivent le travail en arrière-plan."
  }
  entities={[
    {
      id: "sources",
      name: "brain_sources",
      note: "Configuration du connecteur",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Charges utiles brutes ingérées",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Entrées atomiques distillées",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Éléments en attente d'examen",
      fields: [
        {
          name: "id",
          type: "id",
    ...
    },
    {
      ...
      note: "Journal d'audit de synchronisation",
      fields: [
        {
          name: "source_id",
          type: "id",
    ...
    },
    {
      ...
      note: "File d'attente de distillation en arrière-plan",
      fields: [
        {
          name: "operation",
          type: "text",
...
<Endpoint id="doc-block-1h85ywi" title={"Webhook d'ingestion signé"} summary={"Les importations Clips et transcript/capture génériques affichent un RawCapturePayload avec un jeton de porteur par source."} method="POST" path="/api/_agent-native/brain/ingest" summary={"Importer une capture brute depuis Clips ou une source générique"} auth={"Bearer <ingestToken> issued per source via its sourceKey"} request={{
...
Effectuez ces changements dans le flux de développement du dépôt. L'agent
intégré ne peut participer aux modifications de source que si l'hôte accorde
intentionnellement des outils d'écriture repository/workspace ; sinon, il agit
via les actions et les données. Voir
...
- [**Ressources de l'agent**](/docs/agent-resources) – ressources partagées entre les applications
- [**A2A Protocol**](/docs/a2a-protocol) – délégation inter-applications
- [**Modèles**](/docs/cloneable-saas) — le modèle de clonage et de propriété, ainsi que tous les autres modèles de domaine pour commencer
- [**Premiers pas**](/docs/getting-started) — les principes fondamentaux du framework : actions, état de l'application et synchronisation en direct
