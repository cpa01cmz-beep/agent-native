---
title: "Extensions"
description: "Une capacité facultative du toolkit pour des blocs personnalisés ponctuels et isolés. Le code de l’application reste la voie par défaut pour les comportements durables."
---

# Extensions

Les extensions sont une **capacité facultative du toolkit pour des blocs personnalisés isolés**. Elles sont désactivées par défaut et le modèle doit les activer explicitement. Analytics les utilise pour des visualisations ponctuelles demandées explicitement. Pour un comportement durable, modifiez le code de l’application.
...
> **Les extensions ajoutent une interface ; elles ne réécrivent pas l'interface
> de l'application hôte.** Une extension s'affiche sur sa propre page ou dans un
> [emplacement](#slots) nommé exposé par le modèle. Elle ne peut pas injecter
> d'éléments dans n'importe quel composant natif — par exemple à côté de chaque
> participant dans Calendrier — sauf si le modèle expose un emplacement à cet
> endroit. Pour une personnalisation plus poussée, modifiez le template dans
> votre dépôt et votre flux de développement. L'agent intégré ne peut aider que
> si l'hôte accorde intentionnellement des outils d'écriture
> repository/workspace ; les extensions restent la couche rapide sans
> déploiement.
...
Les extensions sont désactivées par défaut. Pour les activer, définissez
`extensionTools: true` sur `createAgentChatPlugin()` et
`createCoreRoutesPlugin()`. Le premier ajoute les actions et les instructions
destinées à l’agent ; le second autorise la création authentifiée via
`POST /_agent-native/extensions`. Le runtime isolé et les routes existantes de
lecture, modification et liens profonds restent disponibles par compatibilité.

<Diagram id="doc-block-1d1pxwt" title={"Le pont du bac à sable"} summary={"L'extension HTML s'exécute dans une iframe isolée et atteint l'hôte uniquement via un ensemble fixe d'assistants de pont : chaque appel est limité et son accès est vérifié."}>
...
<div class="ext-bridge">
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      Template hôte<br /><small class="diagram-muted"
        >actions, SQL auto-scopé</small
      >
    </div>
    <div class="diagram-box">
      Secret proxy<br /><small class="diagram-muted"
        ><code>${keys.NAME}</code>, verrouillé au domaine</small
      >
    </div>
    <div class="diagram-box">
      APIs externes<br /><small class="diagram-muted"
        >uniquement via extensionFetch</small
      >
    </div>
  </div>
</div>
...
1. Ouvrez **Settings → Extensions** (ou demandez simplement dans le chat), puis cliquez sur **"Nouvelle extension"**.
...
<Diagram id="doc-block-1t87u8e" title="Les emplacements poussent le contexte dans le widget" summary={"Le modèle hôte possède des emplacements nommés ; l'installation d'une extension dans une extension l'alimente window.slotContext pour tout ce que l'utilisateur consulte actuellement."}>

```html
<div class="slot">
  <div class="diagram-card">
    <span class="diagram-pill">Fil de mail</span
    ><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill">Notes de contact</span
    ><small class="diagram-muted"
...
- `dbQuery` et `extensionFetch` sont bloqués pour le moment.
...
## Dois-je utiliser une extension ou le flux de source de l’application ? {#vs-app-code}

L'agent intégré ne modifie pas le code source par défaut ; il ne peut le faire que si l'hôte accorde intentionnellement des outils d'écriture repository/workspace. Utilisez le flux de source pour un comportement durable et les extensions pour une personnalisation rapide au runtime.

- **Commencez par le code de l’application** pour le comportement du produit, l’interface native, la mise en page, les itinéraires, la logique métier ou tout ce qui doit être réutilisé.
- **Utilisez un bloc personnalisé uniquement si l’application l’active** et si l’utilisateur demande explicitement une visualisation ou interaction ponctuelle dans une surface existante. Il est rapide et sans déploiement, mais reste un correctif isolé créé par l’agent.
- **Promouvez les blocs durables.** Si un bloc devient utile au-delà de son contexte initial, utilisez **Promouvoir vers le code de l’application** et conservez-le jusqu’à la revue et au déploiement du changement.

|                              | Extension                                       | Modification du code de l'application               |
| ---------------------------- | ----------------------------------------------- | --------------------------------------------------- |
| **Créé par**                 | Agent (ou utilisateur) au moment de l'exécution | Utilisateur ou flux capable de modifier le code     |
...
| **Idéal pour**               | Blocs personnalisés ponctuels                   | Comportement durable du produit                     |

Règle générale : **utilisez le code de l’application par défaut.** Dans une application qui active les extensions, une visualisation ponctuelle peut être un bloc personnalisé ; un nouveau type de graphique destiné à tous les tableaux de bord relève du code du modèle.

Pour une récupération de données personnalisée destinée à alimenter des graphiques et tableaux natifs plutôt qu'une UI de widget sur mesure, voir [Programmes de données](/docs/data-programs) — un script `run-code` stocké et actualisable auquel les panneaux de dashboard se lient directement, sans aucun iframe d'extension.
...
- **Authentification requise** : les extensions ne s'exécutent que pour les utilisateurs connectés et les appels `dbQuery` sont automatiquement étendus.
...
- [**Actions**](/docs/actions-overview) — les opérations qu'un poste appelle via `appAction`
