...
- **Sous-agents** : exécutés avec leur propre thread, leur propre invite système, leur propre ensemble d'outils. Chacun correspond à un profil « agent personnalisé » dans le [Ressources de l'agent](/docs/agent-resources).
...
<Diagram id="doc-block-1pvbwrf" title={"Orchestrateur et spécialistes"} summary={"Le chat principal délègue à des sous-agents qui s'exécutent dans leurs propres threads et rendent compte sous forme de puces en ligne."}>
  ...
  <div class="at-fan">
    ...
    <div class="at-subs">
      <div class="diagram-box">
        Revue de code<br /><small class="diagram-muted"
          >own thread &amp; prompt</small
        >
      </div>
      <div class="diagram-box">
        Analyse BigQuery<br /><small class="diagram-muted">own tools</small>
      </div>
      <div class="diagram-box">
        E-mail à la voix<br /><small class="diagram-muted">own context</small>
      </div>
    </div>
  </div>
...
Les agents personnalisés se trouvent dans l'espace de travail de `agents/<slug>.md` : un fichier Markdown avec le thème principal YAML. Voir [Custom Agents](/docs/agent-resources#custom-agents) pour le format.
...
<Diagram id="doc-block-xepm95" title="Que fait spawnTask()" summary={"Chaque apparition crée un thread, conserve son état dans SQL et diffuse les événements de puce jusqu'à leur achèvement."}>
...
Les sous-agents sont mappés aux profils d'agent personnalisés : fichiers Markdown sur `agents/<slug>.md` dans l'espace de travail qui apparaissent dans la liste déroulante `@mention` et servent de cibles de délégation. [Ressources de l'agent — Custom Agents](/docs/agent-resources#custom-agents) possède le format complet (frontmatter, `tools`, `delegate-default`, remplacements de modèle).
...
<Diagram id="doc-block-gbv1tz" title={"Garde de profondeur de délégation (capuchon par défaut 2)"} summary={"Chaque niveau peut en générer un plus profond jusqu'au plafond ; un spawn passé est refusé côté serveur."}>

```html
<div class="at-depth">
  <div class="diagram-card ok">
    <span class="diagram-pill">depth 0</span
    ><strong>Chat de niveau supérieur</strong
    ><small class="diagram-muted ok">may spawn &darr;</small>
  </div>
  <div class="diagram-card ok">
    <span class="diagram-pill">depth 1</span><strong>Sous-agent</strong
    ><small class="diagram-muted ok">may spawn &darr;</small>
  </div>
  <div class="diagram-card warn">
    <span class="diagram-pill warn">depth 2</span
    ><strong>Sous-agent's sub-agent</strong
...
- [**Ressources de l'agent — Custom Agents**](/docs/agent-resources#custom-agents) — le format de profil
- [**A2A Protocol**](/docs/a2a-protocol) — lorsque le « sous-agent » vit dans une application entièrement différente
- [**Actions**](/docs/actions-overview) — les outils qu'un sous-agent appelle
