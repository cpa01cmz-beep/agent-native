...
> **Quel document sur l'espace de travail ?** Cette page couvre la **forme de déploiement** : un monorepo, de nombreuses applications, une authentification partagée et un déploiement unifié. Pour la couche de personnalisation de l'agent (Ressources de l'agent : `AGENTS.md`, mémoire, skills, agents personnalisés), voir [Ressources de l'agent](/docs/agent-resources) ; pour la gouvernance (qui examine, approuve et possède quoi), voir [Workspace Governance](/docs/workspace-management).

## Application autonome ou espace de travail ? {#standalone-or-workspace}

Choisissez un espace de travail lorsqu’une entreprise exploite plusieurs applications internes qui doivent partager la configuration de déploiement et le contexte de l’organisation. Une application autonome convient à un outil indépendant ou à un prototype. Cette forme de déploiement est prise en charge, mais elle n’inclut pas les packages partagés ni la configuration racine de l’espace de travail. Consultez [Déploiements administrés](/docs/administered-deployments).
...
<FileTree
  id="doc-block-s8gzfc"
  title={"Un workspace généré"}
  entries={[
    {
      path: "package.json",
      note: "Déclare agent-native.workspaceCore",
    },
    {
    ...
    {
      path: ".env.example",
      note: "ANTHROPIC_API_KEY, A2A_SECRET, DATABASE_URL, ... partagés",
    },
    {
    ...
    },
    {
      ...
      note: "Overrides de plugins seulement si nécessaire",
    },
    {
      path: "packages/shared/src/client/",
      note: "Code React partagé seulement si nécessaire",
    },
    {
      path: "packages/shared/AGENTS.md",
      note: "Instructions à l'échelle du workspace",
    },
    {
      path: "apps/mail/",
    },
...
<Diagram id="doc-block-siffjs" title={"Trois calques, fusionnés par nom de fichier"} summary={"Chaque application résout d'abord les plugins, les compétences, les actions et AGENTS.md à partir de l'application locale, puis du package partagé, puis de la valeur par défaut du framework."}>
...
<Diagram id="doc-block-10xvh45" title={"Déploiement unifié : une origine, un chemin par application"} summary={"Chaque application est livrée derrière une seule origine, donc les sessions de connexion et les A2A inter-applications sont gratuites."}>
...
- [Ressources de l'agent](/docs/agent-resources) : la couche de personnalisation (`AGENTS.md`, `LEARNINGS.md`, mémoire personnelle, skills, agents personnalisés) que chaque application de l'espace de travail partage.
