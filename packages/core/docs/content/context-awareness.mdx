...
The `TAB_ID` imported from `app/lib/tab-id.ts` is the framework's `getBrowserTabId()` value. Page-local WebMCP calls send the same id in `X-Agent-Native-Browser-Tab`, so the server resolves per-tab `navigation`/`selection` state to the exact tab the agent or user is driving — there is no second tab scheme to keep in sync.
...
- [**Application State**](/docs/client-advanced#application-state) — the full `application-state` and client helper reference
